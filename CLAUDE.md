...
- 有声书：`packages/fushi_audio/` + `fushi/lib/src/media/audiobook/`（导入入口 `book_import_dialog.dart` / `audiobook_import_dialog.dart`）。设备端语音转录生成字幕在 `fushi/lib/src/asr/`（ReazonSpeech k2-v2 字符级 zipformer RNN-T + silero-vad v4，经 `flutter_onnxruntime`；Windows fp32 编码器走 DirectML，其余 int8 CPU；PCM 抽取走 ffmpeg `asr_pcm_source.dart`，任务可暂停续跑 `asr_transcribe_job.dart`，产物是单时间轴 SRT 喂既有匹配链路；UI 入口 `media/audiobook/asr_transcribe_sheet.dart`）。OCR/ASR 共用的 ONNX 会话抽象与模型下载器在 `fushi/lib/src/onnx/`。
