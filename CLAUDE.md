...
- 有声书：`packages/fushi_audio/` + `fushi/lib/src/media/audiobook/`（导入入口 `book_import_dialog.dart` / `audiobook_import_dialog.dart`）。设备端语音转录生成字幕在 `fushi/lib/src/asr/`（按语言分模型包 `asr_model_manifest.dart`：日语 ReazonSpeech k2-v2 字符级 / 英语 LibriHeavy BPE 带标点大小写，都是 zipformer RNN-T + silero-vad v4，经 `flutter_onnxruntime`；用户在转录弹层选语言、按需下载，设置页「语音识别模型」可删；Windows fp32 编码器走 DirectML 并在设备上整图转成 fp16（`asr_fp16_graph.dart`，纯 Dart protobuf 转换、缓存为 `*.fp16.onnx`，静态桶行数随之翻倍），其余 int8 CPU；PCM 抽取走 ffmpeg `asr_pcm_source.dart`（每块一个进程、默认 4 块并行按序出块——ffmpeg 解 mp3 单线程，30 分钟 6 块串行 940 ms / 并行 240 ms），任务可暂停续跑 `asr_transcribe_job.dart`（三段流水线 `AsrBatchPipeline`：fbank 走 isolate 池 `asr_fbank_workers.dart`、GPU 上保持 2 批在飞、贪心 Loop 图开 2 个会话对半并行搜——插件 delta #11 给每个 CPU 会话各一条工作线程；2026-09-07 nvidia-smi 实测老流水线编码阶段 GPU 只忙 15%），产物是单时间轴 SRT 喂既有匹配链路，旁边同序写逐 token 时间 sidecar `transcript.tokens.jsonl`，导入链路认出转录产物后挂到 `AudioCue.tokenTiming` 并在匹配后按正文句界重切 cue（`fushi_audio` 的 `cue_sentence_resegmenter.dart`：词中切开的合并、一条盖多句的拆开、新边界取 token 发射时间）；UI 入口 `media/audiobook/asr_transcribe_sheet.dart`）。EPUB↔字幕匹配器有基底/读音双轨：`EpubBook.chapterPlainTextWithRuby` 给出与 `chapterPlainText` 逐码元相同的文本 + ruby 区间，`EpubSection.rubies` 让 `EpubSrtMatcher` 在读音轨上再找一次、命中换算回基底偏移（`fushi-cue://` 坐标系不变）。CTC 强制对齐原型（`asr_ctc_align.dart`：字符级词表编码器 + Viterbi，`AsrCtcDecoder.runLogits` 出整张 logits；真模型量具 `integration_test/asr_forced_align_itest.dart`，未接进导入 UI）。OCR/ASR 共用的 ONNX 会话抽象与模型下载器在 `fushi/lib/src/onnx/`。
