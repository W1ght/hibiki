...
std::vector<TermResult> DictionaryQuery::query(const std::string& expression) const {
  ...
  // query_raw() no longer enriches (BUG-1286); this single-expression entry
...
std::vector<TermResult> DictionaryQuery::query_raw(const std::string& expression) const {
  ...
  // BUG-1286: deliberately NOT enriched here. See the note on query_raw() in
  // query.hpp -- Lookup::lookup() calls this hundreds/thousands of times per
  // user lookup, and frequency/pitch enrichment belongs on the surviving set.
