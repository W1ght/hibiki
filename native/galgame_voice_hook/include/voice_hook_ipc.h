...
// 共享内存头。injector 创建并清零；hook DLL 注入后填格式、持续更新 write_pos/total_written。
// volatile 字段跨进程无锁单写单读：hook DLL 单写 write_pos，host 只读——读到的量至多滞后一个
// 音频包，对「抓最近一句语音」无害。绝不在此放指针（跨进程地址无意义）。
#pragma pack(push, 8)
struct SharedHeader {
  ...
  uint32_t ring_capacity;   // 紧随 header 的环形缓冲字节数（帧对齐，≤kMaxRingBytes）
  ...
  volatile uint64_t total_written;  // 单调累计写入字节（host 判断有多少可读）
};
#pragma pack(pop)
