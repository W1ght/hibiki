...
std::vector<LookupResult> Lookup::lookup(const std::string& lookup_string, int max_results, size_t scan_length,
  ...
  // BUG-1665 folds a true inflected redirect (belongs -> belong) when its
  // canonical lemma also matches. Shared glossary storage does not prove a
  // redirect: independent nouns and verbs may intentionally share a definition.
  // Require source-format provenance for this exact record and its canonical
  // target, then verify that the same dictionary supplies the target glossary.
  // Legacy dictionaries without provenance retain their direct hits.
  ...
  for (auto& r : results) {
    ...
    for (const auto& lemma : results) {
      ...
      std::erase_if(r.term.glossaries, [&](const GlossaryEntry& g) {
        if (g.redirect_target.empty() || g.redirect_target != lemma.term.expression) return false;
