...
class HibikiSyncServer {
  ...
  /// BUG-901(a)：音频查词 token 的数量上限。POST 侧签发前先按 TTL prune、再淘汰最旧
  ...
  /// BUG-901(d)：WebDAV 写操作（PUT / MKCOL / DELETE）的按路径串行闸门。key 是目标
  ...
  Future<shelf.Response> _handleRequest(shelf.Request request) async {
    ...
    switch (method) {
      ...
      // BUG-901(d)：改动文件系统的写操作按目标路径串行化，避免并发 PUT/DELETE/MKCOL
  ...
  /// BUG-901(d)：把 [action] 串到 [fsPath] 的写链尾部——同一路径上的写严格 FIFO 串行，
  ...
  Future<shelf.Response> _handleAudioLookup(shelf.Request request) async {
    ...
    // BUG-901(a)：签发前先按 TTL 清过期，再把数量收束到 [_maxAudioTokens] 内（淘汰
  ...
  /// BUG-901(a)：守住音频 token 上限。TTL prune 之后仍达到 [_maxAudioTokens] 时，按
  ...
  /// BUG-901(a) 测试钩子：当前驻留的音频 token 数（验证 cap 逐出行为）。
  @visibleForTesting
  ...
  Future<shelf.Response> _handlePropfind(
    ...
    // BUG-901(b)：逐项 stat 一律异步，避免在事件循环上做阻塞式系统调用（大目录
    ...
    if (entity == FileSystemEntityType.directory) {
      ...
      if (depth == '1') {
        ...
        await for (final child in dir.list()) {
          ...
          // BUG-901(b)：文件长度用异步 stat（await for 循环里安全 await，不打乱 XML
          // 组装顺序）；目录不必取长度。
