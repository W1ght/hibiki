...
/// 顶层 tab 的逻辑身份（取代写死的整数索引 0/1/2）。条件 tab（video 常驻、games 仅
/// Windows）用枚举身份而非位置来切换/路由——插入条件 tab 不会再打乱「设置/词典」的索引
/// （消除 `==2` / `case 1/2` / `%3` 这类特殊情况）。底栏/侧栏只在渲染层把身份映射成位置。
/// games（galgame 库）紧邻设置之前。
enum HomeTab { books, video, dictionaries, games, settings }
