...
/// BUG-791：EPUB 书 bookKey 集合的响应式来源。`.distinct(listEquals)` 按集合去重
...
/// BUG-791：有声书（SrtBooks）uid 集合的响应式来源，作用同 [_epubBookKeysProvider]。
...
final hibikiBooksProvider =
  ...
  // BUG-791：订阅 EPUB 书集合变化，任意导入路径落库后自动重算书架。
...
final srtBooksProvider = FutureProvider<List<SrtBook>>((ref) {
  // BUG-791：订阅有声书集合变化，任意导入路径落库后自动重算。
