...
class SidecarWriteRequest {
  ...
  /// 危险确认后才可为 true：允许覆盖第三方文件或已被用户修改的 Hibiki 生成物。
...
class VideoSidecarWriter {
  VideoSidecarWriter({
    ...
    this.generatorVersion = 'hibiki-nfo-v1',
  ...
  static Future<void> _atomicWrite(File target, Uint8List bytes) async {
    ...
    for (int attempt = 0; attempt < 20; attempt += 1) {
      ...
      final File candidate = File(
        p.join(parent.path, '.$basename.hibiki-$nonce.tmp'),
      );
      try {
