...
void main([List<String> args = const <String>[]]) {
  ...
  runZonedGuarded<Future<void>>(() async {
    ...
    // BUG-766：把上次运行 present 楔死取证（首帧从未 rasterize）折进错误日志（仅
...
class _HoshiReaderAppState extends ConsumerState<HoshiReaderApp>
  ...
  /// BUG-766：present-watchdog 超时（比 20s 裸加载看门狗更长，确认是硬故障而非 IO 慢）。
  static const Duration _presentWatchdogTimeout = Duration(seconds: 30);

  /// BUG-766：present 层死锁兜底看门狗（仅 Windows arm）。UI-isolate 看门狗的逃生 UI 在
  ...
  void initState() {
    ...
    // BUG-766：仅 Windows 挂 present-watchdog——runApp 后 30s 仍无一帧 rasterize
  ...
  /// BUG-766：present 楔死自愈动作（仅 Windows）——落盘取证 + 一次性自动重启。marker 不
  ...
  Widget build(BuildContext context) {
    ...
    if (appModel.isInitialised ||
      ...
      // BUG-766：启动已推进（首帧早已 rasterize）→ 撤 present-watchdog 并清自动重启
