...
void main() {
  ...
  testWidgets('视频来源添加文件夹、自动归组、即时回库与全部刮削', (WidgetTester tester) async {
    final Directory fixture =
        Directory.systemTemp.createTempSync('hibiki_video_source_itest_');
    addTearDown(() {
      ...
      body: () async {
        ...
        expect(
          await _waitFor(
            tester,
            () => find
                .byType(HibikiAdjustableSegmented<MediaLibraryViewKind>)
                .evaluate()
                .isNotEmpty,
          ),
          isTrue,
        ...
        final Finder scrapeAll = find.byWidgetPredicate(
          (Widget widget) =>
              widget is HibikiIconButton && widget.tooltip == t.scrape_all,
        );
