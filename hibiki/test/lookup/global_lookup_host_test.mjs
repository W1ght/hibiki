...
// B1. BUG-852: beginLookup resets the layer's DOM transform IN LOCK-STEP with
...
//     shift.
{
  ...
  assert.strictEqual(layer.style.left, '0px',
    'BUG-852: beginLookup resets the layer DOM transform (left)');
  assert.strictEqual(layer.style.top, '0px',
    'BUG-852: beginLookup resets the layer DOM transform (top)');
}

// B2. BUG-852: panel mode never dismisses on a hook-forwarded gap click (the
...
//     the panel).
{
  ...
  assert.strictEqual(hit, true,
    'BUG-852: panel mode swallows the global click (no gap semantics)');
  assert.ok(!hostPostLog.some((m) => m.handler === 'dismissPopupAt'),
    'BUG-852: panel mode never posts dismissPopupAt from the global hook');
}
