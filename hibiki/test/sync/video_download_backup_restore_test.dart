...
void main() {
  ...
  setUp(() async {
    root = await Directory.systemTemp.createTemp('hibiki-v71-backup-');
  });
  ...
  Future<int> count(HibikiDatabase database, String table) async =>
      (await database
  ...
  test('shared backup strips all five device-local download tables', () async {
    ...
    final HibikiDatabase source = HibikiDatabase(sourceDirectory.path);
    ...
    final HibikiDatabase restored = HibikiDatabase(restoredDirectory.path);
  ...
  test(
    ...
    () async {
      ...
      final HibikiDatabase backupDatabase =
          HibikiDatabase(backupDirectory.path);
      ...
      final HibikiDatabase before = HibikiDatabase(currentDirectory.path);
      ...
      final HibikiDatabase after = HibikiDatabase(currentDirectory.path);
  ...
  test(
    ...
    () async {
      ...
      final String dbPath = p.join(currentDirectory.path, 'hibiki.db');
      final String bakPath = '$dbPath.pre-restore.bak';
      final String repairBakPath = '$bakPath.repair';
      final String sidecarPath =
          p.join(currentDirectory.path, 'hibiki.db.sync-preserve.json');
      ...
      final HibikiDatabase replacement =
          HibikiDatabase(replacementDirectory.path);
      ...
      await File(p.join(replacementDirectory.path, 'hibiki.db')).copy(dbPath);

      // The common full-restore path can have zero preserved sync prefs. Its
      // marker still has to exist solely for the v71 device-local graph.
      ...
      final HibikiDatabase overwritten = HibikiDatabase(currentDirectory.path);
      for (final String table in <String>[
        'video_download_jobs',
        'video_download_job_files',
        'video_download_job_subtitles',
      ...
      final HibikiDatabase recovered = HibikiDatabase(currentDirectory.path);
  ...
  test('merge import never adopts attached device-local download rows',
    ...
    final HibikiDatabase attached = HibikiDatabase(attachedDirectory.path);
    await seedDownloadGraph(attached);
    await attached.close();

    final HibikiDatabase target =
        HibikiDatabase.forTesting(NativeDatabase.memory());
    addTearDown(target.close);
    final String safePath = p
        .join(attachedDirectory.path, 'hibiki.db')
        .replaceAll(r'\', '/')
        .replaceAll("'", "''");
