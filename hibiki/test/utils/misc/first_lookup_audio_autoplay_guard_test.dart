...
/// BUG-1091 源码接线守卫：「本次启动/本 document 第一次查词自动发音没声音」的
/// 三条根因链一旦有人改回去立刻红。
///
/// 根因（Windows 专属组合）：
...
void main() {
  group('BUG-1091 first-lookup audio autoplay wiring', () {
    test('Windows fork allows media autoplay on the default environment', () {
      ...
      expect(
        ...
        reason: 'WebView2 has no per-view mediaPlaybackRequiresUserGesture; '
            ...
            'auto-read per popup document is silently rejected (BUG-1091)',
      );
    });
