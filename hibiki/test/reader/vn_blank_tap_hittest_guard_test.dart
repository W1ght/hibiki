...
// BUG-718：VN 居中竖排布局下「点击空白翻页」永不触发。_hoshiVnTapIsBlank 靠
...
void main() {
  test('BUG-718: _hoshiVnTapIsBlank uses client-rect hit-test, not just a '
    ...
    final int fnEnd = src.indexOf('function _hoshiReaderCaretRangeAtPoint(',
        fnStart);
    ...
    expect(
      body.contains('r.left') && body.contains('r.right') &&
          body.contains('r.top') && body.contains('r.bottom'),
      isTrue,
      reason: 'hit-test must compare the tap point against the char rect bounds',
    );
  });
}
