...
Future<HibikiDatabase> _openDatabase() async {
  final HibikiDatabase database = HibikiDatabase.forTesting(
...
Future<int> _insertVideoSource(HibikiDatabase database) =>
  ...
  int? season,
    ...
    database.upsertVideoDownloadSubscription(
      VideoDownloadSubscriptionsCompanion.insert(
        ...
        category: const Value<String?>('hibiki-video'),
...
void main() {
  test('Nyaa strict rules, start anchor, and SxxExx item outbox are enforced',
      () async {
    final HibikiDatabase database = await _openDatabase();
