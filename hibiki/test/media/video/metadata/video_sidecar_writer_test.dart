...
void main() {
  ...
  setUp(() async {
    temporary = await Directory.systemTemp.createTemp('hibiki_writer_');
    ...
    expect(
      await source
          ...
          .where((FileSystemEntity entity) =>
              p.basename(entity.path).contains('.hibiki-'))
          .toList(),
      isEmpty,
    );
  });
  ...
  test('overwrite 只更新 hash 未变化的 Hibiki 生成物', () async {
    final String target = p.join(source.path, 'movie.nfo');
  ...
  test('用户修改过 Hibiki 生成物后普通 overwrite 保持保护', () async {
    final String target = p.join(source.path, 'movie.nfo');
