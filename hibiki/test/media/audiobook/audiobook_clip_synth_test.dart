...
void main() {
  group('buildFfmpegImageAudioToVideoArgs (TODO-945 M4, mobile mpeg4/.mp4)',
      () {
    // BUG-1264：移动端默认（h264=false）走 libavcodec 原生 mpeg4（MPEG-4 Part 2）。
    ...
    // BUG-1264：mpeg4 钉近最佳 qscale（-q:v 2，文字边缘保真）+ yuv420p（全平台硬解
  ...
  group('buildFfmpegImageSeqAudioToVideoArgs (TODO-1115 image2 序列帧动态高亮)', () {
    ...
    test('mobile seq uses mpeg4 + aac, never libx264/mjpeg/concat/overlay', () {
      ...
      // BUG-1264：移动序列帧输出走原生 mpeg4；AAR 无 libx264，也无
    ...
    // BUG-1264：序列帧移动路径同样钉 mpeg4 的 -q:v 2 + yuv420p + faststart。
