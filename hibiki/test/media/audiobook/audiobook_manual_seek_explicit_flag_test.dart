...
/// BUG-896：有声书暂停态下点某句（skipToCue）立起显式 seek 抑制窗后，用户拖进度条
...
void main() {
  ...
  group('manual seek clears in-flight explicit-seek suppression (BUG-896)', () {
    test('暂停态 skipToCue 立旗 → seekMs 到新位置复位抑制旗，后续 tick 不再被旧目标抑制', () async {
      ...
      expect(controller.explicitSeekInFlightForTesting, isFalse,
          reason: '手动 seek 必须复位显式 seek 抑制旗（BUG-896 根因）');
  ...
  // 源码守卫：手动改位置路径必须复位抑制窗；删掉复位就会让 BUG-896 复发。
  test('源码守卫：seekMs / noteManualReaderNavigation 均复位显式 seek 抑制窗', () {
    ...
    expect(seekMsBody.contains('_clearExplicitSeekSuppression()'), isTrue,
        reason: 'seekMs 必须复位显式 seek 抑制窗（BUG-896）');
    ...
    expect(noteBody.contains('_clearExplicitSeekSuppression()'), isTrue,
        reason: 'noteManualReaderNavigation 必须复位显式 seek 抑制窗（BUG-896）');
  });
}
