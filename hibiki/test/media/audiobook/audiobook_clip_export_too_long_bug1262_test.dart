...
void main() {
  ...
  // BUG-1262 第二根因：多句路径判出 tooLong 后，调度层旧写法一个 `return null` 把窗口
  ...
  group('multi-cue tooLong reaches the user (BUG-1262)', () {
    // 用户拖选了 6 分钟的多句选区。
  ...
  group('dispatcher wiring keeps the tooLong window (BUG-1262)', () {
    ...
    test('_buildAudiobookClipPlan 用 audiobookClipPlanRange，而不是裸 return null',
      ...
      expect(
        ...
        isTrue,
        reason: 'BUG-1262：不可导出时若直接 return null，tooLong 与 unsupportedRange '
      ...
      expect(
        ...
        isTrue,
        reason: 'BUG-1262：计划非空时窗口也必须随记录带回，否则 exportable 路径同样会'
            '回落单句锚',
      );
    });
    ...
    test('_exportAudiobookClip 只在计划没窗口时才回落单句锚', () {
      ...
      expect(
        ...
        isTrue,
        reason: 'BUG-1262：窗口必须以多句计划为准，plan 为空不等于「没窗口」——'
