...
class AdminServer {
  ...
  bool _authorized(shelf.Request request) {
    ...
    if (auth != null && auth.startsWith('Bearer ') && _constantTimeEquals(auth.substring(7).trim(), token)) {
      return true;
    }
    ...
    if (cookie != null) {
      for (final String part in cookie.split(';')) {
        ...
        if (kv.startsWith('fushi_admin=') && _constantTimeEquals(kv.substring('fushi_admin='.length), token)) {
          return true;
        }
      }
    }
  ...
  Future<shelf.Response> _login(shelf.Request request) async {
    ...
    final bool secure = securityContext != null;
    return shelf.Response.found('/', headers: <String, String>{
      'Set-Cookie': 'fushi_admin=${Uri.encodeComponent(token)}; Path=/; HttpOnly; SameSite=Strict'
