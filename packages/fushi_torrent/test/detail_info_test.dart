...
void main() {
  ...
  test(
    'TODO-2526：首尾提优在改文件优先级后被重放；priority=0 不被强下',
    () async {
      final EmbeddedTorrentSession? session = EmbeddedTorrentSession.open(
        engine!,
        listenInterfaces: '127.0.0.1:0',
  ...
  test(
    'trackers：本地种子（无 tracker）返回空列表而非报错',
    () async {
      final EmbeddedTorrentSession? session = EmbeddedTorrentSession.open(
        engine!,
        listenInterfaces: '127.0.0.1:0',
  ...
  test(
    '快照新字段：num_seeds/num_connections/时长/is_paused 有值域',
    () async {
      final EmbeddedTorrentSession? session = EmbeddedTorrentSession.open(
        engine!,
        listenInterfaces: '127.0.0.1:0',
      ...
      expect(added.ok, isTrue);
      final FtTorrentStatus status = session.listTorrents().singleWhere(
        (FtTorrentStatus t) => t.id == added.id,
      );
