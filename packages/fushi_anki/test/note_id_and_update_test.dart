...
void main() {
  ...
  group('updateMinedNote reuses the field render (task C1, repository layer)',
      () {
    test('source-less overwrite preserves the old MiscInfo return link', () async {
      final CardSourceLink link = CardSourceLink(kind: CardSourceKind.book,
        uid: 'book-uid', sourceId: '12345678-1234-4234-8234-123456789abc',
        chapterIndex: 3, charOffset: 456, charLength: 8);
      final _RecordingService service = _RecordingService()
        ..existingFields = <String, String>{'Expression': 'old',
      ...
      final MineOutcome outcome = await repo.updateMinedNote(noteId: 888,
        rawPayloadJson: _payload,
        context: const AnkiMiningContext(sentence: '', documentTitle: 'Updated title'));
      expect(outcome.result, MineResult.success);
