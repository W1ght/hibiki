...
/// 图片防剧透遮罩揭开状态持久化（BUG-891）：建表迁移 revealed_images（v45 -> v46）守护。
///
/// 覆盖：
/// ① v45 -> v46 升级：建出 revealed_images，且迁移落在当前 schema 版本（活值断言，
...
/// 的 ladder 步，只有 `if (from < 46)` 的 createTable(revealedImages) 会跑，故最小 seed
...
void main() {
  test('v45 -> v46 creates revealed_images, lands on current version, empty',
      () async {
    ...
    // revealed_images 自 v46 引入；断言下界而非瞬时值，避免后续 bump 拖 stale。
    expect(db.schemaVersion, greaterThanOrEqualTo(46),
        reason: 'revealed_images 自 v46 引入，schema 版本不应回退到其之前');
