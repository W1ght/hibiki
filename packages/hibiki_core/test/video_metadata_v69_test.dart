...
HibikiDatabase _freshDatabase() => HibikiDatabase.forTesting(
...
Future<int> _insertVideoSource(HibikiDatabase db) => db.insertMediaSource(
...
void main() {
  ...
  test('opening the database interrupts a scrape left running by a hard exit',
    ...
    final Directory temp = await Directory.systemTemp.createTemp(
      'hibiki-video-scrape-recovery-',
    );
    addTearDown(() => temp.delete(recursive: true));
    final String databasePath =
        '${temp.path}${Platform.pathSeparator}hibiki.db';

    final HibikiDatabase first = HibikiDatabase.atFile(databasePath);
    ...
    final HibikiDatabase secondary = HibikiDatabase.atFile(
      databasePath,
      isMainProcess: false,
    );
    ...
    final HibikiDatabase reopened = HibikiDatabase.atFile(databasePath);
