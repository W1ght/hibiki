...
void main() {
  group('BUG-899 A: prefs_version concurrency', () {
    test('concurrent setPref never loses a prefs_version increment', () async {
      ...
      await Future.wait(<Future<void>>[
        for (int i = 0; i < writes; i++) db.setPref('bug899_key_$i', 'v$i'),
      ]);
      ...
      expect(await db.getPref('bug899_key_0'), 'v0');
      expect(await db.getPref('bug899_key_49'), 'v49');
    });
    ...
    test('setPref of the version key itself does not double-bump', () async {
      ...
      await db.setPref('bug899_seed', 'x'); // version -> 1
  ...
  group('BUG-899 B: hot-path indexes exist after onCreate', () {
