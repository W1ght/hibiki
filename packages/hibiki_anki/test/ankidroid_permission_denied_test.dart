...
// BUG-823：AnkiDroid 权限（READ_WRITE_DATABASE）未授予时制卡失败没有明显提醒。
//
...
void main() {
  ...
  group('BUG-823: mineEntry classifies AnkiDroid permission failures', () {
  ...
  group('BUG-823: native provider writes are guarded by requirePermission', () {
    ...
    test('addNote case guards with requirePermission before touching provider',
      ...
      expect(
        ...
        isTrue,
        reason: 'BUG-823: addNote must call requirePermission before addNote() '
    ...
    test('addFileToMedia case guards with requirePermission', () {
      ...
      expect(body.contains('requirePermission(result)'), isTrue,
          reason: 'BUG-823: media insert also needs READ_WRITE_DATABASE');
    });
  });
}
