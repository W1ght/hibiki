...
void main() {
  ...
  test(
    'triggerImagePause resume re-reveals current cue via snapReaderToAudio',
    () {
  ...
  test(
    'manual play during image-pause cancels the pause timer and snaps back',
    () {
      ...
      expect(
        body,
        contains('_invalidateImageChapterPause()'),
        reason:
            '手动 play 须成对作废待恢复的图片暂停（Timer + Completer），'
            '否则计时器到点不 snap，且跨章停留的 await 永久挂起',
      );
      expect(
        body,
        contains('isImagePaused'),
