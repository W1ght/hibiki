...
Future<void> _insertVideo(FushiDatabase db, String bookUid, {int? sourceId}) =>
    db.upsertVideoBook(
...
void main() {
  group('schema v77 video metadata', () {
    ...
    test(
      'work owner constraint and stable season/episode replacement',
      () async {
        ...
        expect(
          (await db.getVideoMetadataWorkByCollection(collectionId))!.id,
          tvWorkId,
        );
        ...
        final int seasonId = (await db.getVideoMetadataSeasons(
          tvWorkId,
        )).single.id;
        await db.replaceVideoMetadataSeasons(
          tvWorkId,
        ...
        final List<VideoMetadataSeasonRow> seasons = await db
            .getVideoMetadataSeasons(tvWorkId);
        expect(
        ...
        final int episodeId = (await db.getVideoMetadataEpisodes(
          seasonId,
        )).single.id;
        await db.replaceVideoMetadataEpisodes(
          seasonId,
        ...
        expect(
          (await db.getVideoMetadataEpisodes(seasonId)).single.bookUid,
          isNull,
    ...
    test(
      'provider graph, terms, people, credits and images replace by owner',
      () async {
        ...
        expect(
          (await db.getVideoMetadataRawSnapshots(
            'work:$workId:tmdb',
          )).single.rawJson,
          '{"id":100}',
        );
        ...
        expect(
          (await db.getVideoMetadataTermsForWork(workId)).single.name,
          'Animation',
        );
        ...
        final VideoMetadataCreditRow credit = (await db.getVideoMetadataCredits(
          workId: workId,
        )).single;
        ...
        expect(
          (await db.getVideoMetadataImages(workId: workId)).single.provider,
          'fanart',
        );
      },
    );
    ...
    test(
      'source settings, run and artifact survive/remove with intended FKs',
      () async {
        final int sourceId = await _insertVideoSource(db);
        await _insertVideo(db, 'movie', sourceId: sourceId);
        final int workId = await db.upsertVideoMetadataWork(
        ...
        expect(
          await db.getVideoSidecarArtifacts(sourceId: sourceId),
          hasLength(1),
        );
        ...
        expect(
          (await db.getVideoSourceScrapeRun(runId))!.sourceId,
          isNull,
          reason: 'run audit remains after source removal',
        );
        final VideoSidecarArtifactRow artifact = (await db
        ...
        expect(
          (await db.getVideoBookByBookUid('movie'))!.sourceId,
          isNull,
  ...
  test(
    'opening the database interrupts a scrape left running by a hard exit',
    () async {
      ...
      expect(
        (await secondary.getVideoSourceScrapeRun(runId))!.status,
        'running',
      ...
      final VideoSourceScrapeRunRow recovered = (await reopened
          .getVideoSourceScrapeRun(runId))!;
  ...
  test(
    'v68 -> v77 creates all metadata tables without touching old rows',
    () async {
...
CREATE TABLE video_books (
  ...
  video_path TEXT NOT NULL
      ...
      final List<Map<String, Object?>> rows = await db
          ...
          .get()
          .then(
            (List<QueryRow> result) => <Map<String, Object?>>[
  ...
  test('v77 keeps local extras while online extras are reconciled', () async {
    ...
    final List<VideoMetadataExtraRow> extras = await db.getVideoMetadataExtras(
      workId,
    );
    expect(
      extras.map((VideoMetadataExtraRow row) => row.extraKey),
