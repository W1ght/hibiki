...
mixin _FushiDbContentMisc
  ...
  Stream<List<String>> watchEpubBookKeys() =>
      (selectOnly(
        epubBooks,
      )..addColumns(<Expression<Object>>[epubBooks.bookKey])).watch().map(
  ...
  ///
  /// v100：碑按 [profileId]（null = 当前激活 Profile）分区。
  ...
  /// （重复删只抬高、绝不倒退）。
  ///
  /// v100：只删 [profileId]（null = 当前激活 Profile）的段、只立该 Profile 的碑。
  ...
  ///
  /// v100：只清 [profileId]（null = 当前激活 Profile）的段与碑。
  Future<int> clearStudySegments(String mediaKind, {int? profileId}) =>
      transaction(() async {
        ...
        return (delete(studySegments)..where(
              (t) => t.profileId.equals(scope) & t.mediaKind.equals(mediaKind),
            ))
            .go();
      });
  ...
  /// 返回改写的行数。
  ///
  /// v100：只写零当前激活 Profile 的段（时段明细里看到的就是这一份）。
  ...
  Future<int> zeroStudySegmentsByUids(Set<String> uids) {
    ...
    return (update(studySegments)..where((t) => t.uid.isIn(uids))).write(
      StudySegmentsCompanion(
        durationMs: const Value(0),
        chars: const Value(0),
        pages: const Value(0),
  ...
  Future<void> deleteStudySession({
    ...
    if (gameSessionId != null) {
      await (delete(
        galgameSessions,
      )..where((t) => t.id.equals(gameSessionId))).go();
    }
  });
  ...
  Future<void> deleteStudySessions({
    ...
    if (gameSessionIds.isNotEmpty) {
      await (delete(
        galgameSessions,
      )..where((t) => t.id.isIn(gameSessionIds))).go();
    }
  });
  ...
  Future<void> updateStudySession({
    ...
    for (final StudySegmentsCompanion row in segments) {
      await (update(
        studySegments,
      )..where((t) => t.uid.equals(row.uid.value))).write(row);
    }
    for (final StudySegmentsCompanion row in insertSegments) {
      await into(
        studySegments,
      ).insertOnConflictUpdate(await _stampStudySegmentProfile(row));
    }
    if (gameSessionId != null &&
        gameStartMs != null &&
        gameEndMs != null &&
      ...
      await (update(
        galgameSessions,
      )..where((t) => t.id.equals(gameSessionId))).write(
  ...
  Future<void> deleteStatFactsOnDays({
    ...
    // v100：游戏骨架行按当前 Profile 删；legacy 行只有归属 Profile 看得见，
  ...
  ///
  /// v100：v92 段只清当前激活 Profile 的；legacy 家族与计数面没有 Profile 维度，
  ...
  ///
  /// v100：同 [clearAllReadingStatistics]——段按当前 Profile 清，legacy / 计数面
