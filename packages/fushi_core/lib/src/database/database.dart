...
class FushiDatabase extends _$FushiDatabase
  ...
  Future<void> _replayColumnStepsForDriftedSchema(Migrator m) async {
    ...
    final bool renameTag =
        await _tableExists('book_tag_membership_tombstones') &&
  ...
  MigrationStrategy get migration => MigrationStrategy(
    onUpgrade: (m, from, to) async {
      ...
      if (from < 100) {
        // v100（统计按 Profile 隔离）：`study_segments` / `galgame_sessions`
        ...
        if (await _tableExists('study_segment_tombstones') &&
          ...
          await customStatement('''
          CREATE TABLE study_segment_tombstones_v100 (
          ...
          await customStatement(
            'INSERT OR IGNORE INTO study_segment_tombstones_v100 '
          ...
          await customStatement(
            'ALTER TABLE study_segment_tombstones_v100 '
