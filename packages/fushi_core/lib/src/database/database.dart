...
class FushiDatabase extends _$FushiDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
        onUpgrade: (m, from, to) async {
          ...
          if (from < 76) {
            // 本地视频分支曾在 schema 69-71 占用与上游 Fushi 改名迁移相同的
            // 版本号。来自该分支的 v69/v70/v71 数据库会跳过部分同号上游迁移，
            // 因此在 v76 按来源版本精确补跑。所有操作均有表守卫或天然幂等，来自
            // 正常上游版本的数据库重复经过也不会丢数据。
            if (from >= 69 && from <= 71) {
              if (await _tableExists('hibiki_paired_peers') &&
                  !await _tableExists('fushi_paired_peers')) {
                await customStatement(
            ...
            // v76 同时收拢本地 v69-v71 的视频元数据、来源刮削和通用下载真相源。
            // 每张表独立守卫，可兼容上游 v75、旧 develop 和开发期半迁移数据库。
            final List<TableInfo<Table, Object?>> videoTables =
            ...
            for (final TableInfo<Table, Object?> table in videoTables) {
