...
# BUG-1492: Magpie has no download path left, so the bundled slim archive is the
...
foreach ($relativePath in $magpieBundleWorkflows.Keys) {
  ...
  Require-Text $relativePath $content 'tools/build_magpie_slim.ps1' 'Windows bundles must build the slim Magpie archive from the in-tree script (BUG-1217/BUG-1492)'
  Require-Text $relativePath $content $entry.Payload 'Windows bundle payload must contain magpie_bundle/ or window upscaling can never install (BUG-1492)'
  Require-Text $relativePath $content 'Magpie-hibiki-slim-x64.zip.sha256' 'the bundled Magpie archive must ship with its sha256 sidecar; the installer refuses an unverified package (BUG-1492)'
}
