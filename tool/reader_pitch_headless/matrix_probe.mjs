...
async function run(kind, dir, action){
  ...
  const lead = kind==='image' ? `<img id="lead" loading="lazy" src="https://hoshi.local/lead.png" alt="">` : '';
  ...
  pg.on('request',async q=>{const u=q.url();if(u==='https://hoshi.local/chapter')return q.respond({status:200,contentType:'text/html',body:full});if(u==='https://hoshi.local/lead.png'){await ready;return q.respond({status:200,contentType:'image/png',body:bigPng});}if(u.startsWith('https://hoshi.local'))return q.respond({status:404,body:''});return q.continue();});
  ...
  const before=await pg.evaluate(()=>{const r=window.hoshiReader;const c=r.getScrollContext();return{scroll:r.getPagePosition(c),pageStep:c.pageSize};});
  const info=await pg.evaluate((action)=>{const r=window.hoshiReader;const c=r.getScrollContext();const sb=r.getPagePosition(c);const fvco=r.getFirstVisibleCharOffset();
  ...
  const after=await pg.evaluate(()=>{const r=window.hoshiReader;const c=r.getScrollContext();return{scroll:r.getPagePosition(c),pageStep:c.pageSize};});
  await b.close();
