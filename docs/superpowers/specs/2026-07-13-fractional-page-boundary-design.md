...
`pitch=564.490967`，I1/I2/I3/I4/I6 全部通过，因此 BUG-787 的产品累计漂移未复现。
...
实现完成后只更新 BUG-788 状态并重建 bug 索引。不得修改分页 CSS、`getScrollContext()`、
pagination metrics、章节切换、BUG-787 harness，也不得带入任何旧分支的其它 57 个文件。
...
4. **误把 BUG-787 探针修复混入产品修复**：`a6c9904d0` 保持独立；本实现不再改 harness。
...
- 最终产品提交只含上述 3 个代码/测试文件；BUG-788 文档完成另作 docs 提交。
