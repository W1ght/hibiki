...
- 只修改 `reader_pagination_scripts.dart`、`reader_paginate_step_test.dart`、`reader_paginate_js_guard_static_test.dart` 与 BUG-788 完成记录。
- 页边界容差固定为现有语义的 1px；不得随页数累计或扩大。
- 不修改分页 CSS、`getScrollContext()`、pagination metrics、章节切换或 BUG-787 harness。
- 不纳入任何旧分支的其它 57 个文件。
...
Expected: commit 只含上述 3 个文件，不含 `hibiki/macos/Podfile.lock`、BUG-787 harness 或旧分支文件。

---

### Task 2: 在真实 macOS WKWebView 复验并完成 BUG-788

**Files:**
- Verify: `hibiki/integration_test/reader_pagination_test.dart`
- Modify: `docs/bugs/BUG-788-fractional-page-boundary-premature-limit.md`
...
- [ ] **Step 2: 更新 BUG-788 完成证据**

在 `docs/bugs/BUG-788-fractional-page-boundary-premature-limit.md` 中：

- 将 `[ ] ①` 改为 `[x] ①`，记录 Task 1 的真实提交哈希及 JS/Dart 修改位置。
- 将 `[ ] ②` 改为 `[x] ②`，记录两个精确 TDD 用例、JS 源码守卫、聚焦测试结果和 macOS
  WKWebView 全章扫描结果。
- 保留 BUG-787 为「探针假阳性」，不得把本 bug 改名为累计漂移。
...
git add \
  docs/BUGS.md \
  docs/bugs/BUG-788-fractional-page-boundary-premature-limit.md
git diff --cached --check
git commit -m "docs(bug): close fractional page boundary limit"
```

Expected: docs commit 只含 BUG-788 与自动索引；`hibiki/macos/Podfile.lock` 仍未暂存。
