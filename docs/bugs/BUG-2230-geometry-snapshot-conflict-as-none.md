## BUG-2230 · Windows宿主将快照读取冲突误报为没有原生查词provider
...
- bug 工具：`dart run tool/bug.dart new geometry-snapshot-conflict-as-none ...` 自动分配 BUG-2230（扫描 93 个 ref、5 个工作区）；记录后执行 `dart run tool/bug.dart reindex` / `check`。
- 索引检查：`check` 退出码 0，本地不变式/索引通过；跨分支历史占号有 84 条警告，本次 BUG-2230 未撞号。完整输出在同一证据目录 `bug-check.log`；本次不改动无关历史编号。
