...
- **[x] ② 已加自动化测试** — `test/lookup/browser_extension_page_scroll_bug732_test.js`（Node 真执行 popup.js 的 wheel 监听：扩展宿主页不 preventDefault/不 window.scrollBy；扩展弹窗内滚 host；in-app 弹窗仍 window.scrollBy）+ `test/lookup/browser_extension_page_scroll_bug732_test.dart`（node 驱动 + 源码守卫 + 三镜像字节一致）。移除守卫行后 Case A 变红，已本地验证。
- **备注**：BUG 号取 732。730 已被剪贴板制卡 PR#36 占用（已并入 develop），731 被并发落地的同步/视频备份 PR#37 占用，故本 PR（PR#38）落地时改到 732，避免与并发分支撞号。
