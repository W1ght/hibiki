## BUG-950 · 9nine 的 Luna 线程列表缺少 TextRender
...
- **[x] ① 已修复** — 提交 `7653cbfbe` 将共享内存文本环扩展为事件环；合入 develop 后协议统一为 v10。injector 独立发布 Luna `ThreadCreate` 事件，Windows runner/Dart 完整透传事件类型，控制器把发现事件登记到独立线程目录而不伪造空台词。线程选择器现在可先显示 `TextRender · 0xf94600 · 0`，真实输出到达后再累计行数。
