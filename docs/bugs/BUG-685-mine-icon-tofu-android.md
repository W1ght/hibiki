## BUG-685 · 手机制完卡后制卡图标✓乱码/豆腐（Android WebView 缺符号字体）
...
- **[x] ① 已修复** — 根因修（保留用户要的 ✓/✓↩︎ 文本标记）：
  ...
  - 提交：`e714731bc`
- **[x] ② 已加自动化测试** —
  - `hibiki/test/dictionary/popup_cards_nav_icon_guard_test.dart` 新增「内嵌 Hibiki Symbols 字形子集（TODO-1368/BUG-685）」组：对 5 份 CSS 镜像（3 份 popup.css + 2 份生成的 content.css）断言 ①`.mine-button` 栈序=桌面字体→Hibiki Symbols→sans-serif；②`@font-face` data:URI 存在且用独立 Dart WOFF/cmap format-4 解析器验证内嵌字体**真含** U+2713/U+21A9/U+FE0E 字形（不是只查字符串）。
  - 提交：`e714731bc`
