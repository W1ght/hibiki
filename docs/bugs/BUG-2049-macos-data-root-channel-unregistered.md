## BUG-2049 · macOS 更改数据位置失败：data_root_access 通道未注册
...
- **[x] ② 已加自动化测试** — `fushi/test/storage/macos_data_root_bookmark_guard_test.dart` 新增源码守卫：掩掉注释后钉住主窗包装结构、内部 Flutter controller 注册顺序，并禁止恢复顶层 `FlutterViewController` 强转。相关守卫 16 项通过，`flutter build macos --debug --no-pub` 成功。
