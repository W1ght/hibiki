...
- **[x] ① 已修复** — `147ff99e5f`：面板 initState 读回合集的规范作品与身份（`_loadCanonicalIdentity`）：用户未改过查询词时替换为日文原名；请求带 originalTitle / tmdb id、mediaKind 取刮削结论、合集未绑 AniList 而身份里有 anilist id 时直接按 id 搜。另：刮削后字幕补齐（`video_subtitle_backfill.dart`）把刮削出的别名传入 `alternateTitles`，Jimaku 文本兜底链不再只剩显示名。
