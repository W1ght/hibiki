## BUG-2183 · 聚合同步/备份不按 mediaKind 过滤，游戏段跨端外流
...
- **[x] ① 已修复** — 新增 `AggregateMergeService.isStudyKindSyncable`（`mediaKind != kActivityMediaGame`），聚合导出、并集仲裁、落地（旧端直落的游戏段也丢）、备份 ATTACH 四处同一判据过滤段与碑。 提交：待填。
