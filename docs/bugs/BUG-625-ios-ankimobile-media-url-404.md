...
- **备注**：这是 BUG-624 之后暴露出来的第二段链路问题：`.m4a` 让 AnkiMobile 开始识别并抓取媒体 URL，随后才撞到源文件被过早清理的 404。
