...
- **[x] ① 已修复** — 把自定义属性的兜底声明从 `html, body` 拆成只写在 `html` 上（非自定义属性仍留在 `html, body`）；重跑 `node tools/browser-extension/scripts/generate-content-css.mjs` 同步两份 `vendor/content.css`（生成结果行为等价：同一元素、同一顺序）。提交：见下方 commit。
