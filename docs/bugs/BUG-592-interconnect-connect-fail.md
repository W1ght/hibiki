## BUG-592 · 互联LAN token成功仍连失败+公网无pin
...
- `PIN-required approval fires at CREATE not confirm (BUG-592)`：断言 pinRequired 会话在 `/api/pair/v2` 时 host 审批（=PIN 显示）即被调用（`pinRequired=true`），confirm 正确 proof 不再二次弹审批仍派 token。
- `PIN-free approval fires at CONFIRM not create (BUG-592)`：免 PIN 会话 CREATE 阶段不弹审批，仍在 confirm 弹（行为不变）。
- `PIN-required host declines at CREATE yields 403 declined (BUG-592)`：pinRequired 会话 host 在 CREATE 拒绝 → 403 declined。
