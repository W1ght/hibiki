...
- **报告**：2026-08-01（用户截图：首页 dashboard 上焦点环悬在「继续」与「最近添加」两个区块之间的空白处，与任何卡片几何都对不上；与 BUG-1299 同一轮「焦点高亮错误」反馈）
...
- **备注**：与 BUG-1299（视频页隐形 chrome 可聚焦）同轮报告但根因独立：1299 是「焦点落在不可见控件上」（焦点所有权问题），本条是「矩形过期」（几何派生状态问题）。两修复叠加后：环只会画在可聚焦的可见控件上、且始终贴住其实时几何。
