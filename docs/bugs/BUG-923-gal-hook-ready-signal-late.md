## BUG-923 · Galgame helper 已注入但 ready 信号过晚导致 engine_attach_failed
...
- **备注**：2026-07-19 真机复测通过。保存并重启 anemoi 后，新 x86 helper 对 PID 25844 完成注入并保持 hold；Hibiki 从 `degraded / engine_attach_failed / Loopback` 切换为 `waitingSignals / 引擎 PCM / 44100 Hz / 1 ch / 16 bit`，证明 ready、IPC 与引擎音频源均已建立。`waitingSignals` 仅表示尚未收到游戏台词。worktree bootstrap 的 pub-cache 补丁阶段因本机 WSL 不可用失败，但本次目标 Flutter 测试、x86/x64 C++ 构建、CTest 与上述真机路径均已独立通过。
