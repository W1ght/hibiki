...
- **[x] ① 已修复** — 整理器新增 `_isExplicitExtra()`：目录仍走既有宽词表，文件名复用元数据链路的 `classifyLocalVideoExtra()` 严格 token 判据，在解析集号前排除 `NCOP` / `NCED` / `creditless OP|ED` / `PV` 等明确附件。真正的同集 v1/v2 仍保留冲突失败；纯特典种子的二次旧口径回退契约不变。
- **[x] ② 已加自动化测试** — `fushi/test/media/video/download/video_download_organizer_test.dart` 新增用户截图同形用例：平铺的 `24 END` 进入 `Season 01/S01E24`，`24 NCED Version` 进入 `Extras` 且不携带集号。
- **备注**：
