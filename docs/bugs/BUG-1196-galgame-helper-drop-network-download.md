...
- **[x] ① 已修复** —
  ...
  - 当时 `magpie_installer.dart` 仍保留独立下载路径；该临时边界已被后续
    [BUG-1292](BUG-1292-magpie-bundled-only.md) 取代，当前 helper 与 Magpie 都只认主包随附归档。
- **[x] ② 已加自动化测试** — `hibiki/test/mining/galgame_helper_no_network_guard_test.dart`（原 `galgame_helper_background_update_guard_test.dart` 反转重写）：守卫安装器不得出现任何网络通道，不得再有 `updateInstalledHelpersInBackground` 且 `main.dart` 不再调用，随包归档仍是唯一来源且校验/清单复检两道门都在。BUG-1292 后该守卫也覆盖 Magpie，且判据从字面量黑名单（`HttpClient` / `https://` …）升级为 `offline_installer_guard.dart` 的 import 白名单 + 折叠拼接后的能力名扫描 —— 旧判据挡不住 `import 'package:http/http.dart'` + `Uri.parse('htt' 'ps://…')`。`galgame_helper_installer_test.dart` 719 → 286 行（删掉 URL 构造 / 镜像候选 / 自更新判据 / 真实 HTTP 侧车抓取 / zip 下载等已无对象的组）；`galgame_helper_launch_guard_test.dart` 的「确认对话框时序」整组随之移除并留注释指向新守卫。
