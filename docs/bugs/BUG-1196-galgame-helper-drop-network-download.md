...
- **[x] ① 已修复** —
  ...
  - `magpie_installer.dart` 此前复用了 helper 的镜像候选与可信主机白名单；helper 不再联网后 Magpie 成为**唯一**按需下载组件，这些符号随之落回 Magpie 自己（`kMagpieProxyPrefixes` / `magpieDownloadCandidateUrls` / `magpieIsTrustedSidecarUrl` 实体化）。**Magpie 的下载保留** —— 它是独立缩放程序，不注入任何进程，与 helper 的语义完全不同。
- **[x] ② 已加自动化测试** — `hibiki/test/mining/galgame_helper_no_network_guard_test.dart`（原 `galgame_helper_background_update_guard_test.dart` 反转重写）：守卫安装器源码里不得出现 `HttpClient` / `applyUpdateProxy` / `ResumableDownload` / `https://`，不得再有 `updateInstalledHelpersInBackground` 且 `main.dart` 不再调用，随包归档仍是唯一来源且校验/清单复检两道门都在，并额外守住「Magpie 仍可联网」防止有人顺手一起删。`galgame_helper_installer_test.dart` 719 → 286 行（删掉 URL 构造 / 镜像候选 / 自更新判据 / 真实 HTTP 侧车抓取 / zip 下载等已无对象的组）；`galgame_helper_launch_guard_test.dart` 的「确认对话框时序」整组随之移除并留注释指向新守卫。
