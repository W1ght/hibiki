...
- **[x] ① 已修复** — `482174f7f5` 先封顶首页活动渲染/封面解码负载；`e4387f215c` 新增 `FushiScrollController`，在真正消费 pointer delta 的 `ScrollPosition` 边界只对 Windows/Linux 绝对值 ≥80 的粗事件减半、单事件封顶 120px，小 delta 与 macOS/移动端保持 1:1。首页各 tab 和共享 `FushiPageScaffold` 均接入，覆盖首页、书架、视频、词典及普通二级页面的主纵向滚动。
- **[x] ② 已加自动化测试** — `e4387f215c`：`fushi/test/utils/misc/desktop_scroll_physics_test.dart` 覆盖正反方向、粗事件减半、巨 delta 封顶、小 delta 原样；`fushi/test/widgets/fushi_page_scaffold_scroll_test.dart` 从真实鼠标 `PointerScrollEvent` 验证主滚动位置。原首页活动分页测试继续保留。
