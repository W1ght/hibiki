...
- **[x] ① 已修复** — 活动预取与渲染解耦：首批只构建 24 条，保留日期分组并按按钮每次再展开 24 条；切换筛选后复位首批数量。活动小封面统一用 192px 解码上限，避免 40×56 / 68×40 槽位继续承担通用 720px 解码与缓存抖动。
- **[x] ② 已加自动化测试** — `fushi/test/pages/activity_feed_test.dart` 覆盖跨日期截断、顺序保持、完整聚合结果不被修改，以及非正上限零构建。
