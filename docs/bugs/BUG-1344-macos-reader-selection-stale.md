...
- **[x] ① 已修复** — `abb73c300`：整栈关闭改走异步收尾，先 `await _clearSelectionJs()` 真正清除原生 Range、CSS Highlight 与内部 selection，再归还 reader 焦点；await 后复查 `mounted` 和 `isDictionaryShown`，旧会话不会抢新查词弹窗的焦点。查词弹窗仍打开时保留原文锚点，未改变连续查词、收藏或制卡语义。
