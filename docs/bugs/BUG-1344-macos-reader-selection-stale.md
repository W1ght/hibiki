...
- **[x] ① 已修复** — `c3796c723`, `6b048241c`：整栈关闭先 `await _clearSelectionJs()` 再归还 reader 焦点；await 后复查 `mounted`、弹窗可见性及 `activeLookupGeneration`，连“新查词已开始但弹窗尚未显示”的窗口也不会被旧会话抢焦点。原生右键/系统菜单查词则先采完句子与夹图状态、在打开弹窗前立即清原生 selection，避免失焦灰块从一开始就出现。
