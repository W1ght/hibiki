## BUG-2189 · 导航失败/兜底超时 discard 丢掉用户真读过的上一页
...
- **[x] ① 已修复** — `_beginNavigation` 在 loadUrl 之前 `leave()` 结算跳走前那页（见 BUG-2188），`_failNavigation` 的 `discard()` 此后只兜「导航发起后新页曾短暂 arrive」的空状态。
