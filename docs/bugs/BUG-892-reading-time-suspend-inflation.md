...
- **纵轴格式器**（`stat_charts.dart` `formatStatDurationAxis`）：小时量级非整点保留一位小数
  ...
  存量脏数据的防御。
- **Daily avg 口径对齐**（问题 7）：`_buildKpiStrip` 的日均字数旧实现 = 总字数 ÷ 历史全部活跃天
  （终身均值），被历史低产/零星活跃日拉低，与同屏「今日 / 本周 / 30 天字符图」（都近期口径）
  对不上、显得偏低。新增纯函数 `dailyAverageChars(_dailyData)`（stat_summary.dart）——与字符图
  **同一 30 天窗口**、按**活跃日**（chars>0）取均值，正是字符图里非零柱的平均高度，直观可对齐；
  删掉不再使用的 `_activeDays` 字段。
- 提交：见下方 commit。
