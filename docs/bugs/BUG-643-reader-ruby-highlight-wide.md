...
- **备注**：
  - 2026-07-10 更新（BUG-689/TODO-1371）：本条给 active 普通 cue span 加的 `line-height: 1 !important` 被证实会在书籍 CSS strut < 1em 时改写行盒厚度、竖排下逐句平移后续列，已移除；1em 窄条宽度实际由 `background-size`（绘制层）保证，与 line-height 无关，本条「高亮条变宽」不回归（守卫仍在 reader_content_styles_test / ruby_highlight_guard_test）。
