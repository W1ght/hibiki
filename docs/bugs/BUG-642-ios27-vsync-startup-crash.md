## BUG-642 · iOS 27 真机启动在 Flutter VSyncClient 崩溃
- **报告**：2026-07-06（用户：）
...
- **[x] ① 已修复** — 最初修复为 `ios/Runner/Info.plist` 将
  `CADisableMinimumFrameDurationOnPhone` 显式设为 `false`，避开 iOS 27 beta 的 Flutter
  VSyncClient 崩溃路径；BUG-647 后改为源 Info.plist 仍保持 `<false/>`，最终 app plist
  由 `Thin Binary` build phase 按构建配置写入：Debug=`false`，Profile/Release=`true`。
  Debug 继续规避崩溃，实机测试/发布包恢复高刷新率 opt-in。
- **[x] ② 已加自动化测试** — `hibiki/test/ios/uiscene_startup_guard_test.dart` 增加静态守卫：
  源 Info.plist fallback 必须保持 `<false/>`，`Thin Binary` 脚本必须在 `embed_and_thin`
