...
- **[x] ① 已修复** — `03180bae1f`：请求时捕获控制器、导航代际、章节；返回后复核并检查恢复/歌词状态，在解析、位置更新、账本与落库之前丢弃过期结果。章节单独复核覆盖设置重载先变章节再递增代际的窗口。音频优先由复用的 `773d4372a0`（原 `b83831166a`，BUG-2384）处理。
