## BUG-1263 · 选区与字幕文本不一致时长选区导出退化为单句音频
...
- **[x] ② 已加自动化测试** —（见本分支提交）
  - `hibiki/test/media/audiobook/audiobook_clip_mismatch_window_bug1263_test.dart`：结构守卫钉住①一致性判定之后计划构建器不得再出现 `return null;`（丢窗即红）且判定结果以 `cueTextMatches:` 进计划；②dispatcher 用 `!dynamicPlan.cueTextMatches` 门 + `dynamicPlan = null;` 只降级高亮，且 `sentenceRange` 仍取计划整段起止；③计划模型带 `final bool cueTextMatches;` 契约字段。
