...
- **[x] ① 已修复** — 与 BUG-2176 同一改动：墓碑永不退场（live = 全部合并后的碑，本机碑与快照碑同集，不再「只进不出」）；压制判据改为段的 `startAt`（删除时刻附近那一段的归属才受 skew 影响，不会整块翻转）；`upsertStudySegmentTombstone` 收敛为唯一写碑入口，只在 `deletedAt` 更大时覆盖（碑戳只增不减），本机删除与同步 / 备份落地都经它。 提交：待填。
