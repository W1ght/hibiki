## BUG-2172 · 手动暂停计时期间翻页仍 addChars 产出 0 时长字数段
- **报告**：2026-09-06（用户：）
...
- **[x] ① 已修复** — 提交 待填；在 `StudyClock` 层修：`addChars` / `addPages` 在 `!isRunning` 时直接 return（停表 = 不在学习，字数页数一并不计；水位照常由页面推进，这些字之后也不会补计——有意）。漫画页同形变体由此一并解决，不改漫画页。原「无段时以 0 时长开段」用例改成 start 后测。
