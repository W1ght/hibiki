...
- **[x] ① 已修复** — 新墓碑语义（BUG-2176：只压制 `startAt < deletedAt` 的段）下立碑不再毒化身份：`clearStudySegments` 改为同一事务里 `SELECT DISTINCT media_key` 逐身份 `upsertStudySegmentTombstone(deletedAt = now)` 再删行；`clearAllReadingStatistics` / `clearAllVideoStatistics` 的文档改准确（legacy 家族仍不立 title 碑）。 提交：`bef9747e30`。
