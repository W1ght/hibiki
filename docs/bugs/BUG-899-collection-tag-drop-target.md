## BUG-899 · 标签拖到合集行头无接收（合集打标签仅详情页按钮入口）
...
- **[x] ③ 追加：合集行展示标签 chip** — 用户复报「添加了看不见」：PR#128 只在合集**详情页**展示标签 chip，书架/视频列表里的合集**行**从不显示。补：DB `getAllCollectionTagMappings`（`database.dart`）+ 批量 `collectionTagMapProvider`（`tag_filter_sheet.dart`，`Map<collectionId, List<BookTagRow>>`，与 `bookTagMapProvider` 同形）；`CollectionShelfRow` 加 `tags` 参数，行头下方 `_buildTagChips`（`HibikiTagChip`+Wrap，与详情页同款，空则不占位）；两调用点 `ref.watch(collectionTagMapProvider).valueOrNull?[collection.id]` 传入，打标签后同时失效该 provider 即时刷新。测试补「tags 非空展示 chip / 为 null 不占位」2 例。提交：09ab9e695

- **备注**：合集打标签的持久化/同步（`CollectionSyncEngine` 并集透传、备份合并 `_mergeCollectionTags`）与详情页展示均由 PR#128 提供，本修复补「拖放」交互入口 + 「列表行标签 chip 展示」，与已有「详情页 AppBar 按钮」入口等价。已知限制：经详情页 AppBar 按钮增删标签后返回列表，`collectionTagMapProvider` 未在详情页失效（与既有 `filteredCollectionIdsProvider` 同限制），拖放路径已即时刷新。真机验收：视频/书架合集行头拖标签→toast→穿 DB（详情页标签行可见）、标签栏过滤合集卡显隐、重复拖同标签提示「已在此合集上」、回归普通书/视频/成员卡拖标签仍有效。
