...
- **[x] ③ 追加：合集行展示标签 chip** — 用户复报「添加了看不见」：PR#128 只在合集**详情页**展示标签 chip，书架/视频列表里的合集**行**从不显示。补：DB `getAllCollectionTagMappings`（`database.dart`）+ 批量 `collectionTagMapProvider`（`tag_filter_sheet.dart`，`Map<collectionId, List<BookTagRow>>`，与 `bookTagMapProvider` 同形）；`CollectionShelfRow` 加 `tags` 参数，行头下方 `_buildTagChips`（`HibikiTagChip`+Wrap，与详情页同款，空则不占位）；两调用点 `ref.watch(collectionTagMapProvider).valueOrNull?[collection.id]` 传入，打标签后同时失效该 provider 即时刷新。测试补「tags 非空展示 chip / 为 null 不占位」2 例。提交：c24298b26
