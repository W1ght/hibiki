...
- **[x] ① 已修复** — 见提交：四处 CSS 与 Dart 代数镜像一起去掉 F；列高 = V − mt − mb − cT − cB，与 padding 逐项镜像（TODO-729 的 `column-width == content-box`、pageStep == realPitch 恒等式不变），TODO-743 的 `max(F, …)` 坍塌地板保留。`tool/reader_pitch_headless/` 两个自带 CSS 复刻的探针同步。
