...
- **[x] ① 已修复** — `moveSelectionHandle` 在 hit-test 前把两个手柄临时 `pointer-events:none`、取字后还原（`savedStartPe`/`savedEndPe`），使手指坐标穿透到底下字符。全程仍只改 `this.selection` + CSS Custom Highlight，绝不碰原生选区（不复活 TODO-1279 双选区）。提交：<待填>
