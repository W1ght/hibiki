...
- **[x] ① 已实现** —
  ...
  - `magpie_installer.dart`：新增 `kMagpieBundledDirectoryName = 'magpie_bundle'`（与安装落点 `magpie/` 刻意不同名）、`magpieBundledZipName`（带 `slim`）、`_installBundledMagpie`。随包归档仍做 SHA-256 校验：随包不等于可信，主包本身可能被改。最初暂留的网络兜底已由后续 [BUG-1492](BUG-1492-magpie-bundled-only.md) 删除。
