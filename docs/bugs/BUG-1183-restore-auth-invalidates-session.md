## BUG-1183 · restoreAuth 无条件作废已解析地址，每次切页面重跑全候选探测
...
- **真实性**：✅ 真 bug。根因在
  ...
  **无条件** `_sessionResolved = false`，而 `_loadConfig` 是 `restoreAuth`
  ...
  这是 [BUG-1180](BUG-1180-interconnect-remote-list-no-cache.md)「切页面重拉」的
...
- **备注**：与 [BUG-1180](BUG-1180-interconnect-remote-list-no-cache.md) /
  [BUG-1181](BUG-1181-manga-shelf-fetches-remote-books.md) /
  [BUG-1182](BUG-1182-show-remote-entries-gate-too-late.md) 同一轮排查。
