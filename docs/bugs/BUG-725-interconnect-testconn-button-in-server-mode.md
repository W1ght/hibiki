...
- **[x] ② 已加自动化测试** — `hibiki/test/sync/interconnect_pairing_fixes_guard_test.dart` 新增源码切片守卫「BUG-725：服务端模式隐藏「测试连接」按钮」：断言 `t.sync_test_connection` 全语料仅一处、且其前存在 `if (!lockedByServer) ...<Widget>[` 门控（`guardIdx < buttonIdx`）。这类私有客户端 UI widget 无低成本 widget 测试可落地，沿用同文件既有源码守卫风格。
- **备注**：与 PR#28（多基字注音重叠）撞 722——722 已由 PR#28 占用，integration owner 落地时把本条改号为 725（724 已给 PR#30 有声书插图）。
