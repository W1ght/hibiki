...
- **[x] ① 已修复** — `global_lookup_host.js`：
  ...
  提交：5e51a48fb
- **[x] ② 已加自动化测试** — `hibiki/test/lookup/clipboard_panel_guard_test.dart` 新增 group「面板栏图钉/关闭可见（BUG-768 暗背景不可见）」三钉：① renderStack 把主题转写到面板栏 `data-theme`；② `.panel-btn` 规则块到 `:hover` 之间必含 `background:`（常驻 chip，防退回只 hover）；③ 暗主题浅色变种存在。全 23 例通过。提交：5e51a48fb
