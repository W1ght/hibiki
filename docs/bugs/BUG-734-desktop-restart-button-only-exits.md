...
- **备注**：同轮修 [BUG-733](BUG-733-overwrite-import-rename-access-denied.md)（覆盖导入换名失败）。潜在风险：若 app 有单实例锁，detached 新实例与旧进程退出之间有极短竞态；实测 Hibiki 桌面无强单实例锁。真机复测：导入完成点「立即重启」应自动重开 app。
