## BUG-801 · 安卓视频原生SubtitleView与可点浮层字幕重复(控制条显示时上下两条)
...
- **备注**：本条与草稿 PR#94 的 BUG-790 撞号，已手动改 801（合并时与 develop 既有 BUG-791 再撞号，integration owner 改 801）。**待真机复测原始失败路径**（安卓 + 视频旁有同名 sidecar .ass + 显示控制条，确认只剩一条字幕；并验证 PGS 图形字幕不回归）。
