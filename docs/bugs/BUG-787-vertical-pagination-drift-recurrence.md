...
- **真实性**：❌ **当前 develop 未复现原「越翻越偏」产品问题；旧 I1 报错是测试探针
  ...
  - 修正探针及后续 BUG-789 章尾终点后在真实 macOS WKWebView 复跑；最终 raw
  ...
  - 本次复跑另发现 I5 在 page 31 提前停止且仍余 17426px 内容；这是同步 `paginate`
    边界误判的独立问题，已另记 BUG-788，不能回写成累计漂移。
- **[x] ① 产品无需新增修复** — 原累计漂移在保留原始小数精度后未复现；本条未修改
  `reader_pagination_scripts.dart` 或分页 CSS。产品侧后续只处理 BUG-788 的提前 `limit`，
...
- **[x] ② 已修复探针并加自动化测试** — `a6c9904d0`（`test(reader): preserve
  ...
  浏览器 floor 后 30 页不误报、绝对 `N × pitch` 网格、真实整数步长漂移仍能报错、JS 不再取整
...
- **备注**：文件路径保留 bug 工具实际生成的
  `docs/bugs/BUG-787-vertical-pagination-drift-recurrence.md`，标题按最终验真结论修正。
