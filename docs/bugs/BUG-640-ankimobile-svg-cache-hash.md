## BUG-640 · iOS/Anki 制卡外字 SVG 偶发读不到（词典媒体缓存名使用 String.hashCode）
...
- **[x] ① 已修复** — `ankiDictionaryMediaCacheFilename` 改成 `hibiki_dict_<sha1(utf8(path))>.<ext>`，保留无扩展名回退 `bin`。writer 和三个 backend 仍共用同一个 helper，修的是命名真源，不在各端加特例。提交哈希：4e421af0a。
