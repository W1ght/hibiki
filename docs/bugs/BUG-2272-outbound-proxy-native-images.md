...
- **剩余边界**：Cloudflare 交互验证 WebView 仍使用平台浏览器网络配置，其手动代理未由本轮 HTTP 适配覆盖；不通过全局 WebView override 干扰并发阅读器。Android `compileDebugKotlin` 已通过，完整 APK/Apple 包未构建；libmpv 验证为 HTTP 音频加载，HTTPS 播放仅验证到 CONNECT 真 socket 通道。
