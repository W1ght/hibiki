...
- **[x] ① 已修复** — `abb73c300`：macOS 阅读器增加 28pt `DragToMoveArea` 标题栏带，复用 `kMacTitleBarHeight`；正文分页 inset 和词典弹窗顶部避让共享 `_macosWindowTitlebarInset`，其它平台恒为 0。没有改变用户窗口 frame，也没有把“开书”擅自改成原生 Spaces 全屏。
