## BUG-323 · TODO-569 视频字幕描边/残留黑字「一点没修好」（8 层模糊 Shadow glyph 拷贝伪描边）
...
- **[x] ① 已修复**（提交：`ddcbf8624`，rebase 到 develop@1b88aa73a 前为 `f77ba8e38`）— 废弃「8 个模糊 `Shadow` glyph 拷贝伪描边」，改用 Flutter **真**文字描边：
...
- **[x] ② 已加自动化测试** —
  ...
  - widget 行为测试 `hibiki/test/widgets/video_subtitle_overlay_test.dart` 重写描边用例为 `renders real outline as double-layer stroke+fill Text, no shadow residue (BUG-323)`：断言字符 'A' 渲染成**两个** `Text`（双层），用 `foreground == null` 区分出 fill 层（有 color、**无 shadows**——残留黑字源已根除）与 stroke 层（`foreground.style == PaintingStyle.stroke`、`strokeWidth == thickness`、color 匹配、几何与 fill 层一致、无 shadows）；新增 `thickness<=0 renders single fill Text` 用例断言无描边时只有单层 fill `Text`、无空描边层。
