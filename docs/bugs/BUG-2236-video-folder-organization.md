...
- **[x] ① 已修复** — schema v98增加来源 videoGroupingMode（默认series保留旧行为），目录合集保存本机 sourceFolderPath。导入时及来源设置提供目录模式，按一级目录创建普通合集，根直属视频归根合集，重扫事务内复用；不认领同名手动合集。目录模式不创建动画元数据索引/刮削计划，系列页优先按目录合集折叠。提交由本轮集成记录。
