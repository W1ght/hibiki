## BUG-1139 · 代理下「发现」搜索 20s 超时太短，请求本可成功却被掐断
...
- **改号**：本条开工时取到的下一个空号是 1138，但 open PR #461 / #462 / #463 已先认领
  BUG-1138（三者彼此也撞号，非本 PR 引入），故改到 1139。
  提交 `314bea466` 的**提交信息里仍写 BUG-1138**，是改号前留下的陈述——分支已推送、
  不做 force-push，故以本条与 `a1ee91630` 之后的改号提交为准。
