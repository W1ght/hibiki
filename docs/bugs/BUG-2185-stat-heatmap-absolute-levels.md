## BUG-2185 · 热力图档位按窗口最大值线性分级，单日爆量后其余全落最浅档
...
- **[x] ① 已修复** — 对齐 Hoshi Android 的分位数着色：新增纯函数 `statHeatmapRankLevel(value, sortedActive)`，档位按窗口内活跃日数值的**秩**分 1..4（`⌈r/n × levels⌉`，整数运算），零值仍最浅档；`buildStatHeatmap` 改用它。 提交：待填。
