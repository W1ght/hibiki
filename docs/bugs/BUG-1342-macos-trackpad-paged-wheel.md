...
- **[x] ① 已修复** — `df3b89200`, `3cf848329`：分页 wheel 每个 tick 只上报方向与主轴；横向触控板惯性由 reader Dart State 中的尾沿静默 gate 聚合，故翻章重建 WebView document 也不会在同一手势中重新放行。纵向鼠标滚轮不进此 gate，保留原有固定窗口节流；方向统一取绝对值更大的主轴，零位移忽略。
