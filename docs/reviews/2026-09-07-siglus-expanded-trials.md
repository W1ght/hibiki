...
当前首个失败边界是首次选线程后的历史正文恢复漏配已有原生资源：必须保留真实 seq/tick，沿事件身份查配已有和晚到资源，并从同一原路径重跑选择动作。新的 LUNARiA 原生消息绑定和单句真卡证据已在上文记录；不能再把旧的异步 Luna 消息缺口当成该构建仍未实现。其他 family、重复句/读档/重播及旧样本启动门仍未验证，后续逐门推进；独立 LE 源码修补不能替代原始游戏会话。
