...
`98884fa55e` 修复 [BUG-2240](../bugs/BUG-2240-selected-thread-history-resource-pairing.md)：历史回捞保留原始 seq/tick，仅查询对应事件桶及其时间校验；已经导出的资源当场配对，晚到资源保留同一严格所有权，不从无标记时间候选或最新资源借用。两份消费端定向测试共 69/69，含 6 项新增行为回归；focused analyze 无问题、退出 0。错误工作目录造成的首轮测试失败及本机分析器环境故障均保留在日志中，未计为通过。
...
Anemoi仍拒绝完整Native message布局，需要从正常原路径启动后的实际映像定位首个结构差异；不能扩大签名制造通过。磁盘exe属于packed布局，当前静态原exe八个message签名均缺失；既有hydrated元数据首拒绝为surface、次拒绝renderer，renderer调用相对message偏移与LP差22字节，仍不足以决定接线。整段文本聚合及有声多片段未证明，不凭同owner/key/栈地址拼接。LUNARiA的重复句/读档/重播及旧样本启动门仍未验证；独立LE源码修补不能替代原始游戏会话。
