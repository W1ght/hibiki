...
阶段未验收。月彼在 LE/启动阶段失败，新增等待候选尚未真机复验。未升级
...
恢复窗口操作后，为私有测试包准备新版 helper，再从月彼原始 CP932 Start 入口
验证实际子进程就绪后附着及 LE 启动结果。Angel Beats 先完成独立 8 参数 ABI 的
结构准入证明；两者均只推进当前首个失败边界，不以 Loopback 替代引擎语音适配。
