...
> 状态：**已实现**（2026-07-12 用户拍板「按推荐做」——§5 三点全按推荐执行；
> §6 任务表全落地，守卫 + 页面级 widget 测试锁死行为）。
...
| 模式 | 书架 | 视频页 |
|---|---|---|
| `recent`（默认） | 最近阅读 = `reader_positions.updatedAt` 倒序，没读过退导入时间（**BUG-756 修正**：原文「现状历史序零变化」前提为假——provider 序实为 `getAllEpubBooks` 的 importedAt 倒序，从来不是访问序；用户真机实报「刚读的书 hero 不显示」后改为真 recency） | 最近观看 = watch-stats `_watchAtByUid` 倒序，无记录退导入时间倒序 |
...
## 5. 用户拍板（2026-07-12「按推荐做」——三点全按推荐执行）
