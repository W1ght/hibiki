...
2. **未指派提供商 = 不发任何 AI 请求。** `AiFeatureAssignments.resolve()` 返回 null 时决策器直接返回 null，数据面与没有 AI 逐字节一致；刻意不回退到「第一家可用的」。入口按钮可以可见、点击时引导去「设置 › AI」指派（2026-09-26 所有者拍板放宽，BUG-2694；原口径「按钮不显示」让新用户找不到入口。`custom_theme_page` / `lapis_style_editor_page` 的「让 AI 帮忙」区早已是这个形态）。
