...
### 后续推翻（2026-07-12 用户拍板）：整理排序页整体删除

Phase E 重做后用户仍不满意：「编辑排序这个页面整个砍掉，做的太烂了」。执行：
`shelf_reorder_page.dart` / `HibikiReorderableGrid`（零消费者）/ `SeriesReorderFrame` /
两页 sort 接线与标签栏「整理」按钮 / 6 个单测 + 1 个集成测试全删；
守卫改为「页面不存在 + 两页零残留」。**能力不回退**：合集成员移出走详情页
（书=网格详情页原有；视频=剧集详情页本轮补上逐集移出按钮）；分组建立走批量
「组合成合集」。手动排序能力随页面移除（排序退化 ShelfEntries 既有值 + 导入
时间倒序）；组内序对视频以 MediaCollectionItems.sortIndex（导入序）为真相源。
