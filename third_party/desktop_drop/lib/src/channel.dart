    ...
    switch (call.method) {
      ...
      case "performOperation_linux":
        ...
        final paths = const LineSplitter().convert(text).map((e) {
          final line = e.trim();
          // text/uri-list allows blank and '#'-comment lines; skip them.
          if (line.isEmpty || line.startsWith('#')) return '';
          final uri = Uri.tryParse(line);
          // TODO-1306: browser address-bar / hyperlink drags deliver an http(s)
          // URI in the text/uri-list; toFilePath() throws on non-file schemes, so
          // the stock code silently dropped them. Preserve http(s) URIs verbatim
          // (the app classifier imports them as stream videos); only file: URIs
          // and bare paths are converted to a filesystem path.
