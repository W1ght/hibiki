...
public class DesktopDropPlugin: NSObject, FlutterPlugin {
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    // TODO-1306: also accept web URLs (browser address-bar / hyperlink drags) so
    // http(s) links can be imported as stream videos, not just local files.
...
class DropTarget: NSView {
  ...
  override func performDragOperation(_ sender: NSDraggingInfo) -> Bool { 
    ...
    // TODO-1306: the enumeration above is restricted to file URLs
    // (urlReadingFileURLsOnly), so a browser drag's web URL (public.url) is not
    // yielded. Read it directly from the pasteboard and pass it through as a
    // "path" so the Dart classifier can import it as a stream video. Guard on
    // !isFileURL so file drops are never double-counted.
    if let url = NSURL(from: sender.draggingPasteboard), !url.isFileURL,
