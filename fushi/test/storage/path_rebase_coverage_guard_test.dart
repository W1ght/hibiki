...
void main() {
  ...
  final String migratorPath = p.join(
    'lib',
    'src',
    'storage',
  ...
  final RegExp pathLikeName = RegExp(
    'path|dir|root|url|source|file|json',
    caseSensitive: false,
  );
  final RegExp pathLikePrefKey = RegExp(
    'path|dir|root|file',
    caseSensitive: false,
  );
  ...
  test('tables.dart 里每个路径形 TextColumn 都在 kPathRebaseColumns 里登记', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          '这些列名形似路径却没在 kPathRebaseColumns 登记。请判定它是否存数据根内'
  ...
  test('每张含必须 rebase 列的表，迁移器都有 _rebase<Table> 入口且在事务里被调用', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          '这些表声明了必须 rebase 的列，但迁移器缺少对应的 _rebase<Table> 入口，'
          '或该入口没有在 DB 改写事务里被调用：$missing',
    );
  });

  test('lib/ 里每个路径形 pref key 字面量都在 kPathRebasePrefs 里登记', () {
    final Set<String> declared = kPathRebasePrefs
        .map((PathRebasePref s) => s.key)
        .toSet();
    ...
    expect(
      ...
      isEmpty,
      reason:
          '这些 pref key 名字形似路径却没在 kPathRebasePrefs 登记。若它承载数据根内'
  ...
  test('kPathRebasePrefs 里的 key 都还在 lib/ 里被真实使用（无陈旧声明）', () {
    ...
    for (final PathRebasePref spec in kPathRebasePrefs) {
      ...
      final String needle = spec.key.startsWith('src:')
          ? spec.key.split(':').last
          : spec.key;
  ...
  test('每条 pref 声明都给出了非空理由，且 key 不重复', () {
    ...
    final Set<String> keys = kPathRebasePrefs
        .map((PathRebasePref s) => s.key)
        .toSet();
  ...
  test('字体 pref key 由 dbSourcePrefKey 编码器生成且已登记', () {
    ...
    for (final String body in <String>[
      ...
      expect(
        ...
        isNotNull,
        reason:
            'dbSourcePrefKey(reader_fushi, $body) = $expected 未在 '
  ...
  test('迁移器里所有 documents 路径改写都必须过同一个作用域谓词（无旁路）', () {
    ...
    final List<String> bare = RegExp(
      r'^.*\brebasePath\(.*$',
      multiLine: true,
    ).allMatches(src).map((RegExpMatch m) => m.group(0)!.trim()).toList();
    expect(
      bare.length,
      equals(1),
    ...
    final int injected = RegExp(
      r'rewritePath: documents.rebase',
    ).allMatches(src).length;
    expect(
      injected,
      equals(fontCalls),
      reason:
          '$fontCalls 处字体 JSON 改写调用里只有 $injected 处注入了作用域改写器；'
