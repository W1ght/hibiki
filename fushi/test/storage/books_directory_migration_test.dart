...
void main() {
  ...
  test('legacy hoshi_books renamed in place with contents preserved', () {
    final Directory legacy = Directory(
      p.join(root.path, 'hoshi_books', 'BookA'),
    ...
    expect(
      File(
        p.join(root.path, 'fushi_books', 'BookA', 'ch1.html'),
      ).readAsStringSync(),
      'html',
    );
  });
  ...
  test(
    'existing fushi_books wins; legacy left alone; no dir created fresh',
    () {
      ...
      migrateLegacyBooksDirectoryAt(root.path);
      expect(
        Directory(p.join(root.path, 'hoshi_books')).existsSync(),
        isTrue,
        reason: '新目录已存在时绝不合并/覆盖',
      );

      // 全新安装：两者皆无 → 不创建任何目录（建目录归消费方）。
      final Directory fresh = Directory.systemTemp.createTempSync(
        'fushi_books_mig_fresh',
      );
