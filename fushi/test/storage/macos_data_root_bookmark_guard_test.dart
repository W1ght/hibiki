...
void main() {
  group('macOS data root security-scoped bookmark guard', () {
    ...
    test('registers custom channels on the nested Flutter controller', () {
      final String appDelegate = maskComments(
        read('macos/Runner/AppDelegate.swift'),
      );
      final String mainWindow = maskComments(
        read('macos/Runner/MainFlutterWindow.swift'),
      );
      ...
      final int wrapperIdx = appDelegate.indexOf(
        'as? MacOSWindowUtilsViewController',
      );
      final int nestedControllerIdx = appDelegate.indexOf(
        'windowController.flutterViewController',
        wrapperIdx,
      );
      final int dataRootChannelIdx = appDelegate.indexOf(
      ...
      final int foregroundChannelIdx = appDelegate.indexOf(
      ...
      expect(
        ...
        reason:
            'the top-level controller is MacOSWindowUtilsViewController; '
    ...
    test('Dart startup restores bookmark before data root existence check', () {
      ...
      final int restoreIdx = src.indexOf(
        'MacOSDataRootAccess.startAccessingStoredBookmark',
      );
      ...
      final int probeCallIdx = src.indexOf(
        '_probeDataRootExists(Directory(configured))',
      );
      ...
      expect(
        configuredIdx,
        lessThan(probeCallIdx),
        reason:
            'resolve() must obtain configured via '
            '_configuredDataRootPath() (which restores the bookmark) before '
            'probing data-root existence',
      );
      expect(
        src,
        contains('_configuredDataRootPath'),
        reason: 'sandbox permission must be restored before touching dataRoot',
      );
    });
    ...
    test(
      'migration restores the previous bookmark when data_root write fails',
      () {
        final String src = read(
          'lib/src/sync/sync_settings_schema/data_root.part.dart',
        );
        ...
        final int storeIdx = src.indexOf('MacOSDataRootAccess.storeBookmark');
        final int rootWriteIdx = src.indexOf(
          'setString(AppPaths.dataRootPrefKey',
        );
        final int restoreIdx = src.indexOf(
          'MacOSDataRootAccess.restoreBookmark',
        );
