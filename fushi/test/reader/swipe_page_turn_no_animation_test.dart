...
void main() {
  ...
  test(
    'paginated layout disables native touch panning (touch-action: none)',
    () async {
  ...
  test(
    'continuous mode keeps native scrolling (no touch-action: none)',
    () async {
      final FushiDatabase db = FushiDatabase.forTesting(
        NativeDatabase.memory(),
      );
  ...
  test('continuous mode wheel: horizontal pass-through + vertical-writing '
    ...
    expect(
      ...
      greaterThanOrEqualTo(0),
      reason:
          'wheel must branch on continuous mode before the paginated '
    ...
    expect(
      wheelBlock,
      isNot(contains("callHandler('onSwipe'")),
    ...
    expect(
      continuousBranch,
      contains('window.scrollBy({left: 0, top: wheelDelta'),
      reason:
          'horizontal continuous wheel does a real window.scrollBy along scrollTop',
    );
    expect(
      continuousBranch,
      contains('window.scrollBy({left: wheelDelta * sign'),
      reason:
    ...
    expect(
      continuousBranch,
      contains('var moved = Math.abs(after - before) > 1'),
      reason: 'edge is decided by measured real movement, not geometry/clamp',
    );
    // 连续分支不得回传翻页 handler（90% 整屏跳页与原生滚动轴向冲突）；跨章走
    // onBoundarySwipe。TODO-737 后分页通道是 onWheelPaginate，连续分支同样不得回传。
    expect(
      continuousBranch,
      isNot(contains("callHandler('onWheelPaginate'")),
      reason:
