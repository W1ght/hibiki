...
void main() {
  ...
  test('暗色主题有独立描边卡变体（描边转白 + 加深投影）', () {
    ...
    expect(
      RegExp(r'border-color\s*:').hasMatch(body),
      isTrue,
  ...
  test('间距仍走 margin-top + min-width，未破坏 TODO-776 grid 间距模型', () {
    ...
    expect(
      RegExp(r'margin-top\s*:').hasMatch(body),
      isTrue,
      reason: 'TODO-776 行距靠 margin-top，grid 不能改用 gap/row-gap 否则双倍间距',
    );
    expect(
      RegExp(r'min-width\s*:\s*0').hasMatch(body),
      isTrue,
  ...
  test('parent 仍只用 column-gap，不引入会双倍间距的 row-gap/gap', () {
    ...
    expect(
      RegExp(r'column-gap\s*:').hasMatch(body),
      isTrue,
      reason: '多列布局只用 column-gap',
    );
    expect(
      RegExp(r'(^|[^-])row-gap\s*:').hasMatch(body),
      isFalse,
      reason: 'row-gap 会与 .glossary-group 的 margin-top 叠成双倍行距',
    );
    expect(
      RegExp(r'(^|[;{\s])gap\s*:').hasMatch(body),
      isFalse,
  ...
  test('.dict-label 是扁平小标题：display:block + opacity:0.7，非 flex 大方块', () {
    ...
    expect(
      RegExp(r'display\s*:\s*block').hasMatch(body),
      isTrue,
      reason: '标题行扁平态 display:block',
    );
    expect(
      RegExp(r'display\s*:\s*flex').hasMatch(body),
      isFalse,
      reason: '标题不是整条 flex 大方块',
    );
    expect(
      RegExp(r'opacity\s*:\s*0\.7').hasMatch(body),
      isTrue,
  ...
  test('.dict-label.selected 长按选词典视觉语义保留，不被一并删掉', () {
    ...
    expect(
      body.contains('var(--primary-color)'),
      isTrue,
      reason: '选为制卡首选词典时仍用 primary 色高亮，hibiki 独有交互不能丢',
    );
    expect(
      RegExp(r'font-weight\s*:\s*bold').hasMatch(body),
      isTrue,
      reason: '选中态仍加粗',
    );
  });
  ...
  test('首卡不被等高拉伸：.category-body grid 显式 align-items:start (BUG-683)', () {
    ...
    expect(
      RegExp(r'align-items\s*:\s*start').hasMatch(rule),
      isTrue,
      reason:
          '首卡偏高根因 = grid 默认 align-items:stretch 把同行卡片拉到最高；'
          '必须显式 align-items:start 让卡片取自然内容高度',
    );
    expect(
      RegExp(r'align-items\s*:\s*stretch').hasMatch(rule),
      isFalse,
  ...
  test('描边卡不钉死高度，取自然内容高度 (BUG-683)', () {
    ...
    expect(
      RegExp(r'(^|[;{\s])height\s*:').hasMatch(body),
      isFalse,
      reason: '卡片不能钉死 height，否则内容高度失真、又回到等高观感',
    );
    expect(
      RegExp(r'min-height\s*:').hasMatch(body),
      isFalse,
  ...
  test(
    ...
    () {
      final RegExpMatch? rule = RegExp(
        r'\.gloss-image-scroll\s*\{([^}]*)\}',
      ).firstMatch(css);
      expect(
        rule,
        isNotNull,
