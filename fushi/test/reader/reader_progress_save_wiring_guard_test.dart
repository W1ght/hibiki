...
void main() {
  ...
  group('B-3：settle 尾沿去抖接进 _handleReaderScroll（在落库刷新之前）', () {
    test(
      ...
      () {
        ...
        expect(
          body.indexOf('_reanchorClearedAt'),
          greaterThanOrEqualTo(0),
        ...
        expect(
          refreshIdx,
          greaterThan(settleIdx),
          reason:
              'settle 去抖判据必须在 _refreshProgressFromScroll 调用之前——'
  ...
  group('落库门控：_refreshProgress 无条件落库（B-4 已删）', () {
    test('_refreshProgress 调 _debouncedSavePosition，且不再有 B-4 伪归零门控', () {
      ...
      expect(
        body,
        contains('_debouncedSavePosition(progress, charOffset)'),
        reason: '_refreshProgress 必须落库（_debouncedSavePosition）',
      );
      expect(
        body,
        isNot(contains('if (!spuriousDrop)')),
        reason:
            '旧 B-4 伪归零门控已删（复核结论 b：误伤惯性甩动到真章首）；'
            '落库现在无条件（reflow 归零由 _reanchorPending JS 不回传 + B-3 settle 窗双墙覆盖）',
      );
      expect(
        body,
        isNot(contains('readerProgressDropIsSpurious')),
        reason: 'readerProgressDropIsSpurious 已删，不得在 _refreshProgress 复活',
      );
      expect(
        body,
        isNot(contains('_lastUserInputAt')),
    ...
    test('readerProgressDropIsSpurious / _lastUserInputAt 全仓死符号已清', () {
      expect(
        page,
        isNot(contains('bool readerProgressDropIsSpurious(')),
      ...
      expect(
        webview,
        isNot(contains('onReaderUserInput')),
        reason: 'onReaderUserInput JS handler + 打点通道已删',
      );
      expect(
        webview,
        isNot(contains('_fushiNotifyUserInput')),
  ...
  group('B-1：样式重锚两阶段编排接进 _applyStylesLive', () {
    ...
    test('_reanchorForStyleChange 用样式专入口 + 编排 + 打 _reanchorClearedAt', () {
      ...
      expect(
        body,
        contains('runUiScaleReanchorOrchestration'),
        reason: '复用两阶段 begin→commit settle-aware 编排',
      );
      expect(
        body,
        contains('beginStyleReanchorInvocation'),
        reason: '用样式专用 begin 入口（不复用 appUiScale 那对）',
      );
      expect(
        body,
        contains('commitStyleReanchorInvocation'),
      ...
      expect(
        body,
        contains('_reanchorClearedAt = DateTime.now()'),
  ...
  group(
    'TODO-797 回归锁：恢复重锚 / appUiScale 重锚 commit 也武装 B-3（_reanchorClearedAt）',
    () {
      ...
      test('_reanchorContinuousAfterRestore 的 commit 打 _reanchorClearedAt', () {
        ...
        expect(
          body,
          contains('commitUiScaleReanchorInvocation'),
        ...
        expect(
          ...
          contains('_reanchorClearedAt = DateTime.now()'),
          reason:
              'TODO-797：恢复重锚 commit 必须打 _reanchorClearedAt，'
      ...
      test('三条 reanchor-commit 路径全部武装 B-3（统一不可遗漏）', () {
        final int count = '_reanchorClearedAt = DateTime.now()'
            .allMatches(chrome)
            .length;
        expect(
          count,
          greaterThanOrEqualTo(3),
          reason: '恢复(718)/appUiScale(693)/样式(736) 三条 reanchor-commit 路径须各打点一次',
        );
      });
    },
  );
