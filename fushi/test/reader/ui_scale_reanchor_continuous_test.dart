...
void main() {
  group('readerUiScaleReanchorAllowed（appUiScale 连续重锚门控真值表）', () {
    ...
    test('分页模式（continuousMode==false）一律抑制——分页有 snap/lock 保护', () {
      expect(
        ...
        isFalse,
        reason:
            '分页模式 registerSnapScroll/lockRootViewport 已挡住 reflow 归零，'
            '不需要也不应触发本重锚',
      );
    });
  ...
  group('JS 守卫：连续模式两阶段重锚原语 + _reanchorPending 串行契约', () {
    ...
    test('阶段1 beginUiScaleReanchor 先置 _reanchorPending 再暂存锚', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'begin 必须在已有重锚在飞时返回 -1（让 setChromeInsets/updatePageSize '
            '等既有序列接管，不重复采样/不抢旗）',
      );
      // 关键时序：采样首个可见字符 → 置旗 → 暂存锚。置旗必须发生（挡住 reflow 归零 scroll）。
      expect(
        containsCodeLine(body, 'getFirstVisibleCharOffset()'),
        isTrue,
        reason: 'begin 必须用 getFirstVisibleCharOffset 采样精确锚',
      );
      // BUG-493：置旗改走清旗单点 setter（_setReanchorPending，语义不变）。
      expect(
        containsCodeLine(body, 'this._setReanchorPending(true)'),
        isTrue,
        reason:
            'begin 必须置 _reanchorPending=true，否则 reflow 归零 scroll 会经 '
            'onReaderScroll 落库 progress≈0 弹回章首（TODO-693 根因）',
      );
      expect(
        containsCodeLine(body, 'this._uiScaleReanchorOffset = charOffset'),
        isTrue,
    ...
    test('阶段2 commitUiScaleReanchor 滚回锚并在 finally 清旗', () {
      ...
      expect(
        containsCodeLine(body, 'this._setReanchorPending(false)'),
        isTrue,
        reason: 'commit 必须清 _reanchorPending，否则后续滚动回传被永久挡住',
      );
      expect(
        containsCodeLine(body, 'this._uiScaleReanchorOffset = undefined'),
        isTrue,
        reason: 'commit 必须清暂存锚，避免下次缩放误用旧锚',
      );
      expect(
        containsCodeLine(body, 'this._uiScaleReanchorScroll = undefined'),
        isTrue,
    ...
    test('两阶段重锚只存在于连续模式 shell（分页 shell 无此原语）', () {
      ...
      expect(
        'beginUiScaleReanchor: function()'.allMatches(js).length,
        1,
    ...
    test('invocation builder 用 typeof 守卫使分页模式整体 no-op', () {
      expect(
        ...
        isTrue,
        reason:
            'beginUiScaleReanchorInvocation 必须 typeof 守卫——分页 shell 缺此函数，'
            '误调时整体 no-op 返回 -1',
      );
      expect(
        containsCodeLine(
  ...
  group('Dart 守卫：ref.listen(appUiScale) 接线 + 两阶段先置旗后提交', () {
    ...
    test('build 用 ref.listen 监听 appUiScale 变化并调连续重锚', () {
      ...
      expect(
        containsCodeLine(src, '_reanchorContinuousForUiScale()'),
        isTrue,
    ...
    test('_reanchorContinuousForUiScale 委托给 top-level 编排核心并绑入实例字段', () {
      ...
      expect(
        containsCodeLine(body, 'runUiScaleReanchorOrchestration('),
        isTrue,
        reason:
            '方法必须委托给 top-level runUiScaleReanchorOrchestration（编排核心，'
            '运行时序列由 runtime 测试锁定）',
      );
      ...
      expect(
        containsCodeLine(body, 'controllerAvailable: _controller != null'),
        isTrue,
        reason: '必须把控制器存活绑进门控',
      );
      expect(
      ...
      expect(
        containsCodeLine(body, 'addPostFrameCallback'),
        isTrue,
        reason:
    ...
    test(
      'top-level 编排 runUiScaleReanchorOrchestration 保留两阶段先后 + begin<0 早返回',
      () {
        ...
        expect(
          containsCodeLine(body, 'if (!gateAllowed)'),
          isTrue,
          reason:
              '编排核心必须先过调用方注入的门控结果 gateAllowed（TODO-718：不再硬编码'
              '单一门控函数，缩放/恢复两路径各传自己的门控真值表）',
        );
        final int idxBegin = body.indexOf('await evalBegin()');
        final int idxIntResult = body.indexOf(
          'ReaderPaginationScripts.intResult(begin)',
        );
  ...
  group('TODO-718 readerRestoreReanchorAllowed（恢复完成重锚门控真值表）', () {
    ...
    test('门控不含 restoreInFlight 参数——恢复完成路径下 _restoreInFlight 必为 false，'
  ...
  group('TODO-718 Dart 守卫：_onRestoreComplete 归零前采锚 + 恢复重锚接线', () {
    ...
    test(
      ...
      () {
        ...
        final int idxReanchor = src.indexOf(
    ...
    test('_reanchorContinuousAfterRestore 委托编排核心并绑恢复完成门控', () {
      ...
      expect(
        containsCodeLine(body, 'readerUiScaleReanchorAllowed('),
        isFalse,
        reason: '恢复重锚不得复用含 !restoreInFlight 早返回的 readerUiScaleReanchorAllowed',
      );
      // 门控不绑 restoreInFlight（恢复完成路径下必为 false）。
      expect(
        containsCodeLine(body, 'restoreInFlight:'),
        isFalse,
      ...
      expect(
        idxCommit,
        greaterThan(0),
        reason: 'evalCommit 必须绑 commitUiScaleReanchorInvocation（settle 后滚回清旗）',
      );
      expect(
        containsCodeLine(body, 'addPostFrameCallback'),
        isTrue,
