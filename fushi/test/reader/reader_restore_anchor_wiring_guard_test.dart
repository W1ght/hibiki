...
void main() {
  ...
  group('前置 ①：恢复锚由实时进度接管（否则重建 = 回退到章首并落库）', () {
    test('_adoptLiveProgressAsRestoreAnchor 委托纯函数，并写全四个恢复锚字段', () {
      ...
      expect(
        containsCodeLine(body, 'restoreAnchorOnLiveProgress('),
        isTrue,
    ...
    test('_refreshProgress 在写完实时进度之后、落库之前接管恢复锚', () {
      ...
      final int liveWrite = code.indexOf(
        '_lastProgressCharOffset = charOffset;',
      );
      final int adopt = code.indexOf(
        '_adoptLiveProgressAsRestoreAnchor(progress, charOffset)',
      );
      final int save = code.indexOf(
      ...
      expect(
        adopt,
        greaterThan(liveWrite),
        reason:
            '实时进度是恢复锚的唯一来源——不接管，恢复锚就永远停在进章快照，'
  ...
  group('前置 ②：调试钩子断言按所有者身份判，同一 State 重装不炸', () {
    test('onWebViewCreated 用 debugHookOwner 身份判据，不再判「钩子必须为 null」', () {
      ...
      expect(
        containsCodeLine(body, 'ReaderFushiPage.debugHookOwner = this;'),
        isTrue,
      ...
      for (final String stale in <String>[
        ...
        expect(
          containsCodeLine(body, stale),
          isFalse,
          reason:
              '旧断言把「同一页重装」误判成「两个阅读器同时活着」，'
    ...
    test('dispose 释放钩子所有权', () {
      ...
      expect(
        containsCodeLine(body, 'ReaderFushiPage.debugHookOwner = null;'),
        isTrue,
  ...
  group('前置 ③：_refreshProgress 的 evaluateJavascript 有错误边界', () {
    test(
      'try/catch 包住 evaluateJavascript，且 fail-open 补 ErrorLogService.log',
      () {
        final String body = methodBody(
          navigation,
          'Future<void> _refreshProgress() async {',
        ...
        final int evalIdx = code.indexOf(
