...
Map<String, dynamic> _font(String name, {String? path, bool enabled = true}) =>
    <String, dynamic>{'name': name, 'path': path, 'enabled': enabled};

List<Map<String, dynamic>> _fonts(
  Map<String, dynamic> first, [
  Map<String, dynamic>? second,
]) => <Map<String, dynamic>>[first, if (second != null) second];

Future<Map<String, dynamic>> _storedJson(FushiDatabase db, String key) async {
...
void main() {
  ...
  test(
    'exposes the new catalog preference keys alongside legacy target keys',
    () {
  ...
  test(
    'video subtitle target reads/writes independently of other targets',
    () async {
      final ReaderSettings settings = ReaderSettings(db);
      await settings.refreshFromDb();
  ...
  test(
    'setting body fonts first still seeds untouched targets from body',
    () async {
      final ReaderSettings settings = ReaderSettings(db);
      await settings.refreshFromDb();
      await settings.setCustomFonts(
  ...
  test('legacy three keys merge into a de-duplicated catalog', () async {
    final Map<String, dynamic> mincho = _font(
      'Mincho',
      path: '/fonts/mincho.ttf',
      enabled: true,
    );
    final Map<String, dynamic> gothic = _font(
      'Gothic',
    ...
    await db.setPref(
      ...
      jsonEncode(
        _fonts(_font('Mincho', path: '/fonts/mincho.ttf', enabled: false)),
      ),
    );
    ...
    expect(
      idByPath.keys,
      containsAll(<String>['/fonts/mincho.ttf', '/fonts/gothic.ttf']),
    );
    expect(
  ...
  test(
    'same catalog font can appear in multiple targets independently',
    () async {
      final ReaderSettings settings = ReaderSettings(db);
      await settings.refreshFromDb();
      await settings.setFontsForTarget(
      ...
      expect(
        restored.customFonts.map((Map<String, dynamic> e) => e['name']),
  ...
  test(
    'bad catalog json falls back to legacy keys and repairs the model',
    () async {
  ...
  test(
    'catalog de-duplication preserves distinct paths for the same name',
    () async {
      await db.setPref(
        '${_prefPrefix}custom_fonts',
        jsonEncode(
