...
void main() {
  ...
  test('webview.part.dart：注入裸 Space 键盘桥并接上 onSpaceKey handler', () {
    ...
    expect(
      flat,
      contains('webViewKeyBridgeScript('),
    ...
    expect(
      flat,
      contains('_resolveWebViewSpaceAction()'),
  ...
  test(
    ...
    () {
      ...
      expect(
        flat,
        contains('ShortcutAction? _resolveWebViewSpaceAction()'),
      ...
      expect(
        body.contains('resolveReaderSpaceOverride('),
        isTrue,
        reason: '裸 Space 必须交 resolveReaderSpaceOverride 判有声书覆写',
      );
      expect(
        body.contains('hasActiveAudiobook: _hasActiveAudiobook'),
        isTrue,
        reason: '有声书激活判据必须用 _hasActiveAudiobook（与键盘焦点路径同源）',
      );
      expect(
        body.contains('ShortcutScope.reader'),
        isTrue,
