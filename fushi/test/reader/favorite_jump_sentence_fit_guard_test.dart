...
void main() {
  ...
  test('collections 跳转把句长透传成 Bookmark.charAnchorLength (BUG-461)', () {
    expect(
      collectionsSrc.contains('charAnchorLength:'),
      isTrue,
      reason:
          '收藏句跳转必须把 item.normCharLength 透传为 Bookmark.charAnchorLength，'
          '否则连续模式只能锚句首、长句句尾被底栏切',
    );
  });

  test(
    '阅读器据 charAnchorLength 算句尾锚 _initialCharOffsetEnd 并传 shell (BUG-461)',
    () {
      expect(
        pageSrc.contains('charAnchorLength'),
        isTrue,
        reason: '阅读器 restore 必须读 bm.charAnchorLength',
      );
      expect(
        pageSrc.contains('_initialCharOffsetEnd'),
        isTrue,
        reason: '阅读器必须算句尾绝对偏移 _initialCharOffsetEnd',
      );
      expect(
        pageSrc.contains('initialCharOffsetEnd: _initialCharOffsetEnd,'),
        isTrue,
        reason:
            'BUG-1140 第二阶段①后改由 ReaderEngineConfig 下发，'
  ...
  test('shellScript / 连续 shell 接受并透传 initialCharOffsetEnd (BUG-461)', () {
    ...
    expect(
      jsSrc.contains('C.initialCharOffsetEnd > C.initialCharOffset'),
      isTrue,
