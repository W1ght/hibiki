...
void main() {
  group('ReaderChromeController：点出来的栏不会自己消失', () {
    test('showTransient 不武装自动收起', () {
      ...
      expect(c.autoHideArmed, isFalse, reason: '点一下唤出的栏必须留着，只有下一次点击能关掉它');
    });
    ...
    test('showTransient 会停掉上一轮（VN 翻页）武装的计时', () {
      ...
      c.showTransient();
      expect(c.transientVisible, isTrue);
      expect(c.autoHideArmed, isFalse, reason: '旧计时不停掉，就会把刚点出来的栏收走');
    });
  ...
  group('EPUB 阅读器源码守卫', () {
    ...
    test('自动收起只剩 VN 推进一个武装点', () {
      expect(
        ...
        2,
        reason:
            '只该剩「方法自身的定义」+「VN 推进那一次调用」。VN 空白点被翻页'
      ...
      expect(
        src
            ...
            .contains('_armChromeAutoHide()'),
        isTrue,
      );
    });
  });

  // 上游同批的「漫画同口径」未随此次合入：本地没有 manga_reader_chrome.dart
  // （漫画 chrome 仍内联在 manga_fushi_page.dart 里），那一侧的守卫留在上游分支。
