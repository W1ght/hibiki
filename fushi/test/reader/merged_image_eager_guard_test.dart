...
void main() {
  ...
  group('TODO-1339 merge-injected leading illustrations stay eager', () {
    test('_sharedInitImages exempts .fushi-merged-image from loading=lazy', () {
      ...
      expect(
        src.contains('_sharedInitImages'),
        isTrue,
      ...
      expect(
        guardWindow.contains('fushi-merged-image'),
        isTrue,
        reason: '设置 loading=lazy 前必须判定并放行 .fushi-merged-image 合并前导插图（保持 eager）',
      );
    ...
    test(
      'the injection still marks merged images with .fushi-merged-image',
      () {
        ...
        expect(
          src.contains('class="fushi-merged-image"'),
          isTrue,
