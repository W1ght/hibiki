...
void main() {
  ...
  group('ReaderContentStyles chrome inset CSS variables', () {
    ...
    test(
      ...
      // （margin + chrome insets），与 padding-top/padding-bottom 逐项镜像（BUG-2457：
  ...
  group('TODO-362 responsive page margins', () {
    ...
    // ② 回归守卫（BUG-2457 改写）：底部 padding 只含页边距 + chrome inset，**不再**
    // 多留一个字号或旧的 22px 常量——那是 TODO-734 之前列高建在 V+O 上的配对项，基准
    // 改纯 V 后只剩一条空带。把字号抬到 128 断言 padding-bottom / column-width 都不含它。
    test('vertical bottom reserve has no font-size band (BUG-2457)', () async {
      final FushiDatabase db =
          FushiDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
      final ReaderSettings settings = ReaderSettings(db);
    ...
    test('horizontal bottom reserve has no font-size band either (BUG-2457)',
        () async {
      final FushiDatabase db =
          FushiDatabase.forTesting(NativeDatabase.memory());
      addTearDown(db.close);
