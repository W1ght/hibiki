...
void main() {
  ...
  test('webview.part.dart：pointerup 早退只认 nativeMoved，不认残留原生选区', () {
    ...
    expect(
      src,
      contains('if (nativeMoved) {'),
      reason: 'pointerup 原生选词分支早退必须只认 nativeMoved（本次手势真的拖动过）',
    );
    expect(
      src.contains('nativeMoved || hasNativeSelection'),
      isFalse,
      reason:
          'BUG-927 回退：残留原生选区不得再塞进 pointerup 早退条件，'
          '否则纯 tap 被吞、查词永远打不开',
    );
    expect(
      src.contains('hasNativeSelection'),
      isFalse,
  ...
  test('chrome.part.dart：桌面右键复制后清原生选区 + 右键 eval 有 try/catch', () {
    ...
    expect(
      ...
      reason:
          'BUG-927 回退：桌面右键 copy 复制后必须 _clearReaderAppSelection() 清'
          '残留原生选区',
    );
    ...
    expect(
      src,
      contains("'ReaderFushi.showReaderTextContextMenu'"),
      reason:
          'BUG-927 回退：右键菜单首个 evaluateJavascript 必须 try/catch，'
