...
void main() {
  group('TODO-656 跨章诊断埋点 [xchapter]', () {
    ...
    test('触摸/指针边界手势带 src 标记并打印几何', () {
      ...
      expect(
        ...
        reason:
            '已砍掉 PC 鼠标(pointer)边界手势跨章：鼠标左键回归原生选字，'
    ...
    test('滚轮边界判定与 Dart 汇合点都有诊断', () {
      expect(
        corpus,
        contains('[xchapter] wheel '),
        reason: '滚轮边界附近必须打印几何 + armed 状态（对照组）',
      );
      expect(
        corpus,
        contains('[xchapter] onBoundarySwipe '),
        reason: '跨章手势 Dart 汇合点必须打印 dir + chapter',
      );
      expect(
        corpus,
        contains('[xchapter] handlePageTurnLimit '),
  ...
  group('TODO-630 有声书高亮诊断埋点 [sentence-audio-hl]', () {
    ...
    test(
      'Dart 端 prepareCues 决策有日志（书源 + cachedSentenceAudio + payloadLen 留痕）',
      () {
        expect(
          corpus,
          contains('[sentence-audio-hl] prepareCues path='),
          reason:
        ...
        expect(
          corpus,
          contains(r'path=$pathTag'),
          reason: 'BUG-395：SRT 与 Audiobook 两源判据归一后，pathTag 仍标明书源以便定位',
        );
        expect(
          corpus,
          contains(r'path=$pathTag-SENTENCE-AUDIO'),
