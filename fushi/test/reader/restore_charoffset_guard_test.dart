...
void main() {
  ...
  test('fushiProgressDetails 携带 getFirstVisibleCharOffset 精确偏移 (BUG-162)', () {
    ...
    expect(
      body.contains('getFirstVisibleCharOffset'),
      isTrue,
  ...
  test('分页+连续两条恢复路径都支持精确字符偏移恢复 (BUG-162)', () {
    ...
    expect(
      jsSrc.contains('C.initialCharOffset'),
      isTrue,
      reason: 'shell 必须读运行时 C.initialCharOffset 并在 >=0 时走 restoreToCharOffset',
    );
  });
  ...
  test('恢复脚本在 initialCharOffset>=0 时优先精确路径 (BUG-162)', () {
    ...
    final int branches = 'restoreToCharOffset(C.initialCharOffset)'
        .allMatches(jsSrc)
        .length;
    expect(
      branches,
    ...
    expect(
      'C.initialCharOffset >= 0'.allMatches(jsSrc).length,
      greaterThanOrEqualTo(2),
      reason: '两个 shell 都必须保留 initialCharOffset>=0 的优先判据',
    );
    expect(
      'restoreProgress(C.initialProgress)'.allMatches(jsSrc).length,
      greaterThanOrEqualTo(2),
  ...
  test(
    ...
    () {
      expect(
        pageSrc.contains('charOffset:'),
        isTrue,
        reason: 'repo.save 必须带 charOffset',
      );
      expect(
        pageSrc.contains('saved.charOffset'),
        isTrue,
        reason: '恢复必须读 saved.charOffset 作精确锚',
      );
      expect(
        pageSrc.contains('initialCharOffset: _initialCharOffset,'),
        isTrue,
