...
void main() {
  group('pure helpers', () {
    test('enabled on every platform outside lyrics mode', () {
      expect(
        readerStatusFooterEnabled(desktop: true, lyricsMode: false),
        isTrue,
      );
      expect(
        readerStatusFooterEnabled(desktop: true, lyricsMode: true),
        isFalse,
      );
      expect(
        readerStatusFooterEnabled(desktop: false, lyricsMode: false),
        isTrue,
      );
      expect(
        readerStatusFooterEnabled(desktop: false, lyricsMode: true),
        isFalse,
      );
    });
    ...
    test('reserve: enabled -> footerHeight, else 0', () {
      ...
      expect(
        ...
        greaterThan(kReaderStatusFooterFontSize),
        reason: '预留高必须装得下文字行盒（视觉高度 == 预留高度铁律）',
      );
    });

    test('band: footer sits inside the system bottom inset (BUG-2460)', () {
    ...
    test('progress label: same format as the top pill; null when unknown', () {
      expect(
        readerProgressLabel(current: 64988, total: 123962),
        '64988 / 123962  52.43%',
      );
      ...
      expect(
        readerProgressLabel(current: 20, total: 10),
        '20 / 10  100.00%',
  ...
  group('widget', () {
    ...
    testWidgets('renders tracker + progress and re-samples on every tick', (
      ...
      await tester.pumpWidget(
        host(totals: () => (durationMs: ms, chars: 0, active: true)),
      );
    ...
    testWidgets(
      '320px footer keeps long counts inside and actions accessible',
      (WidgetTester tester) async {
        ...
        await tester.pumpWidget(
          host(
            totals: () =>
                (durationMs: 360000000, chars: 999999999, active: true),
            current: 123456789,
            total: 999999999,
        ...
        final Finder tracker = find.byKey(
          const ValueKey<String>('fushi_status_tracker'),
        );
    ...
    testWidgets('tracker and progress sit together at the right end', (
      ...
      final Rect tracker = tester.getRect(
        find.byKey(const ValueKey<String>('fushi_status_tracker')),
      );
      ...
      expect(
        tracker.right,
        lessThanOrEqualTo(progress.left),
        reason: '与 inline 形态同序：计时块在进度左边',
      );
      // 判据是「两段挨在一起」而不是「都在右半边」——两段文字合起来本就可能超过半屏。
      expect(
        progress.left - tracker.right,
        lessThanOrEqualTo(24),
        reason: '两段之间只隔一个间距，不再被撑成左右两角',
      );
      expect(
        strip.right - progress.right,
        closeTo(16, 0.5),
        reason: '右端内边距仍是 16，进度贴着右缘',
      );
      expect(
        tracker.left - strip.left,
        greaterThan(32),
        reason: '左端留白（点它唤出 / 收起 chrome），计时块不再钉在左下角',
      );
    });

    // BUG-2531：竖屏读数独立成行时它是底栏 Column 的最后一行，上面那排传输键居中，
    // 读数贴右角会和它们错开成两个重心（用户 2026-09-14「竖屏做到最底部并且居中」）。
    testWidgets('centered: 读数并进底栏那块遮罩时居中，不再贴右角', (WidgetTester tester) async {
      ...
      // 两段读数合起来的中点落在整条的中点上。左边界量到计时那颗开关键，
      // 计时文字左边还有按钮 + 间距，拿文字左缘算会偏出去。
  ...
  group('source-scan guards', () {
    ...
    test('footer reserve is part of _readerBottomReserve (single source)', () {
      ...
      // BUG-2460：状态行与系统底 inset 取大而不是相加——单一真相源在纯函数里。
      expect(
    ...
    test('footer yields to the squeeze bar that already carries its numbers '
        '(BUG-2453)', () {
      ...
      // BUG-2531 起「该不该画」收进 _statusFooterShouldPaint 这一个真值（屏底独立
      // 成行与并进底栏遮罩两条路共用），吸收判据仍是它的一部分。
      expect(
        src.contains(
          '_statusFooterEnabled && _hasEverLoaded && !_statusFooterAbsorbedByBar;',
        ),
        isTrue,
        reason: '吸收判据必须仍在「状态行该不该画」的单一真值里',
      );
    ...
    test('footer replaces the top progress pill on every platform', () {
      ...
      expect(
        showTop.contains('!_statusFooterEnabled &&'),
        isTrue,
    ...
    test(
      ...
      () {
        ...
        expect(
          build.contains('Focus(') || build.contains('canRequestFocus'),
          isFalse,
          reason: '纯指针面，不进焦点遍历池（TODO-700 不变式）',
        );
        // 只钉顺序，不钉相邻、也不钉缩进宽度：中间允许再插别的覆盖层（有声书悬浮球
        // 就插在这两者之间），而「状态行先画、词典弹层后画」这个不变式一点没变。
        final int footerAt = src.indexOf('_buildStatusFooter(),');
        final int dictAt = src.indexOf('buildDictionary(),');
        expect(footerAt, greaterThan(-1));
        expect(
          dictAt,
    ...
    test('session totals are read from StudyClock, no page-side copy', () {
      ...
      expect(
        totals.contains('_studyClock?.sessionTotals()'),
        isTrue,
