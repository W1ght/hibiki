  ...
  group('flushWithBoundedProbe 行为契约', () {
    ...
    test('探针抛异常时：落库照常执行（这是旧形态真正丢进度丢统计的那条路）', () async {
      ...
      expect(
        ...
        isTrue,
        reason:
            '探针抛错时落库必须照常跑完。旧形态里这个异常会沿 _syncAndFlushPosition → '
    ...
    test('探针在预算内完成时：先探针后落库，且如实返回 true（BUG-203 的实时锚不能被换掉）', () async {
      ...
      expect(
        order,
        <String>['probe', 'persist'],
        reason:
            '实时探针必须在落库之前完成，否则落的还是陈旧缓存锚——正是 BUG-203 '
            '「退出重进落在前面好几页」的成因',
      );
  ...
  group('退出链源码守卫', () {
    ...
    test('_syncAndFlushPosition 必须走限时探针契约，不得裸 await 探针', () {
      ...
      final String body = balancedBlockFrom(
        source,
        start,
        what: '_syncAndFlushPosition 方法体',
      );

      expect(
        body,
        contains('flushWithBoundedProbe('),
        reason:
            '退出/生命周期 flush 必须经 flushWithBoundedProbe 给 WebView 探针加上'
            '延迟上界；退回裸串行 await 就会重现「按返回毫无反应」（TODO-2495）',
      );
      expect(
        body,
        contains('probeBudget: kReaderExitProbeBudget'),
      ...
      expect(
        body.indexOf('probe:'),
        lessThan(body.indexOf('persist:')),
        reason:
            '探针必须挂在 probe 槽、落库必须挂在 persist 槽；两者对调等于给落库'
            '加了上界、给探针去了上界，恰好把契约反过来',
      );
      expect(
        body.indexOf('persist:'),
        lessThan(body.indexOf('_flushPosition()')),
    ...
    test('flushWithBoundedProbe 自身必须真的有上界，且落库在 try 之外', () {
      ...
      final String body = balancedBlockFrom(
        source,
        start,
        openSearchFrom: bodyBrace,
        what: 'flushWithBoundedProbe 方法体',
      );

      expect(
        body,
        contains('.timeout(probeBudget)'),
        reason: '探针必须真的被 probeBudget 限时——去掉它，整个契约只剩注释',
      );
      expect(
        body.indexOf('await persist()'),
    ...
    test('进程退出 flush 仍不得触碰 WebView 探针', () {
      ...
      final int start = source.indexOf(
        'Future<void> _flushAllForProcessExit()',
      );
      expect(start, isNonNegative);
      final String body = balancedBlockFrom(
        source,
        start,
        what: '_flushAllForProcessExit 方法体',
      );
      ...
      // 子串误命中。
      expect(
        containsIdentifier(body, '_syncPositionFromWebViewProgress'),
        isFalse,
        reason:
            '进程退出期 WebView 正在拆除，对它 evaluateJavascript 会挂死整个退出；'
