...
// BUG-2527 regression（用户实报：阅读设置里「字号字重显示不全」）。
//
...
void main() {
  ...
  for (final double textScale in textScales) {
    for (final double width in widths) {
      for (final String title in titles) {
        testWidgets(
          'BUG-2527: stepper row title "$title" stays readable '
  ...
  for (final double textScale in textScales) {
    for (final double width in widths) {
      testWidgets(
        'BUG-2527: stepper row title gets at least the minimum readable width '
        'at ${width}dp / ${textScale}x',
        (WidgetTester tester) async {
  ...
  testWidgets(
    'BUG-2527: a wide row keeps the stepper inline (no needless stacking)',
    (WidgetTester tester) async {
  ...
  testWidgets(
    'BUG-2527: the declared stepper trailing width matches what it renders',
    (WidgetTester tester) async {
