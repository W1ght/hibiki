...
void main() {
  test('update section gated by capability helper, not Platform.isAndroid', () {
    ...
    expect(
      systemDest.contains('visible: (_) => Platform.isAndroid'),
      isFalse,
  ...
  test('UpdateChecker no longer hard-returns on non-Android', () {
    ...
    expect(
      src.contains('if (!Platform.isAndroid) return;'),
      isFalse,
  ...
  test(
    'release workflow publishes platform-filtered debug Android channel',
    () {
      ...
      expect(workflow, contains(r'ANDROID_BUILD_NUMBER=$RELEASE_SEQUENCE'));
  ...
  test(
    ...
    () {
      ...
      expect(
        pushLines.any((String l) => l.trim() == 'paths:'),
        isTrue,
      ...
      final String branchesLine = pushLines
          .firstWhere(
            (String l) => l.trim().startsWith('branches:'),
            orElse: () => '',
          )
          .trim();
      expect(
        ...
        "branches: ['main']",
        reason:
            'push 触发分支范围不得漂移（2026-09-03 起 main-only，'
      ...
      expect(
        workflow,
        contains(r'BUILD_VERSION_NAME="${BUILD_VERSION_NAME%%+*}"'),
      );
      ...
      expect(
        ...
        reason:
            'installed Windows debug build must report the debug run version',
      );
      expect(
        workflow,
        contains(
  ...
  test('build docs describe platform-scoped debug run numbers', () {
    ...
    expect(docs, contains(r'v<version>-debug.<seq>+<short-sha>'));
    expect(docs, contains('git rev-list --count HEAD'));
    expect(docs, contains('github.run_number` / `GITHUB_RUN_NUMBER'));
    expect(docs, contains('single GitHub Release'));
    expect(docs, isNot(contains(r'debug-<short-sha>')));
  });
}
