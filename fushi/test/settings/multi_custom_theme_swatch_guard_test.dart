...
void main() {
  ...
  group('M1 swatch row renders one swatch per custom theme', () {
    test('iterates appModel.customThemes into swatches', () {
      expect(
        actions.contains('appModel.customThemes.map('),
        isTrue,
    ...
    test('tapping a swatch pins the theme by id (custom-theme:<id>)', () {
      expect(
        actions.contains(r'custom-theme:${e.id}'),
        isTrue,
    ...
    test('long-press a swatch edits that specific theme', () {
      expect(
        actions.contains('CustomThemePage(themeId: e.id)'),
        isTrue,
    ...
    test(
      'has a +new swatch that opens a draft editor (BUG-1841: no upsert)',
      () {
        expect(
          actions.contains('(_) => const CustomThemePage()'),
          isTrue,
          reason: 'missing the +new swatch draft-editor path',
        );
        expect(
          actions.contains('Icons.add'),
          isTrue,
        ...
        expect(
          actions.contains('upsertCustomTheme('),
          isFalse,
          reason: 'swatch row must not persist a theme before the editor opens',
        );
        expect(
          actions.contains('createBlankCustomTheme'),
          isFalse,
    ...
    test('keeps a focus-reachable edit button', () {
      expect(
        actions.contains('t.edit_custom_theme'),
        isTrue,
    ...
    test('BUG-1894: edit button is disabled when there is nothing to edit', () {
      expect(
        actions.contains('enabled: activeCustomThemeId != null'),
        isTrue,
        reason:
            'the edit button must be disabled with no active custom '
            'theme instead of falling back to a blank-draft editor',
      );
    });

    test(
      'BUG-1894: the edit target is resolved once, shared by onTap+enabled',
      () {
        // 门和目标必须读同一个值，否则又会长出「按钮亮着但没有目标」的状态。
        expect(
          RegExp(
            r'final String\? activeCustomThemeId =\s*\n?\s*'
        ...
        expect(
          actions.contains('CustomThemePage(themeId: activeCustomThemeId)'),
          isTrue,
          reason: 'the edit button must route to the active entry',
        );
        // 旧的行内解析（onTap 里现算一个 activeId）不得回来。
        expect(
          actions.contains('final String? activeId ='),
          isFalse,
    ...
    test(
      'BUG-1894: the blank-draft editor has exactly one entry point (+)',
      () {
        ...
        expect(
          ...
          1,
          reason:
              'blank-draft CustomThemePage() must have exactly one call '
              'site — the +new swatch',
        );
      },
    );
    ...
    test(
      ...
      () {
        ...
        expect(
          actions.contains('customThemeDisplayName'),
          isFalse,
  ...
  group('M2 editor edits a specific entry with name + delete', () {
    ...
    test('apply routes through the list model (upsert + select + key)', () {
      expect(
        page.contains('appModel.upsertCustomTheme(entry)'),
        isTrue,
    ...
    test('share-code wire is unchanged (id/name not in the code)', () {
      ...
      expect(
        page.contains("var code = 'hibiki-theme:"),
        isTrue,
