...
void main() {
  ...
  group('macOS restart uses LaunchServices', () {
    test('DesktopLifecycleService restarts macOS via open --args', () {
      ...
      expect(
        src.contains('Platform.isMacOS'),
        isTrue,
        reason: 'macOS must not use the raw executable restart path.',
      );
      expect(
        src.contains("'/usr/bin/open'"),
        isTrue,
        reason: 'sandboxed macOS app restarts must go through LaunchServices.',
      );
      expect(
        src.contains("'--args'"),
        isTrue,
      ...
      expect(
        src.contains('macOSAppBundlePathForExecutable'),
        isTrue,
