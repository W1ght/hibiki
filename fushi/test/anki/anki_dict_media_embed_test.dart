...
void main() {
  ...
  group('dictionary media embed — bare filename contract', () {
    test(
      'bare ref replaces placeholder → valid single <img>, no nesting',
      () async {
    ...
    test(
      'null store ref (cache miss / store failure) drops the entry',
      () async {
        final Map<String, String> tags = await repo.tagsFor(
          const <DictionaryMedia>[_gaijiMedia],
  ...
  group(
    'ankiInlineMediaReference — single source of bare ref (both backends)',
    () {
  ...
  group('{video-clip} == {book-cover} within one backend (TODO-843)', () {
    ...
    test(
      'mapping {video-clip} yields the same field value as {book-cover}',
      () {
