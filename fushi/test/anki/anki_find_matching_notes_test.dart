...
void main() {
    ...
    final client = MockClient((req) async {
      ...
      if (action == 'findNotes') {
        return http.Response(
          jsonEncode({'result': findNotesResult, 'error': null}),
          200,
        );
      }
      if (action == 'notesInfo') {
        ...
        return http.Response.bytes(
          utf8.encode(jsonEncode({'result': info, 'error': null})),
          200,
  ...
  test(
    'returns all matches newest-first regardless of overwriteScope=latest',
    () async {
      await seedSettings();
      final repo = repoWith(
  ...
  test(
    'openNoteInAnki issues guiBrowse with nid query and returns true',
    () async {
      await seedSettings();
