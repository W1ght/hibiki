...
void main() {
  ...
  test('extracts into addons21/<id> and writes an enabled meta.json', () async {
    ...
    final Directory addonDir = Directory(
      p.join(tmp.path, 'addons21', kAnkiConnectAddonId),
    );
  ...
  test(
    're-install keeps user meta config and re-enables a disabled addon',
    () async {
      final Directory addonDir = Directory(
        p.join(tmp.path, 'addons21', kAnkiConnectAddonId),
  ...
  test('zip-slip entries are skipped', () async {
    ...
    expect(
      File(
        p.join(tmp.path, 'addons21', kAnkiConnectAddonId, '__init__.py'),
      ).existsSync(),
      isTrue,
    );
  });
  ...
  test('bundled asset is a valid addon zip with the real plugin inside', () {
    ...
    final ArchiveFile init = archive.files.firstWhere(
      (ArchiveFile f) => f.name == '__init__.py',
    );
    final String source = utf8.decode(init.content as List<int>);
    expect(source, contains('anki_version'));
    final ArchiveFile config = archive.files.firstWhere(
      (ArchiveFile f) => f.name == 'config.json',
    );
  ...
  test('bundled asset ships every module its __init__.py imports', () {
    ...
    final File asset = File('assets/anki/ankiconnect.ankiaddon');
    final Archive archive = ZipDecoder().decodeBytes(asset.readAsBytesSync());
    final Set<String> packaged = archive.files
        .map((ArchiveFile f) => f.name)
        .toSet();
    final ArchiveFile init = archive.files.firstWhere(
      (ArchiveFile f) => f.name == '__init__.py',
    ...
    expect(imported, isNotEmpty, reason: '解析不出任何相对导入，正则跟不上上游写法了，这条守卫已经空转');
