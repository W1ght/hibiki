...
void main() {
  ...
  JimakuPlanSubtitleResolver buildResolver(
    MockClient client, {
    String apiKey = 'key',
  }) {
  ...
  test('错季：包 01-12 遇绝对编号条目 13-24 → 失败带原因，且一个字节都没下', () async {
    final MockClient client = jimakuServing(<int>[
      for (int e = 13; e <= 24; e++) e,
    ]);
    final ResolvedPlanSubtitles result = await buildResolver(
      client,
    ).resolve(pendingPlan(), packVideos(12));
  ...
  test('条数收敛：条目 24 集、包只 12 集 → 只下 12 条且都是包里的集', () async {
    ...
    expect(result.subtitles.map((PlanSubtitle s) => s.episode).toList(), <int>[
      for (int e = 1; e <= 12; e++) e,
    ]);
    // 只对这 12 集发过下载请求（第 13 集之后一条都没碰）。
    final List<String> downloads = requested
        .where((String u) => u.contains('/f/'))
        .toList();
  ...
  test('缺 API key / 缺条目 id → 明确原因，不发请求', () async {
    ...
    expect(
      (await buildResolver(
        client,
        apiKey: '  ',
      ).resolve(pendingPlan(), packVideos(1))).failureReason,
      'jimaku api key missing',
    );
    expect(
      (await buildResolver(
        client,
      ).resolve(pendingPlan(entryId: null), packVideos(1))).failureReason,
