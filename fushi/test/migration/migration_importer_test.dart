...
void main() {
  ...
  Future<void> writeBatch(
    ...
    File(
      p.join(tmp.path, MigrationExporter.manifestNameFor(batch)),
  ...
  group('缺「所有文件访问权限」时的扫描行为（用户实测翻车形态）', () {
    test('短路返回，绝不产出「清单损坏」这种假话', () async {
      ...
      final MigrationScanResult r = await importer.scan(
        tmp,
    ...
    test('有权限时照常逐批扫（不因新增的门把正常路径掐掉）', () async {
      ...
      expect(r.ready.map((MigrationImportBatch b) => b.batch), <MigrationBatch>[
        MigrationBatch.core,
      ]);
    });
  });
  ...
  test('源码守卫：清单/归档读失败与「清单损坏」分开归类，且 catch-all 在后', () {
    final String src = File(
      'lib/src/migration/migration_importer.dart',
    ).readAsStringSync();
    ...
    expect(
      corruptIdx,
      greaterThan(manifestFsIdx),
      reason: 'catch-all 必须排在专门分支之后，否则专门分支永远够不着',
    );

    expect(
      src.substring(manifestFsIdx, corruptIdx),
      contains('无法读取'),
      reason:
          '读失败的文案必须说「无法读取」，不能说「损坏」——'
          '后者会把用户推去重新导出 11GB，而重导多少次都一样',
    );

    // 归档校验同样要兜住：不兜的话异常炸穿整个 scan，连「哪批出问题」都丢了。
    expect(
      src.indexOf('on FileSystemException catch', corruptIdx),
  ...
  group('hasTransferData：布尔问题不许走全量校验和', () {
    test('只看 zip 在不在，空目录/不存在为 false', () async {
      expect(
        importer.hasTransferData(Directory(p.join(tmp.path, 'nope'))),
        isFalse,
      );
    ...
    test('归档内容坏掉也仍然算「有数据」（这是入口判据，不是完整性判据）', () async {
      await writeBatch(MigrationBatch.core, positions: 1);
      final File zip = File(
        p.join(tmp.path, MigrationExporter.archiveNameFor(MigrationBatch.core)),
      );
      zip.writeAsBytesSync(<int>[0, 1, 2], flush: true);
      expect(
        importer.hasTransferData(tmp),
        isTrue,
    ...
    test('源码守卫：失败分支必须落日志', () {
      ...
      final String tail = src.substring(
        catchIdx,
        (catchIdx + 600).clamp(0, src.length),
      );
      expect(
        tail,
        contains('debugPrint'),
    ...
    test('源码守卫：首页 banner 不得调用 scan()', () {
      ...
      expect(
        bannerSrc.contains('_importer.scan('),
        isFalse,
        reason: 'banner 只需要布尔答案，用 scan 会让 app 每次回前台都满载算校验和',
      );
      expect(
        bannerSrc.contains('_importer.hasTransferData('),
        isTrue,
        reason: '存在性检查必须走廉价路径',
      );
    });
  ...
  test('scan：齐全批次通过、损坏批次进 problems 且不阻塞其他批', () async {
    ...
    // 弄坏 books 的归档（截断）。
    final File booksZip = File(
      p.join(tmp.path, MigrationExporter.archiveNameFor(MigrationBatch.books)),
    );
  ...
  test('scan：缺清单/清单损坏分别报出；空目录无事', () async {
    ...
    await writeBatch(MigrationBatch.core, positions: 1);
    File(
      p.join(tmp.path, MigrationExporter.manifestNameFor(MigrationBatch.core)),
  ...
  test('aggregateExpectedCounts 取逐表最大；verifyImportedCounts 不足才红', () async {
    ...
    final MigrationScanResult r = await importer.scan(tmp);
    final Map<String, int> expected = MigrationImporter.aggregateExpectedCounts(
      r.ready,
    );
