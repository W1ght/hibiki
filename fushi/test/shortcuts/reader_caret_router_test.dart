...
void main() {
  ...
  group('ReaderCaretRouter.decideGamepad (cursor active)', () {
    test('D-pad maps to physical move directions', () {
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.dpadUp),
        CaretAction.moveUp,
      );
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.dpadDown),
        CaretAction.moveDown,
      );
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.dpadLeft),
        CaretAction.moveLeft,
      );
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.dpadRight),
        CaretAction.moveRight,
      );
    });
    ...
    test('A activates, B dismiss-or-exit', () {
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.a),
        CaretAction.activate,
      );
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.b),
        CaretAction.dismissOrExit,
      );
    });
    ...
    test('RT / LT jump to next / previous dictionary section (TODO-070)', () {
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.rt),
        CaretAction.jumpDictNext,
      );
      expect(
        ReaderCaretRouter.decideGamepad(GamepadButton.lt),
        CaretAction.jumpDictPrev,
      );
    });
    ...
    test(
      'X / Y / shoulders return null (kept for bookmark/chrome/page-turn)',
      () {
  ...
  group('ReaderCaretRouter enter triggers (cursor inactive)', () {
    ...
    test('other keys are not enter triggers', () {
      ...
      expect(
        ReaderCaretRouter.isEnterTriggerKeyboard(LogicalKeyboardKey.arrowRight),
        isFalse,
      );
      expect(
  ...
  group('ReaderCaretRouter enter triggers are remappable (TODO-700 T7)', () {
    test(
      'custom enterKeys: only the bound key enters, default A/Enter do not',
      () {
    ...
    test(
      'an empty override disables enter entirely (no key/button enters)',
      () {
        expect(
          ReaderCaretRouter.isEnterTriggerKeyboard(
            LogicalKeyboardKey.enter,
