...
void main() {
  ...
  test('BUG-1995: barrier 与弹窗表面共用同一份 token 判据（不许各判各的）', () {
    ...
    for (final String path in <String>[
      ...
      expect(
        ...
        isTrue,
        reason:
            '$path 的 barrier 鼠标钩子必须复用 dictionaryPopupPointerToken，'
      ...
      expect(
        ...
        isTrue,
        reason:
            '$path 的 barrier 鼠标钩子必须经 dispatchClaimedMouseAction 派发，'
  ...
  /// 一个文件里两条腿而只有一处认领同样会红。
  test('GUARD: 每条折弹窗指针 token 的腿都经 dispatchClaimedMouseAction 派发', () {
    // 定义处本身不是消费点。
    ...
    final List<File> legs = Directory('lib')
        ...
        .where((File f) => !f.path.replaceAll(r'\', '/').endsWith(definition))
        .toList();
    ...
    for (final File leg in legs) {
      ...
      final int claims = 'dispatchClaimedMouseAction('.allMatches(src).length;
      if (claims < folds) {
    ...
    expect(
      ...
      isEmpty,
      reason:
          '这些腿折了弹窗指针 token 却没经认领入口派发：$unclaimed。'
  ...
  test('GUARD: 每个 LookupDismissBarrier 宿主都接了 onNonPrimaryButtonDown', () {
    ...
    expect(
      ...
      isEmpty,
      reason:
          '这些 barrier 宿主没接鼠标非主键通道：$missing。'
