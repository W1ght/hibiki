...
void main() {
  ...
  String bodyOf(String name) {
    ...
    final int end = index + 1 < all.length
        ? all[index + 1].start
        : gamepadCpp.length;
  ...
  group('BUG-1541 蓝牙手柄待机重连自愈（vendored 原生源码契约）', () {
    test('① 断开回调不得在 GameInput 回调线程上 join', () {
      ...
      expect(
        body.contains('join_and_destroy'),
        isFalse,
        reason:
            '①：on_gamepad_disconnected 跑在 GameInput 回调线程上，'
    ...
    test('① retire 只入队 + 唤醒，收割线程负责 join', () {
      ...
      expect(
        retireBody.contains('.join()'),
        isFalse,
        reason: '①：retire 必须非阻塞（会被 GameInput 回调线程调用）',
      );
      expect(
        retireBody,
        contains('reaper_cv.notify_one()'),
      ...
      expect(
        gamepadH,
        contains('std::thread reaper_thread'),
        reason: '①：收割线程句柄须被持有（可 join，不 detach）',
      );
      expect(
        gamepadH,
        contains('std::condition_variable reaper_cv'),
    ...
    test('① init 在注册设备回调前就把收割线程拉起来', () {
      ...
      expect(
        registerStart,
        greaterThan(reaperStart),
        reason:
            '①：收割线程必须早于 RegisterDeviceCallback 就绪，'
            '否则第一次断开回调就没人接手',
      );
    });
    ...
    test('① stop 仍然 join 收割线程（teardown 不留悬空线程，BUG-116 不变式）', () {
      ...
      expect(
        stopBody,
        contains('reaper_thread.join()'),
        reason: '①：stop 须 join 收割线程',
      );
      expect(
        stopBody,
        contains('drain_retired()'),
    ...
    test('② 断开时 GetDeviceInfo 失败要退化成按设备指针匹配，不得 return', () {
      ...
      expect(
        body,
        contains(r'(*it)->device == device'),
        reason: '②：GetDeviceInfo 返回 null 时须按 AddRef 过的设备指针匹配',
      );
      expect(
        body.contains('failed to read info" << std::endl;\n    return;'),
        isFalse,
    ...
    test('④ 轮询线程掉线/收工时补发「松开」，上层状态不被锁死', () {
      ...
      expect(
        ...
        reason:
            '④：轮询循环须感知设备是否还 Connected，'
            '而不是对着死设备一直 GetCurrentReading',
      );
      expect(
        body,
        contains('GameInputDeviceConnected'),
        reason: '④：须按 Connected 位判断',
      );
      expect(
        body.split('neutralize_inputs').length - 1,
      ...
      expect(
        neutralizeBody,
        contains('diff_states'),
        reason: '④：松开事件须由当前状态与中立状态求差得到',
      );
      expect(
        neutralizeBody,
        contains('event_emitter'),
