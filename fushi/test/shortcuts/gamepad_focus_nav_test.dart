...
void main() {
  ...
  testWidgets('focus root moves geometrically in a two-column grid', (
    ...
    expect(
      ...
      reason:
          'D-pad down in a shelf grid must move to the row below, not the '
    ...
    expect(
      ...
      reason:
          'D-pad down at the grid edge must stop instead of sliding '
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        ...
        reason:
            'down must land on the immediately-next row, not skip the '
  ...
  testWidgets('gamepad escapes a focus root edge to sibling controls', (
    ...
    expect(moved, isTrue);
    expect(
      rail.hasPrimaryFocus,
      isTrue,
      reason:
          'left from the shelf edge must leave the book grid and focus '
  ...
  testWidgets('gamepad escapes a focus root top edge to top controls', (
    ...
    expect(
      ...
      isTrue,
      reason:
          'up from the shelf top edge must leave the book grid and focus '
  ...
  testWidgets('gamepad does not reading-order slide at a focus root edge', (
    ...
    expect(
      ...
      const FushiFocusId('bottom-right'),
      reason:
          'down at the bottom edge must not turn into a sideways '
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        moved,
        isFalse,
        reason: 'already at the bottom: edge takeover must not report movement',
      );
      expect(
        scrollCtrl.offset,
        closeTo(scrollCtrl.position.maxScrollExtent, 0.5),
      );
    },
  );

  testWidgets('focus recovers from the ring-less fallback once targets appear '
