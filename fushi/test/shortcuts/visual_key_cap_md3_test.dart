...
void main() {
  ...
  test('keyboard layout view stays free of bare MD3 literals', () {
    for (final String token in forbidden) {
      expect(
        layoutSrc.contains(token),
        isFalse,
        reason: 'KeyboardLayoutView must not reopen MD3 decision via "$token"',
      );
    }
  });
  ...
  test('gamepad layout view stays free of bare MD3 literals (TODO-942 P1)', () {
    for (final String token in forbidden) {
      expect(
        gamepadLayoutSrc.contains(token),
        isFalse,
        reason: 'GamepadLayoutView must not reopen MD3 decision via "$token"',
      );
    }
  });
  ...
  test('gamepad button widget stays free of bare MD3 literals', () {
    for (final String token in forbidden) {
      expect(
        gamepadButtonSrc.contains(token),
        isFalse,
        reason: 'GamepadButtonWidget must not reopen MD3 decision via "$token"',
      );
    }
  });

  test('gamepad figure shell derives visible contrast from onSurface '
      '(TODO-942 v3)', () {
    ...
    // 任何主题下都有确定的明度差。
    expect(
      gamepadLayoutSrc.contains('Color.alphaBlend'),
      isTrue,
      reason:
          'chassis fill must blend onSurface over surface for a '
          'guaranteed lightness delta in every theme',
    );
    expect(
      gamepadLayoutSrc.contains('scheme.onSurface'),
      isTrue,
      reason: 'chassis colors must derive from the onSurface contrast role',
    );
    expect(
      gamepadLayoutSrc.contains('scheme.outlineVariant'),
      isFalse,
      reason:
          'outlineVariant melts into the settings panel (the TODO-942 '
          'invisible-chassis root cause) and is banned in the chassis',
    );
    expect(
      gamepadLayoutSrc.contains('tokens.surfaces.'),
      isFalse,
      reason:
          'surface-container tokens carry no lightness delta against '
  ...
  test('key cap routes radii/surfaces through FushiDesignTokens', () {
    ...
    expect(
      keyCapSrc.contains('tokens.radii.chipRadius'),
      isTrue,
      reason: 'corner radius must come from the token, not a literal',
    );
    expect(
      keyCapSrc.contains('tokens.surfaces.'),
      isTrue,
  ...
  test('modifier caps use the secondaryContainer partition color', () {
    ...
    expect(
      keyCapSrc.contains('secondaryContainer'),
      isTrue,
