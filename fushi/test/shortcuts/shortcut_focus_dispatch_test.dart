...
void main() {
  testWidgets('#1: ancestor onKeyEvent still fires while a focused descendant '
    ...
    expect(
      ...
      reason:
          'ancestor reader Focus must still receive the key — keys are '
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        received,
        contains(LogicalKeyboardKey.digit1),
        reason:
            'autofocus:true makes the home wrapper the primary focus so '
  ...
  testWidgets('#4: without any focus, no handler receives the key (the gap)', (
    ...
    expect(
      ...
      isEmpty,
      reason:
          'documents the pre-fix mobile gap: with no primary focus the '
