...
void main() {
  group('video scope is registered in the shortcut registry (TODO-134)', () {
    test('the video scope exists and owns actions', () {
      ...
      final List<ShortcutAction> videoActions = ShortcutAction.actionsForScope(
        ShortcutScope.video,
      );
    ...
    test(
      'default video keys resolve through the registry (asbplayer/mpv map)',
      () {
  ...
  group(
    'settings-page scope enumeration covers the video scope (TODO-134)',
    () {
      test('iterating ShortcutScope.values x actionsForScope (how the settings '
  ...
  group('video rebind persists and resolves to the new key (TODO-134)', () {
      ...
      () {
        ...
        registry.updateBinding(
          ...
          const ShortcutBindingSet(
            keyboardBindings: <InputBinding>[newBinding],
          ),
        );
    ...
    test('buildVideoPlayerShortcutsFromRegistry keeps subtitle-blur '
  ...
  test(
    'toJson exposes a video action key (registry persistence covers video)',
    () {
      final FushiShortcutRegistry registry = FushiShortcutRegistry()
        ..loadDefaults(TargetPlatform.windows);
  ...
  group('old snapshots get newly-added default keys back (BUG-318 / TODO-562: '
    ...
    test('legacy snapshot (only F, no schema version) regains F12 after load '
    ...
    test('a snapshot the user actually rebound (fullscreen on G, not F) is NOT '
    ...
    test('a current snapshot (schema version present) is not migrated again — '
