...
void main() {
  ...
  test('client 令牌收进「手动填写」折叠 + 已连接只显示状态，不再带 per-peer 说明文案', () {
    ...
    expect(
      client.contains('ExpansionTile('),
      isTrue,
      reason: 'client 令牌框未收进折叠项——原始令牌又被摆成显眼数字会重现困惑。',
    );
    expect(
      client.contains('t.sync_client_token_manual'),
      isTrue,
      reason: '「手动填写令牌」折叠标题丢失。',
    );
    expect(
      client.contains('t.sync_client_connected'),
      isTrue,
      reason: '已连接状态提示丢失——应以状态代替摆出原始令牌数字。',
    );
    expect(
      client.contains('labelText: t.sync_client_token'),
      isTrue,
      reason: '折叠项内的令牌输入框标签丢失（手填回退路径必须保留）。',
    );
    // 解释「两端为何不一样」的说明已删除——改用隐藏令牌而非加说明。
    expect(
      client.contains('t.sync_client_token_hint'),
      isFalse,
      reason: 'client 不应再带解释性说明——隐藏令牌后无需解释差异。',
    );
    expect(
      client.contains('labelText: t.sync_server_token'),
      isFalse,
  ...
  test(
    'server 令牌显示保留 sync_server_token，且不再带 sync_server_token_self_hint 说明',
    () {
      final String corpus = readSyncSettingsSchemaSource();
      final String server = serverWidgetSlice(corpus);
      expect(
        server.contains('Text(t.sync_server_token'),
        isTrue,
        reason: 'server 共享令牌显示标签丢失。',
      );
      expect(
        server.contains('t.sync_server_token_self_hint'),
        isFalse,
  ...
  test(
    '令牌取值来源不被对调：client 读 getFushiClientToken、server 读 getServerPassword',
    () {
      final String corpus = readSyncSettingsSchemaSource();
      final String client = clientWidgetSlice(corpus);
      final String server = serverWidgetSlice(corpus);
      expect(
        client.contains('getFushiClientToken()'),
        isTrue,
        reason: 'client 必须读 per-peer 客户端令牌（getFushiClientToken）。',
      );
      expect(
        client.contains('getServerPassword()'),
        isFalse,
        reason: 'client 不得读 host 共享令牌。',
      );
      expect(
        server.contains('getServerPassword()'),
        isTrue,
        reason: 'server 必须显示共享服务器令牌（getServerPassword）。',
      );
      expect(
        server.contains('getFushiClientToken()'),
        isFalse,
  ...
  test('server _validateAuth 仍双接受：共享 _token + 任一 per-peer token', () {
    ...
    expect(
      body.contains('utf8.encode(_token)'),
      isTrue,
      reason: '_validateAuth 丢了共享令牌兼容路径（Never break userspace）。',
    );
    expect(
      body.contains('_peerTokens()'),
      isTrue,
