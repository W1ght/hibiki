...
void main() {
  ...
  test(
    'restoreAuth is authenticated when urls + token are configured',
    () async {
      final FushiDatabase db = _testDb();
      addTearDown(db.close);
      final SyncRepository repo = SyncRepository(db);
  ...
  test(
    'ensureResolved selects the reachable url and clears cache on switch',
    () async {
      final FushiDatabase db = _testDb();
      addTearDown(db.close);
      final SyncRepository repo = SyncRepository(db);
      ...
      backend.restoreCache(
        rootFolderId: 'http://lan:8765/$kSyncRootFolderName/',
      );
  ...
  test(
    'listRemoteVideos degrades to empty list on host 404 (old host)',
    () async {
