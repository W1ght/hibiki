...
void main() {
  ...
  group(
    'BUG-406 audiobook download wiring is present at both download sites',
    () {
      test(
        ...
        () {
          ...
          expect(
            src,
            contains('book.downloadId'),
            reason:
                '远端有声书 bookKey 必须 = book.downloadId（host 真实 key），'
                '与 EPUB 下载同源',
          );
          expect(
            src,
            isNot(contains('sanitizeTtuFilename(book.title)')),
            reason: '禁止用 sanitizeTtuFilename(title) 重算有声书 key（BUG-414 致 404）',
          );
          // 有声书失败有专用可见错误（不静默吞）。
          expect(
            src,
            contains('remote_book_audiobook_download_failed'),
      ...
      test(
        ...
        () {
          ...
          expect(
            src,
            contains('_downloadLiveAudiobookFor('),
          ...
          expect(
            src,
            isNot(contains('sanitizeTtuFilename(entry.title)')),
            reason: '禁止用 sanitizeTtuFilename(title) 重算有声书 key（BUG-414 致 404）',
          );
        },
      );
  ...
  group('TODO-809 live audiobook sync pulls remote-only into local books', () {
    test('orchestrator _syncAudiobooksLive wires toPull download + import '
        '(not push-only)', () {
      ...
      expect(
        src,
        contains('diff.toPull'),
        reason:
            '立即/自动同步有声书必须遍历 diff.toPull（双向拉取），'
            '不能再 push-only',
      );
      // Pull 经 live API 下载有声书包。
      expect(
        src,
        contains('backend.getRemoteAudiobook('),
  ...
  group('BUG-414/TODO-809 forward guard: audiobook write paths bind the SOURCE '
    ...
    test('orchestrator pull import binds bookKeyOverride to the local EPUB key '
      ...
      expect(
        src,
        isNot(contains('bookKeyOverride: sanitizeTtuFilename(')),
        reason: '禁止把 sanitizeTtuFilename(...) 作为 audiobook 写入 override',
      );
    });

    test('all three pull write sites import audiobooks WITHOUT '
      ...
      for (final String site in sites) {
        ...
        expect(
          src,
          isNot(contains('sanitizeTtuFilename(a.title)')),
