...
void main() {
  group('isTransientSyncError (BUG-864)', () {
    ...
    test('true for the reported Windows semaphore-timeout SocketException', () {
      ...
      const String reported =
          'ClientException with SocketException: 信号灯超时时间已到 '
    ...
    test('false for permanent auth / permission / not-found errors', () {
      expect(
        isTransientSyncError('insufficient_scope: re-consent required'),
        isFalse,
      );
  ...
  group('retryTransientSync (BUG-864)', () {
    test(
      'retries a transient failure then succeeds, backing off each attempt',
      () async {
        int calls = 0;
    ...
    test(
      'gives up after maxAttempts and rethrows the last transient error',
      () async {
        int calls = 0;
        await expectLater(
    ...
    test(
      'does NOT retry a non-transient error — rethrows on first attempt',
      () async {
        int calls = 0;
        await expectLater(
          retryTransientSync<void>(() async {
            calls++;
    ...
    test(
      'a first-attempt success never sleeps (common path zero overhead)',
      () async {
        int calls = 0;
        bool slept = false;
        final int result = await retryTransientSync<int>(() async {
          calls++;
          return 42;
