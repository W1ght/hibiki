...
void main() {
  group('视频远端断点键（video_remote_position_*）逐字节冻结', () {
    test('位置键 / 时间戳键 / 集数变体', () {
      expect(
        videoRemotePositionPrefKey('video/my_film'),
        'video_remote_position_video/my_film',
      );
      expect(
        videoRemotePositionAtPrefKey('video/my_film'),
        'video_remote_position_at_video/my_film',
      );
      // 集数变体（TODO-885）：index<=0 回退整书键；index>0 带 #ep 后缀。
      expect(
        videoRemotePositionEpisodePrefKey('u', 0),
        'video_remote_position_u',
      );
      expect(
        videoRemotePositionEpisodePrefKey('u', 2),
        'video_remote_position_u#ep2',
      );
      expect(
        videoRemotePositionEpisodeAtPrefKey('u', 0),
        'video_remote_position_at_u',
      );
      expect(
        videoRemotePositionEpisodeAtPrefKey('u', 2),
    ...
    test('位置键逆解析：排除更长前缀的 at_ 时间戳键', () {
      expect(
        videoUidFromRemotePositionPrefKey('video_remote_position_u1'),
        'u1',
      );
      // 时间戳键绝不能被误解析成「以 at_ 开头的 uid」。
      expect(
        videoUidFromRemotePositionPrefKey('video_remote_position_at_u1'),
        isNull,
      );
      expect(
        videoUidFromRemotePositionPrefKey('video_remote_position_'),
        isNull,
      );
  ...
  group('PositionPrefKeys 生成器本体', () {
    test('两个冻结实例的前缀逐字节正确', () {
      expect(
        videoRemotePositionPrefKeys.positionPrefix,
