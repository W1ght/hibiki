...
void main() {
  group('isTtuPerBookFileName (root-spill predicate)', () {
    ...
    test(
      'matches title-PREFIXED spill (slash-less folderId fusion, BUG-845)',
      () {
  ...
  group('SyncOrchestrator.pruneRootSpill', () {
    ...
    test(
      'deletes only the spilled root files, keeps folders + nested + others',
      () async {
        ...
        final Set<String> rootNames = rootChildren
    ...
    test(
      'sweeps title-PREFIXED spill while keeping the book folder (BUG-845)',
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final FushiDatabase db = _memDb();
        addTearDown(db.close);
        ...
        final Set<String> rootNames = (await store.listChildren(
          'root',
    ...
    test(
      ...
      'DB reading/video progress intact (TODO-1346)',
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final FushiDatabase db = _memDb();
        addTearDown(db.close);
        ...
        await seedFile(
          store,
          'root/progress_1_6_collides_as_folder',
          _spilledProgress,
        );
        ...
        expect(
          rootNames,
          contains('progress_1_6_collides_as_folder'),
          reason:
              'a folder must never be swept, even if its name matches the '
        ...
        final VideoBookRow? vb = await db.getVideoBookByBookUid(
          'video/playlist/K-ON! 全集列表',
        );
        expect(vb, isNotNull);
