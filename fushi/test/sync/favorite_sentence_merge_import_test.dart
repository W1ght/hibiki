...
void main() {
  test(
    'merge import content-merges favorite_sentences (union + dedupe)',
    () async {
      ...
      final FavoriteSentenceRepository curRepo = FavoriteSentenceRepository(
        cur,
      );
      ...
      final FavoriteSentenceRepository srcRepo = FavoriteSentenceRepository(
        src,
      );
      ...
      final FavoriteSentence shared = all.firstWhere(
        (FavoriteSentence s) => s.text == '共有文',
      );
      ...
      // local(100).
      expect(all.map((FavoriteSentence s) => s.text).toList(), <String>[
        'バックアップ限定',
        '共有文',
        'ローカル限定',
      ]);
  ...
  test(
    'merge import with no favorite_sentences in backup leaves device blob',
    () async {
      final Directory curDir = await _tempDir('fs_cur2_');
      addTearDown(() => cleanupTempDir(curDir));
      final FushiDatabase cur = FushiDatabase(curDir.path);
      await FavoriteSentenceRepository(cur).add(
        FavoriteSentence(
          id: 'hl_keep',
          text: '残す',
          bookTitle: 'BookA',
