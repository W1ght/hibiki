...
void main() {
  group('resolveReachableFushiCandidate', () {
    ...
    test(
      'falls through to the next url when the first is unreachable',
      () async {
        final List<String> probed = <String>[];
        final FushiClientUrl result = await resolveReachableFushiCandidate(
          const <FushiClientUrl>[
    ...
    test(
      'throws SyncAuthError only after every candidate was rejected',
      () async {
        final List<String> probed = <String>[];
        await expectLater(
    ...
    test(
      'a candidate without its own token falls back to the global one',
      () async {
        final Map<String, String> tokensSeen = <String, String>{};
        final FushiClientUrl result = await resolveReachableFushiCandidate(
    ...
    test(
      'throws a retryable SyncBackendError when none are reachable',
      () async {
        await expectLater(
          resolveReachableFushiCandidate(
            const <FushiClientUrl>[
