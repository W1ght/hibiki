...
void main() {
  ...
  test(
    'sidecar tags round-trip rebuilt by name on the downloading device',
    () async {
      ...
      final Set<String> names = (await db.getTagsForBook(
        bookKey,
      )).map((BookTagRow t) => t.name).toSet();
      expect(names, <String>{'听力', 'N2'});
    },
  );

  test(
    'missing tags.json (legacy) imports fine with no tags and no throw',
    () async {
      final FushiDatabase db = _memDb();
      addTearDown(db.close);
      final _FakeBookFolderBackend backend = _FakeBookFolderBackend(
  ...
  test(
    'existing same-name tag on downloader is reused by name (no duplicate)',
    () async {
      final FushiDatabase db = _memDb();
      addTearDown(db.close);
  ...
  test(
    'malformed sidecar degrades safely to empty tags without throwing',
    () async {
      ...
      for (int i = 0; i < malformedCases.length; i++) {
        ...
        final String bookKey = await _importOnce(db, backend, freshTemp());
        expect(
          await db.getTagsForBook(bookKey),
          isEmpty,
