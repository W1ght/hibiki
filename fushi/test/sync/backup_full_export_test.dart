...
void main() {
  ...
  test(
    'export includes trees + meta roots; import restores files and rebases',
    () async {
  ...
  test('atomic tree restore leaves the existing tree intact when the backup '
    ...
    expect(
      File(p.join(dstBooks, 'Existing', 'keep.epub')).existsSync(),
      isTrue,
      reason: 'empty-prefix backup must not delete the existing tree',
    );
  });

  test('import clears stale .import-old/.import-tmp leftovers from a prior '
    ...
    Directory(dstDbDir).createSync(recursive: true);
    await writeFile(
      '$dstBooks.import-old${Platform.pathSeparator}junk.txt',
      'x',
    );
    await writeFile(
      '$dstBooks.import-tmp${Platform.pathSeparator}junk.txt',
      'x',
    );
