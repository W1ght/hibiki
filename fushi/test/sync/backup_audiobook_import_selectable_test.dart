...
void main() {
  ...
  group('export records the blob audiobook count in meta', () {
    test(
      'audiobooks ticked → count = rows; unticked → 0 + rows stripped',
      () async {
        ...
        final BackupMeta withAb = await service.createBackup(
  ...
  group('summarizeBackupEntries audiobook visibility', () {
    ...
    test(
      'meta.audiobookCount>0 shows audiobooks even with NO packed files',
      () {
  ...
  test(
    ...
    () async {
      ...
      expect(
        summary.has(BackupCategory.audiobooks),
        isTrue,
  ...
  group('overwrite import honors the audiobooks toggle', () {
    ...
    test(
      'unticking audiobooks strips every audiobooks row on import',
      () async {
