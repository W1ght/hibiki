...
void main() {
  ...
  test('半成功也刷新列表：书删掉了就不能留幽灵卡', () {
    ...
    expect(
      containsCodeLine(body, 'bookDeleted = true'),
      isTrue,
    ...
    expect(
      RegExp(
        r'if \(bookDeleted\) _forceRefreshRemoteBooks\(\);',
      ).hasMatch(masked),
      isTrue,
  ...
  test('提示语区分「书没删掉」与「书已删、有声书未删」', () {
    ...
    expect(
      containsCodeLine(body, 't.remote_delete_audiobook_partial'),
      isTrue,
      reason: '半成功必须有自己的文案；沿用「删除失败」与实情相反',
    );
    expect(
      containsCodeLine(body, 'if (!bookDeleted)'),
      isTrue,
  ...
  test('部分成功文案在 17 个语言文件里齐全（Slang 缺 key 直接编译失败）', () {
    ...
    for (final FileSystemEntity f in Directory('lib/i18n').listSync()) {
      ...
      expect(
        map.containsKey('remote_delete_audiobook_partial'),
        isTrue,
