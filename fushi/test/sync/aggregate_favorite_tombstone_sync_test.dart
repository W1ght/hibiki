...
void main() {
  group('mergeSnapshots 收藏删除仲裁（纯函数）', () {
    test('墓碑严格新于收藏 → 收藏出局、墓碑保留（删除传播）', () {
      final AggregateSnapshot merged = AggregateSyncService.mergeSnapshots(
        AggregateSnapshot(
          favoriteWords: <FavoriteWordRecord>[_word('言葉', createdAt: 100)],
        ),
        AggregateSnapshot(
      ...
      expect(
        merged.favoriteWordTombstones.single.deletedAt,
        200,
    ...
    test('重收藏（createdAt >= deletedAt）→ 收藏保留、墓碑退场（防删除僵尸）', () {
      final AggregateSnapshot merged = AggregateSyncService.mergeSnapshots(
        AggregateSnapshot(
          favoriteWords: <FavoriteWordRecord>[_word('言葉', createdAt: 300)],
        ),
        AggregateSnapshot(
          favoriteWordTombstones: <AggregateTombstoneRecord>[
            AggregateTombstoneRecord(itemKey: _key('言葉'), deletedAt: 200),
  ...
  group('端到端删除传播（真 DB fold）', () {
    test('A 取消收藏 → 快照带墓碑 → B fold 后本地行被删且墓碑续传', () async {
      ...
      final AggregateSnapshot snapA = await AggregateSyncService(
        dbA,
      ...
      expect(
        await dbB.getAllFavoriteWords(),
        isEmpty,
        reason: '对端的取消收藏必须把本机同键收藏删掉（此前永远删不掉）',
      );
      final AggregateSnapshot snapB = await AggregateSyncService(
        dbB,
      ).materializeLocalSnapshot();
      expect(
        snapB.favoriteWordTombstones,
        hasLength(1),
