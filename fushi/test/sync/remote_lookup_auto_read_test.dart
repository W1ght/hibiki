...
void main() {
  group('查词后自动朗读偏好下发', () {
    ...
    test('app 侧把偏好接到扩展 server 上（app_model → manager → server → handler）', () {
      ...
      expect(
        appModel,
        contains('autoReadOnLookupProvider:'),
        reason:
            'app_model 没把 autoReadOnLookupProvider 交给 YomitanApiServerManager',
      );
      expect(
        appModel,
        contains('ReaderFushiSource.instance.autoReadOnLookup'),
      ...
      expect(
        server,
        contains('autoReadOnLookupProvider: _autoReadOnLookupProvider'),
    ...
    test('扩展两镜像都按这个字段接了自动朗读（页面弹窗与侧边栏共用一份实现）', () {
      ...
      mirrors.forEach((String name, String root) {
        ...
        expect(
          autoRead,
          contains('window.fushiAutoReadFirstEntry'),
          reason: '[$name] 缺共享的自动朗读实现',
        );
        expect(
          autoRead,
          contains("callHandler('resolveWordAudio'"),
        ...
        for (final String surface in <String>['content.js', 'side-panel.js']) {
          ...
          expect(
            code,
            contains('fushiAutoReadFirstEntry'),
            reason: '[$name] $surface 没接自动朗读——两个表面行为漂开正是 BUG-1210 那个病',
          );
          expect(
            code,
            contains('autoReadOnLookup'),
