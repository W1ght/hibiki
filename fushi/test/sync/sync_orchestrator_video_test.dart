...
void main() {
  ...
  group('syncVideoAssets upload phase', () {
    ...
    test('switch ON uploads missing video + writes correct manifest', () async {
      ...
      final FushiDatabase db = _memDb();
      addTearDown(db.close);
      final String uid = await _seedLocalVideo(
        db,
        tmp,
        uid: 'video/Up',
        title: 'Up',
        contents: 'the-real-bytes',
      );
      ...
      final List<RemoteVideoManifestEntry> entries = await client
    ...
    test(
      're-run is idempotent (same size skipped, manifest byte-stable)',
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final FakeSyncBackend backend = FakeSyncBackend(store);
        final Directory tmp = Directory('${work.path}/tmp')..createSync();
        ...
        final AssetEntry? manifestA = await store.findAsset(
          kSyncVideosNamespace,
        ...
        expect(
          second.videosExported,
          0,
          reason: 'remote already has same-size video → skip upload',
        );
        expect(second.errors, isEmpty);
        final AssetEntry? manifestB = await store.findAsset(
          kSyncVideosNamespace,
          kSyncVideosManifestName,
        );
    ...
    test(
      'streaming video (URL / streamSpec / playlist) is not uploaded',
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final FakeSyncBackend backend = FakeSyncBackend(store);
        final Directory tmp = Directory('${work.path}/tmp')..createSync();
    ...
    test(
      'upload-only union keeps another device\'s remote-only entry',
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final FakeSyncBackend backend = FakeSyncBackend(store);
        final Directory tmp = Directory('${work.path}/tmp')..createSync();
        ...
        expect(
          uids,
          containsAll(<String>['video/FromA', 'video/FromB']),
  ...
  group('adversarial finding fixes', () {
    ...
    test(
      ...
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final SyncBackend backend = ObfuscatingSyncBackend(
          FakeSyncBackend(store),
        );
        final Directory tmp = Directory('${work.path}/tmp')..createSync();
        final FushiDatabase db = _memDb();
        addTearDown(db.close);
        await _seedLocalVideo(
          db,
          tmp,
          uid: 'video/Obf',
          title: 'Obf',
    ...
    // 远端没有封面）。
    test(
      'finding9 keeps prior coverAsset when the local cover file disappears',
      () async {
        final FakeAssetStore store = FakeAssetStore();
        final FakeSyncBackend backend = FakeSyncBackend(store);
        final Directory tmp = Directory('${work.path}/tmp')..createSync();
        ...
        final RemoteVideoManifestEntry e1 = (await CloudRemoteVideoClient(
          backend: store,
          backendType: SyncBackendType.webDav,
        ).listRemoteVideoManifest()).single;
        ...
        expect(
          e2.coverAsset,
          e1.coverAsset,
          reason: 'missing local cover file must not wipe published coverAsset',
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          first.videosExported,
          0,
          reason: 'corrupt manifest → no idempotency judge → skip upload',
        );
        expect(
          first.errors.any((String e) => e.contains('unreadable')),
          isTrue,
        ...
        expect(
          await store.getJsonAsset(manifest.id),
          isNull,
          reason: 'corrupt manifest must NOT be overwritten (would wipe peers)',
        );
