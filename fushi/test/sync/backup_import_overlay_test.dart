...
void main() {
  group('BackupImportOverlayView（修①/修②：两阶段遮罩）', () {
    ...
    testWidgets('running：progress 有值 → 确定进度条（value 非空且跟随更新）', (
      ...
      progress.value = 0.87;
      await tester.pump();
      final LinearProgressIndicator bar2 = tester
          .widget<LinearProgressIndicator>(
            find.byType(LinearProgressIndicator),
          );
  ...
  group('源码守卫 (TODO-1151)', () {
    ...
    test('main.dart：备份导入遮罩分支在裸 loading 分支之前，且渲染专用视图', () {
      ...
      final int backupIdx = src.indexOf(
        'appModel.backupImportOwnsAppRoot',
        buildIdx,
      );
      final int loadingIdx = src.indexOf(
      ...
      expect(
        ...
        isFalse,
        reason:
            'BUG-2106：根分支不得再用 backupImportActive（会把 validating 也换根，'
            '摧毁调用方路由与引导向导）',
      );
      expect(
        src.contains('if (appModel.backupImportOwnsAppRoot) {'),
        isTrue,
      ...
      expect(
        src.contains('onRestart: () => backupImportRestart(appModel)'),
        isTrue,
      );
    });
    ...
    test('backup.part：beginBackupImport 与 closeDatabase 之间必须 await endOfFrame '
      ...
      final int beginIdx = src.indexOf('appModel.beginBackupImport()');
      final int closeIdx = src.indexOf('await appModel.closeDatabase()');
      final int frameIdx = src.indexOf(
    ...
    test(
      ...
      () {
        ...
        expect(
          src.contains('completeBackupImport(t.backup_import_success)'),
          isTrue,
          reason: '导入成功必须切到确认视图，而非直接退出',
        );
        expect(
          src.contains('failBackupImport('),
          isTrue,
    ...
    test('backup.part：backupImportRestart 优先真重启(restartApp)+保留纯退出兜底', () {
      ...
      final int fnIdx = src.indexOf(
        'Future<void> backupImportRestart(AppModel appModel)',
      );
      ...
      expect(
        fnBody.contains('lifecycle.supportsRestart'),
        isTrue,
        reason: '重启前必须先判 supportsRestart',
      );
      expect(
        fnBody.contains('lifecycle.restartApp()'),
        isTrue,
    ...
    test('backup.part：导入成功后延时自动重启，失败态不自动（TODO-1151 自动重启）', () {
      ...
      final int completeIdx = importBody.indexOf(
        'completeBackupImport(t.backup_import_success)',
      );
      final int autoRestartIdx = importBody.indexOf(
        'await backupImportRestart(appModel)',
      );
      expect(completeIdx, greaterThan(0));
      expect(
        autoRestartIdx,
        greaterThan(completeIdx),
        reason: '导入成功后必须自动调 backupImportRestart（用户诉求：不再手动重开）',
      );
      expect(
        importBody.contains('Duration(seconds: 1)'),
        isTrue,
      ...
      expect(
        importBody.indexOf('backupImportRestart', failIdx),
        -1,
    ...
    test(
      'backup.part：validate/preview 期用 validating 遮罩，退出后经全局 navigator 弹确认框',
      () {
        final String src = readSource(
          'lib/src/sync/sync_settings_schema/backup.part.dart',
        ).readAsStringSync();
        ...
        final int beginValIdx = src.indexOf(
          'appModel.beginBackupValidating()',
          importIdx,
        );
        final int endValIdx = src.indexOf(
          'appModel.endBackupValidating()',
          importIdx,
        );
        final int confirmIdx = src.indexOf(
          '_showBackupImportConfirmDialog(',
          importIdx,
        );
        final int beginImportIdx = src.indexOf(
        ...
        expect(
          confirmIdx,
          lessThan(beginImportIdx),
          reason: '确认后才进 running 导入相位',
        );
        // 确认框宿主是全局 navigatorKey 的正常 MaterialApp（本设置页此刻已卸载）。
        expect(
          src.contains('appModel.navigatorKey.currentContext'),
          isTrue,
        ...
        final int overlayIdx = src.indexOf(
          '_BackupValidatingOverlay.show(',
          importIdx,
        );
        expect(
          overlayIdx,
          greaterThan(beginValIdx),
          reason: 'beginBackupValidating 之后必须压上遮罩路由（而非依赖换根）',
        );
        expect(overlayIdx, lessThan(endValIdx), reason: '遮罩必须在校验期间就在栈上');
        // 每条退出路径都得摘掉它：路由 canPop:false，漏一次 app 就被永久挡住。
        expect(
          src.contains('overlay.dismiss();'),
          isTrue,
          reason: '退出路径必须摘掉遮罩路由',
        );
    ...
    test('backup.part：遮罩路由只能 removeRoute 摘除（pop 会被 PopScope 拦下）', () {
      ...
      expect(
        body.contains('navigator.removeRoute<void>(route)'),
        isTrue,
        reason: 'BUG-2106：路由带 PopScope(canPop:false)，pop 摘不掉，必须 removeRoute',
      );
      expect(
        body.contains('navigator.pop('),
        isFalse,
    ...
    test('遮罩路由：不可 pop + 不透明 + 接线取消 (BUG-2106)', () {
      ...
      expect(
        src.contains('canPop: false'),
        isTrue,
        reason: '系统返回不得穿过遮罩去 pop 底下的调用方页面（引导向导）',
      );
      expect(
        src.contains('opaque: true'),
        isTrue,
        reason: '遮罩必须不透明：挡住底下页面的绘制与命中测试',
      );
      expect(
        src.contains('phase: BackupImportPhase.validating'),
        isTrue,
        reason: '本路由只承载 validating 相位',
      );
      expect(
        src.contains('onCancel: onCancel'),
        isTrue,
    ...
    test('validating 取消接线搬到遮罩路由的调用方 (BUG-2106)', () {
      ...
      expect(
        src.contains('appModel.cancelBackupValidating();'),
        isTrue,
