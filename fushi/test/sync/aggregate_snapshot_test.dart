...
void main() {
  group('AggregateSnapshot JSON round-trip', () {
    ...
    test('malformed rows are skipped not fatal', () {
      final AggregateSnapshot snap = AggregateSnapshot.fromJson(
        <String, Object?>{
          'version': 1,
          'readingStats': <Object?>[
            'garbage',
  ...
  group('mergeSnapshots invariants', () {
    ...
    test('merge is idempotent: merge(merge(a,b),b) equals merge(a,b)', () {
      ...
      final AggregateSnapshot twice = AggregateSyncService.mergeSnapshots(
        once,
        b,
      );

      expect(
        twice.readingStats.single.charactersRead,
    ...
    test('commutative on union set: merge(a,b) buckets equal merge(b,a)', () {
      ...
      expect(
        ab.readingStats.single.charactersRead,
        ba.readingStats.single.charactersRead,
      );
      expect(
        ab.readingStats.single.readingTimeMs,
    ...
    test(
      'hourly-by-format MAX per {dateKey, hour, format}; totals untouched',
      () {
    ...
    test(
      'an OLD payload missing readingHourlyByFormat defaults it to empty',
      () {
    ...
    test('lookup/mine counters MAX both columns; bookKey only when agreed', () {
      ...
      final LookupMiningRecord sharedAb = ab.lookupMiningCounters.firstWhere(
        (LookupMiningRecord r) => r.title == 'Book A',
      );
      ...
      final LookupMiningRecord sharedBa = ba.lookupMiningCounters.firstWhere(
        (LookupMiningRecord r) => r.title == 'Book A',
      );
  ...
  group('version compatibility', () {
    test(
      'an OLD payload missing lookupMiningCounters defaults it to empty',
      () {
    ...
    test(
      'an additive UNKNOWN key at the same version is ignored, not dropped',
      () {
        ...
        snap = AggregateSnapshot.fromJson(<String, Object?>{
          'version': AggregateSnapshot.currentVersion,
          'miningStats': <Object?>[
