...
void main() {
  group('summarizeBackupEntries (pure manifest)', () {
    test(
      'counts distinct dirs / files per category, normalizing separators',
      () {
  ...
  group('archiveBooksPrefix (legacy hoshi_books archive fallback)', () {
    ...
    test('archive written by old Hibiki (hoshi_books/) falls back to old '
        'prefix', () {
    ...
    test('archive without any books tree defaults to the legacy prefix '
  ...
  group('restoreBackup per-category gating', () {
    ...
    test(
      'unticking videos + audiobooks skips those trees; books still restore',
      () async {
        ...
        expect(
          Directory(dstAudio).existsSync(),
          isFalse,
          reason: 'audiobooks unticked - audio tree not restored',
        );
        expect(
          Directory(dstVideos).existsSync(),
          isFalse,
