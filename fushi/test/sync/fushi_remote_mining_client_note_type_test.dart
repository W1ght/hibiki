...
void main() {
  test('read：主机回传定义 → 反序列化成 AnkiNoteTypeDefinition', () async {
    ...
    final AnkiNoteTypeDefinition? def = await client.readNoteTypeDefinition(
      'Lapis',
    );
  ...
  test('read：主机可达但无该模型（noteType null）→ null', () async {
    ...
    final FushiRemoteMiningClient client = await _client(
      db,
      (http.Request _) => _json(<String, dynamic>{'noteType': null}),
    );
  ...
  test('read：旧版主机（404，无端点）→ null（按后端不支持降级）', () async {
    ...
    final FushiRemoteMiningClient client = await _client(
      db,
      (http.Request _) => http.Response('nope', 404),
    );
    expect(await client.readNoteTypeDefinition('Lapis'), isNull);
  });
  ...
  test('read：token 被拒（401）→ 抛 SyncAuthError', () async {
    ...
    final FushiRemoteMiningClient client = await _client(
      db,
      (http.Request _) => http.Response('nope', 401),
    );
    expect(
      () => client.readNoteTypeDefinition('Lapis'),
  ...
  test('styling：写穿 ok=true；旧版主机 404 → false', () async {
    ...
    addTearDown(db.close);
    http.Request? seen;
    final FushiRemoteMiningClient okClient = await _client(db, (
      http.Request r,
    ) {
      seen = r;
    ...
    final FushiRemoteMiningClient oldHost = await _client(
      db,
      (http.Request _) => http.Response('nope', 404),
    );
  ...
  test('templates：模板列表序列化进请求体', () async {
    ...
    expect((body['templates'] as List).single, <String, dynamic>{
      'name': 'Card',
      'front': 'F',
      'back': 'B2',
    });
