...
void main() {
  group('RemoteVideoManifest roundtrip', () {
    test('toJson → fromJson preserves every field', () {
      ...
      final RemoteVideoManifest decoded = RemoteVideoManifest.fromJson(
        jsonDecode(manifest.canonicalJson()),
      );

      expect(decoded.version, RemoteVideoManifest.currentVersion);
      expect(decoded.videos.length, 2);
      final RemoteVideoManifestEntry a = decoded.videos.firstWhere(
        (RemoteVideoManifestEntry e) => e.uid == 'video/A',
      );
      ...
      final RemoteVideoManifestEntry b = decoded.videos.firstWhere(
        (RemoteVideoManifestEntry e) => e.uid == 'video/B',
      );
      expect(b.title, '');
      expect(
        b.coverAsset,
    ...
    test('canonicalJson is order-independent (sorted by uid)', () {
      ...
      final String forward = const RemoteVideoManifest(
        videos: <RemoteVideoManifestEntry>[ea, eb],
      ).canonicalJson();
      final String reversed = const RemoteVideoManifest(
        videos: <RemoteVideoManifestEntry>[eb, ea],
      ).canonicalJson();
      expect(
        forward,
        reversed,
  ...
  group('RemoteVideoManifest.fromJson rejects malformed input', () {
    test('non-object throws', () {
      expect(
        () => RemoteVideoManifest.fromJson(<int>[1, 2, 3]),
    ...
    test('bad-typed sizeBytes/importedAtMs degrade to 0, not throw', () {
      final RemoteVideoManifest m = RemoteVideoManifest.fromJson(
        <String, dynamic>{
          'version': 1,
          'videos': <dynamic>[
            <String, dynamic>{
  ...
  group('asset name helpers', () {
    test('videoAssetName is deterministic, keeps extension, sanitizes uid', () {
      ...
      expect(
        name,
        videoAssetName('video/安達 2', '/anime/Ada.mkv'),
    ...
    test(
      'uids that sanitize to the same base still get distinct asset names',
      () {
  ...
  group('tags 稳健档：清单条目携带标签 LWW 时钟', () {
    test('tagsAddedAt / tagTombstones toJson↔fromJson 往返一致（version 仍为 1）', () {
      ...
      final manifest = RemoteVideoManifest(
