...
void main() {
  ...
  test('source guard: manual sync threads progress end-to-end', () {
    ...
    final autoTrigger = File(
      'lib/src/sync/sync_auto_trigger.dart',
    ).readAsStringSync();
    expect(
      autoTrigger.contains('SyncProgressCallback? onProgress'),
      isTrue,
    ...
    expect(
      autoTrigger.contains('ValueNotifier<SyncProgress?>'),
      isTrue,
      reason: 'an app-wide syncProgress notifier must exist',
    );
    expect(
      autoTrigger.contains('syncProgress.value = p'),
      isTrue,
      reason: 'both the manual and auto sweep must publish progress globally',
    );
    expect(
      autoTrigger.contains('onProgress?.call(p)'),
      isTrue,
      reason: 'manual sync must still forward to the caller callback',
    );
    expect(
      autoTrigger.contains('syncProgress.value = null'),
      isTrue,
    ...
    expect(
      widget.contains('LinearProgressIndicator(value: p?.fraction)'),
      isTrue,
    ...
    expect(
      widget.contains('valueListenable: syncInProgress'),
      isTrue,
      reason: 'the Sync-now row must listen to the global in-flight notifier',
    );
    expect(
      widget.contains('valueListenable: syncProgress'),
      isTrue,
  ...
  test(
    'logSyncReportErrors writes per-item sync failures to error log',
    () async {
