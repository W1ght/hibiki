...
RemotePopupDictionaryCss _css({
  ...
  dictionaryStyles: styles ?? <String, String>{'OALDPE': '.opal{color:red}'},
...
void main() {
  group('BUG-1718 弹窗 CSS 尾段下发', () {
    ...
    test('请求没有 stylesRevision 键 ⇒ 只带指纹，不带正文（老扩展不被灌 285 KB）', () async {
      ...
      expect(
        r.containsKey('dictionaryStyles'),
        isFalse,
    ...
    test('popupOnly 快路径与空词请求同样带上这套字段（扩展默认走 popupOnly）', () async {
      final RemotePopupDictionaryCss css = _css();
      final Map<String, dynamic> fast = await _lookup(<String, dynamic>{
        'term': '猫',
        'popupOnly': true,
        'stylesRevision': 'x',
      }, css: css);
      expect(fast['dictionaryStyles'], css.dictionaryStyles);
      final Map<String, dynamic> empty = await _lookup(<String, dynamic>{
        'term': '',
        'stylesRevision': css.revision,
      }, css: css);
    ...
    test(
      ...
      () {
        ...
        expect(
          appModel,
          contains('popupDictionaryCssProvider:'),
          reason:
              'app_model 没把 popupDictionaryCssProvider 交给 YomitanApiServerManager',
        );
        expect(
          appModel,
          contains('browserExtensionPopupDictionaryCss'),
    ...
    test('扩展两镜像都接上了 CSS 尾段与词条资源占位的兑现方', () {
      ...
      mirrors.forEach((String name, String root) {
        ...
        expect(
          dictMedia,
          contains('function applyFushiPopupCss('),
          reason: '[$name] dict-media.js 缺 CSS 尾段落地函数',
        );
        expect(
          dictMedia,
          contains('function resolveDictMediaPlaceholders('),
        ...
        expect(
          background,
          contains('stylesRevision: fushiPopupCss.revision'),
          reason:
              '[$name] background.js 没把已缓存指纹带进查词请求 '
              '⇒ 服务端每次查词都要回全量（数百 KB）或永远不回',
        );
        expect(
          background,
          contains('fushiMergePopupCss(data)'),
        ...
        expect(
          'applyFushiPopupCss(resp.data)'.allMatches(content).length,
          2,
          reason: '[$name] content.js 的首次查词 / 嵌套查词两条渲染路径都必须落 CSS 尾段',
        );
        expect(
          content,
          contains('installDictMediaPlaceholderResolver(shadow)'),
        ...
        expect(
          sidePanel,
          contains('applyFushiPopupCss(data)'),
    ...
    test('内容变了指纹必须变——否则用户导入/删词典后扩展永远用陈旧样式', () {
      ...
      expect(
        _css(
          styles: <String, String>{'OALDPE': '.opal{color:red}', '明鏡': '.mk{}'},
        ).revision,
        isNot(base.revision),
        reason: '新增一本词典',
      );
      expect(
        _css(global: '.g{}').revision,
        isNot(base.revision),
        reason: '用户改全局自定义 CSS',
      );
      expect(
        _css(custom: <String, String>{'明鏡': '.c{}'}).revision,
        isNot(base.revision),
        reason: '用户改单典自定义 CSS',
      );
      expect(
