...
void main() {
  ...
  test('新设备配对落库后通知监听者（列表能当场刷新）', () async {
    ...
    await controller.debugPersistPairedPeer(
    ...
    expect(
      (await controller.pairedPeers()).map((FushiPairedPeerRow p) => p.peerId),
      <String>['device-a'],
    );
    expect(
      notified,
  ...
  group('接线半边：广播得真有人听（否则 notifyListeners 白发）', () {
    test('设置页收到 controller 通知时重拉已配对设备列表', () {
      ...
      expect(
        handler.contains('_reloadPairedPeers()'),
        isTrue,
    ...
    test('取代旧审批框超时时按失败收尾，不拖悬挂态往下走', () {
      ...
      expect(
        body.contains('onTimeout:'),
        isTrue,
        reason: '等旧弹窗 teardown 必须有超时，否则一个卡住的弹窗能挂死配对 handler',
      );
      expect(
        body.contains('if (!closedInTime) return false;'),
        isTrue,
        reason:
            '超时后继续往下走 = 把新会话的 PIN 写进仍属于旧弹窗的共享单值态，'
            '旧弹窗 teardown 时又把它清掉——悬挂弹窗 + PIN 错乱两头落空',
      );
      expect(
        body.contains('if (identical(_pairDialogClosed, closed))'),
        isTrue,
        reason:
            '超时的 completer 必须从共享字段摘掉，否则旧弹窗真 teardown 时会去'
