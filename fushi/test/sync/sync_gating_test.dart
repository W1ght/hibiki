...
void main() {
  ...
  group('syncBook honours the statistics gate', () {
    ...
    test(
      'syncStats:true exports statistics using the discovered file id',
      () async {
        final FushiDatabase db = _memDb();
        addTearDown(db.close);
        final EpubBookRow book = await _seedBookWithPosition(db);
    ...
    test(
      'syncContent:true imports remote metadata without downloading files',
      () async {
        final FushiDatabase db = _memDb();
        addTearDown(db.close);
        final EpubBookRow book = await _seedBookWithPosition(db);
  ...
  group('progress tie-break compares at storage resolution (BUG-162)', () {
    test(
      'imported-then-unmoved re-sync is synced, not a spurious re-export',
      () async {
        final FushiDatabase db = _memDb();
        addTearDown(db.close);
        ...
        final backend = _RecordingExportBackend(
          remoteFiles: const SyncFileTrio(
            progress: SyncFileRef(
              id: 'remote-progress',
              name: 'progress_1_6_1000_0.5005.json',
            ),
          ),
        );
        final manager = SyncManager(db: db, backend: backend);
  ...
  group('createBackup includes dictionary resources whenever present', () {
    ...
    test('included whenever the resource files exist', () async {
      final Directory dbDir = await Directory.systemTemp.createTemp(
        't4_dict_db_',
      );
      final Directory dictDir = await Directory.systemTemp.createTemp(
        't4_dict_res_',
      );
      ...
      try {
        ...
        final secondArchive = ZipDecoder().decodeBytes(
          await File(secondPath).readAsBytes(),
        );
        expect(
          secondArchive.findFile('dictionaryResources/JMdict/blobs.bin'),
          isNotNull,
        );
      } finally {
