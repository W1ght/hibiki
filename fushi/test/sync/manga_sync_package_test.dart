...
void main() {
  ...
  test('repackageMangaBook 打整树包；isMangaPackage 嗅中；extract 内容往返', () async {
    ...
    expect(
      await isMangaPackage(File(zipPath)),
      isTrue,
    ...
    expect(
      File(p.join(unpacked.path, kMangaPackageMarker)).existsSync(),
      isTrue,
    );
    expect(
      File(p.join(unpacked.path, 'images', 'p1.jpg')).readAsBytesSync(),
  ...
  test('非漫画目录不打包；EPUB 样式 zip 嗅探不命中；坏文件不命中不抛', () async {
    final Directory notManga = Directory.systemTemp.createTempSync(
      'hbk_not_manga',
    );
    addTearDown(() => notManga.deleteSync(recursive: true));
    File(
      p.join(notManga.path, 'mimetype'),
  ...
  test(
    '端到端：repackage → importMangaPackageFile 落库 format=manga + 页图落地',
    () async {
      ...
      expect(
        row!.format,
        BookFormat.manga.dbValue,
        reason: '漫画身份必须保真（不能落成 epub 丢 OCR 查词层）',
      );
      expect(row.chapterCount, 3, reason: '漫画 chapterCount = 页数');
      expect(
        File(p.join(row.extractDir, kMangaPackageMarker)).existsSync(),
        isTrue,
      );
      expect(
        File(p.join(row.extractDir, row.coverPath ?? '')).existsSync(),
        isTrue,
