...
void main() {
  ...
  test(
    ...
    () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'triggerAutoSyncAfterClose 必须返回 void（fire-and-forget）。'
  ...
  test(
    '_runAutoSync registers its in-flight future into BookExitSyncScope',
    () {
      ...
      expect(
        ...
        isTrue,
        reason:
            '_runAutoSync 必须把在飞同步登记进 BookExitSyncScope（app-scope），'
            '否则退出杀应用会把关书 export 打成半截（132B/132A 互补）。',
      );
      expect(
        RegExp(
          r"import\s+'package:fushi/src/sync/book_exit_sync_scope\.dart';",
        ).hasMatch(body),
        isTrue,
  ...
  test('book-exit sync does NOT show an interrupting success SnackBar', () {
    ...
    expect(
      ...
      isFalse,
      reason:
          '_runAutoSync 不得再弹任何 SnackBar（含「同步成功」）。退出书静默，'
