...
void main() {
  group('SyncOrchestrator.runCollectionsOnly', () {
    ...
    test('A 建合集 → 双方 runCollectionsOnly → B 收到合集', () async {
      ...
      final MediaCollectionRow? got = await dbB.getMediaCollectionByNaturalKey(
        'Fav',
        'collection',
      );
      expect(got, isNotNull, reason: '轻量路径必须把合集传到对端');
      final List<MediaCollectionItemRow> items = await dbB.getCollectionItems(
        got!.id,
      );
    ...
    test('runCollectionsOnly 不写全量 sweep 冷却戳 lastSyncMs', () async {
      await orch(dbA, 'devA').runCollectionsOnly();
      expect(
        await SyncRepository(dbA).getLastSyncMs(SyncChannelScope.unscoped),
        isNull,
        reason:
            '冷却戳属于完整 sweep 语义（TODO-1332），'
  ...
  group('installCollectionsSyncWatcher', () {
    test('合集表写入触发防抖调度；uninstall 后不再触发', () async {
      ...
      installCollectionsSyncWatcher(db: db, debounce: const Duration(days: 1));
    ...
    test('AppModel 初始化接线守卫（源码级）', () {
      ...
      expect(
        src,
        contains('installCollectionsSyncWatcher(db: database)'),
        reason:
            'AppModel.initialise 必须装载合集变更观察者，'
