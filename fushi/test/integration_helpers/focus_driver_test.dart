...
void main() {
      ...
      MaterialApp(
        home: Scaffold(
          body: Column(
            children: <Widget>[
              for (int i = 0; i < 3; i++)
                TextButton(onPressed: () {}, child: Text('btn$i')),
            ],
          ),
        ),
      ),
  ...
  testWidgets(
    ...
    'the route FocusScope focused',
    (tester) async {
      debugDefaultTargetPlatformOverride = TargetPlatform.macOS;
      final FocusScopeNode routeScope = FocusScopeNode(
        debugLabel: 'route scope',
      );
      ...
      await tester.pump();
      final bool scopeOwnsFocus = identical(
        FocusManager.instance.primaryFocus,
        routeScope,
      );

      final FocusDriver driver = FocusDriver(tester);
      final bool reached = await driver.focusWidget(
        find.text('after native view'),
        maxSteps: 3,
      );
      ...
      expect(
        ...
        isTrue,
        reason:
            'macOS native WebView can hand Flutter back only its route scope; '
  ...
  testWidgets('focusWidget does not mask a stuck route FocusScope on iOS', (
    ...
    final bool reached = await FocusDriver(
      tester,
