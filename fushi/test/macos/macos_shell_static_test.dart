...
void main() {
  ...
  test(
    'macOS routes through the native shell before the size-class switch',
    () {
      ...
      expect(
        ...
        lessThan(sizeClass),
        reason:
            'the macOS branch must come before the mobile/desktop '
            'size-class split.',
      );
  ...
  test('root MacosWindow + Sidebar wrap the navigator (Approach B)', () {
    ...
    expect(
      main,
      isNot(
        contains('if (Theme.of(context).platform == TargetPlatform.macOS)'),
      ),
    );
    expect(
      main,
      contains('MacosWindow('),
      reason: 'main.dart builder must wrap the navigator in MacosWindow.',
    );
    expect(
      main,
      contains('buildFushiMacosSidebar('),
  ...
  test('sidebar is built from the dynamic HomeTab active-tab list', () {
    ...
    expect(
      main,
      contains('homeActiveTabs('),
      reason:
          'root sidebar must derive its destinations from the dynamic '
          'HomeTab active-tab list, not a hardcoded set.',
    );
    expect(
      home,
      contains('required List<HomeTab> activeTabs'),
      reason: 'buildFushiMacosSidebar takes the dynamic active-tab list.',
    );
    expect(
      home,
      contains('homeNavItemFor('),
      reason: 'sidebar/rail/bottom-bar share one HomeTab->nav-item map.',
    );
  });

  test(
    '_buildMacosLayout hosts content in a MacosScaffold + native ToolBar',
    () {
      ...
      expect(
        ...
        reason:
            'macOS content needs a transparent Material ancestor so the '
            'page bodies\' InkWell-based widgets have an ink surface.',
      );
      expect(
        body,
        isNot(contains('MacosWindow(')),
        reason:
            'MacosWindow moved to the root (main.dart); HomePage must not '
            'nest a second one.',
      );
    },
  );
  ...
  test('tab selection is unified through homeShellTabNotifier (HomeTab)', () {
    expect(
      home,
      contains('ValueNotifier<HomeTab> homeShellTabNotifier'),
      reason:
          'the shared shell selection is a HomeTab notifier, not an int '
    ...
    expect(
      ...
      reason:
          'HomePage listens so the root sidebar routes selection through '
          'the same _selectTab path (preserving _previousTab / focus reset).',
    );
    expect(
      home,
      contains('homeShellTabNotifier.value = tab'),
      reason:
          '_selectTab writes back so the root sidebar reflects the '
  ...
  test('MD3 desktop rail has no Apple-style sidebar separator', () {
    ...
    final int layoutEnd = home.indexOf(
    ...
    expect(
      body,
      isNot(contains('VerticalDivider')),
      reason:
          'The MD3 rail and content surface must not be split by the '
