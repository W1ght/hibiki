...
void main() {
  ...
  group('TODO-1059 s2 behaviour: repeated poke keeps controls visible', () {
    test('poke every <2s across controlsHoverDuration keeps it visible', () {
      fakeAsync((FakeAsync async) {
        final _HideTimerController ctrl = _HideTimerController(
          hoverDuration: hover,
        );
    ...
    test('poke while hidden does NOT silently un-hide (restart only)', () {
      fakeAsync((FakeAsync async) {
        ...
        expect(
          ctrl.visible,
          isFalse,
          reason: 'hidden-state poke must be a no-op (!visible early return)',
        );
      });
    });
  ...
  group('TODO-1059 s2 source guard: fork restartHideTimerSignal wiring', () {
    ...
    test('theme data exposes restartHideTimerSignal field', () {
      expect(
        forkSrc.contains('final Listenable? restartHideTimerSignal;'),
        isTrue,
        reason: 'theme data must carry restartHideTimerSignal',
      );
      expect(
        forkSrc.contains('this.restartHideTimerSignal'),
        isTrue,
    ...
    test('State subscribes, restarts hide Timer while visible, detaches', () {
      expect(
        forkSrc.contains('void _restartHideTimer()'),
        isTrue,
        reason: 'State must have _restartHideTimer()',
      );
      expect(
        forkSrc.contains('addListener(_restartHideTimer)'),
        isTrue,
        reason: 'must subscribe',
      );
      expect(
        forkSrc.contains('removeListener(_restartHideTimer)'),
        isTrue,
  ...
  group(
    'TODO-1059 s2 source guard: Hibiki signal / poke / pass to fork theme',
    () {
      ...
      test('_RestartHideTimerSignal define + field dispose', () {
        ...
        expect(
          shellSrc.contains('void poke() => notifyListeners();'),
          isTrue,
          reason: 'poke() must notifyListeners()',
        );
        expect(
          shellSrc.contains('_restartHideTimerSignal.dispose()'),
          isTrue,
      ...
      test(
        '_mobileControlsTheme passes restartHideTimerSignal into fork theme',
        () {
          final int fn = themeSrc.indexOf(
          ...
          expect(
            body.contains('restartHideTimerSignal: _restartHideTimerSignal'),
            isTrue,
            reason:
