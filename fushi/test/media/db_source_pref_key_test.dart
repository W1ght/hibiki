...
void main() {
  group('dbSourcePrefKey 偏好 key 编码单一真相', () {
    ...
    test('media_source / profile_repository 经 dbSourcePrefKey 编码，不再硬编码格式', () {
      final String ms = File(
        'lib/src/media/media_source.dart',
      ).readAsStringSync();
      final String pr = File(
        'lib/src/profile/profile_repository.dart',
      ).readAsStringSync();
      expect(
        ms,
        contains('dbSourcePrefKey(uniqueKey, key)'),
        reason: 'MediaSource._dbPrefKey 应转调编码器',
      );
      expect(
        pr,
        contains('dbSourcePrefKey(kReaderSourcePersistedKey, row.key)'),
        reason: 'profile applyProfile 应转调编码器',
      );
      expect(
        pr.contains("'src:reader_fushi:\${row.key}'"),
        isFalse,
    ...
    test(
      'reader_settings / custom_fonts_page / profile / migrator 字面量已转编码器',
      () {
        final String rs = File(
          'lib/src/reader/reader_settings.dart',
        ).readAsStringSync();
        final String cf = File(
        ...
        final String dm =
            ...
            File(
              'lib/src/storage/path_rebase_coverage.dart',
            ).readAsStringSync();
        expect(
          rs,
          contains("dbSourcePrefKey(kReaderSourcePersistedKey, '')"),
          reason: 'ReaderSettings._prefix 应转调编码器',
        );
        expect(
          rs.contains("'src:reader_fushi:'"),
          isFalse,
          reason: 'reader_settings 不应再持有前缀字面量',
        );
        expect(
          cf,
          contains('dbSourcePrefKey(kReaderSourcePersistedKey, shortKey)'),
          reason: 'custom_fonts_page._readerPrefKey 应转调编码器',
        );
        expect(
          cf.contains("'src:reader_fushi:'"),
          isFalse,
        ...
        for (final String src in <String>[pr, dm]) {
          ...
          expect(
            src.contains("'src:reader_fushi:font_catalog'"),
            isFalse,
