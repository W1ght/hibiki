...
void main() {
  group('DropSurfaceScope 决定谁接这次拖放', () {
    ...
    testWidgets('隐藏 tab 的子树被判为不活跃，可见 tab 放行', (WidgetTester tester) async {
      ...
      expect(
        DropSurfaceScope.activeFor(hidden),
        isFalse,
    ...
    testWidgets('嵌套作用域逐层 AND：外层不活跃则内层一律不活跃', (WidgetTester tester) async {
      ...
      expect(
        DropSurfaceScope.activeFor(inner),
        isFalse,
  ...
  group('目录是事实，不是判断', () {
    test('classifyDroppedFiles 把目录同时记进 directories 与 mangas', () {
      final DroppedFiles files = classifyDroppedFiles(<String>[
        r'C:\anime\[VCB-Studio] Yuru Yuri',
  ...
  group('decideDropIntent：文件夹按落点表面解释', () {
    DroppedFiles folderDrop() => classifyDroppedFiles(<String>[
      r'C:\anime\S01',
    ], isDirectory: (String path) => true);
    ...
    test('视频页：文件夹优先于同批拖入的单个视频文件', () {
      final DroppedFiles mixed = classifyDroppedFiles(<String>[
        r'C:\anime\S01',
        r'C:\anime\ep1.mkv',
