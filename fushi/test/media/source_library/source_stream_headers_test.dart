...
void main() {
  ...
  test(
    'webdav source + secret -> Basic auth header from credential store',
    () async {
      ...
      final int sid = await db.insertMediaSource(
        MediaSourcesCompanion.insert(
          ...
          configJson: Value(
            encodeSourceConfig(<String, Object?>{
  ...
  group('isUrlWithinSourceRoot / isSameHttpOrigin', () {
    test('同 origin 且路径在根之下才算根内', () {
      ...
      expect(
        isUrlWithinSourceRoot('https://h.example.com/dav/a.mkv', root),
        isTrue,
      );
      ...
      expect(
        isUrlWithinSourceRoot('https://h.example.com/other/a.mkv', root),
        isFalse,
      );
    });
    ...
    test('大小写主机名等价，非绝对 URL 一律 false', () {
      expect(
        isSameHttpOrigin('https://H.Example.com/a', 'https://h.example.com/b'),
        isTrue,
      );
  ...
  test(
    'null sourceId / local source / missing source -> empty headers',
    () async {
      final FushiDatabase db = _memDb();
      addTearDown(db.close);
      ...
      expect(
        await resolveSourceStreamHeaders(
          db: db,
          sourceId: null,
          targetUrl: 'https://any.example.com/a.mkv',
        ),
        isEmpty,
      );
      ...
      expect(
        await resolveSourceStreamHeaders(
          db: db,
          sourceId: 999999,
          targetUrl: 'https://h.example.com/a.mkv',
        ),
        isEmpty,
