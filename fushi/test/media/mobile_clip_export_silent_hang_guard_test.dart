...
void main() {
  ...
  group('视频片段导出不再静默（BUG-2542）', () {
    ...
    test('移动端分享失败不得报「已导出」', () {
      expect(
        clipExportPart,
        contains('final bool shared = isDesktop ||'),
      ...
      final int shared = clipExportPart.indexOf(
  ...
  test('ffmpeg-kit 后端每处平台往返都带超时（BUG-2542 不得回退）', () {
    // 本地 ffmpeg 后端两种实现同住一个文件（上游已拆出 ffmpeg_kit_backend.dart）：
    // 只看 ffmpeg-kit 那一段，桌面 CLI 后端的超时分支返回空 output 是另一回事
    // （子进程被 sigkill，本来就没有日志可读）。
    final String backend = libFile('lib/src/media/video/ffmpeg_backend.dart');
    final String kit = backend.substring(
      backend.indexOf('class KitFfmpegBackend implements FfmpegBackend {'),
    );
