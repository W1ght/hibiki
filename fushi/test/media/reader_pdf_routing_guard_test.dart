...
void main() {
  ...
  test(
    'ReaderPdfSource.kUniqueKey 是独立键 reader_pdf，绝不复用 EPUB 的 reader_fushi',
    () {
      ...
      // 路由塌缩到单一源。
      expect(
        ReaderPdfSource.instance.uniqueKey,
  ...
  test('AppModel 注册 ReaderPdfSource.instance（否则打开 PDF 崩于 map 空断言）', () {
    ...
    expect(
      src.contains('ReaderPdfSource.instance'),
      isTrue,
      reason:
          'populateMediaSources 必须注册 ReaderPdfSource，'
  ...
  test('导入对话框把 .pdf 路由到 PdfImporter（不经 TextToEpub 文本转换）', () {
    ...
    expect(
      carrier.contains("ext == '.pdf'"),
      isTrue,
      reason: 'classifyImportCarrier 必须早退分流 .pdf',
    );
    expect(
      carrier.contains('ImportCarrier.pdf'),
      isTrue,
    ...
    expect(
      src.contains('ImportCarrier.pdf'),
      isTrue,
      reason: '_importEpubOnly 必须按 pdf 载体分派，缺这条 → PDF 落文本兜底',
    );
    expect(
      src.contains('PdfImporter.importFromPath'),
      isTrue,
  ...
  test('PDF 页接在共享查词链路上（BaseSourcePage + buildDictionary + 弹窗入口）', () {
    ...
    expect(
      src.contains('class ReaderPdfPage extends BaseSourcePage'),
      isTrue,
  ...
  test('书架 PDF 进度按页计，且用 1-based 页序（第 1 页也算在读）', () {
    ...
    expect(
      src.contains('book.chapterCount'),
      isTrue,
  ...
  test('书签复用 EPUB 的 Bookmarks 表，sectionIndex 存页码', () {
    ...
    expect(
      src.contains('BookmarkRepository('),
      isTrue,
      reason: '复用同一张 Bookmarks 表（外键指 EpubBooks，删书自动级联清书签）',
    );
    expect(
      src.contains('sectionIndex: _currentPageIndex'),
      isTrue,
