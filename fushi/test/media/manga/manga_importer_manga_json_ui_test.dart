...
void main() {
  ...
  test(
    'importFromMangaJson stages an EpubBooks(manga) row + copies pages',
    () async {
      final String jsonPath = _writeMangaJson(
        p.join(srcRoot.path, 'vol'),
        basename: 'vol',
      );
      ...
      expect(
        File(p.join(row.extractDir, 'images', 'p001.png')).existsSync(),
        isTrue,
      );
      expect(
        File(p.join(row.extractDir, 'images', 'p002.png')).existsSync(),
        isTrue,
      );
    },
  );
  ...
  test(
    'imageRootPath resolves pages relative to the volume root (OCR layout)',
    () async {
