...
void main() {
  ...
  test('停留门：入账只经 _armPageDwellCount 的定时器，不许到达即计', () {
    expect(
      src.contains('void _armPageDwellCount()'),
      isTrue,
      reason: '停留门入口必须存在',
    );
    ...
    expect(
      '_countVisiblePages();'.allMatches(src).length,
      1,
      reason:
          '除停留定时器外的任何直接调用都是「到达即计」回潮：'
          '快速翻过/扫过的页会被记成已读',
    );
    // 三个「位置变化」入口全部走停留门，而不是直接入账。
    expect(
      '_armPageDwellCount();'.allMatches(src).length,
      3,
      reason: '开书摆位（本地/在线）与 _recordProgress 三处都必须经停留门',
    );
    // webtoon 页内滚动不得重置同页计时（否则慢速连续滚读永远攒不满停留门）。
    expect(
      src.contains('if (_pageDwellTimer != null && key == _pageDwellKey)'),
      isTrue,
  ...
  test('续读预置：恢复存档时把恢复位置之前的页预置为已计', () {
    expect(
      src.contains('void _seedCountedPagesFromRestore(int restoredPage)'),
      isTrue,
    );
    // 本地卷与在线章两条恢复路径都要预置（都以 saved != null 为门）。
    expect(
      '_seedCountedPagesFromRestore(restoredPage);'.allMatches(src).length,
      2,
