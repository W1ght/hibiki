...
String _doc({MangaReadingMode mode = MangaReadingMode.spread}) =>
    mangaWindowDocument(
...
void main() {
  group('BUG-1758 resize 重投影', () {
    test('两种模式都注册 resize 监听并走 _reanchor 重投影', () {
      for (final MangaReadingMode mode in MangaReadingMode.values) {
        ...
        expect(
          doc.contains("window.addEventListener('resize'"),
          isTrue,
          reason: '$mode：没有 resize 监听，窗口一变就停在旧 px 投影的错误页上',
        );
        expect(
          doc.contains('function _reanchor()'),
          isTrue,
          reason: '$mode：摆位与 resize 重投影必须统一在 _reanchor（无过渡落位）',
        );
        expect(
          doc.contains('if (_reanchorRaf) return;'),
          isTrue,
    ...
    test('resize 与重投影之间的窗口期挡住 webtoon 滚动上报', () {
      ...
      expect(
        doc.contains('if (_resizePending) return;'),
        isTrue,
        reason:
            '窗口期几何已新、scrollY 还旧，视觉停在错误页；此刻上报会把错页'
            '记成语义真值，重投影反而钉死漂移',
      );
  ...
  group('BUG-1759 栅格化尺度', () {
    test('样式表不得常驻 will-change（钉 CSS 规则形态）', () {
      for (final MangaReadingMode mode in MangaReadingMode.values) {
        final String doc = _doc(mode: mode);
        expect(
          doc.contains('will-change:transform;}'),
          isFalse,
          reason:
              '$mode：常驻 will-change 让栅格化尺度只升不降，缩小态永远拿旧'
    ...
    test('提升改为手势期临时挂、静止后摘除', () {
      ...
      expect(
        doc.contains('_hintWillChange(canvas, 200);'),
        isTrue,
        reason: '缩放/平移的唯一提交口 _applyCanvas 必须走临时提升',
      );
      expect(
        doc.contains("el.style.willChange = '';"),
        isTrue,
  ...
  group('BUG-1760 放大态滚轮平移优先', () {
    test('ZOOM>1 时滚轮先 _panBy，动了就消费掉本事件', () {
      final String doc = _doc();
      expect(
        doc.contains('_panBy(-wdx, -wdy);'),
        isTrue,
        reason: '放大态滚轮必须复用 _panBy（含钳制），不能直接翻页',
      );
      expect(
        doc.contains('if (PAN_X !== px || PAN_Y !== py)'),
        isTrue,
    ...
    test('贴边翻页仍走累计阈值 + 反向清账，不是一格就跨页', () {
      ...
      expect(
        doc.contains('if (_wheelAccum < 40) return;'),
        isTrue,
