...
String _writeVolumeSubdirSample(
  ...
  final Directory pages = Directory(p.join(srcDir, volume))
    ..createSync(recursive: true);
  File(
    p.join(pages.path, '${pagePrefix}00001.jpeg'),
  ).writeAsBytesSync(<int>[11, 12, 13]);
  File(
    p.join(pages.path, '${pagePrefix}00002.jpeg'),
...
Map<String, Object?> _readMangaJson(String extractDir) =>
    jsonDecode(
          File(
            p.join(extractDir, MangaStorage.kMangaJsonFileName),
          ).readAsStringSync(),
        )
...
void main() {
  ...
  test(
    'importFromMokuroPath stages an EpubBooks(manga) row + on-disk dir',
    () async {
      final String mokuro = _writeValidSample(
        p.join(srcRoot.path, 'a'),
        title: 'vol1',
      );
      ...
      expect(
        File(p.join(row.extractDir, 'images', 'p001.jpg')).existsSync(),
        isTrue,
      );
      expect(
        File(p.join(row.extractDir, 'images', 'p002.jpg')).existsSync(),
        isTrue,
      );
    },
  );
  ...
  test(
    ...
    () async {
      ...
      expect(
        pages.every(
          (Object? page) => ((page! as Map)['blocks'] as List).isEmpty,
        ),
        isTrue,
      );
      expect(row.format, 'manga');
  ...
  test('rollback on bad JSON: no orphan row, no orphan dir', () async {
    ...
    // 无残留书目录。
    final Directory booksRoot = Directory(
      p.join(appDocDir.path, 'fushi_books'),
    );
    final bool hasResidual =
        booksRoot.existsSync() &&
  ...
  test(
    'same-basename pages in different subdirs do not alias (ERRATA H2)',
    () async {
      ...
      File(
        p.join(srcDir.path, 'vol1', 'p001.jpg'),
      ).writeAsBytesSync(<int>[1, 1]);
      File(
        p.join(srcDir.path, 'vol2', 'p001.jpg'),
      ).writeAsBytesSync(<int>[2, 2]);
      final Map<String, Object?> payload = <String, Object?>{
        'version': '0.2.0',
        'title': 'multivol',
        'pages': <Object?>[
      ...
      final File f1 = File(
        p.join(row.extractDir, 'images', 'vol1', 'p001.jpg'),
      );
      final File f2 = File(
        p.join(row.extractDir, 'images', 'vol2', 'p001.jpg'),
      );
  ...
  test(
    'manga.json urls use forward slashes (portable), never backslashes',
    () async {
      ...
      final EpubBookRow row = (await db.getEpubBook(bookKey))!;
      final String u0 =
          ((_readMangaJson(row.extractDir)['pages'] as List<Object?>)[0]
  ...
  test(
    'colliding sanitized paths get unique dest (no alias, ERRATA H2/LOW)',
    () async {
      final Directory srcDir = Directory(p.join(srcRoot.path, 'collide'))
        ..createSync(recursive: true);
      Directory(p.join(srcDir.path, 'images')).createSync(recursive: true);
      File(
        p.join(srcDir.path, 'images', 'a.jpg'),
      ).writeAsBytesSync(<int>[1, 1]);
      ...
      final File f0 = File(
        p.joinAll(<String>[row.extractDir, ...u0.split('/')]),
      );
      final File f1 = File(
        p.joinAll(<String>[row.extractDir, ...u1.split('/')]),
      );
  ...
  test(
    'duplicate title gets numeric suffix bookKey, never overwrites',
    () async {
      final String first = _writeValidSample(
        p.join(srcRoot.path, 'one'),
        title: 'dupe',
      );
      final String second = _writeValidSample(
        p.join(srcRoot.path, 'two'),
        title: 'dupe',
      );

      final String key1 = await MangaImporter.importFromMokuroPath(
        db: db,
        mokuroPath: first,
      );
      final String key2 = await MangaImporter.importFromMokuroPath(
        db: db,
        mokuroPath: second,
      );
  ...
  test(
    'title/volume are combined so each volume gets a unique identity',
    () async {
      final Directory srcDir = Directory(p.join(srcRoot.path, 'series'))
        ..createSync(recursive: true);
      Directory(p.join(srcDir.path, 'images')).createSync(recursive: true);
      File(
        p.join(srcDir.path, 'images', 'p001.jpg'),
  ...
  test(
    'missing page image gives a readable error and no partial import',
    () async {
  ...
  test(
    'not-a-manga-folder (.mokuro with no sibling images) is rejected',
    () async {
  ...
  test(
    'user cancelling the duplicate dialog bubbles as cancellation',
    () async {
      final String first = _writeValidSample(
        p.join(srcRoot.path, 'c1'),
        title: 'same',
      );
      final String second = _writeValidSample(
        p.join(srcRoot.path, 'c2'),
        title: 'same',
      );
      ...
      await expectLater(
        MangaImporter.importFromMokuroPath(
          ...
          policy: DuplicatePolicy.ask(
            (String _) async => DuplicateChoice.cancel,
          ),
        ),
  ...
  test('卷子目录布局（裸 img_path）从 <卷名>/ 解析页图并成功导入', () async {
    ...
    final File cover = File(
      p.join(row.extractDir, 'images', 'DLRAW.TO_00001.jpeg'),
    );
    ...
    expect(
      File(
        p.join(row.extractDir, 'images', 'DLRAW.TO_00002.jpeg'),
      ).readAsBytesSync(),
      <int>[14, 15, 16],
    );
    final Map<String, Object?> json = _readMangaJson(row.extractDir);
  ...
  test('准入判定放行的卷子目录布局，执行也必须真能导入（门与执行不再两义）', () async {
    final String mokuro = _writeVolumeSubdirSample(
  ...
  test('卷子目录只绑自己那一卷，不会误取同批另一卷的同名页图', () async {
    ...
    for (final (String vol, List<int> bytes) in <(String, List<int>)>[
      ...
      File(p.join(batch.path, '$vol.mokuro')).writeAsStringSync(
