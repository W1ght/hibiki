...
void main() {
  test('三个书族源恒 implementsHistory:false（翻了就把一本书的历史劈成两条）', () {
    ...
    const String why =
        '书族源必须保持 implementsHistory:false。'
  ...
  test('分裂机制是真的：同一本书三种 format 派生出三个互异的 MediaItem.uniqueKey', () {
    ...
    final String mediaIdentifier = ReaderFushiSource.mediaIdentifierFor(
      bookKey,
    );
    ...
    expect(
      uniqueKeys,
      hasLength(BookFormat.values.length),
      reason:
          'mediaIdentifier 三种 format 完全相同（转化不改 bookKey），'
    ...
    expect(
      ...
      reason:
          'mediaIdentifier 只由 bookKey 决定，与 format 无关——'
          '转化不改 bookKey ⇒ 它逐字节不变',
    );
  });
  ...
  test('书族源就这三个——新增第四个必须来这里登记并守住 false', () {
    ...
    final RegExp declaration = RegExp(
      r'class\s+(\w+)\s+extends\s+ReaderMediaSource',
    );
    ...
    expect(
      ...
      reason:
          '发现了未登记的书族媒体源：${found.toList()..sort()}。'
