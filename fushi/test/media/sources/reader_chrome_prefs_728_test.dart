...
void main() {
  group('showBottomBarCue is per-reader (TODO-728②)', () {
    ...
    test(
      'defaults to true and round-trips through the global source pref',
      () async {
    ...
    test(
      'reads/writes through ReaderSettings (per-reader) when open',
      () async {
        final db = FushiDatabase.forTesting(NativeDatabase.memory());
        addTearDown(db.close);
        MediaSource.setDatabase(db);
    ...
    test(
      'two ReaderSettings instances (two books) do not cross-contaminate',
      () async {
  ...
  group('topProgressPosition is per-reader (TODO-728 3)', () {
    ...
    test(
      'defaults to center and round-trips through the global source pref',
      () async {
        final db = FushiDatabase.forTesting(NativeDatabase.memory());
        addTearDown(db.close);
        MediaSource.setDatabase(db);
