...
double _outerPadding(WidgetTester tester, String text, {required bool top}) {
...
void main() {
  group('① 主/副字幕位置各自独立（overlay 真几何）', () {
    testWidgets('两层同屏：主字幕吃 bottomPadding，副字幕吃 secondaryBottomPadding', (
      ...
      await _pumpOverlay(
        ...
        c,
        bottomPadding: 20,
        secondaryBottomPadding: 160,
      );

      expect(
        _outerPadding(tester, '主', top: false),
        20,
        reason: '主字幕底距 = 主字幕基线',
      );
      expect(
        _outerPadding(tester, '副', top: true),
        160,
    ...
    testWidgets('改副字幕基线不牵动主字幕（反向守卫）', (WidgetTester tester) async {
      ...
      await _pumpOverlay(
        ...
        c,
        bottomPadding: 30,
        secondaryBottomPadding: 40,
      );
      await _pumpOverlay(
        tester,
        c,
        bottomPadding: 30,
        secondaryBottomPadding: 220,
      );
  ...
  group('④ 设置滑杆 → style → 视频页 → overlay 接线（源码守卫）', () {
    ...
    test('设置页有独立的副字幕位置滑杆，且写的是 secondaryBottomPadding', () {
      expect(
        schemaSrc,
        contains("id: 'video.subtitle.position_secondary'"),
        reason: '副字幕垂直位置必须有自己的滑杆，否则用户没有解耦入口',
      );
      expect(
        schemaSrc,
        contains('t.video_setting_subtitle_position_secondary'),
      ...
      expect(
        schemaSrc,
        contains('s.secondaryBottomPadding ?? s.bottomPadding'),
    ...
    test('主字幕滑杆只写 bottomPadding，不再连带改副字幕', () {
      expect(
        schemaSrc,
        contains("id: 'video.subtitle.position'"),
        reason: '主字幕位置滑杆必须保留',
      );
      expect(
        schemaSrc,
        isNot(contains('copyWith(bottomPadding: v, ')),
    ...
    test('视频页把 style 的副字幕基线真传给 overlay', () {
      expect(
        pageSrc,
        contains('secondaryBottomPadding:'),
        reason: 'overlay 必须接上副字幕基线参数，否则滑杆写穿 DB 也不生效',
      );
      expect(
        pageSrc,
        contains('_subtitleStyle.secondaryBottomPadding'),
        reason: '传给 overlay 的必须是 style 里的真值，不能钉常量或恒 null',
      );
      expect(
        pageSrc,
        contains('bottomPadding: _subtitleStyle.bottomPadding'),
