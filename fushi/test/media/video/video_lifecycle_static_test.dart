...
void main() {
  ...
  group('VideoPlayerController exit flush (问题 1)', () {
    ...
    test('a public flushPosition() awaits the persistence write', () {
      ...
      expect(
        body!.group(1),
        contains('await onPositionWrite?.call('),
    ...
    test('exposes video width/height and subscribes to stream (BUG-105)', () {
      expect(
        src.contains('get videoWidth'),
        isTrue,
        reason: 'overlay 的 \\pos 映射需要视频原始宽度',
      );
      expect(
        src.contains('get videoHeight'),
        isTrue,
        reason: 'overlay 的 \\pos 映射需要视频原始高度',
      );
      expect(
        src.contains('stream.width'),
        isTrue,
        reason: '分辨率到位后须 notify 让 overlay 重定位 \\pos',
      );
      expect(
        src.contains('stream.height'),
        isTrue,
        reason: '分辨率到位后须 notify 让 overlay 重定位 \\pos',
      );
    });
  ...
  group('VideoPlayerController network cache tuning (TODO-033 #1)', () {
    ...
    test('load() injects network cache tuning after player.open', () {
      ...
      final int bodyStart = src.indexOf(
        '}) async {',
        src.indexOf('Future<void> load({'),
      );
  ...
  group('VideoPlayerController restore bootstrap (TODO-250)', () {
    ...
    test('load() does not persist synthetic initialPositionMs', () {
      ...
      expect(
        b,
        isNot(contains('updateCueForPosition(initialPositionMs)')),
  ...
  group('VideoFushiPage exit/background flush wiring (问题 1)', () {
    ...
    test(
      'PopScope intercepts the route pop (canPop:false) to await the flush',
      () {
        expect(
          page,
    ...
    test('pop handler delegates to _handleBackOrExit (PopScope/Esc 共用汇聚点)', () {
      ...
      expect(
        body!.group(1),
        contains('_handleBackOrExit()'),
        reason: 'onPop 必须委托给退出汇聚点 _handleBackOrExit',
      );
    });

    test(
      '_handleBackOrExit starts flushPosition() via exitAfterPersist, no await',
      () {
    ...
    test(
      'background lifecycle flushes the playback position (hard-kill cover)',
      () {
        final RegExpMatch? body = RegExp(
        ...
        expect(
          body!.group(1),
          contains('_controller?.flushPosition()'),
  ...
  group('VideoPlayerController guards against libmpv UAF on dispose (BUG-344)', () {
    ...
    test(
      'A: selectEmbeddedGraphicTrack captures loadToken before first await',
      () {
        ...
        final int firstAwaitAt = b.indexOf(
    ...
    test('A: selectEmbeddedGraphicTrack re-checks _isCurrentLoad after every '
        'native await (>=3, no bare _player!=player)', () {
      final String b = graphicBody();
      final int rechecks = RegExp(
        r'_isCurrentLoad\(player, loadToken\)',
      ).allMatches(b).length;
      expect(
        rechecks,
      ...
      expect(
        b.contains('if (_player != player) return false;'),
        isFalse,
        reason:
    ...
    test('A: selectEmbeddedGraphicTrack inserts a recheck between each native '
        'send', () {
      final String b = graphicBody();
      final int setTrackAt = b.indexOf('await player.setSubtitleTrack(real[');
      final int firstApplyAt = b.indexOf(
      ...
      final int recheckAfterSetTrack = b.indexOf(
        '_isCurrentLoad(player, loadToken)',
        setTrackAt,
      );
      expect(recheckAfterSetTrack, greaterThan(setTrackAt));
      expect(
        recheckAfterSetTrack,
        lessThan(firstApplyAt),
        reason: 'setSubtitleTrack 后、下一次原生下发前必须重校验',
      );
      final int recheckAfterFirstApply = b.indexOf(
        '_isCurrentLoad(player, loadToken)',
        firstApplyAt,
      );
      expect(recheckAfterFirstApply, greaterThan(firstApplyAt));
      expect(
        recheckAfterFirstApply,
        lessThan(secondApplyAt),
    ...
    test('selectSubtitleTrack captures loadToken before the native await and '
      ...
      expect(
        ...
        greaterThan(sendAt),
        reason:
            'setSubtitleTrack 后必须重校验 _isCurrentLoad，否则退出/换集期间向已释放 '
            'libmpv handle 下发 → 原生 UAF（回退字幕闪退，TODO-409）',
      );
      expect(
        b.contains('if (!_isCurrentLoad(player, loadToken)) return;'),
        isTrue,
    ...
    test('load() re-checks _isCurrentLoad after every early native await '
    ...
    test(
      'load() autoPlay play() uses the dual predicate, not bare identity',
      () {
        final String b = loadBody();
        expect(
          b.contains('if (autoPlay && _isCurrentLoad(player, loadToken))'),
          isTrue,
          reason:
              'autoPlay 守卫必须用 _isCurrentLoad 双判据——换集复用同一 player 时单判 '
              '`_player == player` 会误向被新 load 接管的 player 发 play()',
        );
        expect(
          b.contains('if (autoPlay && _player == player)'),
          isFalse,
  ...
  group('VideoFushiPage forced landscape wiring (TODO-099)', () {
    ...
    test('initState locks landscape on entering the video page', () {
      ...
      expect(
        body!.group(1),
        contains('_lockLandscapeForVideo()'),
    ...
    test('the lock method is mobile-gated and locks only landscape', () {
      ...
      expect(
        b,
        contains('if (!isMobilePlatform) return;'),
      ...
      expect(
        b.contains('DeviceOrientation.portraitUp'),
        isFalse,
    ...
    test('dispose restores nothing unconditionally (BUG-2105)', () {
      ...
      expect(
        b,
        contains('_releaseVideoDisplayClaim()'),
    ...
    test(
      '_releaseVideoDisplayClaim restores only for the last owner (BUG-2105)',
      () {
        final RegExpMatch? body = RegExp(
