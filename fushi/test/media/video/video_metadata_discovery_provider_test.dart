...
void main() {
  test(
    'adapts anime metadata search without changing canonical media kind',
    () async {
      ...
      expect(
        items.map(
          (VideoDiscoveryItem item) => item.reference.discoveryCategory,
        ),
    ...
    return <VideoMetadataWork>[
      VideoMetadataWork(
        ...
        ids: <VideoMetadataId>[
          VideoMetadataId(
            type: 'anilist',
            value: request.mediaKind == VideoMetadataMediaKind.movie
                ? '1'
                : '2',
            isDefault: true,
          ),
