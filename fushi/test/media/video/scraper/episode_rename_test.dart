...
void main() {
  ...
  group('proposeEpisodeDisplayName', () {
    test('formats E<padded> <title>, pure E<padded> without title', () {
      ...
      expect(
        proposeEpisodeDisplayName(episodeNumber: 7, episodeTitle: '  '),
        'E07',
        reason: '空白集名等同无集名',
      );
      expect(
      ...
      expect(
        proposeEpisodeDisplayName(episodeNumber: 123),
        'E123',
  ...
  group('renameCollectionEpisodes', () {
    test('dryRun returns old->new table without writing', () async {
      ...
      expect(
        (await db.getVideoBookByBookUid('u1'))!.title,
        'Show - 01',
        reason: 'dryRun 绝不写库',
      );
    ...
    test(
      'execute writes titles through video_books and skips already-equal',
      () async {
        final FushiDatabase db = await openDb();
        final int id = await boundCollection(db);
        await addScrapedEpisode(
    ...
    test('pad width follows max episode number (three digits)', () async {
      ...
      expect(
        proposals.map((EpisodeRenameProposal p) => p.newTitle).toList(),
