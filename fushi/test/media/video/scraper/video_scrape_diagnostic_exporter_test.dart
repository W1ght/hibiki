...
void main() {
  ...
  setUp(() async {
    ...
    await Directory(
      p.join(sourceRoot.path, 'Season 01'),
  ...
  test(
    'exports relative tree and original NFO without media or secrets',
    () async {
      final String videoPath = p.join(
        sourceRoot.path,
        'Season 01',
        '[Group] Example S01E03.mkv',
      );
      final String nfoPath = p.join(
        sourceRoot.path,
        'Season 01',
        '[Group] Example S01E03.nfo',
      );
      ...
      await File(
        p.join(sourceRoot.path, 'Season 01', 'subtitle.srt'),
      ).writeAsString('subtitle bytes must not be copied');
      await File(
        p.join(sourceRoot.path, 'poster.jpg'),
  ...
  test('records NFO omitted by size limit', () async {
    await File(
      p.join(sourceRoot.path, 'large.nfo'),
    ...
    expect(
      _readText(archive, 'manifest.json'),
