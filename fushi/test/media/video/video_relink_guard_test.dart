...
void main() {
  ...
  test('_relinkMissingResource 复用统一拾取并持久化重链 videoPath', () {
    ...
    final int end = source.indexOf(
      'Future<void> _confirmMissingResourceDelete(',
    );
    ...
    expect(
      body.contains('videoPath: newPath'),
      isTrue,
  ...
  test('BUG-805: 缺失对话框与 inline 正文的「重新导入」都接真动作，无空操作/无独立 relink', () {
    ...
    expect(
      source.contains('await _relinkMissingResource(row);'),
      isTrue,
      reason: '单视频重新导入应委托 _relinkMissingResource 重链',
    );
    expect(
      source.contains('VideoImportDialog(repo: widget.repo)'),
      isTrue,
    ...
    expect(
      source.contains('await _reimportMissingResource(row);'),
      isTrue,
      reason: '缺失对话框「重新导入」必须调 _reimportMissingResource（非空 pop）',
    );
    expect(
      source.contains('unawaited(_reimportMissingResource(row))'),
      isTrue,
    ...
    expect(
      source.contains('_MissingResourceChoice.relink'),
      isFalse,
      reason: 'relink 枚举项应已移除（合并进 reimport）',
    );
    expect(
      source.contains('video_resource_missing_relink'),
      isFalse,
