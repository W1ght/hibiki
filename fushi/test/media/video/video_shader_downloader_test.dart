...
void main() {
  group('anime4kMirrorUrls', () {
    test('jsDelivr 多 CDN 节点优先、gh 代理前缀其次、raw 官方兜底（BUG-271 多镜像）', () {
      ...
      expect(
        urls.first,
        endsWith('glsl/Restore/Anime4K_Clamp_Highlights.glsl'),
      );
  ...
  group('kAnime4kPresets', () {
    test('六个预设，id 稳定且唯一（Fast A/B/C + HQ A/B/C）', () {
      ...
      final Set<String> ids = kAnime4kPresets
          .map((Anime4kPreset e) => e.id)
          .toSet();
      expect(ids, <String>{
        'mode_a_fast',
        'mode_b_fast',
    ...
    test('每个预设的 fileNames 去重保序，全部 .glsl', () {
      for (final Anime4kPreset preset in kAnime4kPresets) {
        ...
        expect(
          preset.fileNames.length,
          lessThanOrEqualTo(preset.shaders.length),
        );
      }
    });

    test('Mode A (Fast) 链顺序符合官方模板（Clamp → Restore → Upscale …）', () {
      final Anime4kPreset a = kAnime4kPresets.firstWhere(
        (Anime4kPreset e) => e.id == 'mode_a_fast',
      );
      expect(
        a.shaders.map((Anime4kShaderFile s) => s.fileName).toList(),
        <String>[
  ...
  group('looksLikeGlslShader', () {
    test(
      '空 → false',
      () => expect(looksLikeGlslShader(const <int>[]), isFalse),
    );
    test(
      '含 //! 指令头 → true',
    ...
    test('开头是大段 MIT License 注释、//!HOOK 在数百字节之后 → true（回归）', () {
      ...
      final String license = List<String>.filled(
        40,
  ...
  group('downloadAnime4kFiles', () {
    ...
    test('镜像返回 HTML（非着色器）→ 跳过继续回退', () async {
      ...
      final String content = File(
        p.join(dir.path, 'Anime4K_Clamp_Highlights.glsl'),
      ).readAsStringSync();
    ...
    test('所有镜像 + 所有重试轮都失败 → 计入 failed', () async {
      ...
      final int candidates = anime4kMirrorUrls(
        tiny.shaders.first.repoPath,
      ).length;
  ...
  group('shaderFileNameFromUrl（粘链接下载）', () {
    ...
    test('去查询串/锚点', () {
      expect(
        shaderFileNameFromUrl('https://x.com/a/Foo.hook?raw=1#frag'),
        'Foo.hook',
      );
    });
    ...
    test('非法字符折叠为 _', () {
      expect(
        shaderFileNameFromUrl('https://x.com/a/My Shader.glsl'),
        'My_Shader.glsl',
      );
    });
  });
  ...
  test('经典推荐着色器（RAVU/NNEDI3）符号已删除，不再内置', () {
    final String src = File(
      'lib/src/media/video/video_shader_downloader.dart',
    ).readAsStringSync();
    expect(
      src,
      isNot(contains('kRecommendedShaders')),
      reason: 'TODO-125 删经典推荐着色器目录',
    );
    expect(
      src,
      isNot(contains('class RecommendedShader')),
      reason: 'TODO-125 删 RecommendedShader 模型',
    );
    expect(
      src,
      isNot(contains('mpv-prescalers')),
