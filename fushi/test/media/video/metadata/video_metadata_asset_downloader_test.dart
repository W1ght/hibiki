...
void main() {
  ...
  for (final ({String name, String contentType, List<int> bytes, String ext})
      image
      in <({String name, String contentType, List<int> bytes, String ext})>[
        ...
        (name: 'PNG', contentType: 'image/png', bytes: _pngBytes, ext: '.png'),
        (
          name: 'WebP',
        ...
        (name: 'GIF', contentType: 'image/gif', bytes: _gifBytes, ext: '.gif'),
      ]) {
    test(
      'image downloader accepts ${image.name} magic and derives extension',
      () async {
        final VideoMetadataAssetDownloader downloader =
            VideoMetadataAssetDownloader(
              client: MockClient(
