...
void main() {
  group('PreferencesRepository video danmaku prefs', () {
    ...
    test(
      'persists enabled flag and clamps active limit across reload',
      () async {
    ...
    test(
      'persists danmaku server config round-trip and pushes the static',
      () async {
    ...
    test(
      'loadFromDb resets the static to defaults when no config persisted',
      () async {
        DandanplayConfig.current = const DandanplayConfig(
  ...
  group('PreferencesRepository video danmaku style + block rules', () {
    ...
    test('persists style (clamped) and block rules across reload', () async {
      await repo.setVideoDanmakuStyle(
      ...
      await repo.setVideoDanmakuBlockRulesText(
        'spoiler\n'
        r'/^\d+$/',
      );
      ...
      expect(
        style.fontScale,
        VideoDanmakuStyle.maxFontScale,
      ...
      expect(
        reloaded.videoDanmakuBlockRulesText,
        'spoiler\n'
        r'/^\d+$/',
      );
      reloaded.dispose();
