...
void main() {
  testWidgets(
    'TODO-1315: mount does NOT probe waveform (lazy); entry visible',
    (WidgetTester tester) async {
      ...
      expect(
        find.byWidgetPredicate(
          (Widget w) =>
              w is CustomPaint && w.painter is SubtitleWaveformPainter,
        ),
        findsNothing,
      );
    },
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        find.text(t.video_subtitle_waveform_legend_energy),
        findsOneWidget,
      );
      expect(find.text(t.video_subtitle_waveform_legend_cue), findsOneWidget);
      expect(
        find.text(t.video_subtitle_waveform_legend_playhead),
        findsOneWidget,
      );
  ...
  testWidgets(
    'TODO-1316: auto-align low confidence (null) => hint, no commit',
    (WidgetTester tester) async {
      final List<int> committed = <int>[];
      await tester.pumpWidget(
        _host(
      ...
      expect(_zoomPainter(tester).previewDelayMs, 0);
      expect(
        find.text(t.video_subtitle_auto_align_low_confidence),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    'TODO-1316: no onAutoAlign => no auto-align button in zoom view',
    (WidgetTester tester) async {
      await tester.pumpWidget(
        _host(
          cues: <AudioCue>[_cue(1000, 2000)],
  ...
  testWidgets('cue list: shows all non-empty cues; empty-text cue is skipped', (
    ...
    final Finder list = find.byKey(
      const ValueKey<String>('subtitle-waveform-cue-list'),
    );
    expect(list, findsOneWidget);
    // 列表里两句都在（文本条另有一份，故用 descendant 限定到列表内各一处）。
    expect(
      find.descendant(of: list, matching: find.text('ohayou')),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets(
    'playhead ticker: currentPositionMs drives painter live (~30fps)',
    (WidgetTester tester) async {
      int posMs = 1500;
      await tester.pumpWidget(
        _host(
  ...
  testWidgets(
    'keyboard shortcuts: space binding fires inside the zoom dialog',
    (WidgetTester tester) async {
      int spaceHits = 0;
  ...
  testWidgets(
    ...
    'without clobbering the typed text',
    (WidgetTester tester) async {
      final List<int> committed = <int>[];
      await tester.pumpWidget(
        _host(
  ...
  List<AudioCue> secondlyCues() => <AudioCue>[
    for (int i = 0; i < 60; i++) _cue(i * 1000, i * 1000 + 800, text: 'S$i'),
  ];
  ...
  group('BUG-1729: layoutCueStripChips greedy lane assignment (pure)', () {
    ...
    test(
      'pixel fake-overlap (min chip width at low zoom) also splits lanes',
      () {
    ...
    test('non-overlapping cues all stay on lane 0', () {
      ...
      expect(slots.map((CueStripSlot? s) => s!.lane), everyElement(0));
    });

    test(
      'lane cap: 4 mutually overlapping cues cap at 3 lanes (last shared)',
      () {
        final List<CueStripSlot?> slots = layout(<AudioCue>[
