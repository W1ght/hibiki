...
void main() {
  test('default is high-contrast white text + soft black shadow (Niratan)', () {
    ...
    expect(
      s.resolveTextColor(const Color(0xFF112233)),
  ...
  test(
    'decode of pre-TODO-051 stored data keeps theme-following (back-compat)',
    () {
  ...
  test(
    'default white/black round-trips and persists explicitly (TODO-051)',
    () {
      ...
      expect(
        back.resolveTextColor(const Color(0xFF112233)),
        const Color(0xFFFFFFFF),
      );
      expect(
        back.resolveShadowColor(const Color(0xFF112233)),
        const Color(0xE6000000),
      );
    },
  );
  ...
  test('decode tolerates empty/garbage -> defaults', () {
    ...
    expect(
      VideoSubtitleStyle.decode('not json').textColor,
      const Color(0xFFFFFFFF),
    );
  ...
  test('decode migrates stored asb defaults to scale-derived defaults', () {
    ...
    final VideoSubtitleStyle s = VideoSubtitleStyle.decode(
      '{"fontWeight":700,"shadowThickness":3}',
    );
  ...
  group('dynamic subtitle dodge of the bottom controls bar (TODO-129)', () {
    ...
    test(
      'controls reserve = 进度条上缘高度（一个按钮行），不再含整条按钮行 + margin 累加（TODO-171）',
      () {
  ...
  group('videoSubtitleControlsReserve 按平台真实几何 + 随缩放（BUG-238 / BUG-901）', () {
    ...
    test('移动端 reserve = 进度条触摸热区上缘 + 呼吸间距，字幕命中区整体清出 seek 命中区（BUG-901）', () {
      ...
      expect(
        mobileReserve(1.0),
        greaterThanOrEqualTo(seekHotzoneTop(1.0)),
        reason: '字幕底缘必须骑在进度条触摸热区上缘之上，否则命中区与 seek 重叠误触（BUG-901）',
      );
      expect(
        mobileReserve(1.0) - seekHotzoneTop(1.0),
    ...
    test('reserve 随界面缩放放大（缩放敏感几何项 ×scale）', () {
      ...
      expect(
        mobileReserve(2.0),
        greaterThanOrEqualTo(seekHotzoneTop(2.0)),
      ...
      expect(
        desktopReserve(2.0),
        greaterThanOrEqualTo(desktopSeekHotzoneTop(2.0)),
    ...
    test('桌面 reserve = 进度条触摸热区上缘 + 呼吸间距，点进度条不再被字幕吸走（BUG-1224）', () {
      ...
      expect(
        desktopReserve(1.0),
        greaterThanOrEqualTo(desktopSeekHotzoneTop(1.0)),
        reason: '桌面字幕底缘必须骑在进度条触摸热区上缘之上，否则点进度条被字幕吸走成查词（BUG-1224）',
      );
      expect(
        desktopReserve(1.0) - desktopSeekHotzoneTop(1.0),
        closeTo(breathingBase, 0.001),
        reason: 'reserve 应恰为热区上缘 + 一个呼吸间距',
      );
  ...
  group('buildSubtitleShadows (BUG-222 对称描边而非单向投影)', () {
    ...
    test('正粗细生成八方向对称描边、无单向下方投影', () {
      ...
      final double sumDx = shadows.fold(
        0.0,
        (double a, Shadow s) => a + s.offset.dx,
      );
      final double sumDy = shadows.fold(
        0.0,
        (double a, Shadow s) => a + s.offset.dy,
      );
    ...
    test('描边偏移围绕文字四周（上下左右四个正交方向都有）', () {
      ...
      bool hasDir(double dx, double dy) => shadows.any(
        (Shadow s) =>
  ...
  group('buildSubtitleSoftShadow (抄 Niratan 默认统一外观的柔和投影)', () {
    ...
    test('正粗细生成单枚柔和投影：色 / 模糊半径 / 零偏移', () {
      ...
      expect(
        ...
        Offset.zero,
        reason:
            'BUG-1603：任何非零偏移都会让投影变方向性。真机（DPR=2）实测 '
    ...
    test('BUG-1603 守卫：任意 thickness 下偏移都必须为零', () {
      for (final double t in <double>[0.5, 1, 2, 3, 4.5, 6, 10, 12]) {
        expect(
          buildSubtitleSoftShadow(c, t).single.offset,
          Offset.zero,
  ...
  group('subtitleScreenScaleFactor (TODO-1199 字幕字号屏幕自适应)', () {
    ...
    test('手动基准仍被尊重：有效字号 = 基准 × 因子（因子只是叠加乘数）', () {
      ...
      final double smallScreen = subtitleScreenScaleFactor(
        const Size(640, 360),
      );
      // 同一基准在大屏比小屏放得更大。
      expect(36.0 * bigScreen, greaterThan(36.0 * smallScreen));
      // 同屏下有效字号与基准成正比（改基准 → 有效字号按比例变，手动可调）。
      expect((48.0 * bigScreen) / (24.0 * bigScreen), closeTo(2.0, 1e-9));
    });
  });
}
