...
void main() {
  group('videoResourceRequiresLocalCheck (TODO-897 纯函数)', () {
    ...
    test('http/https 流 URL 豁免（不校验）', () {
      ...
      expect(videoResourceRequiresLocalCheck('http://host/video.mp4'), isFalse);
      expect(videoResourceRequiresLocalCheck('HTTPS://Host/a.ts'), isFalse);
    });

    test('本地绝对路径 / file:// 需校验（true）', () {
      expect(videoResourceRequiresLocalCheck(r'D:\movies\ep01.mkv'), isTrue);
      expect(videoResourceRequiresLocalCheck('/home/user/ep01.mp4'), isTrue);
      // file:// 不是 http(s) 流，按本地处理需校验。
      expect(videoResourceRequiresLocalCheck('file:///tmp/a.mp4'), isTrue);
    });
  });
  ...
  group('isLocalVideoResourceMissing (TODO-897 异步)', () {
    test('流 / 远端 / 空 恒不缺失（豁免，照常 load）', () async {
      ...
      expect(await isLocalVideoResourceMissing('https://host/a.m3u8'), isFalse);
    });
  ...
  group('relocateMissingAppDocumentPath', () {
    ...
    test(
      ...
      'the container Documents (<Documents>/Hibiki/data)',
      () async {
        ...
        final File current = File('${currentDocs.path}/video_covers/cover.jpg')
          ..createSync(recursive: true);
        final String stale =
            '${dir.path}/Containers/Data/Application/OLD/'
        ...
        expect(
          relocated.contains('Hibiki${p.separator}data${p.separator}Hibiki'),
          isFalse,
        );
      },
    );
