...
void main() {
  group('parseAniListAiringResponse', () {
    test('parses schedules with media titles and hasNextPage', () {
      final AniListAiringPage page = parseAniListAiringResponse(
        _pageBody(
          hasNextPage: true,
  ...
  group('AniListClient.fetchAiringSchedulePage', () {
    test(
      'sends window variables and omits ids when mediaIds is null',
      () async {
      ...
      await expectLater(
        ...
        throwsA(
          isA<AniListRequestException>()
              .having(
                (AniListRequestException e) => e.statusCode,
                'statusCode',
                429,
              )
              .having(
