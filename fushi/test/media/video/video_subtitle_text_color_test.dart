...
void main() {
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pump();
      expect(
        _fillText(tester, 'A').style?.color,
        userColor,
  ...
  group('BUG-1285 纯字幕模式颜色语义归零（行内 \\c 不穿透）', () {
    testWidgets('OFF: 行内 \\c 黑色不生效，填充恒为用户 textColor', (
      ...
      await tester.pump();
      expect(
        _fillText(tester, '歌').style?.color,
        userColor,
  ...
  test(
    ...
    () {
      ...
      expect(
        actionsSrc.contains('video_setting_subtitle_text_color'),
        isTrue,
      ...
      expect(
        actionsSrc.contains('s.copyWith(textColor: c)'),
        isTrue,
        reason: '取色必须落 VideoSubtitleStyle.textColor',
      );
      // 固定预设的旧实现必须彻底移除（否则等于没换成调色盘）。
      expect(
        actionsSrc.contains('_textColorPresets'),
        isFalse,
        reason: '固定文字色预设应已移除，改为调色盘',
      );
      expect(
        actionsSrc.contains('_textColorOptionIndex'),
        isFalse,
  ...
  test(
    ...
    () {
      ...
      final VideoSubtitleStyle picked = VideoSubtitleStyle.defaults.copyWith(
        textColor: preset,
      );
      expect(picked.textColor, preset);
      final VideoSubtitleStyle back = VideoSubtitleStyle.decode(
        VideoSubtitleStyle.encode(picked),
      );
      expect(
        back.textColor,
        preset,
