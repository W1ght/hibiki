...
void main() {
  ...
  group('BUG-772 首开在途 controller 主动取消 (Task 1)', () {
    ...
    test('声明 _pendingController 字段', () {
      expect(
        src.contains('VideoPlayerController? _pendingController'),
        isTrue,
    ...
    test('首开路径把在途 controller 存入 _pendingController（仅新建，换集不设）', () {
      expect(
        RegExp(
          r'if \(isInitialVideoOpen\)\s*_pendingController = controller;',
        ).hasMatch(src),
        isTrue,
    ...
    test('dispose() 主动 dispose 在途 controller 且不与 _controller 二次 dispose', () {
      ...
      expect(
        b.contains('_pendingController'),
        isTrue,
        reason: 'dispose 必须处理在途 controller（否则 native churn 跑完污染 GPU）',
      );
      expect(
        b.contains('!identical(_pendingController, _controller)'),
        isTrue,
    ...
    test('首开成功赋值后清空 _pendingController', () {
      expect(
        RegExp(
          r'_controller = controller;\s*_pendingController = null;',
        ).hasMatch(src),
        isTrue,
