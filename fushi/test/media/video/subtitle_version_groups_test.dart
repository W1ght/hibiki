...
void main() {
  group('subtitleReleaseGroupTag', () {
    test('取开头方括号组名；CRC/分辨率/语言标签不算', () {
      expect(
        subtitleReleaseGroupTag('[SubsPlease] Show - 01.ass'),
        'SubsPlease',
      );
  ...
  group('buildSubtitleVersionGroups', () {
    test('同合集同变体折成一张卡；不同格式/组分开', () {
      final List<SubtitleVersionGroup> groups =
          buildSubtitleVersionGroups(<VideoSubtitleCandidate>[
      ...
      final SubtitleVersionGroup ass = groups.firstWhere(
    ...
    test('卡内按集号升序；latest 按上传时间，无时间退化取集号最大', () {
      ...
      final SubtitleVersionGroup noTime =
          buildSubtitleVersionGroups(<VideoSubtitleCandidate>[
  ...
  group('pickGroupCandidateForEpisode', () {
    ...
    test('精确集号命中', () {
      ...
      expect(
        pickGroupCandidateForEpisode(g, 5),
        isNull,
