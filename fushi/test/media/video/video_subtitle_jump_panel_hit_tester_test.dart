...
void main() {
  group(
    'subtitle grapheme offset helpers (BUG-874 shared with barrier hit)',
    () {
      test(
        'start/end offsets track UTF-16 lengths incl. multi-unit graphemes',
        () {
  ...
  group('resolveSubtitleListGraphemeHit (BUG-916 几何命中，不偏左)', () {
    ...
    test('点在字内 → 该字（含左半格，回归病根）', () {
      ...
      expect(
        resolveSubtitleListGraphemeHit(rects, const Offset(35, 20)),
        1,
    ...
    test('空矩形跳过；全空 → -1', () {
      expect(
        resolveSubtitleListGraphemeHit(<Rect>[
          Rect.zero,
          const Rect.fromLTWH(30, 0, 30, 40),
        ], const Offset(40, 20)),
        1,
  ...
  group('VideoSubtitleListHitTester wiring (BUG-874)', () {
    testWidgets('panel binds a hit tester that reverse-hits a row char to '
      ...
      expect(
        ...
        isNotNull,
        reason:
            'tapping a list row char while a popup is open must resolve '
      ...
      expect(
        hit.charRect.contains(target.globalPoint),
        isTrue,
    ...
    testWidgets('without onLookupCue no rows register → hitTest is null', (
      ...
      expect(
        hitTester.hitTest(textRect.center),
        isNull,
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        final SubtitleListHit? onGlyph = hitTester.hitTest(
          target.globalPoint,
          exactOnly: true,
        );
        ...
        final SubtitleListHit? headHit = hitTester.hitTest(
          _pointForGrapheme(tester, 'second line', 's').globalPoint,
        );
        ...
        if (generous != null) {
          expect(
            hitTester.hitTest(leftSkirt, exactOnly: true),
            isNull,
    ...
    testWidgets('unbinds on panel dispose (hidden sidebar)', (
      ...
      expect(
        hitTester.hitTest(Offset.zero),
        isNull,
        reason:
            'disposed panel must unbind so the barrier never calls a '
            'stale hit impl',
      );
    });
  });
