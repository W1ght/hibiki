...
void main() {
  testWidgets(
    'VideoDanmakuOverlay is IgnorePointer and caps rendered widgets',
    (WidgetTester tester) async {
      int positionMs = 1000;
      await _pump(
        tester,
        VideoDanmakuOverlay(
          items: <VideoDanmakuItem>[_item(0, 'first'), _item(10, 'second')],
          enabled: true,
          maxActive: 1,
  ...
  testWidgets(
    'disabled overlay renders nothing but keeps pointer passthrough',
    (WidgetTester tester) async {
      await _pump(
        tester,
        VideoDanmakuOverlay(
  ...
  testWidgets(
    'scroll danmaku is monotonic: backward raw jitter never reverses',
    (WidgetTester tester) async {
  ...
  testWidgets(
    'scroll danmaku has left the viewport on its last rendered frame',
    (WidgetTester tester) async {
  ...
  testWidgets('scroll danmaku stays fully opaque all the way out', (
    ...
    Future<double> opacityAt(int positionMs) async {
      ...
      return tester
          .widget<Opacity>(
            find
                .ancestor(of: find.text('コメント'), matching: find.byType(Opacity))
                .first,
          )
          .opacity;
    }
