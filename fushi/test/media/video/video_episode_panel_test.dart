...
void main() {
  ...
  List<VideoEpisodeEntry> episodes(int n, {ImageProvider? cover}) =>
      <VideoEpisodeEntry>[
        for (int i = 0; i < n; i++)
  ...
  testWidgets('lists episodes; tap reports the episode index (TODO-638)', (
    ...
    await tester.tap(
      find.byKey(const ValueKey<String>('video-episode-card-2')),
    );
    expect(tapped, <int>[2]);
  });

  testWidgets('highlights the current episode card with a play icon and keeps '
    ...
    final Finder currentCard = find.byKey(
      const ValueKey<String>('video-episode-card-1'),
    );
    expect(
      find.descendant(
        of: currentCard,
    ...
    final Finder otherCard = find.byKey(
      const ValueKey<String>('video-episode-card-0'),
    );
    expect(
      find.descendant(
        of: otherCard,
