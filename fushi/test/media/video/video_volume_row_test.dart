...
void main() {
  ...
  final String volumeOverlays = read(
  ...
  group('TODO-438 紧凑音量入口结构', () {
    ...
    test('音量控件是图标锚点，底栏不内联 Slider 或 Row', () {
      expect(
        build,
        contains('_controlPopoverAnchor('),
        reason: '底栏音量按钮必须是固定尺寸锚点，由锚点打开浮层',
      );
      expect(
        build,
        contains('_VideoControlPopoverKind.volume'),
      ...
      expect(
        build,
        isNot(contains('_volumeControlPopoverLink')),
      ...
      expect(
        containsIdentifierCall(build, 'Slider'),
        isFalse,
        reason:
            '底栏按钮内不得内联 Slider（含 Slider.adaptive），'
            '避免占位随需求回退到横向滑条',
      );
      expect(
        containsIdentifierCall(build, 'Row'),
        isFalse,
    ...
    test('浮层锚定按钮上方，hover / tap 不改变底栏几何', () {
      ...
      expect(
        page,
        contains('showWhenUnlinked: false'),
        reason: '锚点消失时浮层必须自动不可见',
      );
      expect(
        page,
        contains('_activeControlPopoverPlacement'),
        reason: '浮层应根据播放器边界和来源 slot 计算锚点/宽度',
      );
      expect(
        page,
        isNot(contains('_volumeSliderWidth')),
        reason: 'TODO-438 不再保留底栏横滑条固定宽度占位',
      );
      expect(
        page,
        isNot(contains('MaterialDesktopVolumeButton')),
    ...
    test('桌面悬停滑条区域滚轮调音量，但不改任何尺寸', () {
      ...
      expect(
        build.contains('AnimatedContainer'),
        isFalse,
        reason: 'hover 不得用 AnimatedContainer 撑宽（BUG-248A 原症状）',
      );
      expect(
        build.contains('OverlayEntry'),
        isFalse,
    ...
    test('点击图标打开浮层；浮层内保留静音与横向滑条', () {
      expect(
        build,
        contains('_toggleControlPopover('),
        reason: '点击 / tap 图标应打开或固定音量浮层',
      );
      expect(
        build,
        isNot(contains('_toggleMute()')),
      ...
      expect(
        toggle,
        contains('_controlPopoverPinned'),
        reason: 'hover 已打开时点击应 pin；已 pin 时再点击才关闭',
      );
      expect(
        toggle,
        contains('popoverLink: popoverLink'),
      ...
      expect(
        popover,
        contains('VideoVolumePopoverCard('),
    ...
    test('可移动音量键只从底栏左右槽渲染完整音量按钮', () {
      ...
      expect(
        bottom,
        contains('_controlLayout.itemsIn(slot)'),
        reason: '底栏出控件的顺序必须直接取自布局真相源',
      );
      expect(
        bottom,
        contains('if (item == VideoControlItem.volume)'),
      ...
      expect(
        ...
        reason:
            '不得再维护第二份「先画完 chip 再追加 volume」的列表——'
            '那正是槽内顺序被丢掉的原因',
      );
      ...
      expect(
        chipItems,
        contains('item != VideoControlItem.volume'),
  ...
  group('TODO-377 音量显示单一真相源', () {
    test('_volumeDisplay 是 ValueNotifier 并被 ValueListenableBuilder 消费', () {
      ...
      expect(
        page,
        contains('ValueListenableBuilder<double>'),
    ...
    test('_setVolumeFromSlider 经统一 helper 写 controller + 同步显示真相源', () {
      final String body = methodBody(
        page,
        RegExp(
          r'void _setVolumeFromSlider\(double value\) \{(.*?)\n  \}',
          dotAll: true,
        ),
      ...
      expect(
        helper,
        contains('controller.setVolume(clamped)'),
        reason: '统一 helper 须真写穿 controller.setVolume',
      );
      expect(
        helper,
        contains('_syncVolumeDisplay(clamped)'),
    ...
    test('_syncVolumeDisplay 写 _volumeDisplay.value 并 clamp 到 0..100', () {
      final String body = methodBody(
        page,
        RegExp(
          r'void _syncVolumeDisplay\(double volume\) \{(.*?)\n  \}',
          dotAll: true,
        ),
        '_syncVolumeDisplay',
      );
    ...
    test('键盘音量键 / 静音切换 / media_kit 移动竖滑都走统一显示 helper', () {
      ...
      final String mute = methodBody(
        page,
        RegExp(
          r'Future<void> _toggleMute\(\) async \{(.*?)\n  \}',
          dotAll: true,
        ),
        '_toggleMute',
      );
  ...
  group('TODO-438 旧 hover 改宽 / OverlayEntry 复杂度不恢复', () {
    ...
    test('dispose 释放音量显示与浮层状态', () {
      ...
      expect(
        m!.group(1),
        contains('_volumeDisplay.dispose()'),
        reason: 'dispose 须释放 _volumeDisplay',
      );
      expect(
        m.group(1),
        contains('_controlPopoverHideTimer?.cancel()'),
        reason: 'dispose 须取消浮层延迟关闭 timer',
      );
      expect(
        m.group(1),
        contains('_videoControlPopover.dispose()'),
