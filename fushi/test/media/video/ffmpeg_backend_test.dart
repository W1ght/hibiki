...
void main() {
  group('FfmpegRunResult', () {
    test('isSuccess 仅当 returnCode == 0', () {
      expect(
        const FfmpegRunResult(returnCode: 0, output: '').isSuccess,
        isTrue,
      );
      expect(
        const FfmpegRunResult(returnCode: 1, output: '').isSuccess,
        isFalse,
      );
      expect(
        const FfmpegRunResult(returnCode: null, output: 'x').isSuccess,
        isFalse,
      );
    });
    ...
    test('failureSummary names Windows invalid-image exits and executable', () {
      ...
      expect(
        ...
        contains(r'C:\Hibiki\ffmpeg.exe -> ffmpeg'),
      );
    ...
    test('failureSummary surfaces the tail error line, not the leading banner', () {
      ...
      expect(
        ...
        reason:
            'The real ffmpeg failure line (tail) must survive '
            'summarization instead of the leading banner.',
      );
      // banner（version 行）不得成为唯一显示内容——旧的从头截断正是只剩它。
      expect(
        summary.contains('ffmpeg version n6.0'),
        isFalse,
        reason:
            'The leading version banner must not crowd out the real '
            'error (old head-truncation showed only the banner).',
      );
      expect(
        summary.contains('configuration:'),
        isFalse,
        reason:
            'The long configuration banner must not be surfaced instead '
  ...
  group('CliFfmpegBackend.run（需本机 ffmpeg，缺失则跳过）', () {
    test('ffmpeg -version 成功且 output 含版本串', () async {
      ...
      final FfmpegRunResult r = await const CliFfmpegBackend().run(<String>[
        '-version',
