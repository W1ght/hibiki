...
void main() {
    ...
    for (final String path in backingSources) {
      ...
      for (final RegExp re in contradictoryMobilePhrases) {
        for (final Match m in re.allMatches(src)) {
          ...
          fail(
            '$path 出现被复核退回的过度承诺/矛盾措辞 "${m.group(0)}"：'
  ...
  test('manager doc 必须留下「移动端生效」的可验证依据出处（media_kit 源码 file:line）', () {
    ...
    expect(
      mgr.contains('android_video_controller'),
      isTrue,
      reason:
          '注释须引用 media_kit_video 的 android_video_controller real.dart 作为'
          '「移动端 vo=gpu 着色器生效」的依据出处',
    );
    expect(
      mgr.contains('vo=gpu') || mgr.contains("vo ?? 'gpu'"),
      isTrue,
      reason: '注释须点明 media_kit 移动端默认 vo=gpu（着色器在该渲染路径生效）',
    );
    // applyShadersToPlayer 必须显式声明「不是移动端 no-op，只在非 libmpv 后端 no-op」。
    expect(
      mgr.contains('非 libmpv'),
      isTrue,
  ...
  test('移动端性能提示文案存在，且不对效果做无条件「可用」承诺', () {
    ...
    expect(
      zhHint.contains('因机型') || zhHint.contains('渲染路径'),
      isTrue,
      reason: 'zh 文案须限定「效果因机型 GPU 而异 / 仅标准渲染路径生效」，不得无条件断言可用',
    );
    expect(
      zhHint.contains('掉帧') || zhHint.contains('发热'),
      isTrue,
    ...
    expect(
      ...
      isTrue,
      reason: 'en 文案须限定「effectiveness varies / standard render path」，不得无条件断言可用',
    );

    // 不得出现把着色器说成「一定提升画质 / 保证可用」的绝对措辞。
    expect(
      zhHint.contains('一定') || zhHint.contains('保证'),
      isFalse,
