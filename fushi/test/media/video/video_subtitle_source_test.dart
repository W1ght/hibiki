...
''';
  ...
  group('远端直出容器流的内嵌字幕抽取（BUG-2590）', () {
    const String kOneCueSrt = '1\n00:00:00,000 --> 00:00:01,000\nx\n';

    test('remoteSubtitleExtractTimeout 取「按体积 / 按时长」较大者、夹在 [120s, 2h]', () {
      // 网络远比磁盘慢（实测 ≈ 11 MB/s ≈ 90 s/GiB），本地的 8 s/GiB 会把远端整读
      // 一遍判成超时。
      expect(remoteSubtitleExtractTimeout(sizeBytes: 0).inSeconds, 120);
      const int gb = 1024 * 1024 * 1024;
      // 1.77 GB = 1.65 GiB（源站直读实测 152 s）→ 120 + 1.65*300 ≈ 615s。
      final int bySize =
          remoteSubtitleExtractTimeout(sizeBytes: 1772761745).inSeconds;
      expect(bySize, inInclusiveRange(610, 620));
      expect(bySize,
          greaterThan(subtitleExtractTimeoutForBytes(1772761745).inSeconds));
      // 同一集 24:04：流代理「突发后 ≈1x 实时限速」时整读≈一集，按时长下限
      // 120 + 1444*1.5 ≈ 2286s 胜出。
      final int byDuration = remoteSubtitleExtractTimeout(
        sizeBytes: 1772761745,
        durationMs: 1444000,
      ).inSeconds;
      expect(byDuration, inInclusiveRange(2280, 2290));
      expect(remoteSubtitleExtractTimeout(sizeBytes: 100 * gb).inSeconds, 7200);
      expect(
        remoteSubtitleExtractTimeout(sizeBytes: 0, durationMs: 4 * 3600 * 1000)
            .inSeconds,
        7200,
      );
      final int t1 = remoteSubtitleExtractTimeout(sizeBytes: 2 * gb).inSeconds;
      final int t2 = remoteSubtitleExtractTimeout(sizeBytes: 8 * gb).inSeconds;
      expect(t2, greaterThan(t1));
    });

    test('remoteEmbeddedSubtitleCacheDir 按键稳定、与本地缓存同根、非法字符折成 _', () {
      final Directory a = remoteEmbeddedSubtitleCacheDir('item-1_1772761745');
      final Directory b = remoteEmbeddedSubtitleCacheDir('item-1_1772761745');
      expect(a.path, b.path);
      expect(p.basename(a.path), 'remote_item-1_1772761745');
      expect(p.basename(p.dirname(a.path)), 'hibiki_vsub_cache',
          reason: '与 embeddedSubtitleCacheDir 同一父目录，清理策略一致');
      expect(
        p.basename(remoteEmbeddedSubtitleCacheDir('a/b:c d').path),
        'remote_a_b_c_d',
      );
      expect(
        remoteEmbeddedSubtitleCacheDir('x_1').path,
        isNot(remoteEmbeddedSubtitleCacheDir('x_2').path),
        reason: '字节数变了（服务器换了文件）不能命中旧缓存',
      );
    });

    test('cachedEmbeddedSubtitleFile 只认存在且非空的文本轨缓存', () async {
      final Directory dir =
          await Directory.systemTemp.createTemp('vsub_remote_');
      addTearDown(() => dir.delete(recursive: true));
      expect(cachedEmbeddedSubtitleFile(dir, 0, 'subrip'), isNull);
      File(p.join(dir.path, 'sub_0.srt')).writeAsStringSync('');
      expect(cachedEmbeddedSubtitleFile(dir, 0, 'subrip'), isNull,
          reason: '空文件是失败残留，不算命中');
      File(p.join(dir.path, 'sub_0.srt')).writeAsStringSync(kOneCueSrt);
      expect(cachedEmbeddedSubtitleFile(dir, 0, 'subrip')?.path,
          p.join(dir.path, 'sub_0.srt'));
      // 命名与本地抽取一致：ass 轨落 .ass。
      File(p.join(dir.path, 'sub_2.ass')).writeAsStringSync('[Script Info]');
      expect(cachedEmbeddedSubtitleFile(dir, 2, 'ass')?.path,
          p.join(dir.path, 'sub_2.ass'));
      expect(cachedEmbeddedSubtitleFile(dir, 2, 'subrip'), isNull,
          reason: '扩展名随 codec，subrip 不会误吃 .ass');
      expect(cachedEmbeddedSubtitleFile(dir, 1, 'hdmv_pgs_subtitle'), isNull,
          reason: '图形轨没有文本缓存');
    });

    test('extractRemoteEmbeddedSubtitle 缓存命中时不碰 ffmpeg', () async {
      final Directory dir =
          await Directory.systemTemp.createTemp('vsub_remote_');
      addTearDown(() => dir.delete(recursive: true));
      File(p.join(dir.path, 'sub_1.srt')).writeAsStringSync(kOneCueSrt);
      final File? hit = await extractRemoteEmbeddedSubtitle(
        streamUrl: 'https://server.invalid/Videos/x/stream',
        cacheDir: dir,
        ordinal: 1,
        codec: 'subrip',
        sizeBytes: 1,
      );
      expect(hit?.path, p.join(dir.path, 'sub_1.srt'));
    });
  });
