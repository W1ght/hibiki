...
void main() {
  group('kVideoShaderTiers 五档映射', () {
    test('恰好五档，顺序 无→低→中→高→极高，id 稳定唯一', () {
      ...
      expect(
        kVideoShaderTiers.map((VideoShaderTierSpec s) => s.id).toList(),
    ...
    test('低 = mpv 内置 ewa_lanczossharp（零下载）：内置缩放 on + 空 GLSL', () {
      ...
      expect(
        ...
        isEmpty,
        reason:
            '低档只靠 mpv 内置 scale 链（buildMpvProperties highQuality 分支 '
    ...
    test('极高 = Anime4K Mode A VL + 额外去模糊修复：内置缩放 on + VL 类链（非 UL）', () {
      ...
      expect(
        ultra.shaderFileNames,
        contains('Anime4K_Restore_CNN_Soft_VL.glsl'),
      );
  ...
  group('tierFromState（状态→档位反查投影）', () {
    ...
    test('内置 on + 空集 → 低（仅靠 highQuality 区分 off/low）', () {
      expect(
        tierFromState(highQuality: true, enabledShaders: const <String>[]),
        VideoShaderTier.low,
      );
    });

    test('内置 on + Anime4K Fast 全集（顺序无关）→ 中', () {
      final List<String> shuffled = shaderFilesForTier(
        VideoShaderTier.medium,
      ).reversed.toList();
      expect(
        tierFromState(highQuality: true, enabledShaders: shuffled),
        VideoShaderTier.medium,
      );
    });
    ...
    test('反查唯一：高/极高文件集互不相等，(highQuality, shaderSet) 两两不重复', () {
      ...
      final Set<String> highSet = shaderFilesForTier(
        VideoShaderTier.high,
      ).toSet();
      final Set<String> ultraSet = shaderFilesForTier(
        VideoShaderTier.ultra,
      ).toSet();
    ...
    test('内置 on + 非标准勾选（多一个文件）→ null（自定义）', () {
      final List<String> custom = shaderFilesForTier(
        VideoShaderTier.medium,
      ).toList()..add('SomeUserShader.glsl');
  ...
  group('orderedEnabledForTier（按目录现有文件过滤+保序）', () {
    test('全部存在 → 返回该档全集并保持叠加顺序', () {
      ...
      final List<String> got = orderedEnabledForTier(
        VideoShaderTier.medium,
        want.toSet(),
      );
      expect(got, want);
    ...
    test('无 GLSL 档（低）→ 空集', () {
      expect(
        orderedEnabledForTier(VideoShaderTier.low, <String>{
