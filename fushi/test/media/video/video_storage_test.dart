...
void main() {
  ...
  group('deleteBookAssets (per-book precise delete, BUG-276)', () {
    ...
    test(
      'deleting A never touches playlist B\'s other-episode subtitle copies',
      () async {
    ...
    test(
      'keeps a file still referenced by another book (shared/reused name)',
      () async {
    ...
    test(
      'never deletes a path outside the app-owned dirs (user video)',
      () async {
  ...
  group('gcOrphanCovers (safe cover-only history GC, BUG-276)', () {
    test(
      'removes covers not referenced by the DB, keeps referenced ones',
      () async {
    ...
    test(
      'removes ALL covers when nothing is referenced (full purge)',
      () async {
