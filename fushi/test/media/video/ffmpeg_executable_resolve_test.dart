...
void main() {
  ...
  group('resolveEnvOverrideFrom 新名优先旧名回退', () {
    ...
    test('都没设 → null（交给捆绑/PATH 阶梯）', () {
      expect(
        resolveEnvOverrideFrom(const <String, String>{}, const <String>[
          'FUSHI_FFMPEG',
          'HIBIKI_FFMPEG',
        ]),
        isNull,
  ...
  group('resolveFfmpegExecutableFrom 优先级', () {
    test('FUSHI_FFMPEG 覆盖最高优先', () {
      expect(
        resolveFfmpegExecutableFrom(
          override: '/opt/ff/ffmpeg',
          bundledPath: '/app/ffmpeg',
        ),
        '/opt/ff/ffmpeg',
      );
    });
    ...
    test('无覆盖时用程序旁捆绑 ffmpeg', () {
      ...
      expect(
        resolveFfmpegExecutableFrom(
          override: '   ',
          bundledPath: r'C:\Hibiki\ffmpeg.exe',
        ),
  ...
  group('runCliFfmpegForTesting bundled fallback', () {
    test(
      'Windows bundled ffmpeg invalid image falls back to PATH ffmpeg',
      () async {
        ...
        expect(result.attemptedExecutables, <String>[
          r'C:\App\Hibiki\ffmpeg.exe',
          'ffmpeg',
        ]);
    ...
    test(
      'fallback PATH launch failure keeps bundled invalid-image context',
      () async {
        final List<String> calls = <String>[];
    ...
    // 而非死盯单一错误码。
    test(
      'bundled ffmpeg launch ProcessException (any code) falls back to PATH',
      () async {
    ...
    test(
      'bundled ffmpeg launch failure falls back to PATH on non-Windows too',
      () async {
        final List<String> calls = <String>[];
        final FfmpegRunResult result = await runCliFfmpegForTesting(
          override: null,
    ...
    test(
      'explicit FUSHI_FFMPEG launch ProcessException still propagates',
      () async {
    ...
    test(
      'no bundled, no override: PATH launch ProcessException propagates',
      () async {
    ...
    test(
      'bundled empty output fallback is platform-agnostic (Linux/mac too)',
      () async {
        final List<String> calls = <String>[];
        final FfmpegRunResult result = await runCliFfmpegForTesting(
          override: null,
    ...
    test(
      'normal -i enumeration (non-zero + full stderr) does NOT fall back',
      () async {
        // 正常工作的 ffmpeg：`-i` 无输出文件恒退出非 0，但 stderr 满是流信息——
        // 这是成功枚举，绝不能误判成坏二进制去回退 PATH（否则平添一次进程开销）。
        final List<String> calls = <String>[];
        const String fullLog =
    ...
    test(
      'bundled timeout (returnCode null + empty) does NOT fall back',
      () async {
