...
void main() {
  test(
    'getCollection uses the authenticated username path and auth headers',
    () async {
      ...
      final BangumiApiClient client = BangumiApiClient(
        ...
        client: MockClient((http.Request request) async {
          captured = request;
          return http.Response('{"type":3,"ep_status":4,"vol_status":1}', 200);
        }),
      );
      addTearDown(client.close);

      final BangumiUserCollection? value = await client.getCollection(
        'alice name',
        123,
      );
      ...
      expect(
        ...
        contains('hajisensai/Fushi/1.2.0'),
      );
  ...
  test(
    'search filters the official subject type and parses Chinese title',
    () async {
      late http.Request captured;
      final BangumiApiClient client = BangumiApiClient(
        accessToken: 'token',
      ...
      final List<BangumiSubject> results = await client.searchSubjects(
        keyword: '芙莉莲',
        subjectType: 2,
      );
  ...
  test('watched anime list follows every official collection page', () async {
    ...
    final List<BangumiWatchedItem> watched = await client.getWatchedAnime(
      'alice name',
    );
    ...
    expect(
      captured.first.url.queryParameters,
      containsPair('subject_type', '2'),
    );
    ...
    expect(watched.map((BangumiWatchedItem item) => item.subject.id), <int>[
      41,
      42,
    ]);
