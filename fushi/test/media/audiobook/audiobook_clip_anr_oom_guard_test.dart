...
void main() {
  ...
  group('(a) JPEG 编码卸到后台 isolate（不在 UI isolate 同步跑 2MP 解码/编码）', () {
    test('encodeClipTextFrameAsJpgAsync 存在且经 Isolate.run 卸载', () {
      ...
      expect(
        code.contains('Future<Uint8List?> encodeClipTextFrameAsJpgAsync('),
        isTrue,
      ...
      expect(
        isolateWrap.hasMatch(code),
        isTrue,
        reason:
            '异步封装必须经 Isolate.run 调同步 encodeClipTextFrameAsJpg，'
    ...
    test('导出管线不再同步调 encodeClipTextFrameAsJpg（只走 Async 版）', () {
      ...
      expect(
        syncCall.allMatches(code).length,
        0,
        reason:
            'audiobook.part.dart 内不得同步调 encodeClipTextFrameAsJpg('
            '（UI isolate 冻结 → ANR），一律走 encodeClipTextFrameAsJpgAsync',
      );
      final RegExp asyncCall = RegExp(r'encodeClipTextFrameAsJpgAsync\(');
      expect(
        asyncCall.allMatches(code).length,
  ...
  group('(b) takeScreenshot() 带超时（绝不无限卡死整条导出管线）', () {
    test('每个 takeScreenshot() 调用点都 .timeout(...) 守护', () {
      ...
      expect(
        ...
        calls,
        reason:
            '每个 takeScreenshot() 都必须 .timeout(...)，'
            '否则单帧截图卡死会永久挂住导出管线',
      );
      expect(
        code.contains('_kClipScreenshotTimeout'),
        isTrue,
        reason: '截图超时应由具名常量定义',
      );
      expect(
  ...
  group('(c) 流式不全驻留（不再把 N 帧 PNG/JPEG 同时收进列表）', () {
    test('渲染层改逐帧回调 AudiobookClipFrameSink（不再返回 List<Uint8List?>）', () {
      ...
      expect(
        textRender.contains('typedef AudiobookClipFrameSink'),
        isTrue,
        reason: '定义逐帧回调类型',
      );
      expect(
      ...
      expect(
        textRender.contains('required AudiobookClipFrameSink onFrame'),
        isTrue,
      );
      expect(
        webview.contains('required AudiobookClipFrameSink onFrame'),
        isTrue,
      );
    });
    ...
    test('_synthDynamicClipVideo 逐帧落盘母帧，不再攒满 pngs/jpgByIndex', () {
      final String code = _read(audiobookPartPath);
      expect(
        code.contains('final List<Uint8List?> pngs'),
        isFalse,
        reason: '不得把全部 N 帧 PNG 同时驻留（native OOM 根因）',
      );
      expect(
        code.contains('final Map<int, Uint8List> jpgByIndex'),
        isFalse,
        reason: '不得把全部帧 JPEG 同时驻留内存',
      );
      expect(
        code.contains('encodeClipTextFrameAsJpgAsync(png)'),
        isTrue,
        reason: 'onFrame 里逐帧后台编码',
      );
      expect(
        code.contains('masterJpgPathByIndex'),
        isTrue,
        reason: '母帧按 highlightCueIndex 落盘落路径映射',
      );
      expect(
        code.contains('master.copy('),
        isTrue,
