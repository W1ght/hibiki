...
void main() {
  ...
  test(
    'reader page uses shared MD3 dialog frame for desktop quick settings',
    () {
      final String readerSource = readReaderPageSource();

      final int desktopBranch = readerSource.indexOf('if (isDesktopPlatform)');
      final int mobileBranch = readerSource.indexOf(
      ...
      final String desktopSource = readerSource.substring(
        desktopBranch,
        mobileBranch,
      );
  ...
  test('narrow nav rows and wide categories share the same category order '
      '(TODO-725: nav-first)', () {
    final String source = File(
      'lib/src/media/audiobook/reader_quick_settings_sheet.dart',
    ).readAsStringSync();
    ...
    final List<String> wideOrder = RegExp(
      r"id: '([a-z]+)'",
    ).allMatches(wideSource).map((Match m) => m.group(1)!).toList();
  ...
  test('Ctrl+F opens the sheet straight to the navigation (location) sub-page '
    ...
    expect(
      readerSource,
      contains('case ShortcutAction.readerOpenNavigation:'),
