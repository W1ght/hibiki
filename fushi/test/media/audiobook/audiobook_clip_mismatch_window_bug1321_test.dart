...
void main() {
  ...
  test(
    'plan builder keeps the plan (and its audio window) on text mismatch',
    () {
      ...
      expect(
        afterMatch.contains('return null;'),
        isFalse,
  ...
  test(
    'dispatcher only disables highlight on mismatch, keeps the full window',
    () {
      ...
      expect(
        gateMatch,
        isNotNull,
        reason: '不一致时必须显式禁用逐句高亮（BUG-968 契约），而非丢弃音频窗',
      );
      final int nullAfterGate = body.indexOf(
        'dynamicPlan = null;',
        gateMatch!.start,
      );
      ...
      expect(
        ...
        isTrue,
        reason:
            'BUG-1321/BUG-1320：整段窗口必须来自多句计划；无条件回落单句锚会把长选区'
  ...
  test('plan model carries the cueTextMatches contract field', () {
    ...
    final String classBody = part.substring(
      ...
      part.indexOf(
        '}',
        part.indexOf('final List<List<Uint8List>> imagesByCueIndex;', classAt),
      ),
    );
