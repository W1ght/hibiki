...
void main() {
  ...
  test(
    'default (background-play OFF) stops the session on exit (TODO-702)',
    () {
      final RegExpMatch? body = disposeBody();
  ...
  test(
    'exit-stop is guarded by the pref, not unconditional (keeps TODO-291)',
    () {
      final RegExpMatch? body = disposeBody();
      expect(body, isNotNull);
      final String dispose = body!.group(1)!;
      ...
      expect(
        ...
        isTrue,
        reason:
            'stop 必须在 !audiobookBackgroundPlay 门控内，'
  ...
  test(
    'onSourcePagePop fires the guarded stop without awaiting (BUG-1273)',
    () {
      ...
      expect(
        ...
        isTrue,
        reason:
            '退出即停必须在 onSourcePagePop 里按 !audiobookBackgroundPlay '
      ...
      expect(
        ...
        isFalse,
        reason:
            'BUG-1273：退出路径不得 await stop（native 释放耗时不可控且只在播放态'
  ...
  test(
    'onSourcePagePop stop() is guarded by catchError + ErrorLogService (W1)',
    () {
      final RegExpMatch? body = RegExp(
        r'Future<void> onSourcePagePop\(\) async \{(.*?)\n  \}',
        dotAll: true,
      ...
      expect(
        ...
        isTrue,
        reason:
            'onSourcePagePop 的 fire-and-forget stop 必须 .catchError 记到 '
