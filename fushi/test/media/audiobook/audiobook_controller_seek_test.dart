...
void main() {
  ...
  group('AudiobookPlayerController cue seek mapping', () {
    ...
    test(
        ...
        () async {
      final _HangingJustAudioPlatform platform =
          _installHangingAudioPlatform(ready: true);
      final AudiobookPlayerController controller = AudiobookPlayerController();
      addTearDown(controller.dispose);
      await controller.load(
          audiobook: _audiobook(),
    ...
    test('source restoration rejects invalid file indices and negative offsets',
        () async {
      _installHangingAudioPlatform();
      final AudiobookPlayerController controller = AudiobookPlayerController();
      addTearDown(controller.dispose);
      await controller.load(audiobook: _audiobook(), audioFiles: <File>[
      ...
      expect(controller.position.inMilliseconds, 55000,
          reason:
...
_HangingJustAudioPlatform _installHangingAudioPlatform({bool ready = false}) {
  ...
  final _HangingJustAudioPlatform platform =
      _HangingJustAudioPlatform(ready: ready);
...
class _HangingAudioPlayer extends AudioPlayerPlatform {
  ...
  Future<LoadResponse> load(LoadRequest request) {
    ...
    if (ready) {
      emitPosition(
          request.initialIndex ?? 0, request.initialPosition ?? Duration.zero);
