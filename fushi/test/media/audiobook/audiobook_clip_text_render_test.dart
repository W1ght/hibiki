...
void main() {
  ...
  testWidgets('renderAudiobookClipTextToPng returns non-empty PNG for the '
      'user trigger text (BUG-490)', (WidgetTester tester) async {
  ...
  test('source guard: catch branch logs clipToImageThrew (never a bare '
      'return null) (BUG-490)', () {
    final File src = File(
  ...
  test('computeClipTextLayout: 18-rune yoon-initial trigger stays sane '
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        frames[0],
        isNot(equals(frames[1])),
        reason:
            'moving the highlight to a different sentence must change '
            'the rendered pixels',
      );
    },
  );

  test('source guard: highlighted line uses layout.highlight, others plain fg '
      '(TODO-1115)', () {
    final File src = File(
      'lib/src/media/audiobook/audiobook_clip_text_render.dart',
    );
    ...
    expect(
      code.contains('i == widget.highlightIndex'),
      isTrue,
      reason: 'the highlighted sentence is chosen by highlightIndex',
    );
    expect(
      code.contains('color: layout.highlight'),
      isTrue,
      reason:
