...
void main() {
  ...
  Future<void> insertBook(
    ...
    SrtBooksCompanion.insert(
      ...
      audioPathsJson: Value(audioPaths == null ? null : jsonEncode(audioPaths)),
      srtPath: srtPath,
    ...
    test(
      ...
      () {
        ...
        final AudiobookPathRelocator relocator = AudiobookPathRelocator(
          audiobooksRoot: root.path,
        );
        expect(
          relocator.relocate(
            '/data/user/0/app.hibiki.reader/audiobooks/u1/01.mp3',
          ),
  ...
  group('SrtBookRepository.repairMovedPaths', () {
    test(
      'repairs all four columns of a row stranded on the old data root',
      () async {
        writeFile(p.join(root.path, 'u1', '01.mp3'), 'A');
    ...
    test(
      'does not rewrite anything when several same-name candidates exist',
      () async {
        writeFile(p.join(root.path, 'a', '01.mp3'), 'A');
        writeFile(p.join(root.path, 'b', '01.mp3'), 'B');
    ...
    test(
      'a healthy library is never rewritten (and never scans the tree)',
      () async {
    ...
    test('only the broken row is rewritten (rows are keyed by uid, and the '
      ...
      await insertBook(
        db,
        uid: 'srtbook_broken',
        srtPath: '/old/hibiki/audiobooks/u1/sub.srt',
      );
      ...
      expect(
        (await db.getSrtBookByUid('srtbook_broken'))!.srtPath,
        p.join(root.path, 'u1', 'sub.srt'),
      );
      expect(
        (await db.getSrtBookByUid('srtbook_healthy'))!.srtPath,
        healthySrt,
        reason: '健康行绝不能被同批改写',
      );
    });
    ...
    test(
      'listAll alone self-heals (production wiring: documents root hook)',
      () async {
        writeFile(p.join(root.path, 'u1', 'sub.srt'), 'S');
        writeFile(p.join(root.path, 'u1', '01.mp3'), 'A');
        const String old = '/data/user/0/app.hibiki.reader/files/audiobooks/u1';
        ...
        expect(
          books.single.srtPath,
          p.join(root.path, 'u1', 'sub.srt'),
          reason: '返回的模型必须已经是修好的路径',
        );
        expect(books.single.audioPaths, <String>[
          p.join(root.path, 'u1', '01.mp3'),
        ]);
