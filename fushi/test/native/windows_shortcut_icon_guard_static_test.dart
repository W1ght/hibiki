...
void main() {
  ...
  test('native flutter_window.cpp 含 setShortcutIcon + IShellLink 实现', () {
    ...
    expect(
      src.contains('"setShortcutIcon"'),
      isTrue,
      reason: 'channel 必须处理 setShortcutIcon',
    );
    expect(
      src.contains('flutter::EncodableValue("iconPath")'),
      isTrue,
    ...
    expect(
      src.contains('CLSID_ShellLink'),
      isTrue,
      reason: '必须 CoCreateInstance(CLSID_ShellLink) 改 .lnk',
    );
    expect(
      src.contains('SetIconLocation('),
      isTrue,
    ...
    expect(
      loadIdx >= 0 && saveIdx > loadIdx,
      isTrue,
      reason: '必须先 IPersistFile::Load 现有 .lnk 再 Save（保留 target/args/workdir）',
    );
    ...
    expect(
      RegExp(
        r'FushiShortcutInFolder\(\s*FOLDERID_UserPinned\s*,\s*'
    ...
    expect(
      src.contains('shell_link->GetPath('),
      isTrue,
      reason: '必须读取固定项现有 target 进行校验',
    );
    expect(
      src.contains('GetModuleFileNameW('),
      isTrue,
      reason: '必须以当前进程 exe 作为固定项 target 真值',
    );
    expect(
      src.contains('GetFinalPathNameByHandleW('),
      isTrue,
      reason: '固定项 target 与当前 exe 必须先解析最终路径再比较',
    );
    expect(
      src.contains('FinalPathForComparison(target.data())'),
      isTrue,
      reason: '固定项 target 必须参与最终路径归一化',
    );
    expect(
      src.contains('FinalPathForComparison(executable.data())'),
      isTrue,
    ...
    expect(
      RegExp(
        r'FushiShortcutInFolder\(\s*FOLDERID_Programs\s*,\s*'
        r'L"Fushi\\\\Fushi\.lnk"',
      ).hasMatch(src),
      isTrue,
      reason:
          r'开始菜单 .lnk 必须拼成程序组 Fushi 子文件夹下的 Fushi.lnk'
          r'（FOLDERID_Programs + Fushi\Fushi.lnk），不能用 Programs 根下的裸 Fushi.lnk',
    );
    expect(
      src.contains('SHGetKnownFolderPath('),
      isTrue,
  ...
  test('CMakeLists 链接 ole32 + shell32（IShellLink/KnownFolder 依赖）', () {
    ...
    expect(
      RegExp(
        r'target_link_libraries\(\$\{BINARY_NAME\} PRIVATE[^)]*\bole32\b',
      ).hasMatch(cmake),
      isTrue,
    ...
    expect(
      RegExp(
        r'target_link_libraries\(\$\{BINARY_NAME\} PRIVATE[^)]*\bshell32\b',
      ).hasMatch(cmake),
      isTrue,
  ...
  test('Dart syncWindowsShortcutIcons 经 setShortcutIcon 用 iconPath 下发', () {
    ...
    expect(
      src.contains("MethodChannel('app.fushi/window')"),
      isTrue,
  ...
  test('settings 页换图标成功后调 syncWindowsShortcutIcons', () {
    ...
    expect(
      src.contains('syncWindowsShortcutIcons('),
      isTrue,
      reason: '_switchPreset / _pickCustomIcon 必须在换图标后同步 .lnk 图标',
    );
    // 两个 Windows 触点都接：preset + custom。
    expect(
      'syncWindowsShortcutIcons('.allMatches(src).length >= 2,
      isTrue,
