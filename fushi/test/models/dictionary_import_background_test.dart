...
void main() {
  ...
  group('TODO-082 source guards', () {
    ...
    test('导入失败写入 ErrorLogService（不吞异常）', () {
      expect(
        manager.contains("ErrorLogService.instance.log('DictImport(dir)'"),
        isTrue,
    ...
    test('成功导入给用户提示（数量汇总）', () {
      expect(
        dialog.contains('t.dict_import_success_summary'),
        isTrue,
        reason: '多文件路径成功后给成功提示',
      );
      expect(
        manager.contains('t.dict_import_success_summary'),
        isTrue,
