...
void main() {
  ...
  final String audioCtrlSrc = File(
  ...
  test('① openMedia 不再 await audio handler 冷启（起跑不阻塞导航）', () {
    ...
    expect(
      body.contains('await initialiseAudioHandler()'),
      isFalse,
      reason: 'push 前 await 冷启 = 点击后零反馈的那段，禁止回归',
    );
    expect(
      body.contains('unawaited(initialiseAudioHandler())'),
      isTrue,
  ...
  test('② initialiseHandler 记忆化在飞 future（并发安全单次初始化）', () {
    expect(
      audioCtrlSrc.contains('_handlerInit ??= _initialiseHandlerOnce()'),
      isTrue,
      reason:
          '无记忆化时「openMedia 起跑 + 消费端 await」并发双跑 '
          'AudioService.init',
    );
  });

  test('③ reader _resolveAudioSlot 在会话操作前 await handler 就绪', () {
    final int slotStart = readerSrc.indexOf(
    ...
    expect(
      beforeSession.contains('await appModel.initialiseAudioHandler();'),
      isTrue,
      reason:
          'session attach/start 挂媒体通知前 handler 必须就绪——'
          'openMedia 不再兜底这个时序，消费端自己 await',
    );
  });

  test('③ 书架后台听书入口仍 await handler 就绪', () {
    final int start = appModelSrc.indexOf(
