...
void main() {
  test('round trips through json', () {
    const LocalAudioSourcePref p = LocalAudioSourcePref(
      name: 'nhk16',
      enabled: false,
    );
    final LocalAudioSourcePref restored = LocalAudioSourcePref.fromJson(
      p.toJson(),
    );
    expect(restored, p);
  });
  ...
  test('defaults enabled to true on malformed json', () {
    final LocalAudioSourcePref p = LocalAudioSourcePref.fromJson(
      const <String, dynamic>{'name': 'forvo'},
    );
    expect(p.name, 'forvo');
