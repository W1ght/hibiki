...
void main() {
  ...
  setUp(() async {
    ...
    manager = LocalAudioManager(prefsRepo: prefs, databaseDirectory: directory);
  });
  ...
  test('LocalAudioDbEntry round trips its source prefs through json', () {
    ...
    final LocalAudioDbEntry restored = LocalAudioDbEntry.fromJson(
      entry.toJson(),
    );
  ...
  test(
    'setSourcesFor updates only the matching db, leaving others intact',
    () async {
      await manager.setEntries(const <LocalAudioDbEntry>[
        LocalAudioDbEntry(
          path: '/tmp/one.db',
  ...
  test('reorder ignores out-of-range indexes', () async {
    ...
    expect(jsonDecode(before) as List<dynamic>, hasLength(2));
  });
  ...
  test(
    'importFile throws when the source file does not exist (BUG-446)',
    () async {
      expect(
  ...
  test('importFile rejects a non-audio file (zip / junk) and leaves no copy '
      '(BUG-779)', () async {
  ...
  test(
    'importFile reference=true also rejects a non-audio file (BUG-779)',
    () async {
      final Directory src = await Directory.systemTemp.createTemp(
        'src_junk_ref',
      );
      final File junk = File('${src.path}/notes.txt')
  ...
  test('importFile reference=true still throws when the source is missing '
      '(BUG-483)', () async {
    expect(
      () => manager.importFile(
        '/no/such/ref.db',
  ...
  test(
    'pruneOrphans never touches external referenced files (BUG-483)',
    () async {
      final Directory ext = await Directory.systemTemp.createTemp('ext_prune');
      final File external = File('${ext.path}/ref.db')
  ...
  test(
    'pruneOrphans deletes unreferenced local_audio_*.db files only',
    () async {
      ...
      expect(
        other.existsSync(),
        isTrue,
      ); // not a local_audio_* file -> untouched
    },
  );
}
