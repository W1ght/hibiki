...
void main() {
  group('TODO-1166 built-in star rating tags', () {
    ...
    test(
      'fresh pool: seeds exactly 1..5 stars in ascending sortOrder',
      () async {
        ...
        expect(
          tags.map((BookTagRow t) => t.name).toList(),
          kBuiltInStarTagNames,
        );
        expect(
          tags.map((BookTagRow t) => t.colorValue).toList(),
    ...
    test('idempotent: second call adds nothing, no duplicate rows', () async {
      ...
      expect(
        tags.map((BookTagRow t) => t.name).toSet(),
        kBuiltInStarTagNames.toSet(),
      );
    });

    test(
      'only-add: preserves user tags + edited star color, never deletes',
      () async {
        final FushiDatabase db = await _openDb();
