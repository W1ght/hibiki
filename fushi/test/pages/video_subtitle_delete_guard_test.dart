...
void main() {
  ...
  test('菜单：抽取进行中不弹；坐标经 Overlay.globalToLocal 换算；'
    ...
    expect(
      menu.contains('Overlay.of(context).context.findRenderObject()'),
      isTrue,
    ...
    expect(
      RegExp(
        r'_focusOwnership\.guardOverlay\(\s*\(\) => showMenu<bool>\(',
      ).hasMatch(menu),
      isTrue,
      reason:
    ...
    expect(
      menu.contains('await _deleteSubtitleFile(controller, source);'),
      isTrue,
  ...
  test('删除：先确认（共享销毁确认框、带路径），再清当前主 / 副字幕，然后才删文件，'
    ...
    final int confirm = del.indexOf(
      't.video_subtitle_delete_confirm(path: path)',
    );
    ...
    final int removeImported = del.indexOf(
      '_importedSubtitleSources.where(notDeleted)',
    );
    expect(
      confirm,
    ...
    expect(
      del.contains('FushiDestructiveConfirmDialog('),
      isTrue,
      reason:
    ...
    expect(
      RegExp(
        r'_focusOwnership\.guardOverlay\(\s*\(\) => showAppDialog<FushiDestructiveConfirmResult>\(',
      ).hasMatch(del),
      isTrue,
    ...
    expect(
      del.contains("'[video-playback] delete external subtitle failed"),
      isTrue,
    ...
    expect(
      del.contains('_selectSubtitleOff(controller)'),
      isFalse,
      reason:
    ...
    expect(
      del.contains('_clearRemoteSubtitle(controller)'),
      isFalse,
    ...
    expect(
      del.contains('_clearRemoteSecondarySubtitle(controller)'),
      isTrue,
    ...
    expect(
      del.contains('sameExternalSubtitlePathForMenu(source, primary)'),
      isTrue,
  ...
  test('清当前主字幕：落 null（无偏好）而非 off: 哨兵，三种落库形状齐全', () {
    ...
    expect(
      forget.contains('updateSubtitleSource(widget.bookUid, null)'),
      isTrue,
