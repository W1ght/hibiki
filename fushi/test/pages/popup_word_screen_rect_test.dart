...
void main() {
  testWidgets(
    ...
    'render box, accounting for the header offset (BUG-129)',
    (tester) async {
      ...
      expect(
        screen.top,
        greaterThan(naive.top),
        reason: 'header-corrected top must be lower than the naive shift',
      );
    },
  );

  testWidgets('popupWordScreenRect returns the fallback when the render box is '
  ...
  test('nested popup callbacks position children via popupWordScreenRect, not '
    ...
    for (final String path in <String>[
      ...
      expect(
        src,
        contains('popupWordScreenRect('),
        reason: '$path must map nested-lookup rects through the WebView box',
      );
      expect(
        src,
        isNot(contains('localRect.shift(Offset(')),
        reason:
            '$path must not reconstruct the rect from the Positioned '
