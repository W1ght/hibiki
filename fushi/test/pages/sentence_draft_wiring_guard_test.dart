...
void main() {
  ...
  test('webview registers the generic setSentenceContext JS handler', () {
    ...
    expect(
      src,
      contains('sentenceDraftEnabled: kSentenceContextPickerEnabled &&'),
      reason:
          'webview must still gate the flag on the SAME predicate '
  ...
  test('reader clears the draft on a new lookup, after mine, and on dismiss', () {
    ...
    expect(
      '_miningDraft.clear()'.allMatches(src).length,
  ...
  test('reader and video override clear to empty their draft', () {
    ...
    final String mixin = readSource(
      'lib/src/pages/implementations/dictionary_page_mixin.dart',
    );
    expect(
      mixin,
      contains('onClearSentenceDraft: onClearSentenceDraftToDraft'),
    );
  });
  ...
  test(
    'popup.js exposes resetSentenceContextMirror that zeros both scalars',
    () {
      final String js = readSource('assets/popup/popup.js');
