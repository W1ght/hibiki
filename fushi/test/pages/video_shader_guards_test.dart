...
String _promptMethodBody(String src) {
  ...
  final int end = src.indexOf(
    '_downloadAndEnableDefaultShaderTier',
    start + marker.length,
  );
  expect(
    end,
...
void main() {
  group('着色器对比原画（TODO-127 / BUG-261 / C 快捷键）', () {
    ...
    final String shortcutsSrc = File(
    ...
    test('有 _toggleShaderCompare 走 controller.toggleShaderBypass + OSD', () {
      ...
      expect(
        body.contains('toggleShaderBypass()'),
        isTrue,
    ...
    test('控制条不再放着色器对比按钮（TODO-127；改从右键菜单 / 快捷键 / 设置进入）', () {
      ...
      expect(
        controls.contains('Icons.compare'),
        isFalse,
        reason: '对比按钮应已移出桌面 / 移动控制条',
      );
      expect(
        controls.contains('onPressed: _toggleShaderCompare'),
        isFalse,
    ...
    test('C 快捷键切换着色器对比', () {
      ...
      expect(
        pageSrc.contains('buildVideoPlayerShortcutsFromRegistry('),
        isTrue,
      ...
      expect(
        ...
        )[ShortcutAction.videoToggleShaderCompare]!.keyboardBindings.contains(
          cKey,
        ),
        isTrue,
  ...
  group('TODO-874：手机端抑制 Anime4K 首次提示', () {
    ...
    test('引入 foundation（defaultTargetPlatform / TargetPlatform）', () {
      expect(
        videoSrc.contains("import 'package:flutter/foundation.dart';"),
        isTrue,
    ...
    test('方法体内含移动端平台 early-return', () {
      ...
      expect(
        body.contains('defaultTargetPlatform == TargetPlatform.android'),
        isTrue,
        reason: 'Android 应在该方法内被门控早退',
      );
      expect(
        body.contains('defaultTargetPlatform == TargetPlatform.iOS'),
        isTrue,
        reason: 'iOS 应在该方法内被门控早退',
      );
      // 平台判定后紧跟 return，构成 early-return（移动端直接跳过整段提示）。
      final int iosAt = body.indexOf(
    ...
    test('纯抑制：不在移动端早退路径置 videoAnime4kPromptShown 标记', () {
      ...
      expect(
        gateSlice.contains('setVideoAnime4kPromptShown'),
        isFalse,
