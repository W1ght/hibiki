...
void main() {
  ...
  test('内容就绪兜底超时回调必须连同解开导航态（restore/navigate/completer）', () {
    ...
    // ① 超时回调仍走「强制摘遮罩」这条路径。
    expect(
      body.contains('_readerContentReady = true;'),
      isTrue,
    ...
    expect(
      body.contains('_failNavigation();'),
      isTrue,
      reason:
          '超时回调必须 _failNavigation 清导航态（含 _isNavigatingToChapter / '
    ...
    expect(
      failBody.contains('_isNavigatingToChapter = false;'),
      isTrue,
      reason:
          '_failNavigation 必须清 _isNavigatingToChapter（否则 '
          '_paginationInFlight 恒真、翻页永久卡死）',
    );
    expect(
      failBody.contains('_restoreInFlight = false;'),
      isTrue,
      reason: '_failNavigation 必须清 _restoreInFlight',
    );
    expect(
      failBody.contains('_restoreCompleter!.complete(false);'),
      isTrue,
      reason: '_failNavigation 必须完成挂起的 restore completer，放行等待方',
    );
    expect(
      failBody.contains('_restoreCompleter = null;'),
      isTrue,
