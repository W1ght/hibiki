...
void main() {
  test('BUG-1658: ModuleSettingsView 不得用 DesktopContentLayout 包住共享页头', () {
    ...
    final String code = src
        ...
        .join('\n');
    expect(
      code.contains('DesktopContentLayout'),
      isFalse,
      reason:
          '设置分区的页头必须与兄弟分区（readerShelf 全出血）同构对齐，'
          '不得再叠 DesktopContentLayout 的侧向留白/限宽（BUG-1658）',
    );
    expect(
      code.contains('DesktopContentKind'),
      isFalse,
    ...
    expect(
      ...
      isTrue,
      reason:
          '分段导航页头必须仍经 FushiPageHeader.customTitle 渲染，'
          '与兄弟分区同一页头组件',
    );
  });
  ...
  test('BUG-1658: 下载页 / 浏览器扩展页不得回退旧 AppBar 小标题页头', () {
    for (final String path in <String>[
      ...
      final String code = File(path)
          ...
          .join('\n');
      expect(
        code.contains('appBar:'),
        isFalse,
        reason:
            '$path 页头已统一为 FushiPageHeader 大标题（BUG-1658），'
