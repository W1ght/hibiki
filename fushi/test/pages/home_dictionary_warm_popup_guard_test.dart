...
void main() {
  ...
  test('home dictionary seeds and reuses a persistent warm popup slot', () {
    ...
    expect(
      src,
      isNot(contains('replaceStack: true')),
      reason: '连点 replaceStack 反复 create/destroy WebView 正是 UAF 崩溃源，必须改为复用热槽',
    );
  });
  ...
  test(
    ...
    () {
      ...
      expect(
        src,
        contains('bool get _hasVisiblePopup => _popup.hasVisiblePopup;'),
      );
      expect(
        src,
        contains('canPop: !_hasActiveQuery && !_popup.hasVisiblePopup'),
        reason: 'back/exit must ignore the hidden warm slot',
      );
      expect(
        src,
        contains('if (_popup.hasVisiblePopup) {'),
      ...
      expect(
        src,
        isNot(contains('_popup.clear();')),
        reason: 'clearing the stack would dispose the warm WebView mid-session',
      );
    },
  );
}
