...
void main() {
  ...
  test('着色器视图有「指定 mpv 目录并搜索」流程 + override 优先发现', () {
    ...
    expect(
      src.contains('_pickMpvDirAndSearch'),
      isTrue,
    ...
    expect(
      src.contains('discoverLocalMpvShaders(overrideDir:'),
      isTrue,
    ...
    expect(
      src.contains('_pickMpvDirAndSearch(autoFallback: true)'),
      isTrue,
  ...
  test('设置面板 → 视频页把 mpv 目录初值/回调接到 appModel 持久化', () {
    ...
    expect(
      actions.contains('onMpvShaderDirChanged'),
      isTrue,
    ...
    expect(
      host.contains('onMpvShaderDirChanged'),
      isTrue,
    ...
    expect(
      page.contains('appModel.setVideoMpvShaderDir('),
      isTrue,
  ...
  test('着色器视图有「粘贴链接下载」流程（不必装 mpv）', () {
    ...
    expect(
      src.contains('downloadShaderFromUrl('),
      isTrue,
      reason: '走 downloadShaderFromUrl（镜像+校验）',
    );
    expect(
      src.contains('t.video_shader_download_url'),
      isTrue,
      reason: '有粘贴链接下载按钮',
    );
  });
