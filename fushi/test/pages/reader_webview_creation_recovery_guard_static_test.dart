...
void main() {
  ...
  test('fork in_app_webview 把创建失败合成 sentinel error 经 onReceivedError 转交', () {
    ...
    expect(
      src.contains('kInAppWebViewCreationFailedSentinel ='),
      isTrue,
    ...
    expect(
      src.contains('onCreationError: _onCreationError'),
      isTrue,
    ...
    expect(
      handler.contains('onReceivedError'),
      isTrue,
  ...
  test('reader onReceivedError 命中 sentinel 走 toast + pop 恢复', () {
    ...
    expect(
      block.contains('kReaderWebViewCreationFailedSentinel'),
      isTrue,
    ...
    expect(
      block.contains('if (!mounted) return;'),
      isTrue,
