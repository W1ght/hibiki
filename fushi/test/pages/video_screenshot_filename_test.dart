...
void main() {
  ...
  group('uniqueVideoScreenshotBaseName', () {
    test('已存在同名时追加递增计数后缀（同一视频同一播放秒连点截图靠这层兜唯一）', () {
      ...
      expect(
        uniqueVideoScreenshotBaseName(desired, exists: existing.contains),
  ...
  group('_saveScreenshot 源码守卫', () {
    ...
    test('桌面保存对话框和移动分享临时文件都使用新截图 basename', () {
      ...
      expect(
        body.contains('fileName: screenshotName'),
        isTrue,
        reason: '桌面 save dialog 默认名必须来自新命名 helper',
      );
      expect(
        body.contains('XFile(tmp.path, mimeType: \'image/jpeg\')'),
        isTrue,
      );
      expect(
        body.contains('subject: screenshotName'),
        isTrue,
