...
void main() {
  ...
  Widget wrap(Widget child) => TranslationProvider(
    child: MaterialApp(
      home: Scaffold(body: SizedBox(width: 200, height: 320, child: child)),
    ),
  );
  ...
  testWidgets('multi-cover tiles member covers into a folder grid', (
    ...
    expect(
      find.byKey(const ValueKey<String>('series-folder-cell-0')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('series-folder-cell-1')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('series-folder-cell-2')),
      findsOneWidget,
    );
    expect(
      find.byKey(const ValueKey<String>('series-folder-cell-3')),
      findsNothing,
    );
  });
  ...
  testWidgets('mosaic caps at 4 cells even with more members', (tester) async {
    ...
    expect(
      find.byKey(const ValueKey<String>('series-folder-cell-4')),
      findsNothing,
    );
