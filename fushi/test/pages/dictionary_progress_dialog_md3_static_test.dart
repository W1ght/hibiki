...
void main() {
  test(
    'dictionary progress dialogs use shared MD3 dialog chrome and tokens',
    () {
      ...
      for (final String path in paths) {
        ...
        expect(
          source,
          isNot(contains('package:fushi/src/utils/spacing.dart')),
          reason: path,
        );
        expect(source, isNot(contains('Space.')), reason: path);
        expect(
          source,
          isNot(contains('padding: const EdgeInsets.all(4)')),
          reason: path,
        );
      }
    },
