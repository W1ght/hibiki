...
void main() {
  ...
  test('popup.css gives each furigana base its own positioned .ruby-unit '
      'anchor (BUG-722)', () {
    ...
    expect(
      RegExp(r'position\s*:\s*relative').hasMatch(m!.group(1)!),
      isTrue,
      reason:
          '.ruby-unit must be position:relative so the absolute <rt> '
          'anchors to its own kanji, not the full-width <ruby> (BUG-722)',
    );
  });

  test('postProcessRuby wraps each base in .ruby-unit and moves its <rt> into '
      'the unit (BUG-722)', () {
    expect(
      js.contains('function postProcessRuby('),
      isTrue,
      reason: 'popup.js must define postProcessRuby',
    );
    // Per-base unit is created…
    expect(
      js.contains("className = 'ruby-unit'"),
      isTrue,
      reason:
          'postProcessRuby must wrap each base in a .ruby-unit span so '
    ...
    expect(
      js.contains("rtBox.className = 'ruby-rt'"),
      isTrue,
      reason:
          'postProcessRuby must wrap each reading in a .ruby-rt box — '
          'that span, not the <rt>, carries position:absolute (BUG-1487)',
    );
    expect(
      js.contains('unit.appendChild(rtBox)'),
      isTrue,
      reason:
          'postProcessRuby must append the reading box to the per-base '
          '.ruby-unit — otherwise the reading anchors to the whole <ruby> and '
          'multi-kanji-word readings superimpose (BUG-722)',
    );
    expect(
      js.contains('rtBox.appendChild(sib)'),
      isTrue,
      reason:
