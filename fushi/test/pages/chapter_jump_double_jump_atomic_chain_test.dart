...
void main() {
  ...
  test('_navigateToChapterAndWait 收 progress + preciseLocateJs 并全部转发', () {
    ...
    expect(
      body,
      contains('double progress = 0.0,'),
      reason: '必须能把目标章内分数烘进导航（书签/收藏/字符跳转）',
    );
    expect(
      body,
      contains('String? preciseLocateJs,'),
    ...
    expect(
      body,
      contains('_navigateToChapter(index,'),
      reason: 'progress/charOffset 必须转发给 _navigateToChapter → _beginNavigation',
    );
    expect(
      body,
    ...
    expect(
      body,
      contains('preciseLocateJs: preciseLocateJs'),
  ...
  test('_beginNavigation 在 loadUrl 前清旧 pending 并绑定本次代际', () {
    ...
    expect(
      body,
      contains('_preciseLocateQueue.clear();'),
    ...
    expect(
      body,
      contains('_preciseLocateQueue.replace('),
      reason: '本次定位必须在 loadUrl 前进入 last-write-wins 队列',
    );
    expect(
      body,
      contains('generation: _navigateGeneration,'),
  ...
  test('_applyPendingPreciseLocate 有代际守卫且消费一次', () {
    ...
    expect(
      body,
      contains('_preciseLocateQueue.consume('),
    ...
    expect(
      body,
      contains('canApply: mounted && _controller != null,'),
      reason: 'dispose/控制器释放后必须丢弃 pending，不执行旧 DOM 脚本',
    );
  });

  test('dispose 立即中止导航等待并清除 pending', () {
    final String body = slice('void dispose() {', 'super.dispose();');
    expect(
      body,
      contains('_failNavigation();'),
      reason: 'dispose 必须立即 complete(false) 并清 pending，不能让搜索回调挂到超时',
    );
  });

  test(
    '_onRestoreComplete 在 settle 之后应用 pending（分派互斥：非连续直接 / 连续走 reanchor）',
    () {
      ...
      expect(
        onAfter,
        contains('await _applyPendingPreciseLocate();'),
  ...
  test('onSearchJump 按 logical chapter + DOM ready 分流，跨章排队且同章在飞最后写胜出', () {
    ...
    expect(
      body,
      contains('scrollToSearchMatchInvocation'),
      reason: '搜索定位仍用 scrollToSearchMatch（文本命中）',
    );
    expect(
      body,
      contains('decideReaderSearchJump('),
      reason: '不得只比较 sectionIndex/currentChapter；必须同时判断 DOM ready',
    );
    expect(
      body,
      contains('if (!mounted || _book == null || _controller == null) return;'),
      reason: '搜索弹层回调晚到 dispose 后不得继续排队或求值旧 WebView',
    );
    expect(
      body,
      contains('ReaderSearchJumpAction.replacePending'),
  ...
  test('收藏/字符跨章跳转把分数烘进 _navigateToChapterAndWait(progress:)', () {
    ...
    expect(
      fav,
      contains('_navigateToChapterAndWait('),
      reason: '收藏跨章走 navigate-with-baked-charOffset（原子恢复链）',
    );
    expect(
      fav,
      contains('charOffset: useOffset ? normCharOffset :'),
      reason:
          '收藏把目标绝对字符锚烘进导航（不再 /10000 当分数落章首）；'
          'BUG-876 起 charOffset 条件化(useOffset ? … : null)，收藏缺 offset 回退文本搜索',
    );
    expect(
      fav.contains('progress:'),
      isFalse,
      reason: '收藏跳转不得再把绝对字符索引当分数烘进 progress',
    );
    expect(
      'fushiReader.restoreProgress('.allMatches(fav).length,
      0,
      reason: '收藏同章分支改用 restoreToCharOffset 精确锚，不再 restoreProgress',
    );
    expect(
      'restoreToCharOffset('.allMatches(fav).length,
      1,
    ...
    expect(
      charJump,
      contains('progress: target.progress,'),
