...
void main() {
  ...
  group('子A：音量与倍速使用固定锚点轻浮层（TODO-438）', () {
    ...
    test('音量浮层保留静音按钮和横向滑条，滑条仍走现有音量同步通道', () {
      ...
      expect(
        body.contains('RotatedBox('),
        isFalse,
        reason: '本轮音量浮层保持横向 Slider，不再旋转成竖条',
      );
      expect(
        body.contains('VideoVolumePopoverCard('),
        isTrue,
      ...
      expect(
        body.contains('onChanged: _setVolumeFromSlider'),
        isTrue,
        reason: '浮层滑条拖动继续走 _setVolumeFromSlider',
      );
      expect(
        body.contains('_volumeDisplay'),
        isTrue,
        reason: '浮层显示值应读 _volumeDisplay，与滚轮/键盘/移动竖滑共用同步通道',
      );
      expect(
        overlaySrc.contains('Row('),
        isTrue,
        reason: 'helper 内的可见浮层必须是横向布局',
      );
      expect(
        overlaySrc.contains('videoVolumePopoverFrameKey'),
        isTrue,
        reason: 'helper 必须暴露可见 frame key 供真实尺寸断言量测',
      );
      expect(
        overlaySrc.contains('videoVolumePopoverSliderKey'),
        isTrue,
    ...
    test('音量完整按钮只从底栏左右槽渲染一次，不进入 top bar / side rail', () {
      ...
      expect(
        bottom.contains('_controlLayout.itemsIn(slot)'),
        isTrue,
        reason: '底栏出控件的顺序必须直接取自布局真相源',
      );
      expect(
        bottom.contains('if (item == VideoControlItem.volume)'),
        isTrue,
        reason: '音量按位分派，不得在循环之外追加（会丢掉槽内真实下标）',
      );
      expect(
        bottom.contains('_buildVolumeButton(') && bottom.contains('slot: slot'),
        isTrue,
      );

      final String top = methodBody('Widget _topBarSlotGroup(');
      expect(
        top.contains('_buildVolumeButton'),
        isFalse,
        reason: 'topLeft/topRight 不得渲染音量完整控件',
      );
      expect(
        top.contains('VideoControlItem.volume'),
        isFalse,
      ...
      expect(
        rails.contains('_buildVolumeButton'),
        isFalse,
        reason: 'screenLeft/screenRight rail 不得渲染音量完整控件',
      );
      expect(
        rails.contains('VideoControlItem.volume'),
        isFalse,
    ...
    test('倍速按钮打开紧凑浮层，复用 presets / _setSpeed / 1.0x 复位', () {
      ...
      expect(
        body.contains('_speedMenuPresets()'),
        isTrue,
      ...
      expect(
        body.contains('_setSpeed('),
        isTrue,
        reason: '倍速浮层改速必须走 _setSpeed',
      );
      expect(
        body.contains('_setSpeed(1.0'),
        isTrue,
    ...
    test('可移动 speed 控件在 top bar / side rail / bottom 都有自己的浮层锚点', () {
      expect(
        src.contains('_controlPopoverLinkFor('),
        isTrue,
        reason: '可移动 speed 控件需要按所在 slot 取稳定 LayerLink',
      );
      expect(
        src.contains('LayerLink? popoverLink'),
        isTrue,
      ...
      final String top = methodBody('Widget _topBarSlotGroup(');
      expect(
        top.contains('_controlPopoverLinkFor(slot, item)'),
        isTrue,
        reason: 'top bar speed 来自 _slotChipItems(slot)，必须使用该 slot 的锚点',
      );
      expect(
        top.contains('_controlPopoverAnchor('),
        isTrue,
        reason: 'top bar speed 按钮必须渲染 CompositedTransformTarget',
      );
      expect(
        top.contains('popoverLink: popoverLink'),
        isTrue,
      ...
      expect(
        rail.contains('_controlPopoverLinkFor(slot, item)'),
        isTrue,
        reason: 'side rail speed 来自 _slotChipItems(slot)，必须使用该 slot 的锚点',
      );
      expect(
        rail.contains('_controlPopoverAnchor('),
        isTrue,
        reason: 'side rail speed 按钮必须渲染 CompositedTransformTarget',
      );
      expect(
        rail.contains('popoverLink: popoverLink'),
        isTrue,
      ...
      expect(
        bottom.contains('_controlPopoverLinkFor(slot, VideoControlItem.speed)'),
        isTrue,
  ...
  group('子B：顶栏设置入口去重', () {
    ...
    test('_showPlayerSettings 方法 + rightRail settings 接线保留（设置仍可打开）', () {
      expect(
        src.contains('void _showPlayerSettings('),
        isTrue,
      ...
      final int actEnd = src.indexOf(
        '}',
        src.indexOf('switch (button)', actStart),
      );
      final String actBody = src.substring(actStart, actEnd);
      expect(
        actBody.contains('case VideoControlButton.settings:') &&
            RegExp(
              r'_showPlayerSettings\(\s*(?:sourceSlot:\s*sourceSlot,?\s*)?\)',
            ).hasMatch(actBody),
        isTrue,
        reason:
