...
void main() {
  ...
  List<AniListMedia> makeSeriesMatches(int n) {
    return List<AniListMedia>.generate(
      ...
      (int i) => AniListMedia(
        id: 100 + i,
        romaji:
            'Keikenzumi na Kimi to, Keiken Zero na Ore ga, '
  ...
  testWidgets('regression: series entries live inside a scrollable pane', (
    ...
    // 不再是外层 Column 的固定槽位。
    await pumpDialog(tester, screen: const Size(360, 640));
    final Finder firstSeries = find.textContaining(
      'Otsukiai suru Hanashi 0',
      skipOffstage: false,
    );
  ...
  testWidgets('wide screen: filter pane left, results list right (two-pane)', (
    ...
    expect(
      listRect.left,
      greaterThan(queryRect.right),
      reason: '宽屏下结果列表应在筛选面板右侧（两栏并排）',
    );
    expect(
      listRect.top,
      lessThan(queryRect.bottom),
  ...
  test(
    'BUG-1509: search paints loading frame before persistence and network',
    () {
      final String source =
      ...
      final int paintFrame = search.indexOf(
        'await WidgetsBinding.instance.endOfFrame;',
      );
      final int persistKey = search.indexOf(
  ...
  testWidgets('search button sits in the fixed bottom action bar with cancel', (
    ...
    expect(
      btnRect.center.dy,
      closeTo(cancelRect.center.dy, 1.0),
      reason: '搜索与取消同一行',
    );
    expect(btnRect.left, greaterThan(cancelRect.right), reason: '主操作在取消右侧');
    expect(
      btnRect.bottom,
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      for (final AppLocale locale in AppLocale.values) {
        ...
        expect(
          btn.right,
          lessThanOrEqualTo(dialog.right + 0.5),
