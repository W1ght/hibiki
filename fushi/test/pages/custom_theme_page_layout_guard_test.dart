...
void main() {
  ...
  group('CustomThemePage TODO-072 three-section layout', () {
    test('uses the system-theme section title', () {
      expect(
        source.contains('t.section_system_theme'),
        isTrue,
        reason: '系统主题色板块标题缺失',
      );
    });
    ...
    test('uses the audiobook & lyrics section title', () {
      expect(
        source.contains('t.section_audiobook_lyrics'),
        isTrue,
        reason: '有声书与歌词板块标题缺失',
      );
    });
    ...
    test('keeps the reader-text section title', () {
      expect(
        source.contains('t.section_reader_colors'),
        isTrue,
        reason: '阅读器文字板块标题缺失',
      );
    });
    ...
    test('shows the video-subtitle note line', () {
      expect(
        source.contains('t.video_subtitle_color_note'),
        isTrue,
        reason: '视频字幕颜色说明行缺失',
      );
      expect(
        source.contains('_buildNoteRow('),
        isTrue,
        reason: '说明行未通过 _buildNoteRow 渲染',
      );
    });

    test('sentenceAudioHighlight + selection sit in the audiobook section, '
      ...
      final int sentenceAudioIdx = source.indexOf(
        't.color_sentence_audio_highlight',
      );
  ...
  group('CustomThemePage TODO-928 自定义主题跟随全局明暗 + 折叠种子色', () {
    test('不再引用自定义专属的「深色模式」开关（已删第二明暗真值的 UI）', () {
      ...
      expect(
        source.contains('t.dark_mode'),
        isFalse,
        reason: '自定义页仍引用 dark_mode 文案 → 深色开关未删干净',
      );
      expect(
        source.contains('_setBrightnessMode'),
        isFalse,
    ...
    test('applyCustomTheme 调用不再传 brightnessMode（停写第二真值）', () {
      expect(
        source.contains('brightnessMode: _brightnessMode'),
        isFalse,
    ...
    test('种子色走折叠式 _buildSeedColorPicker（默认收起，防误触）', () {
      expect(
        source.contains('_buildSeedColorPicker('),
        isTrue,
        reason: '种子色应改为折叠选色区，不再裸挂在滚动主路径',
      );
      expect(
        source.contains('bool _seedExpanded = false'),
        isTrue,
        reason: '折叠状态应默认收起',
      );
    });
  ...
  group('CustomThemePage TODO-071 seed preview hint', () {
    test('renders the seed-preview hint through _buildHintRow', () {
      expect(
        source.contains('t.theme_seed_preview_hint'),
        isTrue,
        reason: '种子色预览提示文案缺失',
      );
      expect(
        source.contains('_buildHintRow('),
        isTrue,
    ...
    test(
      'the hint lives in the system-theme section, above the primary toggle',
      () {
