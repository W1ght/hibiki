...
void main() {
  ...
  test('_pokeControlsVisible 在面板打开时早返回', () {
    ...
    expect(
      body.contains('if (_videoSidePanel.value != null) return;'),
      isTrue,
  ...
  test(
    ...
    () {
      ...
      expect(
        flat.contains('final bool gated ='),
        isTrue,
      ...
      expect(
        flat.contains('_subtitleListVisible.value'),
        isFalse,
        reason: 'BUG-371：字幕列表 push-aside 不遮控制条，门控**不应含** _subtitleListVisible',
      );
      expect(
        flat.contains('!gated && _mediaKitControlsVisible.value'),
        isTrue,
  ...
  test('右侧操作 rail 强压制门控含真 overlay 侧栏，但不含字幕列表（BUG-371）', () {
    ...
    expect(
      getterBody.contains('_videoSidePanel.value != null'),
      isTrue,
      reason: 'rail 强压制应含真 overlay 侧栏门控（面板盖控制条则不显示 rail）',
    );
    expect(
      getterBody.contains('_videoControlEditMode.value'),
      isTrue,
      reason: 'rail 强压制应含编辑态门控',
    );
    expect(
      getterBody.contains('_subtitleListVisible.value'),
      isFalse,
      reason: 'BUG-371：字幕列表 push-aside 不遮 rail，强压制**不应含** _subtitleListVisible',
    );
    ...
    final int braceIdx = body.indexOf(
      'return const SizedBox.shrink();',
      gateIdx,
    );
    expect(braceIdx, greaterThan(gateIdx), reason: 'gate 块应闭合到 shrink');
    expect(
      body.contains('if (_immersiveLocked.value)'),
      isTrue,
  ...
  test('_showVideoSidePanel 不再唤起控制条、_hideVideoSidePanel 关闭后唤回', () {
    ...
    expect(
      show.contains('_pokeControlsVisible();'),
      isFalse,
      reason: '_showVideoSidePanel 打开面板时不应再 _pokeControlsVisible（否则点亮背景控制条）',
    );
    expect(
      show.contains('_markControlsVisible(false);'),
      isTrue,
    ...
    expect(
      hide.contains('_pokeControlsVisible();'),
      isTrue,
