...
SettingsDestination _noticeOnlyDestination() {
  ...
  expect(
    noticeItems,
    hasLength(1),
    reason: 'image-quality group must own exactly one Windows black-flash row',
  );
  return SettingsDestination(
    id: video.id,
    title: video.title,
    icon: video.icon,
    sections: <SettingsSection>[SettingsSection(items: noticeItems)],
  );
}
...
void main() {
  testWidgets(
    ...
    'on Windows host through the real renderer',
    (WidgetTester tester) async {
      ...
      expect(
        find.text(t.video_windows_black_flash_notice_title),
        findsOneWidget,
        reason: 'notice title must render on a Windows host',
      );
      expect(
        find.text(t.video_windows_black_flash_notice_body),
        findsOneWidget,
