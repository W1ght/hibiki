...
void main() {
  group('resolveBackend 按平台能力规约（BUG-1207 根因层）', () {
    ...
    test('auto 的既有行为不变（有引擎 embedded / 无引擎 qb）', () {
      ...
      expect(
        auto.resolveBackend(embeddedSupported: true),
        QbConnectionConfig.backendEmbedded,
      );
      expect(
        auto.resolveBackend(embeddedSupported: false),
        QbConnectionConfig.backendQbittorrent,
      );
    });
  ...
  testWidgets('无内置引擎平台：后端选择器与下载目录整块不渲染，落到外接 qb 分支', (
    ...
    expect(
      find.byType(FushiSegmentedStrip<String>),
      findsNothing,
      reason: '本平台没有第二个可用后端，选择器必须整块不渲染',
    );
    expect(
      find.text(t.video_setting_torrent_backend_embedded),
      findsNothing,
    ...
    expect(
      find.text(t.download_save_root_title),
      findsNothing,
    ...
    expect(
      find.text(t.video_setting_qb_url),
      findsOneWidget,
