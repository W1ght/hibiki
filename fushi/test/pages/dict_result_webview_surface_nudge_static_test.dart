...
void main() {
  ...
  test(
    ...
    () {
      ...
      expect(
        src.contains('final bool nudgeSurfaceOnRender;'),
        isTrue,
        reason: 'the widget must expose a nudgeSurfaceOnRender flag',
      );
      expect(
        src.contains('this.nudgeSurfaceOnRender = false,'),
        isTrue,
  ...
  test(
    'popupRendered fires the surface nudge only when opted-in, Windows-only',
    () {
      final src = read(
        'lib/src/pages/implementations/dictionary_popup_webview.dart',
      );
      ...
      final int nudgeCallIdx = src.indexOf(
        'if (widget.nudgeSurfaceOnRender) _nudgeSurfaceRepaint();',
      );
      expect(
        nudgeCallIdx,
        greaterThan(renderedCallIdx),
        reason: 'the nudge must fire right after onRendered, gated by the flag',
      );
      ...
      final int windowsGateIdx = src.indexOf(
      ...
      expect(
        src.contains('_surfaceRepaintNudgeJs'),
        isTrue,
        reason: 'the nudge must evaluate the repaint JS',
      );
      expect(
        src.contains("el.style.opacity = '0.9999';"),
        isTrue,
  ...
  test('home dictionary result WebView opts into the surface nudge', () {
    ...
    final int nudgeIdx = src.indexOf(
      'nudgeSurfaceOnRender: true,',
      resultKeyIdx,
    );
    expect(
      nudgeIdx,
      greaterThan(resultKeyIdx),
