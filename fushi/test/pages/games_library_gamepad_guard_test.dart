...
void main() {
  ...
  test('游戏卡挂了 X=详情 且非 X 按钮显式转发给祖先', () {
    ...
    expect(
      code.contains('GamepadButtonForwardingAction('),
      isTrue,
    ...
    expect(
      code.contains('ancestorContext: context'),
      isTrue,
      reason: '必须传本层 Actions 之上的 context，否则转发原地自我循环',
    );
    expect(
      code.contains('bool isEnabled(GamepadButtonIntent intent)'),
      isFalse,
    ...
    expect(
      code.contains('button != GamepadButton.x'),
      isTrue,
      reason: 'X 是详情键的唯一判据',
    );
    expect(
      code.contains('GamepadButton.y'),
      isFalse,
