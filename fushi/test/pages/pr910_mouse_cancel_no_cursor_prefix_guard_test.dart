...
void main() {
  ...
  test('规划器把坐标建模成可空：没有可信坐标就不下发 setCursorPos', () {
    ...
    expect(
      src.contains('required Offset? position,'),
      isTrue,
      reason:
          '坐标必须可空——cancel 事件根本没有可信坐标，用 Offset.zero 冒充'
          '会让 native 把光标挪到 (0,0)',
    );
    // 断言的关键字面量：
  ...
  test('onPointerCancel 只走按钮差分，坐标传 null', () {
    ...
    expect(
      cancelAnchor.allMatches(src).length,
      1,
      reason: 'onPointerCancel 锚点必须唯一，否则窗口会被同形 token 抢走',
    );
    expect(
      moveAnchor.allMatches(src).length,
      1,
    ...
    expect(
      cancelBody.contains('_syncMouseInput(null, ev.buttons);'),
      isTrue,
      reason:
          'cancel 必须以 null 坐标补发 up，沿用 native 最后一次有效的 '
          'lastCursorPos_',
    );
    // 断言的关键字面量（反向）：'_syncMouseInput(ev.localPosition'
    expect(
      cancelBody.contains('_syncMouseInput(ev.localPosition'),
      isFalse,
      reason:
          'BUG-1419 回归：cancel 的 ev.localPosition 是 Offset.zero，'
          '前置它会发出「带左键按下的 MOVE 到 (0,0)」→ 选区被刷到文档左上角',
    );
  });

  test('带真实坐标的四个入口不受影响，down 仍先落坐标（BUG-1652 不回退）', () {
    final int positional = '_syncMouseInput(ev.localPosition, ev.buttons)'
        .allMatches(src)
        .length;
    expect(
      positional,
      ...
      reason:
          'hover / down / up / move 四个入口都必须继续带坐标同步，'
          '实际只有 $positional 处',
    );
  });
}
