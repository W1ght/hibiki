...
Widget _stubSettings(BuildContext _, Widget navigation) =>
    Column(children: <Widget>[navigation, const Text('game-settings')]);

Widget _testMonitorWithSections(BuildContext _, VoidCallback onShowLibrary) {
...
void main() {
  ...
  testWidgets('800x600 top tabs replace diagnostics with settings', (
    ...
    final Finder gameSections = find.byType(
      FushiAdjustableSegmented<GameSection>,
    );
    ...
    expect(
      find.text(t.settings),
      findsOneWidget,
      reason: '800px 页头必须完整呈现首页、游戏库、工作台、设置四个分段',
    );
    expect(
      controller.requestById(const FushiFocusId('game-library-tab-sections')),
      isTrue,
    );
    await _settleOnLibrary(tester);
    expect(
    ...
    expect(
      controller.requestById(const FushiFocusId('game-discover-tab-sections')),
      isTrue,
    ...
    expect(
      controller.requestById(const FushiFocusId('game-capture-tab-sections')),
      isTrue,
    ...
    expect(
      controller.requestById(const FushiFocusId('game-import-tab-sections')),
      isTrue,
