...
void main() {
  group('BUG-931 视频收藏 OSD / 无进度条守卫', () {
    test(
      '收藏快捷键不再唤起控制条进度条（_toggleFavoriteCurrentCue 无 _pokeControlsVisible）',
      () {
        ...
        expect(
          ...
          isFalse,
          reason:
              'BUG-931：收藏不得调 `_pokeControlsVisible()`，否则会把底栏进度条弹出来'
        ...
        expect(
          compactCode(
            body,
          ).contains(compactCode('_showOsd(t.no_sentence_selected,')),
          isTrue,
    ...
    test('视频页所有短提示走左上角 OSD，全域无 FushiToast.show（底部 toast）', () {
      ...
      expect(
        ...
        isFalse,
        reason:
            'BUG-931：视频页短提示统一走左上角 `_showOsd(...)`；出现 '
            '`FushiToast.show` 说明有提示回退到了屏幕底部。',
      );
      // 收藏加/移除确实用 OSD（防止有人把提示整个删掉来「绕过」守卫）。
      expect(
        compactCode(src).contains(compactCode('_showOsd(t.favorite_added,')),
        isTrue,
        reason: '收藏成功应走左上角 `_showOsd(t.favorite_added ...)`。',
      );
      expect(
        compactCode(src).contains(compactCode('_showOsd(t.favorite_removed,')),
        isTrue,
