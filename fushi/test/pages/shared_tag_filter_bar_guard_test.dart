...
void main() {
  test('共享标签栏组件存在且用 MD3 chip + 拖拽重排 + 管理动作', () {
    ...
    expect(
      src.contains('LongPressDraggable<BookTagRow>'),
      isTrue,
      reason: '长按拖拽重排',
    );
  ...
  test('视频 tab 标签栏改用共享 FushiTagFilterBar（与书架同一组件）', () {
    final String video = _read(
      'lib/src/pages/implementations/home_video_page.dart',
    );
    expect(
      video.contains('FushiTagFilterBar('),
      isTrue,
      reason: '视频标签栏要和书架用同一组件',
    );
    // 旧实现：筛选图标打开 TagFilterSheet 的 modal。
    expect(
      video.contains('builder: (_) => const TagFilterSheet()'),
      isFalse,
  ...
  test('视频 tab 启用批量选择（标签栏旁的「选择」+ 批量打标签/删除）', () {
    ...
    expect(
      video.contains('selectionMode: _selectionMode'),
      isTrue,
      reason: 'TODO-063：视频标签栏旁要有「选择」按钮，缺它就是用户报的「视频少了选择」',
    );
    expect(
      video.contains('onToggleSelectionMode: _toggleSelectionMode'),
      isTrue,
    ...
    expect(
      video.contains('addTagToVideoBook'),
      isTrue,
