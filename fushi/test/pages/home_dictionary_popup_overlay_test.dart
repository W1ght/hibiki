...
void main() {
  ...
  test(
    ...
    () {
      ...
      expect(
        page.contains('rootOverlay: true'),
        isTrue,
        reason: 'home popup stack must mount in the root Overlay',
      );
      expect(
        page.contains('OverlayEntry'),
        isTrue,
      ...
      expect(
        page.contains('clipBehavior: Clip.none'),
        isTrue,
      ...
      expect(
        ...
        isTrue,
        reason:
            'popup screen must be the whole window (root Overlay), not the '
            'result sub-area',
      );

      expect(
        page.contains('_overlayInert') || page.contains('overlayInert'),
        isTrue,
        reason: 'must guard the root-Overlay rebuild during deactivate',
      );
      expect(
        page.contains('void deactivate()'),
        isTrue,
        reason: 'deactivate must mark the overlay inert',
      );

      expect(
        page.contains('popupWordScreenRect('),
        isTrue,
        reason:
            'top-level result-WebView lookup maps localRect to screen coords',
      );
      expect(
        page.contains('globalCoordinates: true'),
        isTrue,
  ...
  test('dictionary_page_mixin still positions nested popups via '
      'popupWordScreenRect (screen-space) keeps its contract', () {
    final String mixin = File(
      'lib/src/pages/implementations/dictionary_page_mixin.dart',
    ).readAsStringSync();
    expect(
      mixin.contains('popupWordScreenRect('),
      isTrue,
