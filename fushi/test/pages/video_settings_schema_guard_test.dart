...
void main() {
  ...
  test('TODO-186: global settings has a dedicated video destination', () {
    final String destinationSrc = File(
    ...
    expect(
      destinationSrc.contains('video,'),
      isTrue,
      reason: 'SettingsDestinationId must include a dedicated video tab',
    );
    expect(
      schemaSrc.contains('buildVideoDestination(),'),
      isTrue,
      reason:
  ...
  test('阶段B: every VideoGroup projects a non-empty ordered set', () {
    ...
    for (final VideoGroup group in VideoGroup.values) {
      ...
      final List<int> orders = items
          .map((SettingsItem i) => i.video!.order)
          .toList();
  ...
  test(
    '阶段B: controller-bound rows are host-gated (hidden on the home page)',
    () {
  ...
  test('审查 Finding 9: sheet categories are driven by VideoGroup.values '
    ...
    for (final VideoGroup group in VideoGroup.values) {
      expect(
        sheetSrc.contains('case VideoGroup.${group.name}:'),
        isTrue,
  ...
  test('TODO-286: home video settings stay pref-only (no live controller)', () {
    ...
    for (final String forbidden in <String>[
      ...
      expect(
        videoSrc.contains(forbidden),
        isFalse,
        reason:
            'settings_schema_video.dart must not depend on a live player '
            '($forbidden found)',
      );
    }
  ...
  test(
    ...
    () {
      ...
      expect(
        videoSrc.contains("id: 'video.quality.windows_black_flash_notice'"),
        isTrue,
        reason: 'video quality group must own the black-flicker notice item',
      );
      expect(
        videoSrc.contains('isWindowsPlatform'),
        isTrue,
        reason: 'black-flicker notice must be gated to Windows only',
      );
      expect(
        videoSrc.contains('_buildWindowsBlackFlashNotice'),
        isTrue,
      ...
      expect(
        videoSrc.contains('t.video_windows_black_flash_notice_title'),
        isTrue,
      );
      expect(
        videoSrc.contains('t.video_windows_black_flash_notice_body'),
        isTrue,
      );
      expect(
        videoSrc.contains('Icons.info_outline'),
        isTrue,
        reason: 'notice should render as an informational row',
      );
      expect(
        stringsSrc.contains('"video_windows_black_flash_notice_title"'),
        isTrue,
      );
      expect(
        stringsSrc.contains('"video_windows_black_flash_notice_body"'),
        isTrue,
      );
    },
  );
