...
void main() {
  ...
  test('settings speed slider has a live preview callback before commit', () {
    ...
    expect(
      speedRow.contains('onPreviewSpeed(snapVideoSpeed(v))'),
      isTrue,
      reason:
          'onChanged must preview snapped speed without waiting for DB commit',
    );
    expect(
      speedRow.contains('onSetSpeed(snapVideoSpeed(v))'),
      isTrue,
    ...
    expect(
      commitIdx,
      greaterThan(previewIdx),
      reason: 'preview (onChanged) precedes commit (onChangeEnd)',
    );
  });

  test(
    'video page wires speed preview as non-persistent and commit as durable',
    () {
  ...
  test(
    '_setSpeed updates UI before controller and only debounces persistence',
    () {
      ...
      expect(
        body.contains('final bool changed ='),
        isTrue,
        reason:
      ...
      expect(
        body.contains('bool rebuild = true'),
        isTrue,
  ...
  test(
    'speed persistence debounce is flushed on lifecycle, exit and dispose',
    () {
      expect(pageSrc.contains('double? _pendingSpeedPersist'), isTrue);
      expect(pageSrc.contains('Timer? _speedPersistDebounce'), isTrue);
      expect(
        pageSrc.contains('void _queuePersistVideoSpeed(double speed)'),
        isTrue,
      );
      expect(
        pageSrc.contains('Future<void> _flushPersistedVideoSpeed() async'),
        isTrue,
      );
  ...
  test(
    'long-press temporary speed is non-persistent and skips full rebuild',
    () {
      ...
      expect(
        longPress.contains(
          '_setSpeed(snapped, persist: false, rebuild: false)',
        ),
        isTrue,
