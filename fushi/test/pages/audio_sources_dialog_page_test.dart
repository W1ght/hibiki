...
void main() {
  ...
  test('isValidRemoteUrl enforces http(s) + a term/reading placeholder', () {
    ...
    expect(
      AudioSourcesDialog.isValidRemoteUrl('http://x.com/{reading}'),
      isTrue,
    );
    // 无占位符
  ...
  testWidgets(
    'long-press drag reorders rows even under 0.5 UI scale (BUG-027)',
    (WidgetTester tester) async {
      List<AudioSourceConfig>? saved;
  ...
  // label / enabled / 位置。
  testWidgets('editing a remote row rewrites that row in place, keeping its '
    ...
    await tester.enterText(
      find.byType(TextField),
      'http://192.168.1.9:5050/?term={term}',
    );
    await tester.pump();
  ...
  testWidgets('reordering while editing still writes to the edited row, not '
  ...
  testWidgets('the edit button sits left of the switch and keeps the switch '
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        find.textContaining(t.audio_source_loopback_warning),
        findsOneWidget,
      );
    },
  );
