...
void main() {
  ...
  SettingsSegmentedItem<VideoImmersiveMode> immersiveItem() {
    final SettingsItem item = buildVideoDestination().sections
  ...
  test('TODO-174: four persisted immersive modes exist and default to '
      'shortcut-and-lookup', () {
    ...
    // shortcutAndLookup。
    expect(
      modeSrc.contains("shortcutAndLookup('seek_and_lookup')"),
      isTrue,
      reason:
    ...
    expect(
      prefsSrc.contains("'video_immersive_mode'"),
      isTrue,
      reason: 'immersive mode must persist in preferences',
    );
    expect(
      appModelSrc.contains('VideoImmersiveMode get videoImmersiveMode'),
      isTrue,
  ...
  test('TODO-174: shortcut-and-lookup blocks touch double-tap seek, no mode '
    ...
    final String doubleTapGetter = pageSrc.substring(
      dtStart,
      pageSrc.indexOf(';', dtStart) + 1,
    );
    expect(
      doubleTapGetter.contains('VideoImmersiveMode.shortcutAndLookup'),
      isFalse,
      reason:
          'double-tap seek gate must NOT admit shortcut-and-lookup (touch seek off)',
    );
    expect(
      doubleTapGetter.contains('VideoImmersiveMode.full'),
      isTrue,
    ...
    final int seekReturnIdx = body.indexOf(
    ...
    expect(
      body.contains('VideoImmersiveMode.'),
      isFalse,
      reason:
    ...
    expect(
      platformBranch,
      greaterThan(seekReturnIdx),
      reason: 'platform pause/fullscreen fallback follows the seek attempt',
    );
  });

  test(
    'TODO-174: locked context menu is available only to full-control mode',
    () {
      ...
      final int fullControlsGate = handleBody.indexOf(
      ...
      final int delegateIdx = handleBody.indexOf(
        '_showVideoContextMenu(globalPosition);',
      );
      expect(
        delegateIdx,
        greaterThan(fullControlsGate),
        reason: 'full-control gate must run before scheduling the context menu',
      );
      expect(
        showBody.contains('showMenu<VoidCallback>('),
        isTrue,
        reason: 'the gated delegate must remain the method that opens the menu',
      );
    },
  );
  ...
  test('TODO-174: video settings schema exposes the four-mode selector', () {
    ...
    expect(
      item.options.length,
      VideoImmersiveMode.values.length,
    ...
    expect(
      schemaSrc.contains('setVideoImmersiveModeDual('),
      isTrue,
    ...
    expect(
      hostSrc.contains('onImmersiveModeChanged'),
      isTrue,
  ...
  test('TODO-209: immersive selector is a dropdown picker, not a 4-segment strip '
    ...
    final int picker = widgetsSrc.indexOf(
    ...
    expect(
      widgetsSrc.contains('AdaptiveSettingsPickerOption<T>'),
      isTrue,
