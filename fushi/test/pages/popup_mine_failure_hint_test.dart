...
void main() {
  ...
  test('popup.js 解析 reply.message 并在制卡失败时就地提示（BUG-1908）', () {
    expect(
      popupJs.contains('reply.message'),
      isTrue,
    ...
    expect(
      injection.contains('window.i18nMineFailed'),
      isTrue,
  ...
  test('Dart 三条回程都能带失败原因（BUG-1908）', () {
    ...
    expect(
      coordinator.contains('toPopupReply({String? message})'),
      isTrue,
    ...
    expect(
      controller.contains('toPopupReply(message:'),
      isTrue,
      reason:
          '浮窗控制器必须把已本地化的失败文案回给浮窗，'
    ...
    expect(
      bridge.contains("'message': message"),
      isTrue,
      reason: '裸浮窗（非 gal）的制卡/覆写回程同样要带原因',
    );
    expect(
      bridge.contains("'message': t.card_export_failed"),
      isTrue,
  ...
  test('popup.js 三镜像同步（BUG-1908 改动不得只落一份）', () {
    final String vendorApp = File(
      'assets/browser_extension/vendor/popup.js',
    ).readAsStringSync();
    final String vendorRepo = File(
  ...
  test('覆写（✓⤺）失败必须说出原因，且不把卡说没（BUG-1908）', () {
    ...
    expect(
      overwrite.contains('!result.ankiConnect'),
      isTrue,
      reason: 'ankiConnect 才是覆写的真成败位；无条件画 ✓ 等于把失败画成成功',
    );
    expect(
      overwrite.contains('showInlineHint('),
      isTrue,
      reason:
          '覆写失败必须走页内提示车道——gal 浮窗是独立 native 窗口，'
          '宿主的 Flutter toast 在游戏全屏时用户一个也看不见',
    );
    expect(
      overwrite.contains('result.message'),
      isTrue,
      reason:
          '宿主为覆写失败专门算了本地化文案'
  ...
  test('制卡失败的按钮态只能来自宿主的确定答复，不许硬猜也不许回查（BUG-1908/TODO-448）', () {
    ...
    expect(
      failure.contains('setMineState(result.duplicate === true)'),
      isTrue,
      reason:
          '失败态必须照宿主给的 duplicate 位画：'
          'MineResult.duplicate 走的正是 success:false，此时 Anki 里确定有卡，'
          '硬写 setMineState(false) 会画成可制卡 ＋ 并把 ↗ 入口藏起来',
    );
    expect(
      failure.contains("callHandler('duplicateCheck'"),
      isFalse,
      reason:
          'TODO-448：失败/不确定后绝不回查 Anki 再把按钮翻成 ✓'
    ...
    expect(
      popupJs.contains('duplicate: reply.duplicate === true'),
      isTrue,
  ...
  test('桥自身 reject 时不得静默（BUG-1908）', () {
    ...
    expect(
      catchBlock.contains('showInlineHint('),
      isTrue,
      reason:
          '只写 console.error 对用户等于「点了没反应」；'
  ...
  test('已存在卡的操作单回程也要带出宿主的原因（BUG-1908）', () {
    ...
    expect(
      handled.contains('action.result.message'),
      isTrue,
      reason: '覆写/新增重复卡失败时宿主的原因不能在这条路上被丢掉',
    );
    expect(
      handled.contains('showInlineHint('),
      isTrue,
  ...
  test('Dart 侧把「重复」与「真没制成」分开回传（BUG-1908）', () {
    ...
    expect(
      coordinator.contains('bool get duplicate'),
      isTrue,
      reason: 'gal 回程必须能区分 MineResult.duplicate',
    );
    expect(
      coordinator.contains("if (duplicate) 'duplicate': true"),
      isTrue,
    ...
    expect(
      popupResult.contains('this.duplicate = false'),
      isTrue,
      reason: 'MinePopupResult 必须带 duplicate 位',
    );
    expect(
      popupResult.contains("if (duplicate) 'duplicate': true"),
      isTrue,
