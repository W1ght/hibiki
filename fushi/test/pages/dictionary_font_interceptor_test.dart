...
void main() {
  ...
  Future<WebResourceResponse?> serve(String url, {Set<String>? whitelist}) =>
      dictionaryFontWebResourceResponse(
        Uri.parse(url),
        allowedRoots: <String>[allowed.path],
        whitelistedPaths: whitelist ?? <String>{p.canonicalize(goodFont.path)},
      );
  ...
  test('非字体 URL 返回 null（原有 image:// / dictmedia:// 分支不受影响）', () async {
    ...
    expect(
      await serve('https://fushi.local/fonts/whatever'),
      isNull,
  ...
  test('白名单内 + 魔数合法 → 200，带 CORS 头与字节', () async {
    final WebResourceResponse? r = await serve(
      dictionaryFontUrl(goodFont.path),
    );
    expect(r, isNotNull);
    ...
    expect(
      ...
      '*',
      reason:
          '字体是强制 CORS 模式的子资源，弹窗文档与 fushi.local 从不同源，'
  ...
  test('URL 带内容版本戳，且版本戳不影响路径解析', () async {
    ...
    await goodFont.writeAsBytes(
      Uint8List.fromList(<int>[...ttfBytes(), ...List<int>.filled(32, 0)]),
    );
  ...
  test('isDictionaryFontUrl 只认本前缀（拦截器靠它做廉价前缀判定）', () {
    expect(
      isDictionaryFontUrl(Uri.parse(dictionaryFontUrl(goodFont.path))),
      isTrue,
    );
    expect(isDictionaryFontUrl(Uri.parse('image://?dictionary=X')), isFalse);
    expect(
      isDictionaryFontUrl(Uri.parse('https://fushi.local/fonts/x')),
      isFalse,
    );
  });
  ...
  test('.ttc 在 URL 模式下的 Content-Type 与内联模式一致（font/collection）', () async {
    ...
    await ttc.writeAsBytes(
      Uint8List.fromList(<int>[0x74, 0x74, 0x63, 0x66, 0x00, 0x01, 0x00, 0x00]),
    );
    final WebResourceResponse? r = await serve(
    ...
    expect(
      ...
      'font/collection',
      reason:
          '内联模式用 DictionaryFontCss._fontTypes 的 font/collection；'
  ...
  group('configuredDictionaryFontPaths（生产取值路径）', () {
    ...
    test('readerSettings 为 null 但 DB 就绪时白名单非空（app 外查词字体不能失效）', () async {
    ...
    test('停用的字体不进白名单（可读集合收敛到最小）', () {
      ...
      expect(
        whitelist,
        isNot(contains(p.canonicalize(disabledFont.path))),
        reason:
            '产 CSS 的 DictionaryFontCss.build 过滤了 enabled==false；'
            '白名单不过滤的话，被用户停用的字体文件仍可经 /dictfonts/ 读出',
      );
    });

    test('注入侧产的每条字体 URL，拦截器都必须放行（两侧同源的端到端断言）', () async {
      ...
      final List<String> urls = RegExp(
        r'url\("([^"]+)"\)',
      ).allMatches(css.fontFaces).map((RegExpMatch m) => m.group(1)!).toList();
      expect(urls, isNotEmpty, reason: '注入侧必须真的产出了 /dictfonts/ URL，否则这条断言是空转');
