...
void main() {
  ...
  test('popup engine holder runs popupMain on a cached engine', () {
    ...
    expect(
      ...
      lessThan(executeIdx),
      reason:
          'handler 必须在 executeDartEntrypoint 之前注册，否则 Dart '
  ...
  test('popup flutter activity is transparent, cached-engine, pushes text', () {
    ...
    expect(
      src,
      contains('PopupEngineHolder.setOnFinish(this)'),
      reason: '注册必须带 owner，holder 才知道该不该让某个实例注销',
    );
    expect(
      src,
      contains('PopupEngineHolder.clearOnFinish(this)'),
      reason: '注销必须带 owner——旧 Activity 不能清掉新窗刚注册的回调',
    );
    expect(
      src,
      isNot(contains('setOnFinish(null)')),
  ...
  test('floating dict registrant registers popup-render runtime plugins', () {
    ...
    expect(
      src,
      contains('io.flutter.plugins.urllauncher.UrlLauncherPlugin'),
