...
void main() {
  ...
  group('TODO-951 symptom C: no ValueKey rebuild + warm slot', () {
    ...
    test('popup page seeds a warm slot and keeps its WebView warm', () {
      ...
      expect(
        src,
        contains('keepWebViewWarm: entry.isWarmSlot'),
        reason: '热槽 WebView 全程挂载预热',
      );
      expect(
        src,
        contains('reuseWarmSlot: reuseWarmSlot'),
        reason: '顶层查词复用热槽原地查新词',
      );
      expect(
        src,
        contains('void didUpdateWidget(PopupDictionaryPage'),
        reason: '宿主改 searchTerm/generation 时 didUpdateWidget 复用热槽重查',
      );
      // 顶层重查保留热槽（不 clear 掉热 WebView）。
      expect(
        src,
        contains('_popup.pruneToWarmSlot'),
  ...
  group('TODO-951 symptom B: close decoupled from swipe', () {
    test(
      'search bar no longer carries the close button (moved out of swipe)',
      () {
        final String src = popupSrc();
        // 关闭 X 由 _buildCloseButton 在 swipe wrapper 之外独立渲染。
        expect(
          src,
          contains('Widget _buildCloseButton()'),
          reason: '独立关闭按钮入口存在',
        );
        expect(
          src,
          contains('onClose: null'),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final Iterable<DictionaryPopupLayer> layers = tester
          .widgetList<DictionaryPopupLayer>(find.byType(DictionaryPopupLayer));
      ...
      expect(
        layers.every((DictionaryPopupLayer l) => l.keepWebViewWarm == false),
        isTrue,
  ...
  group('TODO-1336: warm reuse resets the closing latch', () {
    test(
      'didUpdateWidget resets _isClosing so a reused warm page can reclose',
      () {
        final String src = popupSrc();
        ...
        final String body = endIdx > idx
            ? src.substring(idx, endIdx)
            : src.substring(idx);
        expect(
          body,
          contains('_isClosing = false'),
