...
void main() {
  test('collections_page 句子/制卡跳转走 charAnchor + preserveSavedPosition（非分数）', () {
    ...
    expect(
      src,
      contains('item.type == _CollectionType.sentence ||'),
      reason: '必须区分句子/制卡（绝对字符锚）与书签（0-10000 分数）',
    );
    expect(
      src,
      contains('charAnchor: isSentenceJump ? item.normCharOffset'),
      reason: '句子/制卡的 normCharOffset 是绝对字符锚，须经 charAnchor 透传',
    );
    expect(
      src,
      contains('preserveSavedPosition: isSentenceJump'),
  ...
  test('reader 跳转分支：charAnchor 走精确字符锚恢复、preserve 时抑制持久化', () {
    ...
    expect(
      src,
      contains('final int? charAnchor = bm.charAnchor;'),
      reason: '读取跳转携带的绝对字符锚',
    );
    expect(
      src,
      contains('_initialCharOffset = charAnchor;'),
      reason: 'charAnchor 非负时设精确字符锚 → restoreToCharOffset 精确恢复',
    );
    expect(
      src,
      contains('_suppressPositionPersist = bm.preserveSavedPosition;'),
      reason: '临时浏览跳转据 preserveSavedPosition 置位抑制标记',
    );
    // BUG-162 分数兜底路径仍在（真实书签 charAnchor==null）。
    expect(
      src,
      contains('_initialProgress = bm.normCharOffset / 10000.0;'),
