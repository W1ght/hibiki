...
void main() {
  group('kanji card data is wired through the popup injection path', () {
    ...
    test('serializes kanjiResults onto window.kanjiResults', () {
      ...
      expect(
        ...
        reason:
            'kanji results must come from the SAME DictionarySearchResult '
      ...
      expect(
        ...
        reason:
            'kanji results must be serialized via the typed '
      ...
      expect(
        ...
        reason:
            'the kanji payload must be injected into the WebView next to '
            'window.lookupEntries in the same evaluateJavascript call.',
      );
    });

    test(
      'window.kanjiResults injection sits in the same push as lookupEntries',
      () {
        final int entriesIdx = injector.indexOf(
          r'window.lookupEntries = $entriesJson',
        );
        final int kanjiIdx = injector.indexOf(
          r'window.kanjiResults = $kanjiResultsJson',
        );
        expect(entriesIdx, greaterThanOrEqualTo(0));
        expect(
          kanjiIdx,
          greaterThan(entriesIdx),
          reason:
              'kanji results ride alongside the term entries in the same '
  ...
  group('popup.js renders the kanji card from window.kanjiResults', () {
    ...
    test(
      'renderPopup builds the kanji card and tolerates kanji-only results',
      () {
        final int renderIdx = popupJs.indexOf(
          'window.renderPopup = function()',
        );
        expect(renderIdx, greaterThanOrEqualTo(0));
        final String renderBody = popupJs.substring(renderIdx);
        expect(
          renderBody.contains('buildKanjiCards()'),
          isTrue,
        ...
        expect(
          ...
          isTrue,
          reason:
              'a kanji-only lookup (no term entries) must still render the '
    ...
    test('an empty kanji payload renders nothing', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'a missing / empty kanji array must produce no card, leaving '
