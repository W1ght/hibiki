...
void main() {
  test('controller exposes the pending-reveal state machine', () {
    ...
    expect(
      ctrl.contains('bool revealOnRender'),
      isTrue,
      reason: '条目需带「等渲染才显示」挂起标记',
    );
    expect(
      ctrl.contains('void markPendingReveal('),
      isTrue,
      reason: '冷层挂起 API',
    );
    expect(
      ctrl.contains('bool revealRendered('),
      isTrue,
      reason: '渲染完成翻可见 API',
    );
  });
  ...
  test('reader (base_source_page) gates a cold nested popup on render', () {
    ...
    expect(
      base.contains('revealImmediately && needsWebViewRender'),
      isTrue,
      reason: '复用 warm slot 但有可渲染内容时也要等 popupRendered',
    );
    expect(
      base.contains('result.kanjiResults.isNotEmpty'),
      isTrue,
  ...
  test(
    'video/home (dictionary_page_mixin) gates a cold nested popup on render',
    () {
      final String mixin = File(
        'lib/src/pages/implementations/dictionary_page_mixin.dart',
      ).readAsStringSync();
      expect(
        mixin.contains('controller.markPendingReveal('),
        isTrue,
        reason: '冷嵌套层就绪后挂起，不立即 show',
      );
      expect(
        mixin.contains('controller.revealRendered(entry)'),
        isTrue,
        reason: 'onRendered 翻可见挂起层',
      );
      expect(
        mixin.contains('final bool needsWebViewRender ='),
        isTrue,
        reason: 'mixin 必须先判断结果是否需要 WebView 渲染',
      );
      expect(
        mixin.contains('result.kanjiResults.isNotEmpty'),
        isTrue,
        reason: 'kanji-only 结果也需要 WebView 渲染',
      );
      expect(
        mixin.contains('if (!needsWebViewRender) {'),
        isTrue,
  ...
  test('popup webview surfaces a load-error reveal signal (onRenderError)', () {
    ...
    expect(
      wv.contains('widget.onRenderError?.call()'),
      isTrue,
      reason: '加载失败通知宿主翻可见',
    );
  });
  ...
  test('reader/mixin wire onRenderError to the same reveal path', () {
    ...
    expect(
      base.contains('onRenderError:'),
      isTrue,
      reason: 'reader 把加载失败接到翻可见入口',
    );
    expect(
      base.contains('_popup.dispose()'),
      isTrue,
    ...
    expect(
      mixin.contains('onRenderError:'),
      isTrue,
      reason: 'mixin 把加载失败接到翻可见入口',
    );
    expect(
      mixin.contains('onForcedReveal:'),
      isTrue,
