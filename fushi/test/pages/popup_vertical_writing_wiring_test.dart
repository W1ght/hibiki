...
void main() {
  ...
  test('calcPopupPosition has a verticalWriting branch', () {
    ...
    expect(
      RegExp(r'bool\s+verticalWriting').hasMatch(src),
      isTrue,
  ...
  test(
    'base_source_page exposes popupVerticalWriting and threads it per layer',
    () {
      ...
      expect(
        RegExp(
          r'_layerVerticalWriting\(int index\)\s*=>\s*index == 0 && '
      ...
      expect(
        src.contains('verticalWriting: _layerVerticalWriting('),
        isTrue,
        reason: '_calculatePopupPosition 必须按层级而非无脑透传 popupVerticalWriting',
      );
      expect(
        src.contains('verticalWriting: popupVerticalWriting'),
        isFalse,
  ...
  test('reader overrides popupVerticalWriting from writingMode', () {
    ...
    expect(
      RegExp(r'popupVerticalWriting\s*=>[\s\S]{0,80}writingMode').hasMatch(src),
      isTrue,
    ...
    expect(
      RegExp(r'popupVerticalWriting\s*=>\s*!_lyricsMode').hasMatch(src),
      isTrue,
