...
void main() {
  test('reader onMineFromPopup 成功分支把制卡计入书籍统计', () {
    ...
    expect(
      containsIdentifierCall(src, 'describeMineOutcome'),
      isTrue,
      reason: 'reader 应经 describeMineOutcome 判定制卡结果',
    );
    expect(
      containsCodeLine(src, 'if (described.record) unawaited(_recordMined());'),
      isTrue,
      reason: 'reader 成功（described.record）必须记账，否则统计页「制卡」恒为 0',
    );
    // helper 缺失时 methodBody 直接 fail（不会静默返回空串再让下面的断言假绿）。
    final String recordBody = methodBody(
      src,
      'Future<void> _recordMined() async {',
    );
    expect(
      containsIdentifierCall(recordBody, 'recordMiningEvent'),
      isTrue,
      reason:
          'reader 记账必须走 FushiDatabase.recordMiningEvent 复合入口'
          '（同事务写 mining_statistics + per-book），不得回到散点直调底层 DAO',
    );
    expect(
      containsCodeLine(recordBody, 'sourceType: kStatSourceBook'),
      isTrue,
  ...
  test('mixin 暴露 protected recordMined（供 video 等覆写页调用）', () {
    ...
    expect(
      masked.substring(0, signatureAt).trimRight().endsWith('@protected'),
      isTrue,
    ...
    expect(
      ...
      isTrue,
      reason:
          'mixin 记账必须走 FushiDatabase.recordMiningEvent 复合入口'
          '（同事务写 mining_statistics + per-book），不得回到散点直调底层 DAO',
    );
    // 基类 onMineEntry 经 describeMineOutcome 路由，record 时记账（书内/独立查词页这条不回归）。
    expect(
      containsCodeLine(src, 'if (described.record) unawaited(recordMined());'),
      isTrue,
      reason: '基类成功分支记账不得丢',
    );
  });
  ...
  test('stat_activity 暴露公开 statTodayKey（记账日期键的唯一权威实现）', () {
    ...
    expect(
      containsCodeLine(src, 'String statTodayKey() =>'),
      isTrue,
