...
void main() {
  ...
  Widget buildApp(Widget home) => TranslationProvider(
  ...
  group('merge', () {
    test('keeps saved order and drops sources no longer in the db', () {
      ...
      expect(merged.map((LocalAudioSourcePref s) => s.name), <String>[
        'forvo',
        'nhk16',
      ]);
      // 保留各自 enabled
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      await tester.pumpWidget(
        buildScaledApp(
          threeSourceDialog((List<LocalAudioSourcePref> p) {
            applied = p;
          }),
          0.5,
        ),
      ...
      expect(applied, isNotNull);
      final List<String> order = applied!
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      final List<String> manySources = List<String>.generate(
        24,
        (int i) => 'source_$i',
      );
      await tester.pumpWidget(
