...
void main() {
  ...
  test(
    'popupRendered reports content height in host CSS px (node behavior)',
    () async {
      ...
      final File jsTest = File(
        'test/pages/pr910_popup_content_zoom_report_test.js',
      );
      ...
      expect(
        ...
        0,
        reason:
            'popup content-height unit test failed.\n'
            'stdout:\n${result.stdout}\nstderr:\n${result.stderr}',
      );
  ...
  jsMirrors.forEach((String name, String relPath) {
    test('[$name] popupRendered 不得再直接上报未乘 zoom 的 layout px', () {
      ...
      expect(
        js.contains('function __fushiReportedContentHeight()'),
        isTrue,
        reason: '换算入口缺失',
      );
      expect(
      ...
      for (final Match m in sites) {
        ...
        expect(
          argWindow.contains('__fushiReportedContentHeight()'),
          isTrue,
      ...
      expect(
        js.contains('__fushiScrollHeight(),'),
        isFalse,
        reason:
            'BUG-1651 回归：裸 __fushiScrollHeight() 是未乘 zoom 的 layout px，'
