...
void main() {
  ...
  test('gallery button is wired into the bottom settings bar (barItems)', () {
    // 本地这条栏还是 barItems 直写（上游已改成 ReaderControlLayout 槽位系统）。
  ...
  test('gallery page is a chapter-grouped grid (extracted component)', () {
    ...
    expect(
      gallery.contains('class ReaderGalleryPage extends StatefulWidget'),
      isTrue,
    );
    expect(
      gallery.contains('SliverGrid('),
      isTrue,
      reason: '插图册是按章分组的 sliver 网格，不是缩略图带',
    );
    expect(
      gallery.contains('childCount: section.images.length'),
      isTrue,
    ...
    expect(
      src.contains('class _ReaderGalleryPage'),
      isFalse,
  ...
  test('未解锁卡片用共享遮罩视觉，且两个插图表面共用同一份', () {
    ...
    expect(
      gallery.contains('maskedIllustrationCover('),
      isTrue,
      reason: '锁着的卡是这张图自己的高斯模糊，不是写着「尚未读到」的占位卡',
    );
    expect(
      gallery.contains('class _LockedCardBody'),
      isFalse,
  ...
  test('卡片长按 / 右键菜单提供跳转与恢复遮罩', () {
    ...
    expect(
      gallery.contains('onLongPress: () => unawaited(_showCardMenu(ref))'),
      isTrue,
      reason: '长按卡片必须唤出菜单（锁着的图此前没有任何跳转入口）',
    );
    expect(
      gallery.contains('ContextMenuTrigger('),
      isTrue,
    ...
    // 没有键位就等于对键盘 / 手柄用户不存在。
    expect(
      gallery.contains('_isContextMenuKey(event)'),
      isTrue,
      reason: '卡片菜单必须有键盘入口（菜单键 / Shift+F10）',
    );
    expect(
      gallery.contains('_restoreGridFocus()'),
      isTrue,
      reason: '菜单关闭后要把焦点还给网格，否则键盘导航一次性废掉',
    );
    expect(
      gallery.contains("ValueKey<String>('fushi_gallery_menu_jump')"),
      isTrue,
    );
    expect(
      gallery.contains("ValueKey<String>('fushi_gallery_menu_relock')"),
      isTrue,
    );
    // 恢复遮罩必须真落到宿主（会话集 + Drift + 正文），不是只改本页外观。
    expect(gallery.contains('widget.onUnrevealImage?.call(key)'), isTrue);
    expect(
      src.contains('onUnrevealImage: (String key)'),
      isTrue,
      reason: 'chrome.part.dart 必须接住撤销揭开',
    );
    expect(
      src.contains('unmarkImageRevealed(bookUid, key)'),
      isTrue,
      reason: '撤销要落 Drift，否则下次开书又是揭开态',
    );
    expect(
      src.contains('__fushiUnmarkImageRevealed'),
      isTrue,
