...
void main() {
  ...
  final String homeVideoSrc = File(
  ...
  test('BUG-130: 桌面控制条启用单击播放/暂停', () {
    ...
    expect(
      src.contains('playAndPauseOnTap: _asbConfig.tapTogglesPlayback'),
      isTrue,
  ...
  test('BUG-131: 关闭字幕加载遮罩后归还焦点给视频', () {
    ...
    expect(
      ...
      isTrue,
      reason:
          '加载遮罩是模态对话框、会夺焦；关闭后必须主动把焦点还给 Video，'
          '否则空格等快捷键失灵（BUG-131）',
    );
  ...
  test('BUG-132: 恢复字幕源时对导入的外挂文件按路径直接加载', () {
    ...
    expect(
      ...
      isTrue,
      reason:
          'app 文档目录里的导入字幕不在剧集目录、listAllSubtitleSources 扫不到，'
  ...
  test('BUG-165: 换集时导入字幕捷径按目录归属判定（剧集同目录 sidecar 不沿用）', () {
    ...
    expect(
      body.contains('shouldReusePersistedSubtitleAcrossEpisode('),
      isTrue,
      reason:
          '换集（crossEpisode）时捷径必须按目录归属区分导入字幕 vs 剧集同目录 '
          'sidecar，剧集同目录 sidecar 要落回枚举按新集名重新匹配（BUG-165）',
    );
    expect(
      body.contains('crossEpisode'),
      isTrue,
  ...
  test('TODO-016: 字幕菜单只补入当前持久化的导入字幕源', () {
    ...
    expect(
      menu.contains('_subtitleSourcesForMenu('),
      isTrue,
      reason: '菜单不能只用 listAllSubtitleSources；重进后还要补入当前视频持久化的导入字幕',
    );
    expect(
      menu.contains('currentSubtitleSource: _currentSubtitleSource'),
      isTrue,
    ...
    expect(
      helper.contains('includeCurrentPersistedSubtitleForMenu('),
      isTrue,
      reason: '当前持久化导入字幕的补入逻辑要走可行为测试的 helper',
    );
    expect(
      helper.contains('currentSubtitleSource: currentSubtitleSource'),
      isTrue,
      reason: '只有持久化值是本地可加载字幕路径时才允许补入菜单',
    );
    expect(
      helper.contains('currentCues: currentCues'),
      isTrue,
      reason: '已有 DB cues 的重进路径必须把当前 controller cues 传给菜单 helper',
    );
    expect(
      helper.contains('Directory('),
      isFalse,
    ...
    expect(
      helper.contains('currentSecondaryCues: currentSecondaryCues'),
      isTrue,
    ...
    expect(
      ensure.contains('currentSecondaryCues: controller.secondaryCues'),
      isTrue,
  ...
  test('BUG-133: 视频页有页级拖放目标 + 导入去重防护', () {
    ...
    expect(
      src.contains('Widget _pageDropTarget('),
      isTrue,
    ...
    expect(
      importOuter.contains('_subtitleImportsInFlight'),
      isTrue,
  ...
  test('视频播放页拖放提示使用当前视频语义', () {
    ...
    expect(
      playbackDrop.contains('classifyDroppedFiles('),
      isTrue,
      reason: '播放页拖放必须先识别字幕/音频，避免把当前视频页误当成库页卡片落点',
    );
    expect(
      playbackDrop.contains('_importExternalSubtitle('),
      isTrue,
      reason: '受支持字幕应直接导入到当前播放视频',
    );
    expect(
      playbackDrop.contains('video_drop_audio_unsupported'),
      isTrue,
      reason: '音频文件在播放页应给出当前视频页语义，而不是要求拖到卡片',
    );
    expect(
      playbackDrop.contains('drag_drop_need_card_target'),
      isFalse,
      reason: '播放页不能复用“某本书或某个视频”这类库页文案',
    );
    expect(
      pageDrop.contains('_handlePlaybackDrop('),
      isTrue,
  ...
  test('被视频播放页覆盖的库页不再响应同一次文件拖放', () {
    ...
    expect(
      videoDrop.contains('!route.isCurrent'),
      isTrue,
    ...
    expect(
      shelfDrop.contains('!route.isCurrent'),
      isTrue,
  ...
  test('视频通知走 mpv 式角标 OSD，不再用 Material SnackBar', () {
    ...
    expect(
      osd.contains('IgnorePointer'),
      isTrue,
      reason: 'OSD 必须 IgnorePointer 包裹，绝不拦截点击（单击暂停/拖放/字幕查词）',
    );
    expect(
      osd.contains('_osdNotifier'),
      isTrue,
  ...
  test('TODO-446: 自动文本内封字幕抽取失败必须走 OSD 提示', () {
    ...
    expect(
      handler.contains('DefaultEmbeddedSubtitleLoadStatus.loaded'),
      isTrue,
    );
    expect(
      handler.contains('_currentSubtitleSource'),
      isTrue,
      reason: '自动加载成功后菜单高亮应跟随默认内封文本轨',
    );
    expect(handler.contains('_showOsd('), isTrue, reason: '自动加载失败必须给用户可见反馈');
    expect(
      handler.contains('t.video_subtitle_load_failed'),
      isTrue,
  ...
  test('TODO-573: 「自动获取字幕(Jimaku)」入口对本地和远端视频都显示', () {
    ...
    expect(
      panel.contains('if (_jimakuQuery() != null)'),
      isTrue,
      reason:
          'Jimaku 入口门控必须按「能否算出番名 query」判定，不能用 !_isRemote '
          '把远端视频整条入口隐藏（TODO-573）',
    );
    expect(
      panel.contains('if (!_isRemote && _currentVideoPath != null)'),
      isFalse,
  ...
  test('TODO-573: 远端 Jimaku query 取 host 标题，下载后走内存应用', () {
    ...
    expect(
      dialog.contains('_jimakuQuery()'),
      isTrue,
      reason: '对话框 query 复用 _jimakuQuery，不再直接读 _currentVideoPath',
    );
    expect(
      dialog.contains('if (_isRemote)'),
      isTrue,
      reason: '远端必须按 _isRemote 分流：没有本地 DB 行，不能走持久化链路',
    );
    expect(
      dialog.contains('_applyRemoteSubtitle(controller, downloaded)'),
      isTrue,
      reason:
          '远端下载的字幕只能内存应用（_applyRemoteSubtitle），与远端「本地导入字幕」'
          '同一不落 DB 的链路（TODO-573）',
    );
    expect(
      dialog.contains('_selectSubtitleSource(controller, source)'),
      isTrue,
      reason: '本地视频仍走 _selectSubtitleSource 持久化外挂字幕源',
    );
    // 远端分支必须早返回，不要再落到本地持久化分支。
    final int remoteApply = dialog.indexOf(
      '_applyRemoteSubtitle(controller, downloaded)',
    );
    final int localSelect = dialog.indexOf(
