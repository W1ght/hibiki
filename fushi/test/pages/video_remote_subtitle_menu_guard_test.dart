...
void main() {
  ...
  test('字幕轨切换区远端分支提供 关闭 / host 字幕 / 本地导入', () {
    ...
    expect(
      src.contains('Widget _buildSubtitleTrackRows('),
      isTrue,
  ...
  test('sidecar 和外挂字幕加载按扩展名路由到 async parser', () {
    ...
    expect(
      loadBody.contains('parseSubtitleContent('),
      isFalse,
      reason: 'external subtitle load must not synchronously parse large files',
    );
