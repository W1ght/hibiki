...
void main() {
  test(
    'image lightbox closes on any tap (no stopPropagation on the image)',
    () {
      ...
      // with the real file.
      final String source = maskJsComments(
        File('assets/popup/popup.js').readAsStringSync(),
      );
      ...
      expect(
        body,
        contains("overlay.addEventListener('click'"),
        reason: 'lightbox overlay must close on tap',
      );
      expect(
        body,
        contains('closeImageLightbox()'),
      ...
      expect(
        body.contains("image.addEventListener('click'"),
        isFalse,
        reason:
