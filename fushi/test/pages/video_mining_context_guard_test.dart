...
void main() {
  ...
  test('video mining caches the cue at subtitle lookup time', () {
    expect(
      src.contains('AudioCue? _lastLookupCue'),
      isTrue,
      reason: 'Video mining needs the subtitle cue from the original lookup.',
    );
    ...
    expect(
      lookup.contains('_lastLookupCue = resolveVideoLookupAnchorCue('),
      isTrue,
      reason:
          'Tapping a subtitle character must snapshot the anchor cue at '
          'lookup time (via resolveVideoLookupAnchorCue).',
    );
    // currentCue 仍被透传作为默认锚点（gap / 末句后被清成 null，BUG-074）。
    expect(
      lookup.contains('currentCue: controller.currentCue'),
      isTrue,
      reason: 'lookup 仍以 currentCue 作为默认锚点。',
    );
    // BUG-966: 字幕列表点词把被点条目作为 overrideCue 透传，优先于播放位置那句。
    expect(
      lookup.contains('overrideCue: overrideCue'),
      isTrue,
  ...
  test('video mining exports media from the cached lookup cue', () {
    ...
    final int snapshotIndex = mine.indexOf(
    ...
    expect(
      record,
      contains('snapshot.sectionIndex'),
  ...
  test(
    ...
    () {
      ...
      expect(
        mineCard,
        contains('String? audioFailure'),
        reason: 'OSD/日志应携带底层 ffmpeg 诊断摘要，而不是只有泛化失败文案。',
      );
      expect(
        mineCard,
        contains('onAudioFailure:'),
        reason: '音频抽取器的失败摘要必须按来源传回视频制卡路径（BUG-1205）。',
      );
      expect(
        mineCard,
        contains('onCoverFailure:'),
        reason: '封面抽取器的失败摘要必须按来源传回视频制卡路径（BUG-1205）。',
      );
      expect(
        mineCard,
        contains(r'sentence audio export failed: $audioFailure'),
      ...
      final String tryGifBody = methodBody(
        engine,
      ...
      expect(
        tryGifBody,
        contains('onFailure: reportCover'),
      ...
      final String audioBody = methodBody(
        engine,
      ...
      expect(
        audioBody,
        contains('onFailure: reportAudio'),
      ...
      final int engineAbortIdx = engineNorm.indexOf(
        "abortReason: _withRootCause('required audio missing'",
      );
      final int engineCtxIdx = engineNorm.indexOf(
        'AnkiMiningContext context =',
      );
      expect(engineAbortIdx, greaterThanOrEqualTo(0));
      expect(
        engineCtxIdx,
        greaterThan(engineAbortIdx),
      ...
      expect(
        shellOutcomeIdx,
        greaterThan(shellAbortIdx),
        reason: '句子音频导出失败后必须中止，不能继续读落卡产物 res.outcome!。',
      );
    },
  );

  test('TODO-816 ③: GIF fallback grabs the cue-time frame, not the current '
      'decoded frame', () {
    ...
    expect(
      engine,
      contains('extractVideoFrameViaFfmpeg'),
      reason: 'GIF 不可用时须按 cue 时间从视频文件抽单帧（而非截当前解码帧）。',
    );
    expect(
      engineNorm,
      contains('atSeconds: req.clipStartMs / 1000.0'),
    ...
    expect(
      mineCard,
      contains('stillFallback: () => currentFrameSnapshot'),
  ...
  test('TODO-816 ④: degrading a clip to a still frame surfaces an OSD', () {
    ...
    expect(
      mineCard,
      contains('res.degradedToStill && mounted'),
      reason: '降级提示须据引擎回传状态 + mounted 守卫，避免向已销毁页面 _showOsd。',
    );
    expect(
      mineCard,
      contains('card_cover_degraded_to_static'),
  ...
  test('TODO-971：制卡成功 OSD 用突出变体（醒目，区别于音量小角标）', () {
    ...
    expect(
      mineImpl.contains('describeMineOutcome('),
      isTrue,
