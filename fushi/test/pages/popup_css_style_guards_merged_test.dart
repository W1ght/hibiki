...
void main() {
  ...
  group('popup_header_button_spacing', () {
    ...
    String? ruleBody(String selector) {
      final RegExp rule = RegExp(RegExp.escape(selector) + r'\s*\{([^}]*)\}');
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          isTrue,
          reason:
              'the header button spacing must be a single-source responsive '
    ...
    test('.mine-button no longer carries its own margin-left (gap is the single '
    ...
    test('.sentence-context-picker no longer carries its own margin-left '
        '(TODO-846)', () {
      ...
      expect(
        ...
        isFalse,
        reason:
            'the .sentence-context-picker block must not add its own '
  ...
  group('popup_pitch_frequency_spacing', () {
    test('popup.css gives the pitch section that follows the frequency section '
      ...
      expect(
        ...
        isNotNull,
        reason:
            'popup.css must target the freq→pitch adjacency '
      ...
      final RegExpMatch? marginMatch = RegExp(
        r'margin-top\s*:\s*(\d+(?:\.\d+)?)\s*px',
      ).firstMatch(body);
      expect(
        marginMatch,
        isNotNull,
