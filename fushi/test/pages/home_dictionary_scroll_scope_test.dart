...
void main() {
  ...
  group('home dictionary result area shares the TODO-460 wheel scope', () {
    test(
      'renders results through DictionaryPopupWebView (the popup.js scope)',
      () {
        ...
        expect(
          ...
          reason:
              'The result area must depend on the shared dictionary popup '
              'WebView so it inherits the TODO-460 smooth-wheel channel.',
        );

        final int resultBodyAt = source.indexOf(
        ...
        final int webViewAt = source.indexOf(
          'DictionaryPopupWebView(',
          resultBodyAt,
        );
        expect(
          webViewAt,
          greaterThan(resultBodyAt),
          reason:
              'The result body must render results through '
    ...
    test('keeps the result WebView under the UI-scale neutralizer (zoom=1)', () {
      ...
      final int neutralizerAt = source.indexOf(
      ...
      expect(
        ...
        greaterThan(resultBodyAt),
        reason:
            'The result body must wrap the result WebView in '
            'FushiAppUiScaleNeutralizer.',
      );
      expect(
        webViewAt,
        greaterThan(neutralizerAt),
        reason:
            'popup.js refines each wheel notch and divides by '
  ...
  group('the shared popup WebView still loads the TODO-460 wheel asset', () {
    test('DictionaryPopupWebView loads assets/popup/popup.html', () {
      ...
      expect(
        source,
        contains("webViewAssetUrl('assets/popup/popup.html')"),
        reason:
            'The shared dictionary WebView must load the popup HTML that '
            'pulls in popup.js.',
      );
    });
    ...
    test('popup.html pulls in popup.js', () {
      ...
      expect(
        html,
        contains('src="popup.js"'),
        reason:
            'popup.html must include popup.js, which carries the '
  ...
  test('popup.js still carries the TODO-460 smooth-wheel channel', () async {
    ...
    expect(
      js,
      contains('POPUP_WHEEL_MAX_VISUAL_STEP'),
      reason: 'TODO-460 single-notch visual-step cap must remain in popup.js.',
    );
    expect(
      js,
      contains('popupClampWheelVisualStep'),
