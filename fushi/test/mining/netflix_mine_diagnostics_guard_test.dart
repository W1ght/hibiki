  ...
  group('TODO-1303 扩展制卡诊断 toast + audioWarning 区分守卫', () {
    mirrors.forEach((String name, String root) {
      test('[$name] fushiClassifyMineResp 读诊断 + 弹原因 toast', () {
        ...
        expect(
          src.contains('d.message || d.detail'),
          isTrue,
          reason: '$root classify 未读取 message/detail 诊断',
        );
        // 失败弹 toast 显因。
        expect(
          src.contains("window.fushiToast('✗ '"),
          isTrue,
        ...
        expect(
          src.contains('function fushiMineHttpFailureReason(resp)'),
          isTrue,
        ...
        expect(
          src.contains('鉴权失败(401)'),
          isTrue,
          reason: '$root 未区分 401 鉴权失败原因',
        );
        expect(
          src.contains('Yomitan API server'),
          isTrue,
