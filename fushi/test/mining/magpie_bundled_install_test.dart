...
Iterable<File> _workflowFiles() sync* {
  ...
  for (final FileSystemEntity entity in workflowDirectory.listSync(
    recursive: true,
    followLinks: false,
  )) {
...
void main() {
  ...
  void writeBundle({required String arch, String? shaOverride}) {
    ...
    final File zipFile = File(
      p.join(bundleDir.path, magpieBundledZipName(arch)),
  ...
  group('随包安装的校验强度', () {
    test('没有随包归档 → 返回 false，由调用方提示更新 Hibiki，不抛', () async {
      final MagpieInstaller installer = MagpieInstaller(
        bundledDirectory: bundleDir,
      );
    ...
    test('只有 zip 没有侧车 → 硬失败，绝不装无法证明来源的包', () async {
      final List<int> zip = _buildFakeMagpieZip();
      File(
        p.join(bundleDir.path, magpieBundledZipName('x64')),
      ).writeAsBytesSync(zip);
      final MagpieInstaller installer = MagpieInstaller(
        bundledDirectory: bundleDir,
      );
  ...
  group('组包脚本契约', () {
    ...
    test('源文件保留 UTF-8 BOM，避免 Windows PowerShell 5.1 按代码页误解码', () {
      ...
      expect(
        ...
        <int>[0xEF, 0xBB, 0xBF],
        reason:
            'BUG-1217 脚本含中文；无 BOM 时 Windows PowerShell 5.1 '
            '会按系统代码页读取并在执行前 parser error',
      );
    });

    test('Windows PowerShell 5.1 Parser.ParseFile 对脚本零错误', () {
      final String escapedPath = scriptFile.absolute.path.replaceAll("'", "''");
    ...
    test('裁剪计数、zip 与 sha256 侧车仍由脚本真实产出', () {
      ...
      expect(script, contains(r'Write-Host "  保留 $kept 个文件，裁掉 $dropped 个"'));
      expect(script, contains(r'Set-Content -LiteralPath "${slimZip}.sha256"'));
    });
  ...
  group('Windows 主包随包资产构建契约', () {
    test('递归扫描 workflow，Magpie slim 只有两个规范 pwsh 调用点', () {
      ...
      for (final File file in _workflowFiles()) {
        ...
        final List<String> lines = maskHashComments(
        ...
        for (int index = 0; index < lines.length; index++) {
          ...
          expect(
            ...
            canonicalInvocation,
            reason:
                'Magpie slim 禁止回退到 Windows PowerShell 5.1：'
          ...
          expect(
            step.any((String value) => value.trim() == 'shell: pwsh'),
            isTrue,
    ...
    test('debug 与 release 的脚本单改都触发构建并复制 zip + sha256', () {
      ...
      final String buildWorkflow = File(
        '../.github/workflows/build-multiplatform.yml',
      ).readAsStringSync();
