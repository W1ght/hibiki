...
void main() {
  ...
  mirrors.forEach((String name, String root) {
    ...
    test('[$name] hides Netflix next-episode button gated by a setting', () {
      ...
      expect(
        content.contains('[data-uia="next-episode-seamless-button"]'),
        isTrue,
        reason: '$root content.js missing the seamless next-episode selector',
      );
      expect(
        content.contains('netflixHideNextEpisode'),
        isTrue,
        reason: '$root content.js must gate the hide on netflixHideNextEpisode',
      );
      expect(
        content.contains('display:none!important'),
        isTrue,
        reason: '$root content.js next-episode hide must use display:none',
      );
      // 缺省=隐藏（仅显式 false 才显示），content 与 options 判据一致。
      expect(
        content.contains('netflixHideNextEpisode === false'),
        isTrue,
        reason: '$root content.js default must be hide (only === false shows)',
      );

      final String html = File('$root/options.html').readAsStringSync();
      final String js = File('$root/options.js').readAsStringSync();
      expect(
        html.contains('id="nfHideNext"'),
        isTrue,
        reason: '$root options.html missing the next-episode hide toggle',
      );
      expect(
        js.contains('netflixHideNextEpisode'),
        isTrue,
    ...
    test('[$name] surfaces un-generated Netflix cards at batch completion', () {
      final String content = File('$root/content.js').readAsStringSync();
      expect(
        content.contains('张录制失败未生成，可再点生成重试'),
        isTrue,
    ...
    test('[$name] Netflix mining carries the show title (documentTitle)', () {
      final String adapters = File(
        '$root/subtitle-adapters.js',
      ).readAsStringSync();
      expect(
        adapters.contains('function netflixDocumentTitle'),
        isTrue,
        reason: '$root subtitle-adapters.js missing netflixDocumentTitle',
      );
      expect(
        adapters.contains('[data-uia="video-title"]'),
        isTrue,
        reason:
      ...
      // 入队即抓剧名并随卡持久化。
      expect(
        content.contains('documentTitle: documentTitle'),
        isTrue,
        reason: '$root content.js must persist documentTitle on the queue item',
      );
      ...
      expect(
        background.contains('msg.documentTitle'),
        isTrue,
