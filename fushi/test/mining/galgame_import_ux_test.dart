...
void main() {
  group('① 游戏「导入」页要自己接 drop', () {
    ...
    test('导入视图挂了 FushiFileDropTarget', () {
      ...
      expect(
        body,
        contains('FushiFileDropTarget('),
        reason:
            '此前整个游戏域只有**库**页挂了 drop target，而 DropSurfaceScope '
    ...
    test('drop target 必须包在 _buildImport 的最外层（内层会被布局裁掉命中区）', () {
      ...
      expect(
        ...
        lessThan(layoutAt),
        reason:
            'desktop_drop 按 RenderBox.paintBounds 过滤，drop target 只覆盖'
  ...
  group('② 导入成功要有反馈', () {
    ...
    test('文件选择器导入成功后出 success toast', () {
      ...
      expect(
        ...
        isNonNegative,
        reason:
            '成功路径此前 0 反馈：只有「已在库中」才出 toast，导成功与导失败'
            '在观感上完全一样。',
      );
    ...
    test('「导入」页把跳转接上了游戏库', () {
      ...
      expect(
        page,
        contains('onImported: _showLibrary'),
    ...
    test('批量拖放路径同样出 toast 并回调', () {
      ...
      expect(
        body,
        contains('filterOutDuplicateGameExes('),
    ...
    test('游戏库页的拖放收敛到同一条共享路径（两处各写一份必然走岔）', () {
      ...
      expect(body, contains('addGamesFromPaths('));
      expect(
        body.contains('newGalgameEntryFromExe('),
        isFalse,
        reason:
            '落库细节应当只有共享动作里一份；这里再拼一遍，只要有一处忘了 '
  ...
  group('③ 自定义图标要有裁剪步骤', () {
    ...
    test('选图之后、落盘之前插入裁剪对话框，并锁定 1:1', () {
      ...
      expect(
        body.substring(cropAt, cropAt + 160),
        contains('aspectRatio: 1'),
    ...
    test('落盘与读字节都用裁剪结果，不再碰原图路径', () {
      final String body = methodBody(source, 'Future<void> _pickCustomIcon(');
      expect(
        body.contains('persistCustomIconFile(pickedPath)'),
        isFalse,
        reason: '用原图落盘 = 裁剪白做，而且是**静默**的：UI 上裁完了，图标还是原样。',
      );
      expect(
        body.contains('File(pickedPath).readAsBytes()'),
        isFalse,
        reason: 'Android 分支同理。',
      );
    });
    ...
    test('裁剪对话框支持锁定宽高比，且真的传给了 CropController', () {
      ...
      expect(
        crop,
        contains('CropController(aspectRatio: widget.aspectRatio)'),
        reason: '只加字段不接到 controller 上，锁比例就是个摆设。',
      );
      expect(
        crop,
        contains('Future<File?> showCropImageDialog('),
