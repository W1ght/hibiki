...
void main() {
  group('FushiFocusScroll.signedFractionFor', () {
    test('down/right 为正、up/left 为负', () {
      expect(
        FushiFocusScroll.signedFractionFor(TraversalDirection.down, 0.8),
        0.8,
      );
      expect(
        FushiFocusScroll.signedFractionFor(TraversalDirection.right, 0.8),
        0.8,
      );
      expect(
        FushiFocusScroll.signedFractionFor(TraversalDirection.up, 0.8),
        -0.8,
      );
      expect(
        FushiFocusScroll.signedFractionFor(TraversalDirection.left, 0.8),
        -0.8,
      );
    });
  });
  ...
  group('FushiFocusScroll.scrollByViewportFraction', () {
    testWidgets('无 Scrollable 祖先返回 false', (WidgetTester tester) async {
      ...
      expect(
        FushiFocusScroll.scrollByViewportFraction(ctx, null, 0.8),
        isFalse,
      );
    });
