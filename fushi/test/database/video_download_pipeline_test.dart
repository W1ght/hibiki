...
void main() {
  test(
    'job CRUD/watch and child tables preserve fields then cascade',
    () async {
      ...
      final VideoDownloadJobFileRow file = (await db.getVideoDownloadJobFiles(
        'job-1',
      )).single;
      ...
      expect(
        await db.getVideoDownloadJobSubtitles('job-1'),
        isEmpty,
  ...
  test(
    'claim and retry use lease fields without overloading lifecycle/stage',
    () async {
      final FushiDatabase db = await _openDb();
      ...
      final VideoDownloadJobRow waiting = (await db.getVideoDownloadJob(
        'retry-job',
      ))!;
      ...
      final VideoDownloadJobRow failed = (await db.getVideoDownloadJob(
        'retry-job',
      ))!;
  ...
  test(
    'stage CAS, release and needsAttention preserve orthogonal state',
    () async {
      final FushiDatabase db = await _openDb();
      await db.upsertVideoDownloadJob(
      ...
      final VideoDownloadJobRow advanced = (await db.getVideoDownloadJob(
        'stage-job',
      ))!;
      ...
      final VideoDownloadJobRow attention = (await db.getVideoDownloadJob(
        'stage-job',
      ))!;
  ...
  test(
    'subscription lease/retry, oneShot fulfillment and item dedup',
    () async {
      final FushiDatabase db = await _openDb();
      await db.upsertVideoDownloadSubscription(
        _subscription('one-shot', mode: 'oneShot'),
      );
      final VideoDownloadSubscriptionRow claimed = (await db
      ...
      final VideoDownloadSubscriptionRow fulfilled = (await db
          .getVideoDownloadSubscription('one-shot'))!;
      ...
      final List<VideoDownloadSubscriptionItemRow> items = await db
          .getVideoDownloadSubscriptionItems('one-shot');
      expect(items, hasLength(1));
      expect(
        items.single.selectedResourceId,
        'release-b',
      ...
      final VideoDownloadSubscriptionRow retrying = (await db
          .getVideoDownloadSubscription('ongoing'))!;
  ...
  test(
    'database constraints reject transient URLs and mixed state axes',
    () async {
      final FushiDatabase db = await _openDb();
      await expectLater(
        db.upsertVideoDownloadJob(
          _job('http-job').copyWith(
            magnetUri: const Value<String?>(
              'https://indexer/token/metainfo.torrent',
            ),
          ),
        ),
        throwsA(anything),
      ...
      await expectLater(
        db.updateVideoDownloadJob(
          'valid',
          const VideoDownloadJobsCompanion(lifecycle: Value<String>('running')),
        ),
        throwsA(anything),
      );
      await expectLater(
        db.updateVideoDownloadJob(
          'valid',
          const VideoDownloadJobsCompanion(stage: Value<String>('completed')),
        ),
        throwsA(anything),
      );
    },
  ...
  test('B3：items 状态计数按订阅分组聚合（面板卡片摘要一查拿全量）', () async {
    ...
    Future<void> item(String sid, String key, String resource, String status) =>
        db.upsertVideoDownloadSubscriptionItem(
          VideoDownloadSubscriptionItemsCompanion.insert(
            subscriptionId: sid,
            logicalItemKey: key,
    ...
    final Map<String, Map<String, int>> counts = await db
        .getVideoDownloadSubscriptionItemStatusCounts();
    expect(counts['s1'], <String, int>{
      VideoDownloadSubscriptionItemStatus.processed: 2,
      VideoDownloadSubscriptionItemStatus.queued: 1,
    });
    expect(counts['s2'], <String, int>{
      VideoDownloadSubscriptionItemStatus.failed: 1,
    });
  });
}
