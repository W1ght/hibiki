...
/// v100：统计按 Profile 隔离——`study_segments` / `galgame_sessions` 加 `profile_id`，
...
void main() {
  /// 造一个 v99 形态的库：先用当前代码建全表并写入统计行，再把 v100 的产物
  /// 拆掉（DROP COLUMN / 按旧形态重建碑表）、版本号写回 99。
  Future<({String path, int activeId, int otherId})> seedV99Database(
    Directory directory, {
  ...
  test('v99 → v100：存量段 / 碑 / 游玩会话全部归到升级时激活的 Profile', () async {
    final Directory directory = Directory.systemTemp.createTempSync(
      'study_profile_v100',
    );
    addTearDown(() => directory.deleteSync(recursive: true));
    final ({String path, int activeId, int otherId}) seed =
        await seedV99Database(directory);
    ...
    expect(upgraded.schemaVersion, 100);
  ...
  test('v100 迁移路径上分区索引一并建出（_ensureIndexes 不跑升级路径）', () async {
    final Directory directory = Directory.systemTemp.createTempSync(
      'study_profile_v100_idx',
    );
    addTearDown(() => directory.deleteSync(recursive: true));
    final ({String path, int activeId, int otherId}) seed =
        await seedV99Database(directory);
  ...
  test('库里没有任何 Profile 时升级补建 Default 认领历史、并设为激活', () async {
    final Directory directory = Directory.systemTemp.createTempSync(
      'study_profile_v100_noprof',
    );
    addTearDown(() => directory.deleteSync(recursive: true));
    final ({String path, int activeId, int otherId}) seed =
        await seedV99Database(directory, withProfiles: false);
