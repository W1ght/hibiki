...
void main() {
  group('deleteReadingStatisticsForTitle', () {
    test('clears reading + lookup/mining (book) rows for the title only, and '
      ...
      final List<LookupMiningCounterRow> bookCounters = await db
          .getLookupMiningCountersBySource('book');
      expect(
        bookCounters.map((LookupMiningCounterRow r) => r.title),
        <String>['B'],
        reason: 'book counters for A gone, B kept',
      );
      final List<LookupMiningCounterRow> videoCounters = await db
          .getLookupMiningCountersBySource('video');
      expect(
        videoCounters.single.title,
        'A',
        reason: 'same-title video counter is a different bucket, survives',
      );

      final Set<(String, String)> tombstones = await db
          .getStatisticsTombstoneKeys();
      expect(tombstones, contains(('A', 'book')));
      expect(tombstones, isNot(contains(('A', 'video'))));
    });

    test('does NOT delete mined sentences or favorite words (content, not '
        'statistics)', () async {
      final FushiDatabase db = await _openDb();
      await _setReading(
        db,
      ...
      expect(
        (await db.getAllMinedSentences()).length,
        1,
        reason: 'mined card history is user content, not a statistic',
      );
      expect(
        (await db.getAllFavoriteWords()).length,
        1,
  ...
  group('deleteVideoStatisticsForIdentity', () {
    test('clears video watch + lookup/mining (video) rows and writes a video '
      ...
      await _setReading(db, title: 'A', dateKey: '2026-07-05', chars: 1, ms: 1);
      await db.addLookupCount(
        title: 'A',
        sourceType: 'video',
        dateKey: '2026-07-05',
      ...
      expect(
        (await db.getAllReadingStatistics()).single.title,
        'A',
    ...
    test('v76: uid-scoped delete leaves a same-title sibling video untouched '
      ...
      final List<VideoWatchStatisticRow> watch = await db
          .getAllVideoWatchStatistics();
      expect(
        watch.single.bookUid,
        'uid-2',
    ...
    test('review2-6 回归：改名视频按 uid 删除时，墓碑覆盖被删行的全部历史 title'
      ...
      expect(
        tombstones,
        containsAll(<(String, String)>[('新名', 'video'), ('旧名', 'video')]),
      );
    });

    test('review3-2 回归：includeUnattributed 扫面覆盖被删 uid 的全部历史 title'
    ...
    test('review4-1 回归：歧义历史 title（另一身份也在用）不扫无身份行、不立碑'
      ...
      expect(
        tombstones,
        isNot(contains(('同名', 'video'))),
        reason: '歧义 title 不立碑——立了会压制幸存视频 uid-y 的同步',
      );
    });

    test('review2-10 回归：bookUid 存成空串（而非 NULL）的行也算无身份，'
        '删得掉不复活', () async {
      final FushiDatabase db = await _openDb();
      ...
      expect(
        ...
        isEmpty,
        reason:
            '删除谓词与展示层同判据（NULL 与 空串 都算无身份），'
            '不留「显示得出、删不掉」的行',
      );
    });

    test('v76: includeUnattributed sweeps legacy NULL/empty-identity rows of '
  ...
  group('tombstone lifecycle', () {
    test(
      'new lookup activity clears the matching tombstone (per sourceType)',
      () async {
        final FushiDatabase db = await _openDb();
  ...
  group('v92: 定向删除连带 study_segments 事实 + 按媒体身份立碑', () {
    test(
      ...
      () async {
        ...
        final List<StudySegmentTombstoneRow> tombstones = await db
            .getStudySegmentTombstones();
        expect(
          tombstones.map(
            (StudySegmentTombstoneRow t) => (t.mediaKind, t.mediaKey),
          ),
    ...
    test(
      ...
      '(no identity → nothing to delete, no segment tombstone)',
      () async {
        final FushiDatabase db = await _openDb();
        await _seedSegment(
          db,
    ...
    test(
      ...
      'and writes a (video, bookUid) segment tombstone',
      () async {
        final FushiDatabase db = await _openDb();
        await _seedSegment(
          db,
