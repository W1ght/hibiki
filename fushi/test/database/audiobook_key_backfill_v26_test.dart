...
Future<List<String>> _bookKeys(FushiDatabase db, String table) async {
  final rows = await db
      .customSelect('SELECT book_key FROM $table ORDER BY id')
      .get();
...
void main() {
  group('TODO-809 audiobook book_key backfill (v25 -> v26)', () {
    test('bad DB heals: mismatched key rewritten via unique SRT-title match, '
      ...
      expect(await _bookKeys(db, 'audio_cues'), <String>[
        'real-epub-key',
        'real-epub-key',
      ]);
      ...
      final match = await db
          .customSelect(
            'SELECT COUNT(*) AS c FROM audiobooks a '
    ...
    test(
      'ambiguous (>1 match) and orphan (0 match) rows are left untouched',
      () async {
        final db = await _openV25((raw) {
        ...
        expect(
          abKeys,
          containsAll(<String>['sanitized-dup', 'sanitized-orphan']),
        );
    ...
    test(
      'idempotent: re-running the backfill after heal changes nothing',
      () async {
        final db = await _openV25((raw) {
          _insertEpub(raw, 'real-epub-key', 'Kokoro');
          _insertAudiobook(raw, 'sanitized-kokoro');
    ...
    test('target key already owned by another audiobook is skipped '
      ...
      expect(
        abKeys,
        containsAll(<String>['real-epub-key', 'sanitized-kokoro']),
      );
      expect(abKeys, hasLength(2));
    });
