...
void main() {
  ...
  test(
    'v77→v78 creates the five durable pipeline tables and indexes',
    () async {
      ...
      expect(version, db.schemaVersion, reason: '阶梯必须一路升到当前版');
  ...
  test(
    'v78 job schema persists only magnet locator and text backend profile',
    () async {
      final FushiDatabase db = await openV77Db();
      final Set<String> columns =
          (await db
                  .customSelect('PRAGMA table_info(video_download_jobs)')
                  .get())
              .map((row) => row.read<String>('name'))
              .toSet();
      ...
      final List<Map<String, Object?>> jobForeignKeys =
          (await db
                  .customSelect('PRAGMA foreign_key_list(video_download_jobs)')
                  .get())
              .map((row) => row.data)
              .toList();
      expect(jobForeignKeys.any((fk) => fk['table'] == 'profiles'), isFalse);
    },
  );

  test(
    'v78 child ownership uses cascade and optional job links use setNull',
    () async {
      final FushiDatabase db = await openV77Db();

      final List<Map<String, Object?>> fileForeignKeys =
          (await db
                  .customSelect(
      ...
      final List<Map<String, Object?>> subtitleForeignKeys =
          (await db
                  .customSelect(
      ...
      final List<Map<String, Object?>> itemForeignKeys =
          (await db
                  .customSelect(
