...
void main() {
  ...
  test('main.dart：dataRootUnavailable 逃生屏在裸 loading 分支之前，且接双按钮 (BUG-815)', () {
    ...
    final String region = src.substring(
      escapeIdx,
      loadingIdx < src.length ? loadingIdx : src.length,
    );
    expect(
      region.contains('retryInitialise()'),
      isTrue,
      reason: '逃生屏必须有「重试」→ retryInitialise()',
    );
    expect(
      region.contains('retryInitialiseWithDefaultRoot()'),
      isTrue,
  ...
  test(
    ...
    () {
      ...
      expect(
        mBody.contains('AppPaths.forceDefaultRootForSession = true'),
        isTrue,
        reason:
      ...
      expect(
        cBody.contains('_dataRootUnavailable = e'),
        isTrue,
        reason: 'catch 必须置 _dataRootUnavailable = e',
      );
      expect(
        cBody.contains('_initError = '),
        isFalse,
