...
void main() {
  group('popup.css overlay scrollbar (Niratan 对齐 2026-08-23)', () {
    ...
    test('rest-state thumb is transparent pill', () {
      ...
      expect(
        block,
        contains('background-clip: content-box;'),
    ...
    test('thumb reveals on hover and on popup-scroll-active', () {
      expect(
        css,
        contains('*:hover::-webkit-scrollbar-thumb'),
        reason: '桌面鼠标悬停显形（通用形，任一滚动容器自身悬停即显形）',
      );
      expect(
        css,
        contains('*.popup-scroll-active::-webkit-scrollbar-thumb'),
      ...
      expect(
        block,
        contains('var(--popup-scrollbar-thumb'),
        reason: '显形色走主题变量，非字面量',
      );
    });

    test(
      'standard scrollbar props must NOT come back (Chromium 121+ 全族禁用)',
      () {
        // 声明级匹配（行首缩进 + 属性名 + 冒号），注释里提到属性名不算。
        expect(
          RegExp(r'^\s*scrollbar-color\s*:', multiLine: true).hasMatch(css),
          isFalse,
          reason:
              '标准 scrollbar-color 声明一出现，Chromium 121+ 即禁用全部 '
              '::-webkit-scrollbar 伪元素，悬停显隐失效（BUG-753 行为）',
        );
        final Iterable<RegExpMatch> widthDecls = RegExp(
          r'^\s*scrollbar-width\s*:\s*(\S+?)\s*;',
          multiLine: true,
        ).allMatches(css);
        for (final RegExpMatch m in widthDecls) {
          expect(
            ...
            'none',
            reason:
                'scrollbar-width 只允许 none（.expression-scroll 的 '
    ...
    test('theme blocks define the thumb variable', () {
      for (final String theme in <String>['light', 'dark']) {
        ...
        expect(
          block,
          contains('--popup-scrollbar-thumb:'),
    ...
    test('light theme block pins color-scheme: light', () {
      ...
      expect(
        ...
        reason: 'WebView2 Fluent overlay scrollbar only follows color-scheme',
      );
    });
    ...
    test('popup.js carries the popup-scroll-active toggler', () {
      expect(
        js,
        contains("'popup-scroll-active'"),
        reason: 'popup.js 必须有滚动监听给根节点打 .popup-scroll-active',
      );
      expect(js, contains('900'), reason: '900ms 无滚动后衰减（Niratan 同参）');
    });
  });
}
