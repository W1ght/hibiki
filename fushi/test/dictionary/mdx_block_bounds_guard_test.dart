...
void main() {
  ...
  test(
    'mdx_reader bounds both uncompressed-block copies by decompressed_size',
    () {
  ...
  test('mdx_reader key-block else copy is guarded before the assign', () {
    final String src = File(mdxCpp).readAsStringSync();
    final int guardIdx = src.indexOf(
      RegExp(r'if\s*\(\s*pos\s*\+\s*meta\.decompressed_size'),
    );
  ...
  test('mdx_reader record-block else copy is guarded before the insert', () {
    ...
    final int lastGuard = before.lastIndexOf(
      RegExp(r'pos\s*\+\s*meta\.decompressed_size\s*>\s*size'),
    );
    expect(
      lastGuard,
      greaterThanOrEqualTo(0),
