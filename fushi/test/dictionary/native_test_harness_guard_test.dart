...
void main() {
  ...
  test('tests/CMakeLists.txt aggregates the native suite into ctest', () {
    ...
    expect(
      cmake,
      contains('add_subdirectory(\${FUSHI_ROOT} fushidicts_build)'),
    );
    ...
    for (final String testName in <String>[
      ...
      expect(
        cmake,
        contains('add_fushi_test($testName'),
  ...
  test('CI builds + runs the native ctest suite on Linux', () {
    ...
    final int ctestIdx = workflow.indexOf(
      'Run fushidicts native tests (ctest)',
    );
    ...
    expect(
      ctestIdx,
      lessThan(flutterBuildIdx),
      reason: 'run native ctest before the Flutter Linux build (fail fast).',
    );
  });

  test(
    'the existing macOS ctypes dylib smoke stays intact (not regressed)',
    () {
