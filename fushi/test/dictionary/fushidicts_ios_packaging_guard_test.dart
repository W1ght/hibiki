...
void main() {
  ...
  test('iOS Runner delegates FushiDicts build to a cache-safe script', () {
    ...
    expect(
      script,
      contains(r'case ";$cmake_archs;"'),
      reason:
          'Xcode can feed duplicate ARCHS values; CMake 4.x then keeps '
  ...
  test('iOS deployment targets stay within Xcode 27 supported floor', () {
    ...
    final List<double> runnerTargets =
    ...
    expect(
      ...
      reason:
          'post_install 会覆盖每个 pod 自己的部署目标，必须与 platform '
  ...
  test('iOS Runner exports force-loaded FushiDicts symbols for dlsym', () {
    final String project = read('ios/Runner.xcodeproj/project.pbxproj');
    final List<String> fushidictsLinkerBlocks =
        RegExp(r'OTHER_LDFLAGS = \(([\s\S]*?)\n\s*\);')
            .allMatches(project)
            .map((RegExpMatch match) => match.group(1)!)
            .where(
              (String block) => block.contains('FUSHIDICTS_MERGED_ARCHIVE'),
            )
            .toList();

    expect(
      fushidictsLinkerBlocks,
      hasLength(3),
      reason:
          'Debug/Profile/Release must export the force-loaded static '
    ...
    for (final String block in fushidictsLinkerBlocks) {
      ...
      expect(
        block.indexOf('"-Wl,-export_dynamic"'),
  ...
  test('iOS Runner keeps global symbols through the archive strip pass', () {
    ...
    final List<String> runnerSettingsBlocks =
    ...
    for (final String block in runnerSettingsBlocks) {
      expect(
        block,
        contains('STRIP_STYLE = "non-global";'),
        reason:
            'BUG-1584：`flutter build ios --release` 用 ACTION=build，'
