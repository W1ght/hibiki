...
void main() {
  ...
  group('fallbackMimeType', () {
    ...
    group(
      'BUG-1203: content documents are classified by declared media-type',
      () {
        test(
          'isHtmlMediaType accepts every EPUB content-document media-type',
          () {
        ...
        for (final String href in <String>[
          ...
          test('declared media-type wins for unusual href "$href"', () {
            ...
            expect(
              fallbackMimeType(href),
              isNot('text/html'),
              reason:
                  '前提：这个 href 靠扩展名表是判不出 HTML 的，'
                  '否则本用例证明不了 media-type 优先',
            );
            expect(
              isHtmlMediaType(book.mediaType(href)),
              isTrue,
              reason:
                  'OPF 声明是内容文档，拦截器必须当网页处理'
                  '（否则整本正文空白且无错误日志）',
            );
          });
        }
        ...
        for (final String ext in <String>['xhtml', 'html', 'htm', 'xht']) {
          test('.$ext falls back to HTML when the OPF declares nothing', () {
            ...
            expect(
              isHtmlMediaType(book.mediaType('OEBPS/c01.$ext')),
              isTrue,
              reason:
                  '.$ext must not fall back to octet-stream — the reader '
        ...
        test(
          'a malformed OPF declaration cannot un-classify a known extension',
          () {
        ...
        test('the shared predicate has no parallel copy in the parser', () {
          ...
          expect(
            parser.contains('_isHtmlMediaType'),
            isFalse,
            reason:
                '解析器又抄回了一份私有谓词——两份判据必然漂开，'
                '一边认怪 media-type 另一边不认就是静默空白页',
          );
          expect(
            parser.contains('isHtmlMediaType('),
            isTrue,
        ...
        test(
          'interceptor classifies by media-type and still serves text/html',
          () {
            final String reader = readReaderPageSource();
            final int payloadIdx = reader.indexOf(
            ...
            final int endIdx = reader.indexOf(
            ...
            expect(
              payloadBody.contains('_book?.mediaType('),
              isTrue,
              reason:
                  '拦截器不再读 EPUB 声明的 media-type，退回「按扩展名猜」——'
                  'BUG-1203 的根因原样复活',
            );
            expect(
              payloadBody.contains('isHtmlMediaType('),
              isTrue,
              reason:
                  '拦截器必须复用共享谓词 isHtmlMediaType，'
                  '不得在本地另写一份平行判据',
            );
            expect(
              payloadBody.contains('fallbackMimeType(filePath)'),
              isTrue,
              reason: 'OPF 允许缺 media-type，扩展名兜底不能被删掉',
            );
            expect(
              payloadBody.contains("isHtmlDocument ? 'text/html'"),
              isTrue,
              reason:
                  '内容文档必须一律以 text/html 下发；一旦改成回声 OPF 声明的'
            ...
            expect(
              payloadBody.contains("'application/xhtml+xml'"),
              isFalse,
              reason:
                  '拦截器方法体里出现这个字符串字面量，几乎必然是把它当成了下发的'
  ...
  group('EpubBook.resolveInternalLink', () {
    ...
    test('resolves link with fragment', () {
      ...
      final result = book.resolveInternalLink(
        'https://fushi.local/epub/ch1.xhtml#section2',
      );
      expect(result, isNotNull);
    ...
    test('returns null for href not matching any chapter', () {
      ...
      expect(
        book.resolveInternalLink('https://fushi.local/epub/nonexistent.xhtml'),
        isNull,
      );
    });
    ...
    group('BUG-097 path normalization (../ ./ // resolve, not external)', () {
      ...
      test('duplicate slashes resolve to the chapter', () {
        final result = book.resolveInternalLink(
          'https://fushi.local/epub/OEBPS//ch1.xhtml',
        );
        expect(result, isNotNull);
        expect(result!.chapterIndex, 0);
      });
  ...
  group('EpubBook.chapterIndexForHref (TODO-796 cover TOC matching)', () {
    ...
    test('percent-encoded Japanese cover href resolves', () {
      ...
      expect(jpBook.chapterIndexForHref('%E8%A1%A8%E7%B4%99.xhtml'), 0);
    });
    ...
    test(
      'href owned by no spine chapter returns -1 (dirty TOC item skipped)',
      () {
