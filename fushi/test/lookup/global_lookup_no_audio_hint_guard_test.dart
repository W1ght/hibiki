...
void main() {
  ...
  group('popup.js: no-audio feedback (TODO-1251)', () {
    ...
    test('无音频源分支调用 showNoAudioHint 而非静默的 showAudioError', () {
      ...
      expect(
        js.contains('function showNoAudioHint('),
        isTrue,
      ...
      expect(
        body.contains('showNoAudioHint(button)'),
        isTrue,
    ...
    test('showNoAudioHint 用本地化文案 + 生成 .audio-hint 提示 + 切静音态', () {
      ...
      final String body = js.substring(start, end < 0 ? js.length : end);
      expect(
        body.contains('window.i18nNoAudioAvailable'),
        isTrue,
        reason: '文案走宿主注入的 i18n（回退中文默认值）',
      );
      expect(
        body.contains("el('div', { className: 'audio-hint'"),
        isTrue,
        reason: '必须创建 .audio-hint 提示元素',
      );
      expect(
        body.contains("classList.add('audio-unavailable')"),
        isTrue,
    ...
    test('播放失败仍走透明 showAudioError（只改无音频反馈，不动播放路径）', () {
      ...
      expect(
        body.contains('showAudioError(button)'),
        isTrue,
  ...
  test('popup.css 定义 .audio-hint 与 .audio-button.audio-unavailable', () {
    ...
    bool definesSelector(String selector) =>
        RegExp('${RegExp.escape(selector)}\\s*[,{]').hasMatch(css);
    expect(
      definesSelector('.audio-hint'),
      isTrue,
      reason: '.audio-hint 必须作为选择器出现（可与 .fushi-btn-tip 分组）',
    );
    expect(
      definesSelector('.audio-hint.visible'),
      isTrue,
      reason: '.audio-hint.visible 必须定义可见态（可分组）',
    );
    expect(
      definesSelector('.audio-button.audio-unavailable'),
      isTrue,
