...
void main() {
  ...
  group('Dart controller: mine + duplicate bridge branches', () {
    ...
    test(
      '_onJsMessage routes mineEntry / duplicateCheck to deferred handlers',
      () {
        expect(
          src.contains("case 'mineEntry':"),
          isTrue,
          reason: '_onJsMessage 必须有制卡分支（过去根本没有 → ➕ 按钮永挂）',
        );
        expect(
          src.contains('_handleMineBridge('),
          isTrue,
        ...
        expect(
          src.contains('_handleDuplicateBridge('),
          isTrue,
    ...
    test(
      'mine handler goes through the real Anki repo and resolves the bridge',
      () {
        // 真走 AnkiConnect/AnkiDroid repo（非 UI 假动作），与 in-app onMineEntry 同路径。
        expect(
          src.contains('createAnkiRepository()'),
          isTrue,
          reason: '制卡/查重必须调真 Anki repo',
        );
        expect(
          src.contains('repo.mineEntry('),
          isTrue,
          reason: '制卡走 repo.mineEntry（AnkiConnect/AnkiDroid）',
        );
        expect(
          src.contains('repo.isDuplicate('),
          isTrue,
        ...
        expect(
          src.contains('resolveBridge(id, reply)'),
          isTrue,
  ...
  test('C++ overlay DEFERS mineEntry / duplicateCheck', () {
    ...
    expect(
      cpp.contains('body.find("\\"mineEntry\\"")'),
      isTrue,
      reason: 'mineEntry 必须纳入 deferred（不即时 null-resolve）',
    );
    expect(
      cpp.contains('body.find("\\"duplicateCheck\\"")'),
      isTrue,
