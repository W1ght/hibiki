  ...
  mirrors.forEach((String name, String root) {
    group('extension mirror [$name] $root', () {
      ...
      test('1184 manifest 设 default_popup 指向 action-popup.html', () {
        ...
        expect(
          src.contains('"default_popup"'),
          isTrue,
          reason: '$root manifest 未设 default_popup',
        );
        expect(
          src.contains('vendor/action-popup.html'),
          isTrue,
      ...
      test('1185 content.css 消费 --fushi-popup-zoom + --dict-columns', () {
        ...
        expect(
          src.contains('zoom: var(--fushi-popup-zoom'),
          isTrue,
          reason: '$root content.css 未消费 --fushi-popup-zoom',
        );
        expect(
          src.contains('repeat(var(--dict-columns'),
          isTrue,
      ...
      test('1185 content.js 定义 __fushiOnLinkClick（嵌套查词重发 lookup）', () {
        ...
        expect(
          src.contains('window.__fushiOnLinkClick = function'),
          isTrue,
          reason: '$root content.js 未定义 __fushiOnLinkClick（点释义里的词无反应）',
        );
        // place() 按 zoom 除算 fixed 定位坐标，避免 CSS zoom 放大偏移。
        expect(
          src.contains('(pos.left / zoom)') && src.contains('(pos.top / zoom)'),
          isTrue,
      ...
      test(
        '1190 content.js 强制 DOM 包裹高亮（__fushiCssHighlightsSupported=false）',
        () {
          final String src = content.readAsStringSync();
          expect(
            src.contains('window.__fushiCssHighlightsSupported = false'),
            isTrue,
      ...
      test('1881 action-popup.js 生成按钮状态机 + 清空 + 条目跳转 + 连接重检', () {
        final String src = actionJs.readAsStringSync();
        expect(
          src.contains('function fushiGenButtonState'),
          isTrue,
          reason: '$root action-popup.js 缺 fushiGenButtonState 状态机（回归成隐形三态按钮）',
        );
        expect(
          src.contains('function fushiTabSite'),
          isTrue,
          reason: '$root action-popup.js 缺 fushiTabSite 站点判定',
        );
        expect(
          src.contains('function fushiQueueItemUrl'),
          isTrue,
          reason: '$root action-popup.js 缺 fushiQueueItemUrl（队列条目跳回视频页）',
        );
        // 取消逃生口必须保留：batch active 时按钮可点（cancel 态）。
        expect(
          src.contains("mode: 'cancel'"),
          isTrue,
          reason: '$root fushiGenButtonState 丢了 cancel 逃生口',
        );
        // 清空队列（二次确认）。
        expect(
          src.contains("'hp-clear'"),
          isTrue,
        ...
        expect(
          src.contains('function refreshConnection'),
          isTrue,
          reason: '$root action-popup.js 连接探测未抽成 refreshConnection（行点击无法重检）',
        );
        expect(
          src.contains("connEl.addEventListener('click'"),
          isTrue,
