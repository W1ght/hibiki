...
void main() {
  ...
  group('M1a-1 autoRead wiring (global lookup controller)', () {
    ...
    test('autoRead is gated on the autoReadOnLookup preference', () {
      expect(
        shared.contains('ReaderFushiSource.instance.autoReadOnLookup'),
        isTrue,
      );
    });
    ...
    test('play step goes through the unified fast path (BUG-1127)', () {
      ...
      for (final String s in <String>[src, shared]) {
        expect(
          ...
          isFalse,
          reason:
              'the Dart fallback lives INSIDE autoReadWordUnified with the '
        ...
        expect(
          ...
          isFalse,
          reason:
              'global lookup must reuse autoReadWordUnified, not the '
    ...
    test('BUG-1127: WebView play reports the real audio.play() outcome', () {
      ...
      // playback).
      expect(
        shared.contains("handler != 'wordAudioPlayed'"),
        isTrue,
        reason: 'the shared handler must still dispatch on wordAudioPlayed',
      );
      expect(
        src.contains('_autoRead.maybeHandleWordAudioPlayed('),
        isTrue,
        reason: 'the controller must route the report into the shared handler',
      );
    ...
    test('autoRead fires on both first lookup and nested re-lookup', () {
      expect(
        ...
        reason:
            'one delegating wrapper + two call sites '
  ...
  group('M1a-2 swipe-close JS (pointer/mouse for desktop WebView2)', () {
    ...
    test('in-app popup webview reuses the shared constant (single truth)', () {
      ...
      expect(
        src.contains('_topPullReleaseJs = kPopupTopPullReleaseJs'),
        isTrue,
      );
    });
    ...
    test('top-pull swipe JS migrated off the retired single-frame render path', () {
      ...
      expect(
        src.contains('String buildOverlayRenderScript('),
        isFalse,
      ...
      final String shared = read(
        'lib/src/reader/popup_swipe_close_script.dart',
      );
      expect(
        shared.contains("callHandler('topPullReleased')"),
        isTrue,
        reason:
