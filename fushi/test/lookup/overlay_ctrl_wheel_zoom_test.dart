...
void main() {
  ...
  group('BUG-1139 (3) 源码守卫', () {
    ...
    test('app 外注入装 Ctrl+滚轮监听，回传净档数，且不就地改 zoom', () {
      ...
      expect(
        js.contains("callHandler('popupZoomFontStep'"),
        isTrue,
      ...
      expect(
        RegExp(
          r'options\.globalLookup\s*\?\s*_globalLookupZoomWheelJs\s*:\s*',
        ).hasMatch(dart),
        isTrue,
    ...
    test('host 测高把 iframe layout px 换算回 host CSS px（③ 几何闭环）', () {
      ...
      final String host = _repoFile(
        'fushi/assets/popup/global_lookup_host.js',
      ).readAsStringSync();
      expect(
        host.contains('function frameContentZoom('),
        isTrue,
        reason: '缺 zoom 读取：iframe 的 documentElement.style.zoom 是唯一来源',
      );
      expect(
        host.contains('layoutPx * frameContentZoom(record)'),
        isTrue,
        reason:
            'measureContentHeight 必须把 layout px 乘回 z，'
      ...
      expect(
        host.indexOf('layoutPx * frameContentZoom(record)'),
    ...
    test('瞬态覆盖窗接了这根 handler', () {
      for (final String path in <String>[
        ...
        expect(
          src.contains('maybeHandleOverlayZoomFontStep('),
          isTrue,
          reason: '$path 必须接 Ctrl+滚轮缩放',
        );
        expect(
          src.contains('onFontSizeChanged:'),
          isTrue,
