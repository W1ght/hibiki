...
void main() {
  test(
    ...
    () async {
      ...
      expect(
        ...
        0,
        reason:
            'BUG-732 wheel behavior test failed.\n'
            'stdout:\n${result.stdout}\nstderr:\n${result.stderr}',
      );
      expect(
  ...
  test('popup.js wheel guard bails on the extension host page (source guard)', () {
    ...
    for (final String path in popupCopies) {
      ...
      expect(
        src.contains('chrome.runtime && chrome.runtime.id'),
        isTrue,
      ...
      expect(
        src.contains('if (!scroller && inExtensionContentScript) return;'),
        isTrue,
      ...
      expect(
        guardIdx >= 0 && preventIdx > guardIdx,
        isTrue,
