...
void main() {
  group('Phase C 覆盖窗 resize 接线', () {
    test(
      'host.js — 瞬态 overlay ROOT 卡渲染右下角 resize grip 发 beginWindowResize',
      () {
        ...
        expect(
          src.contains('global-lookup-resize-grip'),
          isTrue,
          reason: 'grip 的 class/CSS 被删——覆盖窗拖不动',
        );
        expect(
          src.contains('function createResizeGrip'),
          isTrue,
          reason: 'createResizeGrip 接线被删',
        );
        expect(
          src.contains("postToHost('beginWindowResize'"),
          isTrue,
    ...
    test('controller — windowMoved 落 overlay 键（拖即解锁）且不串 app 内键', () {
      ...
      final String src = file.readAsStringSync();
      expect(
        src.contains("handler == 'windowMoved'"),
        isTrue,
        reason: 'overlay 未接 windowMoved 回报——拖完不落库',
      );
      expect(
        src.contains('resolveOverlayResizeFromDelta'),
        isTrue,
        reason: '倒推未走同源纯函数（增量折算）',
      );
      expect(
        src.contains('setOverlayLookupIndependentSize'),
        isTrue,
    ...
    test('C++ — 瞬态窗放开模态 resize + 拖拽期挂起 shell 区域裁剪', () {
      ...
      final String src = file.readAsStringSync();
      expect(
        src.contains('WM_ENTERSIZEMOVE'),
        isTrue,
        reason: '未在进入模态 size 循环时挂起区域裁剪',
      );
      expect(
        src.contains('resizing_'),
        isTrue,
