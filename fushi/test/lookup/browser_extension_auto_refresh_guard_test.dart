...
void main() {
  group('BUG-726 extension auto-refresh wiring', () {
    test('AppModel.initialise fires refreshBrowserExtensionCopy', () {
      ...
      expect(
        RegExp(
          'unawaited\\(\\s*refreshBrowserExtensionCopy\\(\\)',
        ).hasMatch(src),
        isTrue,
    ...
    test('installer refresh is conditional on an existing install', () {
      ...
      expect(
        body,
        contains('if (!dir.existsSync()) continue;'),
    ...
    test('legacy extension dir keeps being refreshed after the rename', () {
      ...
      expect(
        _functionBody(src, 'Future<Directory> _legacyExtensionDestDir'),
    ...
    test(
      'background.js self-reload is guarded (loop / recording / legacy)',
      () {
        final String src = File(
        ...
        final int nextBranch = src.indexOf(
          '} else if (msg.type === ',
          lookupBranch + 1,
        );
        expect(
          nextBranch,
          greaterThan(lookupBranch),
          reason: 'lookup 分支边界无法定位（后面必须还有别的 msg.type 分支）',
        );
        final String lookupBody = src.substring(lookupBranch, nextBranch);
        final int bodyRead = lookupBody.indexOf(
          RegExp(r'await r\.(text|json)\('),
        );
        ...
        expect(
          RegExp(
            r'JSON\.parse\(raw\)|const data = r\.ok \? await r\.json\(\)',
          ).hasMatch(lookupBody),
          isTrue,
    ...
    test('mirror copy of background.js stays byte-identical', () {
      ...
      final List<int> bundled = File(
        'assets/browser_extension/background.js',
      ).readAsBytesSync();
      final List<int> source = File(
