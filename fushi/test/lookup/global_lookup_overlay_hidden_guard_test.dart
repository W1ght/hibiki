  ...
  group('三端接线（源码扫描）', () {
    ...
    test(
      ...
      () {
        ...
        //   失败收尾 2 处：_lookupExternal 里 showAt 被拒 / 抛异常时把一张从未
        //     给用户看到的卡收掉（#1119）。这两处同样不是用户关闭，发
        //     overlayHidden 会把暂停的视频当成“用户关了卡”恢复播放。
        expect(
          'GlobalLookupChannel.hide(notify: false)'.allMatches(c).length,
          5,
          reason: '只有非用户关闭的前导/失败收尾才能用 notify:false',
        );
        // 反向判据（光数 notify:false 的条数可以被绕：把一条真用户关闭改成
        // 静默、再删掉一条前导，总数不变而语义已反）：_onJsMessage 的四条真实
        // 关闭路径（tapOutside 两分支、dismissPopupAt、topPullReleased）必须走默认
        // notify:true，否则暂停的视频永远恢复不了。
        expect(
