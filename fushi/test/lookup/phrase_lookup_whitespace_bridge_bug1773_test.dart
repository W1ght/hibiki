...
void main() {
    ...
    () async {
      ...
      try {
        ...
        expect(
          ...
          0,
          reason:
              'BUG-1773 whitespace-bridge behavior test failed.\n'
              'stdout:\n${result.stdout}\nstderr:\n${result.stderr}',
        );
  ...
  test(
    'forward scan uses isScanStop + whitespace bridge, not isScanBoundary',
    () {
      ...
      sources.forEach((String label, String src) {
        ...
        // 它把空白也当终点 → 短语被截断。
        expect(
          src.contains('if (this.isScanStop(char)) break;'),
          isTrue,
          reason: '[$label] 前向扫描必须以 isScanStop 为终点',
        );
        expect(
          src.contains('if (this.isScanBoundary(char)) break;'),
          isFalse,
        ...
        expect(
          ...
          isTrue,
          reason:
              '[$label] 空白桥接必须要求左边已有本节点扫入的内容（跨节点不粘空白）'
              '、且节点末尾的空白终止扫描',
        );
        expect(
          src.contains(
            'this.isScanWhitespace(nextChar) || '
        ...
        expect(
          src.contains('!this.isScanBoundary(hitContent[startOffset - 1])'),
          isTrue,
