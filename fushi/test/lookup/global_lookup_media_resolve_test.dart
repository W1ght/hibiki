...
void main() {
  ...
  group('TODO-867 P1 scheme registration symmetry (native vs in-app)', () {
    test('overlay WebView2 registers a superset of in-app media schemes', () {
      ...
      final String inApp = read(
        'lib/src/pages/implementations/dictionary_webview_media.dart',
      );
      ...
      final Set<String> inAppSchemes = RegExp(
        "'([a-z]+)'",
      ).allMatches(m!.group(1)!).map((Match e) => e.group(1)!).toSet();
      ...
      expect(
        wiredSchemes,
        containsAll(inAppSchemes),
        reason:
            'native overlay must WIRE every in-app media scheme into '
            'regs[]; in-app=$inAppSchemes wired=$wiredSchemes',
      );
      // And SetCustomSchemeRegistrations count must match the array size.
      final Match? count = RegExp(
        r'SetCustomSchemeRegistrations\((\d+),',
      ).firstMatch(cpp);
      expect(count, isNotNull);
      expect(
        int.parse(count!.group(1)!),
        wiredSchemes.length,
    ...
    test(
      'overlay routes both schemes to the media resolver (not passthrough)',
      () {
        final String cpp = read('windows/runner/global_lookup_window.cpp');
  ...
  group('TODO-867 P1 resolveGlobalLookupMedia (pure parse)', () {
    ...
    test('image:// — jpg/gif/webp/svg/unknown extension MIME', () {
      String? mime(String ext) => resolveGlobalLookupMedia(
        'image://?dictionary=d&path=a.$ext',
      )?.contentType;
    ...
    test('missing fields / unknown scheme -> null (served as 404)', () {
      expect(
        resolveGlobalLookupMedia('image://?path=a.png'),
        isNull,
      ); // no dict
