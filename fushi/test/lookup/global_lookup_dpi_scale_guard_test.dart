...
void main() {
  ...
  test('① DPI 只有一个入口，三项设置都在里面（BUG-1104）', () {
    expect(
      hdr.contains('void ApplyDpiScale();'),
      isTrue,
    ...
    final String fn = src.substring(
      i,
      src.indexOf('void GlobalLookupWindow::ConfigureWebView', i),
    );
    ...
    expect(
      fn.contains('GetDpiForWindow(hwnd_)'),
      isTrue,
  ...
  test('② 三项 put_ 在全文只出现一次（两条路径都从同一个入口走）', () {
    ...
    expect(
      ...
      3,
      reason:
          'composition 创建路径、windowed 回退路径、WM_DPICHANGED 三处都必须调；'
  ...
  test('③ WM_DPICHANGED 跟随新显示器，且不会弄可见预热中的隐藏窗', () {
    ...
    final String block = src.substring(
      i,
      src.indexOf('case WM_ENTERSIZEMOVE:', i),
    );
    expect(
    ...
    expect(
      block.contains('SWP_NOZORDER | SWP_NOACTIVATE'),
      isTrue,
      reason: '不动 Z 序、不抢焦点',
    );
    expect(
    ...
    expect(
      block.contains('ApplyRoundedRegion();'),
      isTrue,
  ...
  test('④ 既有机制不得被顺手改掉', () {
    expect(
      src.contains('void GlobalLookupWindow::RecoverDeadWebView'),
      isTrue,
      reason: 'BUG-693 的死 WebView 自愈重建',
    );
    expect(
      src.contains('put_IsStatusBarEnabled(FALSE)'),
      isTrue,
