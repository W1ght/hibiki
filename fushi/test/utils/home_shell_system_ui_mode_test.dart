...
void main() {
  ...
  test(
    'setHomeShellSystemUiMode emits a single setEnabledSystemUIMode call',
    () async {
  ...
  group('source guards', () {
    ...
    test('home-shell helper hides the Android status bar, keeps the nav bar', () {
      ...
      expect(
        fn.contains('SystemUiOverlay.top'),
        isFalse,
    ...
    test('closeMedia returns to the home-shell mode, not bare edgeToEdge', () {
      ...
      expect(
        fn,
        contains('setHomeShellSystemUiMode()'),
        reason: 'exiting media must restore the status-bar-hidden home shell',
      );
      expect(
        fn.contains(
          'SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge)',
