...
void main() {
  ...
  group('deriveSeriesDefaultName', () {
    ...
    test('CJK 第N巻 尾标记 → 公共前缀', () {
      expect(
        deriveSeriesDefaultName(const <String>[
          '転生したらスライムだった件 第1巻',
          '転生したらスライムだった件 第2巻',
        ], fallback: fb),
        '転生したらスライムだった件',
    ...
    test('无公共前缀 → fallback', () {
      expect(
        deriveSeriesDefaultName(const <String>['苹果之书', '香蕉物语'], fallback: fb),
        fb,
      );
    });
    ...
    test('公共前缀剥后只剩「第」等悬挂标记 → 不残留半截', () {
      ...
      final String r = deriveSeriesDefaultName(const <String>[
        '第1巻',
        '第2巻',
      ], fallback: fb);
      expect(r, fb);
    ...
    test('视频侧复用 parseVideoFilename 得系列名后再取公共前缀', () {
      ...
      expect(deriveSeriesDefaultName(series, fallback: fb), 'SPYxFAMILY');
    });
  });
}
