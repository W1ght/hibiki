...
void main() {
  group('LookupAutoReadCoordinator', () {
    ...
    LookupAutoReadCoordinator newCoordinator({
      ...
      return LookupAutoReadCoordinator(dedupeWindow: window, now: () => now);
    }
    ...
    test(
      'allows the same automatic playback after the window expires',
      () async {
        final LookupAutoReadCoordinator coordinator = newCoordinator();
        int playCount = 0;
    ...
    test('keeps reading and source in the automatic playback key', () async {
      ...
      expect(played, <String>['きょう/lookup', 'こんにち/lookup', 'きょう/preview']);
    });

    test(
      'propagates failures and does not keep failed playback deduped',
      () async {
        final LookupAutoReadCoordinator coordinator = newCoordinator();
        int attempts = 0;
    ...
    test('BUG-1127: a silent playback failure (play -> false) releases the '
