  ...
  group('sentence-audio ffmpeg routing guard (TODO-970)', () {
    test('TtsChannel.extractAudioSegment routes to ffmpeg on all platforms', () {
      ...
      expect(
        ...
        reason:
            'Sentence-audio cutting must delegate to the ffmpeg path so '
            'Android stops using the native Transformer handler (TODO-970).',
      );
      expect(
        body.contains("invokeMethod('extractAudioSegment'"),
        isFalse,
        reason:
            'The native MethodChannel sentence-audio handler '
      ...
      expect(
        body.contains('if (!_isSupported)'),
        isFalse,
        reason:
            'extractAudioSegment must not branch on platform anymore; '
            'both desktop and Android share the single ffmpeg path.',
      );
    });

    test('book sentence-audio container is platform-aware '
      ...
      expect(
        source,
        contains(r"'sentence.${immersionMiningAudioExtension()}'"),
        reason:
            'Book/audiobook sentence audio must derive its container '
      ...
      expect(
        source.contains("'sentence.aac'"),
        isFalse,
        reason:
            'Do not hardcode .aac for book sentence audio: iOS '
            'AnkiMobile leaves the localhost URL visible instead of '
            'embedding playable media (BUG-644).',
      );
      expect(
        source.contains("'sentence.m4a'"),
        isFalse,
        reason:
            'Do not switch book sentence audio to .m4a: the bundled '
            'desktop ffmpeg-min cannot mux it (BUG-460).',
      );
    });

    test('video sentence-audio container is platform-aware '
      ...
      expect(
        ...
        reason:
            'Immersion sentence audio must derive its container from '
      ...
      expect(
        engine.contains("immersion_audio.m4a'"),
        isFalse,
        reason:
            'Do not hardcode .m4a for immersion sentence audio: the '
            'bundled desktop ffmpeg-min cannot mux it (no ipod muxer) → '
            'exit -22 → silent cards (TODO-1217/BUG-460).',
      );
      expect(
        engine.contains("immersion_audio.aac'"),
        isFalse,
        reason:
            'Do not hardcode .aac either: iOS AnkiMobile leaves a raw '
            '.aac localhost URL as visible text (da22cd42a). Use the '
            'platform-aware helper.',
      );
      expect(
        engine,
        contains('await _audio('),
        reason:
            'Sentence audio must be cut via the ffmpeg audio extractor '
            '(_audio defaults to extractAudioSegmentViaFfmpeg), TODO-1000.',
      );
      expect(
        engine.contains('extractAudioSegmentViaFfmpeg'),
        isTrue,
        reason:
            'The engine audio extractor default must be the ffmpeg path '
