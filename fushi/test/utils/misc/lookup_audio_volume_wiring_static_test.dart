...
void main() {
  group('lookup audio volume wiring', () {
    test('auto-read routes through the unified WebView-first path with a '
      ...
      expect(
        ...
        reason:
            'the Dart fallback (playLookupAudio) must still pass the '
            'configured volume',
      );
      ...
      expect(
        ...
        reason:
            'autoReadWordUnified must reuse the single resolved ref for '
      ...
      expect(
        ...
        reason:
            'autoReadWordUnified must fall back to the Dart player with '
    ...
    test('manual + auto word audio applies the configured volume in the '
      ...
      expect(
        popupJs,
        contains('new Audio('),
        reason: 'popup.js must play word audio with an HTML5 <audio> element',
      );
      expect(
        popupJs,
      ...
      expect(
        ...
        reason:
            'the playWordAudio Dart bridge must be removed (unified to '
            '<audio>)',
      );
      expect(
        popupWebView,
        ...
        reason:
            'the popup must not round-trip word audio to a native/libmpv '
            'player',
      );
    });
    ...
    test('TtsChannel forwards volume to platform playback', () {
      ...
      expect(
        channel,
        contains('DesktopAudioPlayback.playFile(filePath, volume:'),
      );
    });

    test('Android and desktop audio backends apply volume before playback', () {
      final String desktop = _read(
      ...
      expect(
        android,
        contains('mp.setVolume(volume, volume)'),
        reason: 'Android must apply volume before prepare/start',
      );
    });

    test(
      'only automatic lookup playback is routed through the dedupe gate',
      () {
