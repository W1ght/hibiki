...
Future<Future<void> Function()> _lockExclusively(String dbPath) async {
  ...
  final Isolate isolate = await Isolate.spawn(_holdExclusiveLock, <Object?>[
    dbPath,
    ready.sendPort,
  ]);
...
void main() {
  group('BUG-1413: 库忙 vs 库里真没这个词（LocalAudioDb 层）', () {
    ...
    test(
      'queryMeta：真·未命中仍是 null，撞锁改抛 busy（两者不再同形）',
      () async {
        // 无锁基线：命中 = 记录，未命中 = null。
        expect(LocalAudioDb.queryMeta(dbPath, '勉強', 'べんきょう')?.file, 'a.mp3');
        expect(
          LocalAudioDb.queryMeta(dbPath, 'ないよ', ''),
          isNull,
        ...
        try {
          ...
          expect(
            ...
            throwsA(
              isA<LocalAudioUnavailableError>().having(
    ...
    test(
      'extractBlob：撞锁改抛 busy，不再吞成「这个词条没有音频」',
      () async {
    ...
    test(
      'TtsChannel.queryLocalAudio 把 busy 穿透 Isolate.run 边界',
      () async {
        ...
        expect(
          await TtsChannel.instance.queryLocalAudio('ないよ', ''),
          isNull,
          reason: '真·未命中仍是 null',
        );
  ...
  group('BUG-1413: WordAudioResolver 分开处置「没答上」与「没有这个词」', () {
    ...
    List<AudioSourceConfig> sourcesLocalThenRemote() => <AudioSourceConfig>[
      ...
      AudioSourceConfig.remoteAudio(url: 'https://remote.test/?term={term}'),
    ];
    ...
    test('本地库没答上：记一条用户可见错误日志，并继续下一源（不当成 miss）', () async {
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'BUG-1413：库在忙必须留下用户可见的痕迹，'
            '否则用户看到「暂无发音」永远不知道是数据库忙',
      );
      expect(
        added.map((ErrorLogEntry e) => e.source).join('\n'),
  ...
  group('BUG-1413 源码守卫：查询预算耗尽不得再被吞成同形 null', () {
    ...
    test(
      ...
      () {
        ...
        for (final Match m in 'on TimeoutException {'.allMatches(playbackSrc)) {
          ...
          expect(
            block,
            contains('throw const LocalAudioUnavailableError('),
            reason: 'BUG-1413：预算耗尽必须归成可区分的 unavailable',
          );
          expect(
            block.contains('return null;'),
            isFalse,
    ...
    test('查询预算是共享常量，不再各处手写 500ms 魔数', () {
      ...
      final String enhancement = File(
        'lib/src/creator/enhancements/local_audio_enhancement.dart',
      ).readAsStringSync();
      expect(
        enhancement,
        contains('kLocalAudioQueryBudget'),
        reason: '制卡链与查词链必须用同一个预算，否则两边行为会漂开',
      );
      expect(
        enhancement,
        contains('LocalAudioUnavailableError'),
    ...
    test('local_audio_db 对 BUSY 分类，且不靠调大 busy_timeout 掩盖', () {
      ...
      expect(
        'PRAGMA busy_timeout = 3000'.allMatches(dbSrc).length,
        2,
