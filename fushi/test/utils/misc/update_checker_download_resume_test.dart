...
void main() {
  group('downloadUpdateAsset cache and resume', () {
    ...
    test(
      'resumes an existing part file with Range and promotes atomically',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
        final UpdateDownloadPaths paths = UpdateDownloadPaths.forAsset(
    ...
    test('reports diagnostics for source switch and chunk progress', () async {
      ...
      expect(
        diagnostics
            .where((UpdateDownloadDiagnostics d) {
    ...
    test(
      'reports diagnostics for accepted resume and restart-from-zero',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
        final UpdateDownloadPaths resumePaths = UpdateDownloadPaths.forAsset(
          updatesDir,
          asset,
        );
        await resumePaths.partFile.writeAsBytes(
          payload.sublist(0, 4),
          flush: true,
        );
        await _writeMetadata(
          resumePaths.metadataFile,
          asset,
          sizeBytes: payload.length,
        );
        ...
        final Directory restartDir = await Directory.systemTemp.createTemp(
          'hibiki-update-restart',
        );
        addTearDown(() => _deleteDirectoryWithRetry(restartDir));
        final UpdateDownloadPaths restartPaths = UpdateDownloadPaths.forAsset(
          restartDir,
          asset,
        );
        await restartPaths.partFile.writeAsBytes(<int>[
          99,
          98,
          97,
          96,
    ...
    test(
      'restarts from zero when server ignores Range with HTTP 200',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
        final UpdateDownloadPaths paths = UpdateDownloadPaths.forAsset(
    ...
    test(
      'does not reuse a truncated final package even with an MZ header',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
        final UpdateDownloadPaths paths = UpdateDownloadPaths.forAsset(
    ...
    test('uses unique staging when the previous final exe is locked', () async {
      ...
      final RandomAccessFile lockedFinal = await paths.file.open(
        mode: FileMode.read,
      );
    ...
    test(
      'ignores an unavailable legacy fixed part path and stages freshly',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
        final UpdateDownloadPaths paths = UpdateDownloadPaths.forAsset(
    ...
    test(
      'resumes the same unique staging owner after a failed download',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
    ...
    test(
      'coalesces concurrent downloads for the same asset and version',
      () async {
        final List<int> payload = _payload();
        final UpdateAsset asset = _asset(payload);
    ...
    test(
      'downloadAndInstall active flow shares one in-flight operation',
      () async {
