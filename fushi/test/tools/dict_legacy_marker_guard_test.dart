...
void main() {
  test('query.cpp 的完成标记/版本判定同时接受新旧全部名字', () {
    ...
    final String addBody = src.substring(
      addIdx,
      (addIdx + 400).clamp(0, src.length),
    );
    expect(
      addBody,
      contains('dict_format_version'),
      reason: 'add_dict 必须用共享判定，否则兼容改了也够不着',
    );
    expect(
      addBody.contains('".fushidicts_1"'),
      isFalse,
