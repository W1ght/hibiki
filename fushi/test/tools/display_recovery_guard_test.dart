...
void main() {
  group('Windows runner display recovery contract (TODO-689)', () {
    final String win32 = File(
      'windows/runner/win32_window.cpp',
    ).readAsStringSync();
    final String win32Header = File(
    ...
    test(
      'win32_window handles display-change and monitor power-on messages',
      () {
        expect(
          win32,
        ...
        expect(
          win32,
          contains('InvalidateRect'),
          reason: 'Must invalidate the window so it repaints.',
        );
        expect(
          win32,
          contains('OnDisplayRecovered'),
    ...
    test('win32_window registers AND unregisters the power notification', () {
      expect(
        win32,
        contains('RegisterPowerSettingNotification'),
      ...
      expect(
        win32,
        contains('UnregisterPowerSettingNotification'),
        reason: 'Must release the power notification handle on Destroy().',
      );
      expect(
        win32Header,
        contains('HPOWERNOTIFY power_notify_'),
    ...
    test('flutter_window forces a redraw when the display recovers', () {
      ...
      expect(
        flutterWindow,
        contains('void FlutterWindow::OnDisplayRecovered'),
