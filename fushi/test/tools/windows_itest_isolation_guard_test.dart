...
void main() {
  group('Windows integration test isolation contract', () {
    ...
    test('does not kill or reject existing user Hibiki processes', () {
      ...
      expect(
        script,
        isNot(contains('close it first')),
        reason: 'A running user Hibiki instance is evidence, not a blocker.',
      );
      expect(
        script,
        isNot(contains('hibiki.exe is running')),
        reason: 'The script must not fail just because a user instance exists.',
      );
    ...
    test('only reaps isolated stale test-runner processes', () {
      final List<RegExpMatch> stopProcessMatches = RegExp(
        r'\bStop-Process\b',
      ...
      final int cleanupStart = script.lastIndexOf(
        r'foreach ($proc in $before)',
        killIndex,
      );
      ...
      final int snapshotStart = script.indexOf(
      ...
      expect(
        snapshotBlock,
        contains(r'$path = [string]$cim.ExecutablePath'),
        reason: 'The test-runner scope must be based on executable path.',
      );
      expect(
        snapshotBlock,
        contains(r'$path.StartsWith($RunnerPathPrefix,'),
        reason: 'The stale runner check must be scoped to this worktree.',
      );
      expect(
        snapshotBlock,
        contains('[System.StringComparison]::OrdinalIgnoreCase'),
        reason: 'The test-runner scope must stay path based, not name based.',
      );
      expect(
        snapshotBlock,
        contains(r'isTestRunner = $isRunner'),
    ...
    test(
      'runs with isolated test root, app data, logs, and WebView2 profile',
      () {
