...
void main() {
  ...
  test('controller：激活/backend 迁移路径接线 _syncTrackAutoRefresh（BUG-1027 ①）', () {
    ...
    for (final String activator in <String>[
      ...
      expect(
        ...
        greaterThan(at),
        reason:
            '$activator 内（或其后）必须调用 _syncTrackAutoRefresh——'
  ...
  test('controller：_stopSources 回收音轨刷新定时器', () {
    ...
    final int cancelAt = controllerSrc.indexOf(
  ...
  test('诊断页：「刷新音轨」按钮在活跃音轨卡片内、不在页头 actions', () {
    ...
    final String headerActions = pageSrc.substring(
      headerActionsAt,
      headerActionsEnd,
    );
    expect(
      headerActions.contains('game_refresh_tracks'),
      isFalse,
  ...
  test('诊断页：gameResource / Loopback 空轨走解释态分支（BUG-1027 ②）', () {
    ...
    expect(
      tracksCard.contains('GalAudioTracksPanel('),
      isTrue,
      reason: '诊断页音轨卡片必须接共享面板组件，不许再写第二份轨列表',
    );

    expect(
      panelSrc.contains('galTrackEmptyHintFor'),
      isTrue,
      reason: '空轨解释态必须按 audioBackend 分支（纯函数 galTrackEmptyHintFor）',
    );
    expect(
      panelSrc.contains('game_tracks_resource_mode_hint'),
      isTrue,
      reason: 'gameResource 空轨必须显示资源模式解释而非通用「尚无音轨数据」',
    );
    expect(
      panelSrc.contains('game_tracks_loopback_hint'),
      isTrue,
  ...
  test('诊断页：逐轨试听接线（grabUtterance 导出 + 播放/停止切换）', () {
    expect(
      pageSrc.contains('exportTrackPreview'),
      isTrue,
      reason: '音轨行必须能经 exportTrackPreview 抓取该轨片段试听',
    );
    expect(
      pageSrc.contains('game_track_preview_failed'),
      isTrue,
      reason: '试听失败必须 toast 反馈，不得静默',
    );
    final int previewAt = controllerSrc.indexOf(
      'Future<GalTrackPreview?> exportTrackPreview(',
    );
    expect(previewAt, greaterThan(0));
    final String previewBody = controllerSrc.substring(
      previewAt,
      previewAt + 2200,
    );
    expect(
      previewBody.contains('grabUtterance'),
      isTrue,
