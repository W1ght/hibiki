...
void main() {
  group('BUG-1483 WebView2 user-data fallback on read-only install dir', () {
    ...
    final String installerIss = File(
      'windows/installer/fushi.iss',
    ).readAsStringSync();

    test(
      'runner probes exe-dir writability and falls back to LOCALAPPDATA',
      () {
        // 回退函数存在，且尊重已设变量（itest harness 隔离，TODO-1003）。
        expect(
          mainCpp,
          contains('EnsureWritableWebView2UserDataFolder'),
        ...
        expect(
          mainCpp,
          contains('.fushi-webview2-write-probe'),
    ...
    test('fallback runs before the Flutter engine window is created', () {
      final int callIndex = mainCpp.indexOf(
    ...
    test('fork environment sites still honor the override env var', () {
      ...
      expect(
        inAppWebView,
        contains('FUSHI_WEBVIEW2_USER_DATA_FOLDER'),
