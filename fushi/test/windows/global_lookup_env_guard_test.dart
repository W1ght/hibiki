...
void main() {
  ...
  setUpAll(() {
    ...
    dartChannel = File(
      'lib/src/lookup/overlay_window_channel.dart',
    ).readAsStringSync();
  });
  ...
  group('TODO-1153 direction 1: overlay env uses a dedicated user data folder', () {
    test('a dedicated overlay user data folder helper exists', () {
      expect(
        cpp.contains('OverlayUserDataFolder('),
        isTrue,
    ...
    test('the env create consumes the dedicated folder', () {
      ...
      // 0x8007139F).
      expect(
        cpp.contains('overlay_folder = OverlayUserDataFolder(user_data_leaf_)'),
        isTrue,
      );
      ...
      expect(
        ...
        greaterThan(create),
        reason:
            'CreateCoreWebView2EnvironmentWithOptions must consume the '
  ...
  group('TODO-1153 direction 2: create failures are never swallowed', () {
    test('a single non-swallowing failure sink exists', () {
      expect(
        cpp.contains('void GlobalLookupWindow::ReportOverlayError('),
        isTrue,
    ...
    test('the synchronous env-create HRESULT is captured and checked', () {
      expect(
        cpp.contains('HRESULT create_hr ='),
        isTrue,
        reason:
            'the synchronous HRESULT (0x8007139F on conflict) must be '
    ...
    test('the env completion callback checks its HRESULT / null env', () {
      expect(
        cpp.contains('FAILED(env_hr) || env == nullptr'),
        isTrue,
        reason:
            'a failed environment create must be reported, not '
    ...
    test('the controller callback no longer returns S_OK silently on null', () {
      expect(
        cpp.contains('FAILED(ctrl_hr) || ctrl == nullptr'),
        isTrue,
      ...
      expect(
        bareReturn.hasMatch(cpp),
        isFalse,
        reason:
            'the silent ctrl==nullptr early return must be replaced by a '
            'reported one.',
      );
    });
    ...
    test('Dart surfaces native overlay errors via ErrorLogService', () {
      expect(
        dartChannel.contains("case 'nativeError':"),
        isTrue,
        reason: 'the channel must handle the native error report.',
      );
      expect(
        dartChannel.contains('ErrorLogService.instance.log('),
        isTrue,
        reason:
            'native overlay bring-up errors must reach ErrorLogService '
