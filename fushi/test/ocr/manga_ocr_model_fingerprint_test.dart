...
void main() {
  ...
  test('指纹由模型内容派生：同名同长度但内容不同 → 指纹不同', () async {
    ...
    final String? a = await resolveMangaOcrModelFingerprint(
      root,
      manifest: _manifest,
    );
    expect(a, isNotNull);
    ...
    await File(
      p.join(other.path, 'detector.onnx'),
    ).writeAsString('detector-v4');
    await File(p.join(other.path, 'encoder.onnx')).writeAsString('encoder-b');
    final String? b = await resolveMangaOcrModelFingerprint(
      other,
      manifest: _manifest,
    );

    expect(b, isNotNull);
    expect(b, isNot(a), reason: '换了模型权重却复用同一个缓存目录，全库结果会陈旧且无人察觉');
    expect(
  ...
  test('上游换模型后重下（内容变、mtime 变）→ 缓存签名变', () async {
    await writeModels('detector-v4', 'encoder-a');
    final String signatureBefore = await resolveLocalMangaOcrEngineSignature(
      root,
      manifest: _manifest,
    );
  ...
  test('内容不变时指纹稳定，且哈希被 (size, mtime) 记忆化', () async {
    await writeModels('detector-v4', 'encoder-a');
    final String? first = await resolveMangaOcrModelFingerprint(
      root,
      manifest: _manifest,
    );
    final File sidecar = File(
      p.join(root.path, kMangaOcrModelFingerprintFileName),
    );
    ...
    expect(
      (memo['files'] as Map<String, dynamic>).keys,
  ...
  test('损坏的记忆化文件不影响指纹正确性', () async {
    await writeModels('detector-v4', 'encoder-a');
    final String? expected = await resolveMangaOcrModelFingerprint(
      root,
      manifest: _manifest,
    );
    await File(
