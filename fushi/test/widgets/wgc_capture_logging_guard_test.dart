...
void main() {
  ...
  test(
    ...
    () {
      ...
      expect(
        src.contains('#include "../utils/wgc_log.h"'),
        isTrue,
        reason: 'texture_bridge.cc must include wgc_log.h',
      );
      expect(
        platformViewSrc.contains('#include "../utils/wgc_log.h"'),
        isTrue,
        reason:
      ...
      for (final String evt in <String>[
        ...
        expect(
          platformViewSrc.contains('WgcLog::Write("$evt"'),
          isTrue,
          reason:
      ...
      for (final String detail in <String>[
        ...
        expect(
          src.contains(detail) || platformViewSrc.contains(detail),
          isTrue,
          reason:
      ...
      expect(
        src.contains('WgcLog::Write("retire-remove-closed"'),
        isFalse,
        reason:
      ...
      expect(
        src.contains('WgcLog::Write("remove-before-close-fail"'),
        isFalse,
      ...
      expect(
        ...
        isFalse,
        reason: 'default WGC path must not remove an event it never registered',
      );

      final int retireStart = src.indexOf(
        'void TextureBridge::RetireFramePoolLocked(',
      );
      expect(retireStart, greaterThanOrEqualTo(0));
      final int retireEnd = src.indexOf(
        'void TextureBridge::',
        retireStart + 1,
      );
      final String retireBody = src.substring(retireStart, retireEnd);
      expect(
        retireBody.contains('WgcLog::Write("retire"'),
        isTrue,
        reason:
      ...
      final int removeTick = src.indexOf(
        'WgcLog::Write("pump-remove-tick-done"',
        pumpStop,
      );
      final int sessionCloseStart = src.indexOf(
        'WgcLog::Write("session-close-start"',
        removeTick,
      );
      final int poolCloseStart = src.indexOf(
      ...
      expect(
        pumpBody.contains('WgcLog::Write("frame-getfail"'),
        isTrue,
      ...
      expect(
        afterDeliver.contains('WgcLog::Write'),
        isFalse,
        reason:
  ...
  test(
    ...
    () {
      ...
      expect(
        mainDart.contains('WgcCaptureLog.foldIntoErrorLog()'),
        isTrue,
        reason:
