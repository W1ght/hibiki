...
void main() {
  group('FushiTag', () {
    test('uses design token chip radius for every rounded tag surface', () {
      ...
      expect(
        source,
        isNot(contains('BorderRadius.circular(tokens.radii.chip)')),
      );
    });
    ...
    testWidgets('does not render icon when null', (tester) async {
      await tester.pumpWidget(
        buildTestApp(
          const FushiTag(text: 'adj', backgroundColor: Colors.orange),
        ),
      );
    ...
    testWidgets('InkWell has no onTap when message is null', (tester) async {
      await tester.pumpWidget(
        buildTestApp(
          const FushiTag(text: 'no action', backgroundColor: Colors.grey),
        ),
      );
