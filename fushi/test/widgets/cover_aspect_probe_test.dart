...
void main() {
  ...
  group('共享内核', () {
    test('两个组件的 State 都 with CoverAspectProbe', () {
      for (final String path in componentPaths) {
        ...
        expect(
          containsCodeLine(src, 'with CoverAspectProbe<'),
          isTrue,
          reason: '$path 必须复用共享的宽高比探测内核，不得自建',
        );
        expect(
          containsCodeLine(src, 'ImageProvider probedImageOf('),
          isTrue,
  ...
  group('阈值真相源', () {
    test('横图阈值三处同值，且组件里不得再写裸字面量', () {
      expect(
        PortraitCoverImage.landscapeAspectThreshold,
        kCoverLandscapeAspectThreshold,
      );
      expect(
        LandscapeCoverImage.landscapeAspectThreshold,
        kCoverLandscapeAspectThreshold,
      );
      for (final String path in componentPaths) {
        final String src = File(path).readAsStringSync();
        expect(
          containsCodeLine(src, 'landscapeAspectThreshold = 1.2'),
          isFalse,
    ...
    test('压暗色同值，且竖槽自己的 0.85 判据必须保留（不是该收敛的东西）', () {
      ...
      expect(
        ...
        isFalse,
        reason:
            '竖槽默认判据（w/h > 0.85 才垫底）与横图判据是两个不同的东西，'
            '收敛不得把它一并抹成 1.2',
      );
    });
  ...
  group('内核行为：换 provider 的生命周期', () {
    ...
    testWidgets('换成另一朝向的图，解码完成后槽向判定跟着切换', (WidgetTester tester) async {
      final Uint8List portraitBytes = (await tester.runAsync<Uint8List>(
        () => pngBytes(20, 30),
      ))!;
      final Uint8List landscapeBytes = (await tester.runAsync<Uint8List>(
        () => pngBytes(32, 18),
      ))!;
    ...
    testWidgets('换 provider 的那一帧必须先丢掉旧宽高比，不能拿上一张图的朝向渲染', (
      ...
      final Uint8List otherBytes = (await tester.runAsync<Uint8List>(
        () => pngBytes(21, 31),
      ))!;
