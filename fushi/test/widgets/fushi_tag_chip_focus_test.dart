...
void main() {
  group('FushiTagChip default focusability', () {
    ...
    testWidgets(
      'a passive FushiTagChip (onTap == null) is not a focus target',
      (WidgetTester tester) async {
        await tester.pumpWidget(
          buildTestApp(
            const FushiFocusRoot(
              child: Column(children: <Widget>[FushiTagChip(label: 'Passive')]),
            ),
          ),
        );
        await tester.pump();
    ...
    testWidgets(
      ...
      (WidgetTester tester) async {
        ...
        expect(
          ...
          isNotNull,
          reason:
              'a deletable chip is a real gamepad target, not just a '
