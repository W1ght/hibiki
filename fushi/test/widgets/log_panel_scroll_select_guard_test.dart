...
void main() {
  ...
  test('FushiLogPanel source uses ListView.builder + SelectionArea on the '
  ...
  // 把「退化成视口复制」钉死在源码层。
  test('copy-all / share route through full widget.log, not the viewport '
    ...
    expect(
      panel,
      contains('Clipboard.setData(ClipboardData(text: widget.log))'),
      reason: '复制全部必须复制 widget.log 全量',
    );
    // 始终可见的「复制全部」入口存在。
    expect(
      panel,
      contains('t.log_copy_all'),
  ...
  group(
    ...
    () {
      test(
        'drag inactive -> always allow (non-selection scroll untouched)',
        () {
          expect(
      ...
      test('drag active + downward ANIMATED scroll -> ALLOW '
      ...
      test('drag active + upward ANIMATED scroll -> ALLOW '
          '(edge auto-scroll extends selection)', () {
        expect(
          logSelectionScrollDecision(
            pointerSelectionActive: true,
      ...
      test('drag active + downward JUMP scroll -> BLOCK '
      ...
      test('drag active + upward JUMP scroll -> BLOCK '
  ...
  testWidgets(
    ...
    'not throw (line Text width is bounded)',
    (WidgetTester tester) async {
  ...
  test(
    ...
    () {
      ...
      expect(
        panel,
        contains('maxWidth: constraints.maxWidth'),
      ...
      expect(
        panel,
        contains('copy-all to clipboard failed'),
  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      expect(
        maxExtent,
        greaterThan(2000),
        reason: '5000 行应产生远超视口的可滚动范围（虚拟化前提）',
      );
      double target = 0;
      ...
      expect(
        maxRenderedDuringScroll,
        lessThan(500),
        reason:
            '滚动期间渲染行数（≈ Selectable 数）必须有界；接近已滚过总行数说明'
  ...
  test(
    ...
    () {
      ...
      expect(
        source,
        contains('_allowProgrammaticScroll(value, animated: false)'),
