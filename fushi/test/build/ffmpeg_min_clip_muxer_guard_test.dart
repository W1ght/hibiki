...
void main() {
  ...
  test('ffmpeg-min build whitelist enables the mov muxer for clip export', () {
    ...
    expect(
      ...
      contains('mov'),
      reason:
          'clip video synth writes .mov (mjpeg+aac); without the mov '
  ...
  test('ffmpeg-min build whitelist enables the mp4 muxer for video clips', () {
    ...
    expect(
      ...
      contains('mp4'),
      reason:
          'video clip export targets .mp4 (h264/hevc copy or libx264 '
          're-encode); without the mp4 muxer it exits -22. BUG-917.',
    );
    expect(
      list,
      isNot(contains('matroska')),
      reason:
          'ffmpeg-min has no matroska muxer — proof the clip export must '
  ...
  test(
    'video clip export names the output .mp4, never the source container',
    () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'clip export must write .mp4 (ffmpeg-min muxable + universal). '
            'BUG-917.',
      );
      expect(
        clip.contains('p.extension(inputPath)'),
        isFalse,
        reason:
            'clip output extension must NOT follow the source container — '
  ...
  test('audiobook clip pipeline emits .aac audio, never .m4a/.mp4', () {
    ...
    expect(
      pipeline,
      contains(r"outputPath: '$base.aac'"),
      reason:
          'sentence audio must be written to .aac (adts); .m4a needs the '
          'absent ipod/mov muxer → exit -22. BUG-460.',
    );
    expect(
      pipeline.contains(r"'$base.m4a'"),
      isFalse,
      reason:
          'clip pipeline must not write .m4a (no m4a/ipod muxer in the '
          'bundled ffmpeg-min build). BUG-460.',
    );
    expect(
      pipeline.contains(r"'$base.mp4'"),
      isFalse,
  ...
  test('immersion audio extension is desktop/Android=aac, iOS=m4a', () {
    expect(
      immersionMiningAudioExtensionFor(isIOS: false),
      'aac',
      reason:
          'Desktop + Android must use .aac (adts): the bundled '
          'ffmpeg-min has no ipod/mp4 muxer, so .m4a exits -22 (TODO-1217).',
    );
    expect(
      immersionMiningAudioExtensionFor(isIOS: true),
      'm4a',
      reason:
          'iOS AnkiMobile only auto-downloads media-recognized URLs; it '
  ...
  test('the desktop immersion audio container is muxable by ffmpeg-min', () {
    ...
    expect(
      ...
      contains('adts'),
      reason:
          'desktop immersion clip audio writes .aac (adts container); '
    ...
    expect(
      ...
      isNot(contains('ipod')),
      reason:
          'ffmpeg-min still has no ipod muxer — proof the desktop branch '
          'must NOT use .m4a (TODO-1217).',
    );
  });

  test('immersion engine + capture channel derive the audio container from the '
      'platform-aware helper, never a hardcoded .m4a/.mp4', () {
    final String engine = libFile(
      'lib/src/mining/immersion_mining_engine.dart',
    );
    final String capture = libFile(
    ...
    for (final MapEntry<String, String> e in <String, String>{
      ...
      expect(
        e.value.contains(r'.${immersionMiningAudioExtension()}'),
        isTrue,
        reason:
            '${e.key} must build the clip audio filename from '
      ...
      expect(
        ...
        isFalse,
        reason:
            '${e.key} must not hardcode a .m4a clip audio name — the '
