...
void main() {
  ...
  test('vendored Windows ffmpeg.exe is committed and is a real PE binary', () {
    ...
    expect(
      ...
      isTrue,
      reason:
          'vendored Windows ffmpeg must exist at '
    ...
    expect(
      ...
      reason:
          'vendored ffmpeg.exe is suspiciously small '
  ...
  test('vendored Windows dir ships only the exes, no side-car runtime DLLs', () {
    ...
    expect(
      names,
      <String>['ffmpeg.exe', 'ffprobe.exe'],
      reason:
          'third_party/ffmpeg-min/windows must contain exactly the two '
  ...
  test('vendored Windows exes import only system DLLs', () {
    ...
    for (final File exe in exes) {
      ...
      expect(
        ...
        reason:
            'DLL-name scan of ${exe.path} found nothing recognisable; '
            'the guard would pass vacuously',
      );
      expect(
        referenced.difference(systemDlls),
        isEmpty,
        reason:
            '${exe.path} references non-system DLLs '
  ...
  test(
    ...
    () {
      ...
      expect(
        ...
        reason:
            'Windows job must source ffmpeg from the vendored third_party '
            'path (TODO-416)',
      );
      expect(
        windowsJob,
      ...
      expect(
        ...
        reason:
            'the install step must copy ffprobe.exe into the bundle '
      ...
      expect(
        ...
        isNotNull,
        reason:
            'could not locate the \$runtimeFiles copy list in the Windows '
      ...
      for (final String dead in <String>['libwinpthread-1.dll', 'zlib1.dll']) {
        expect(
          ...
          isNot(contains(dead)),
          reason:
              '$dead is no longer vendored (the Windows recipe links '
      ...
      expect(
        ...
        reason:
            'vendored ffmpeg must be installed into the Windows Release '
      ...
      expect(
        ...
        contains('Copy-Item'),
        reason:
            'the install step must copy the vendored ffmpeg into the '
            'bundle',
      );
      ...
      final int installIndex = windowsJob.indexOf(
        'Install vendored ffmpeg-min runtime into Windows bundle',
      );
      final int innoIndex = windowsJob.indexOf(
  ...
  test(
    'release-desktop Windows job no longer fetches ffmpeg cross-workflow',
    () {
      final String workflow = readWorkflow('release-desktop.yml');
      ...
      expect(
        workflow,
        isNot(contains('gh run download')),
        reason:
            'must not download ffmpeg artifact from another workflow run '
            '(it expires after 90 days -> Windows ships without ffmpeg)',
      );
      expect(
        workflow,
        isNot(contains('ffmpeg-min.yml')),
        reason:
            'release-desktop must not reference the ffmpeg-min workflow '
            'for artifacts; the binary is vendored (TODO-416)',
      );
      expect(
        workflow,
        isNot(contains('ffmpeg_min_run_id')),
        reason:
            'the cross-workflow run-id input is dead once ffmpeg is '
            'vendored',
      );
    },
  );
  ...
  test('vendored Windows ffmpeg.exe is a CI build, not a local dev build', () {
    ...
    expect(
      ...
      isTrue,
      reason:
          'vendored ffmpeg.exe must be built by ffmpeg-min.yml on a '
    ...
    expect(
      ...
      isFalse,
      reason:
          'the crashing V35 local-build marker /d/ffmpeg715 must never '
  ...
  test('ffmpeg-min workflow has no deprecated branch push trigger', () {
    ...
    expect(
      ...
      reason:
          'the temporary validation branch push trigger must be removed; '
