...
void main() {
  ...
  test('Dart 侧仍在 app bundle 的固定路径上找 Aidoku runtime', () {
    final String runtime = read(
      'lib/src/media/manga/aidoku/aidoku_runtime.dart',
    );
  ...
  test('两条 macOS job 都把 Aidoku runtime 装进 bundle 并验证', () {
    for (final String path in <String>[releaseWorkflow, multiWorkflow]) {
      ...
      expect(
        job,
        contains('tool/aidoku/verify_macos_runtime.sh'),
        reason:
            '$path 的 macos job 缺硬门：没跑 verify 就没人能发现 runtime '
            '没装进去。',
      );
      expect(
        job,
        contains(r'Contents/Resources/aidoku_runtime'),
  ...
  test('发布 job 在 Developer ID 签名之前装 Aidoku runtime', () {
    final String job = extractJob(
      maskHashComments(read(releaseWorkflow)),
      'macos',
    );
    ...
    expect(
      ...
      lessThan(signed),
      reason:
          'Developer ID 那一步按 Mach-O 判定重签 bundle 里每个可执行体。'
  ...
  test('发布 job 用 app 本体核对 runtime 的架构覆盖', () {
    ...
    expect(
      ...
      reason:
          'BUG-1668 同形：runner 是 Apple Silicon，arm64-only 的 helper '
  ...
  test('发布 job 不得把 Aidoku runtime 降级成 host 架构', () {
    ...
    expect(
      job,
      isNot(contains('FUSHI_AIDOKU_ARCHS')),
      reason:
          '发布包必须走脚本默认的 universal。降级成 host 最终会被同一步里的'
  ...
  test('构建脚本默认出 universal，不是 runner 的 host 架构', () {
    ...
    expect(
      script,
      contains(r'${FUSHI_AIDOKU_ARCHS:-universal}'),
    ...
    expect(
      script,
      contains(r'--target "$rust_target"'),
      reason: '裸 cargo build 只出 host 架构。',
    );
    expect(script, contains('lipo -create'), reason: '多架构产物必须合成一个 fat 二进制。');
    expect(
      script,
      isNot(contains(r'/target/release/fushi-aidoku-runtime')),
  ...
  test('verify 脚本自己就是硬门：存在性 + 架构覆盖 + 真启动', () {
    ...
    expect(script, contains('lipo -archs'), reason: '架构覆盖核对是这道门的主要价值。');
    expect(
      script,
      contains('usage: fushi-aidoku-runtime'),
