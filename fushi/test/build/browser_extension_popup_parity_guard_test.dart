...
void main() {
  ...
  group('vendor/content.css stays complete vs popup.css (TODO-1267)', () {
    ...
    for (final String root in const <String>[extAssets, extTools]) {
      test('[$root] content.css contains every popup.css class selector', () {
        final String content = maskCssComments(
          read('$root/vendor/content.css'),
        );
        final List<String> missing = wanted
            .where((String sel) => !content.contains(sel))
            .toList();
        expect(
          missing,
          isEmpty,
      ...
      test('[$root] content.css keeps the extension-only overlay', () {
        ...
        expect(
          content.contains('#fushi-subtitle-panel'),
          isFalse,
          reason:
              '$root/vendor/content.css must not restyle an in-page subtitle panel',
        );
        expect(
          content.contains('--fushi-popup-max-width'),
          isTrue,
          reason:
      ...
      test('[$root] content.css re-roots stay specificity-neutral (BUG-752)', () {
        ...
        expect(
          ...
          isTrue,
          reason:
              '$root/vendor/content.css lost the :where() universal '
              'reset (BUG-752)',
        );
        ...
        final RegExp bareIdReroot = RegExp(
        ...
        expect(
          ...
          isNull,
          reason:
              '$root/vendor/content.css re-introduced a bare-ID re-root '
    ...
    test('scrollbars hidden via ::-webkit-scrollbar also hide via the standard '
        'property (BUG-775)', () {
      ...
      expect(
        ...
        isNotEmpty,
        reason:
            'expected at least .expression-scroll to hide its scrollbar '
      ...
      for (final String base in bases) {
        ...
        final RegExp companion = RegExp(
          '(^|[,\\s}])${RegExp.escape(base)}'
          r'\s*(,[^{]*)?\{[^}]*scrollbar-width:\s*none[^}]*\}',
        );
        expect(
          companion.hasMatch(css),
          isTrue,
          reason:
              'popup.css hides "$base" scrollbars only via '
    ...
    test('the two extension content.css mirrors are byte-identical', () {
      expect(
        bytes('$extAssets/vendor/content.css'),
