...
void main() {
  ...
  test('前置：两条 workflow 都在', () {
    expect(
      releaseDesktop.existsSync(),
      isTrue,
      reason: 'expected ${releaseDesktop.absolute.path}',
    );
    expect(
      validation.existsSync(),
      isTrue,
      reason: 'expected ${validation.absolute.path}',
    );
  });

  final String releaseText = releaseDesktop.existsSync()
      ? releaseDesktop.readAsStringSync()
      : '';
  final String validationText = validation.existsSync()
      ? validation.readAsStringSync()
      : '';

  test('Windows 发布构建带 -v，失败时真错误不会被 MSBuild 折叠掉', () {
    expect(
      releaseText,
      contains('flutter build windows --release'),
  ...
  test('Windows 发布构建失败时会把 CMake 日志传成 artifact', () {
    expect(
      releaseText,
      contains('CMakeFiles/CMakeError.log'),
      reason:
          '-v 打的是 flutter 侧；CMake 配置期的报错只在 CMakeOutput.log / '
  ...
  test('Windows 发布构建前必须抬高 gen_snapshot 栈保留（BUG-1589 根因）', () {
    ...
    expect(
      ...
      reason:
          '栈补丁步骤必须排在 Windows release 构建之前，否则首轮构建仍然'
          '用未打补丁的 gen_snapshot。',
    );
  });
  ...
  test('验证构建是 debug、发布构建是 release —— 别把前者的绿当后者的绿', () {
    ...
    expect(
      validationText,
      contains('flutter build windows --debug'),
      reason:
          '验证构建原本是 --debug（不走 AOT）。若已改成 --release，'
