...
void main() {
  ...
  setUpAll(() {
    ...
    final String overlayCpp = File(
      'windows/runner/global_lookup_window.cpp',
    ).readAsStringSync();
    final int overlayStart = overlayCpp.indexOf(
      'void GlobalLookupWindow::ForwardCompositionMouse',
    );
    expect(
      overlayStart,
      greaterThanOrEqualTo(0),
  ...
  group('BUG-870 native sendScroll carries a sub-unit remainder', () {
    test('reads a per-axis residual and carries the truncated fraction', () {
      ...
      expect(
        sendScrollBody.contains('scrollResidualX_'),
        isTrue,
        reason: 'horizontal axis must carry its own sub-unit remainder',
      );
      expect(
        sendScrollBody.contains('scrollResidualY_'),
        isTrue,
        reason: 'vertical axis must carry its own sub-unit remainder',
      );
      expect(
        sendScrollBody.contains('residual = scaled - offset'),
        isTrue,
    ...
    test(
      'skips sending a wheel only when the accumulated offset is still zero',
      () {
        ...
        expect(
          sendScrollBody.contains('if (offset == 0)'),
          isTrue,
          reason:
    ...
    test('does not truncate the raw delta directly to short (the old bug)', () {
      ...
      expect(
        sendScrollBody.contains(
          'static_cast<short>(delta * kScrollMultiplier)',
        ),
        isFalse,
        reason:
            'truncating the raw delta*multiplier drops small touchpad '
            'frames to 0 (BUG-870); it must accumulate a residual first',
      );
      expect(
        sendScrollBody.contains('static_cast<short>(scaled)'),
        isTrue,
  ...
  group('BUG-1065 wheel delta 与 app 外 overlay 同尺度', () {
    test('in-app sendScroll 按 deviceScaleFactor_ 还原物理像素后再乘倍数', () {
      expect(
        sendScrollBody.contains('deviceScaleFactor_'),
        isTrue,
        reason:
            '滚轮 delta 是逻辑像素，必须用 deviceScaleFactor_'
      ...
      // 整体偏掉——所以这里显式钉死用的是哪一个。
      expect(
        sendScrollBody.contains('captureScaleFactor_'),
        isFalse,
        reason:
            '滚轮还原的是设备 DPR，不是纹理采集缩放；拿错了在 dpr == 采集'
            '缩放的机器上完全看不出来',
      );
      expect(
        RegExp(
          r'delta \* kScrollMultiplier \* \w+ \+ residual',
    ...
    test('deviceScaleFactor_ 为 0/未初始化时回退 1.0，绝不把 wheel 归零', () {
      expect(
        sendScrollBody.contains('1.0'),
        isTrue,
        reason: 'deviceScaleFactor_ 非法（<=0）时必须回退 1.0，否则一乘 0 滚轮全死',
      );
      expect(
        RegExp(r'deviceScaleFactor_ > 0').hasMatch(sendScrollBody),
        isTrue,
    ...
    test('对照端：app 外 overlay 仍原样转发系统 WHEEL_DELTA', () {
      ...
      expect(
        RegExp(
          r'GET_WHEEL_DELTA_WPARAM\(wparam\)\s*[*/]',
