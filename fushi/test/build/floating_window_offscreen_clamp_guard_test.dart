...
void main() {
  ...
  group('TODO-832 Windows strip off-screen clamp', () {
    ...
    test('the drag (WM_MOUSEMOVE) path clamps against the cursor monitor', () {
      ...
      expect(
        cpp.substring(move, leave).contains('MoveBodyTo('),
        isTrue,
      ...
      expect(
        primitive.contains('MonitorFromPoint('),
        isTrue,
  ...
  group('TODO-832 Android overlay off-screen clamp', () {
    test('BaseFloatingService declares the 48dp margin + clampToScreen', () {
      ...
      expect(
        base.contains('MIN_VISIBLE_DP = 48'),
        isTrue,
        reason: 'min-visible margin must be a single 48dp constant.',
      );
    ...
    test('setupOverlay re-clamps the restored saved position', () {
      ...
      final int next = base.indexOf(
        'protected WindowManager.LayoutParams '
        'createLayoutParams()',
      );
      ...
      expect(
        body.contains('clampToScreen()'),
        isTrue,
        reason: 'a saved off-screen position must be pulled back on restore.',
      );
      expect(
        body.contains('rootView.post('),
        isTrue,
    ...
    test(
      'clampToScreen uses WindowMetrics/DisplayMetrics + fixed-width source',
      () {
        final String base = readJava('BaseFloatingService.java');
        ...
        expect(
          body.contains('getCurrentWindowMetrics()'),
          isTrue,
          reason: 'API30+ screen bounds source.',
        );
        expect(
          body.contains('getRealMetrics('),
          isTrue,
        ...
        // them off-screen.
        expect(
          body.contains('layoutParams.width > 0'),
          isTrue,
          reason:
              'fixed layoutParams.width is authoritative, not view '
