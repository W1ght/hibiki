...
void main() {
  ...
  test('desktop icon entry is gated so updates never rewrite it', () {
    ...
    expect(
      ...
      isTrue,
      reason:
          'desktop icon must be guarded by Check: ShouldCreateDesktopIcon '
    ...
    expect(
      ...
      isTrue,
      reason:
          'desktop icon must be an optional "desktopicon" task. '
          'Line: $line',
    );
  });
  ...
  test('ShouldCreateDesktopIcon skips creation when the shortcut exists', () {
    ...
    expect(
      ...
      isTrue,
      reason:
          'ShouldCreateDesktopIcon must return '
  ...
  test('desktopicon task is declared and defaults to checked', () {
    ...
    expect(
      ...
      isFalse,
      reason:
          'desktopicon must default to checked to preserve the '
  ...
  test(
    'no [Code] block comment embeds an Inno constant that closes it early',
    () {
      final String iss = readInstallerScript();
      ...
      final RegExpMatch? nextSection = RegExp(
        r'^\[[A-Za-z]',
        multiLine: true,
      ...
      expect(
        ...
        isEmpty,
        reason:
            '[Code] 段的 { } 块注释里嵌入了会提前闭合它的 Inno 常量（内层 {…}），'
