...
void main() {
  ...
  setUpAll(() {
    ...
    expect(
      ...
      reason: 'setPointerUpdate must exist in the vendored in_app_webview.cpp',
    );
    final int end = cpp.indexOf(
      'void InAppWebView::setPointerButtonState',
      start,
    );
    expect(
      end,
  ...
  group('BUG-871 injected touch flags a primary contact', () {
    test('the first active contact is tracked and flagged PRIMARY', () {
      expect(
        setPointerBody.contains('primaryTouchPointerId_'),
        isTrue,
        reason: 'the first active touch contact must be tracked',
      );
      expect(
        setPointerBody.contains('POINTER_FLAG_PRIMARY'),
        isTrue,
        reason:
            'the primary contact must carry POINTER_FLAG_PRIMARY so '
            'Chromium recognises a pan/scroll manipulation',
      );
      // Only the primary contact gets the flag; it is cleared on its own up.
      expect(
        setPointerBody.contains('pointer == primaryTouchPointerId_'),
        isTrue,
        reason: 'only the primary contact may carry the flag',
      );
      expect(
        setPointerBody.contains('primaryTouchPointerId_ = -1'),
        isTrue,
        reason:
            'the primary must be released on its up so a later gesture '
            'can claim it',
      );
    });
    ...
    test('a cancelled touch is forwarded as an up (no stranded primary)', () {
      ...
      final String cancelBody = customView.substring(
        cancel,
        nextHandler > cancel ? nextHandler : customView.length,
      );
      expect(
        cancelBody.contains('PointerDeviceKind.touch'),
        isTrue,
        reason: 'a cancelled touch must be handled distinctly',
      );
      expect(
        cancelBody.contains('InAppWebViewPointerEventKind.up'),
        isTrue,
