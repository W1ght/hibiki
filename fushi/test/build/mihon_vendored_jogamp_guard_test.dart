...
void main() {
  group('vendored org.jogamp 离线仓库', () {
    test('4 个构件存在且字节与上游一致', () {
      expect(
        Directory(_jogampRoot).existsSync(),
        isTrue,
        reason:
            '$_jogampRoot 不存在 —— vendored 仓库被删了？'
      ...
      _artifactSha256.forEach((String relative, String expected) {
        ...
        expect(
          ...
          expected,
          reason:
              '$relative 的字节变了（实际 $actual）。vendored 构件必须与上游'
    ...
    test('.sha1 伴生文件与构件自洽（Gradle 解析本地仓库时会自校验）', () {
      _artifactSha256.forEach((String relative, String _) {
        ...
        expect(
          ...
          actual,
          reason:
              '$relative.sha1 与实际不符（声明 $declared / 实际 $actual）。'
    ...
    test('没有夹带 native classifier jar（依赖链里没有它们，多带就是白占体积）', () {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'jcef 的 POM 只声明 gluegen-rt 与 jogl-all 两个主构件，'
  ...
  group('构建补丁的仓库声明', () {
    ...
    test('声明了构建树内的本地仓库，路径与脚本逐字一致', () {
      expect(
        patch.contains(_inTreeRepoPath),
        isTrue,
        reason:
            '补丁里找不到 "$_inTreeRepoPath"。补丁、ps1、sh 三处路径必须逐字'
            '相同，否则 Gradle 找不到本地仓库、静默回落远端。',
      );
      expect(
        patch.contains('rootProject.file("$_inTreeRepoPath")'),
        isTrue,
        reason:
            '本地仓库必须用 rootProject.file("$_inTreeRepoPath") 定位；'
    ...
    test('本地仓库排在两个远端镜像之前（顺序即优先级）', () {
      ...
      final int scijava = added.indexOf(
        'maven("https://maven.scijava.org/content/groups/public")',
      );
      final int jogamp = added.indexOf(
      ...
      expect(
        ...
        lessThan(scijava),
        reason:
            '本地仓库必须排在 scijava 之前，否则每次构建仍然先打那台'
            '2026-08-25 超时过的主机，vendored 就白做了。',
      );
      expect(local, lessThan(jogamp), reason: '本地仓库必须排在 jogamp.org 之前（同上）。');
    });
    ...
    test('三个 org.jogamp 仓库都带 content filter', () {
      ...
      final int filters = RegExp(
        r'includeGroupByRegex',
      ).allMatches(patch).length;
      expect(
        filters,
        3,
        reason:
            '期望本地 + scijava + jogamp.org 三处各有一个 content filter，'
            '实际 $filters 处。',
      );
    });
  });
  ...
  group('两个构建脚本的搬运', () {
    ...
    test('ps1 把 third_party/jogamp 搬进构建树，并在缺失时硬失败', () {
      expect(
        ps.contains(r'third_party\jogamp'),
        isTrue,
        reason:
            'ps1 里找不到源目录 third_party\\jogamp（已剥注释，'
            '所以不是被解释性注释骗到的）。',
      );
      expect(
        ps.contains(r'hibiki-offline-maven\jogamp'),
        isTrue,
        reason:
            'ps1 里找不到目标路径 hibiki-offline-maven\\jogamp；'
            '它必须与补丁里的 "$_inTreeRepoPath" 指同一处。',
      );
      expect(
        ps.contains('throw "Vendored org.jogamp repository is missing'),
        isTrue,
        reason:
            'ps1 必须在 vendored 仓库缺失时 throw。静默继续 = 悄悄回落到'
            '那两台主机，病灶复活且没人看得见。',
      );
    });
    ...
    test('sh 把 third_party/jogamp 搬进构建树，并在缺失时硬失败', () {
      expect(
        sh.contains('third_party/jogamp'),
        isTrue,
        reason: 'sh 里找不到源目录 third_party/jogamp（已剥注释）。',
      );
      expect(
        sh.contains('hibiki-offline-maven/jogamp'),
        isTrue,
