...
class ReaderPaginationScripts {
  ...
  /// BUG-2192：允许回吃前一条命中尾巴的字数（与 JS `OVERLAP`、matcher
  /// `EpubSrtMatcher.cueTailOverlap` 同值）。
  static const int kSentenceAudioTailOverlap = 4;

  /// BUG-2192：「延续」判定的前向容差（与 JS `ADJACENT` 同值）：紧接前一条命中
  ...
  ///
  /// 规则：游标 `cursor` 单调（BUG-2192 起允许回到前一条命中的尾巴 [kSentenceAudioTailOverlap]
...
window.__fushiInstallShell = function(C) {
  ...
  collectSentenceAudioCueRanges: function(cues) {
    ...
    // 不一致导致的累积偏移。不变量：① 游标 cursor 单调不回退（BUG-2192 起允许
    ...
    //
    // BUG-2192：ASR 字幕前句多带下一句首字（「…高い所。と」+「とはいえ、」）时，
