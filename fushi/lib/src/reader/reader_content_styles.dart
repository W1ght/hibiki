...
class ReaderContentStyles {
  ...
  // 保持，列底边 = V−mb−cB（漏 0）。
  //
  // BUG-2457：padding-bottom 曾再多加一个字号 F（`+ ${fontSize}px`，列高相应
  ...
  /// V=视口高，F=字号（只作坍塌地板，不再从列高里扣，BUG-2457），mt/mb=上下页边距(px)，
  ...
  /// BUG-2459：WebKit（macOS / iOS WKWebView）把**首行含振假名的段落**整体撑高一截：
  ...
  static String css({
    ...
    //    chrome top/bottom inset)，与 padding-top/padding-bottom 逐项对应（BUG-2457：
    //    不再多扣一个字号）。
