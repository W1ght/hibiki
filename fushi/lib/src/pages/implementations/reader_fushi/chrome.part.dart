...
extension _ReaderChrome on _ReaderFushiPageState {
  ...
  Widget _readerControlButton(ReaderHeaderAction a) =>
  ...
  List<Widget> _bottomSlotButtons() {
    final List<ReaderHeaderAction> left = _readerControlActionsIn(
      ReaderControlSlot.bottomLeft,
    );
    final List<ReaderHeaderAction> center = _readerControlActionsIn(
      ReaderControlSlot.bottomCenter,
    );
    final List<ReaderHeaderAction> right = _readerControlActionsIn(
  ...
  /// 播放条右端：底栏槽位里的按钮（用户拖进去的）+ 状态读数。播放条在场时底栏
  /// 只有这一条，槽位按钮并进它的右端，读数仍在最右。
  ...
  String _labelWithShortcut(String label, ShortcutAction action) {
    final List<InputBinding> keys = appModel.shortcutRegistry
        .bindingsFor(action)
        .keyboardBindings;
  ...
  Widget _buildDesktopHeader() {
    ...
    return Positioned(
      ...
      child: RepaintBoundary(
        ...
        // 顶栏本体因此不再挂 MouseRegion；按钮来自布局槽位（设置里可拖）。
  ...
  void _openReadingStatistics() {
    final int? remainingChapter = _progress.remainingChapterChars(
      _currentChapter,
    );
  ...
  Future<void> _transcribeFromAudiobookPanel() async {
    ...
    final EpubBookRow? book = await appModel.database.getEpubBook(
      widget.bookKey,
    );
    if (!mounted) return;
  ...
  /// home indicator 那 34pt 里，而不是在它上面再叠一条 28pt（BUG-2460）。
  ///
  ...
  void _syncDictionaryTheme() {
    ...
    appModel.setOverrideDictionaryTheme(
      ThemeData(
        useMaterial3: true,
        colorScheme: appModel
            .buildColorScheme(brightness)
            .copyWith(
