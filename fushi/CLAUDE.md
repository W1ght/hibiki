...
- 硬边界：**AI 只产出配置或在已取回的候选里做排序/选择，热路径永远是本地确定性代码**；未指派提供商时**不发任何 AI 请求**（2026-09-26 所有者拍板：入口可以可见，点击引导去「设置 › AI」指派，见 BUG-2694；入口仍须过 iOS 合规门与用户关掉的模块门）；AI 产物必须经本地校验（正则可编译、CSS 选择器白名单、候选 key 在集合内）才落地，且先进草稿/待确认而不是直接保存。AI 配置是设备本地的，不进备份/同步/Profile。设计见 `docs/specs/2026-09-15-ai-feature-expansion.md`。
