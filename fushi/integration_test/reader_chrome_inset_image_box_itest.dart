...
/// BUG-2454 / BUG-2453 真 app 证据（Windows 离屏 runner，真 WebView2 + 真分页 JS）。
///
/// A. BUG-2454 —— 竖排分页 shell 的 `window.fushiReader.setChromeInsets(top, bottom)`
...
///
/// B. BUG-2453 —— 桌面端底栏挤压模式（`tap_empty_hide_chrome=false`）下状态行
...
void main() {
  ...
  testWidgets(
    'BUG-2454: setChromeInsets re-derives the image max box to the new column '
    'height; BUG-2453: squeeze-mode play bar absorbs the status footer',
      ...
      await runFushiItest(
        ...
        body: () async {
          ...
          expect(grown.maxHeight, closeTo(grown.columnWidth, 1.0),
              reason:
                  'BUG-2454: after setChromeInsets the image max-height must '
          ...
          expect(base.maxHeight - grown.maxHeight, closeTo(expectedShrink, 2.0),
              reason: 'BUG-2454: max-height must shrink by exactly the inset '
          ...
          expect(restored.maxHeight, closeTo(restored.columnWidth, 1.0),
              reason: 'BUG-2454: restoring the insets must re-derive the image '
                  'max-height again');
          ...
          try {
            ...
            final String audiobookKey = await seedAudiobook(tester,
                title: 'BUG-2453 Squeeze Audiobook');
            ...
            expect(footers, 0,
                reason: 'BUG-2453: with the readout inlined into the occupying '
                    'bar, the status footer must not paint a second copy');
            expect(footerProgress, 0,
                reason: 'BUG-2453: only ONE progress readout may be on screen');
