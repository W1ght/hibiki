...
void main() {
  ...
  testWidgets('reader automation can open an audiobook and enter lyrics mode', (
    WidgetTester tester,
  ) async {
    await runFushiItest(
      ...
      body: () async {
        ...
        expect(
          controller.chapterCueCount,
          greaterThan(0),
          reason: 'fixture must provide subtitle cues for lyrics mode',
        );

        expect(
          ReaderFushiPage.debugOpenQuickSettings,
          isNotNull,
        ...
        expect(
          ...
          findsOneWidget,
          reason: 'lyrics action must not activate the neighboring exit action',
        );
        await _pumpUntil(
          tester,
        ...
        final dynamic sentinel = await ReaderFushiPage.debugEvaluateJavascript
            ?.call(
