...
void main() {
  ...
  testWidgets('选中的释义段在导出的卡片释义里被 mark 标在同一位置', (WidgetTester tester) async {
    ...
    expect(int.parse('$anchors'), 3, reason: '三条义项都必须带 data-fushi-gloss 锚点');

    // ---- 1. 跨元素选中 -------------------------------------------------
    final Object? crossRaw = await controller.evaluateJavascript(
      source: '''
      (function() {
    ...
    expect(cross['picked'], '一日に三回食べる', reason: '测试自身的前提：选中的确实是跨三个文本节点的整句');
    expect(cross['nodeCount'], 3, reason: '这一条义项必须真的由三个文本节点组成');
    final List<dynamic> marks = cross['marks'] as List<dynamic>;
    expect(marks.join(), '一日に三回食べる', reason: '导出的释义里，mark 拼起来必须逐字等于选中的文本');
    expect(marks.length, 3, reason: '跨元素选中不能把结构压平，应当每个文本节点各自成 mark');
    expect(cross['markedItems'], <int>[1], reason: '高亮只能落在被选中的那一条义项上，不能污染其它义项');
    expect(
      (cross['glossaryMarks'] as List<dynamic>).join(),
      '一日に三回食べる',
      reason: 'Glossary 字段（全部词典）与 MainDefinition 必须同样带高亮',
    );
    expect(
      '${cross['style']}'.contains('background-color'),
      isTrue,
      reason: '出厂默认色必须写成 inline style，不能依赖笔记类型 CSS 存在',
    );
    expect(
      '${cross['style']}'.contains('color: inherit'),
      isTrue,
      reason: '不显式写 color 的话 <mark> 的浏览器默认前景色在暗色卡上是黑字',
    );

    // ---- 2. 图片之后的文本（导出端多出 alt 文本的缺口）------------------
    final Object? afterImageRaw = await controller.evaluateJavascript(
      source: '''
      (function() {
        var h = window.__t.helpers;
        var host = h.host(2);
    ...
    expect(afterImage['picked'], '図のあと', reason: '测试自身的前提：选中的是图片后面那段文本的前三个字');
    expect(
      afterImage['exportHasAlt'],
      isTrue,
      reason:
    ...
    expect(
      (afterImage['marks'] as List<dynamic>).join(),
      '図のあと',
      reason: '图片 alt 只存在于导出树，不跳过它就会把 mark 整体顶偏 alt 的长度',
    );
    expect(afterImage['markedItems'], <int>[2], reason: '高亮必须落在含图片的那一条义项上');

    // ---- 3. 选中落在释义之外 -------------------------------------------
    final Object? outsideRaw = await controller.evaluateJavascript(
      source: '''
      (function() {
        var h = window.__t.helpers;
    ...
    expect(
      '${outside['picked']}'.isNotEmpty,
      isTrue,
      reason: '测试自身的前提：词头上确实选中了文本',
    );
    expect(outside['marks'], isEmpty, reason: '选中落在释义之外时一个 mark 都不该有');
    ...
    expect(
      '$keptRaw'.isNotEmpty && '$keptRaw' != 'null',
      isTrue,
    ...
    final Object? clearedRaw = await controller.evaluateJavascript(
      source: '''
      (function() {
        var h = window.__t.helpers;
        var host = h.host(1);
    ...
    expect(
      '$keptWhenHighlighted'.contains('一日に三回食べる'),
      isTrue,
      reason:
          'popup.js 不许自作主张清空——它不知道用户的笔记类型和字段映射，'
          '清了就可能让选中的内容凭空消失',
    );
    ...
    expect('$clearedGlossary', 'true', reason: '让位的前提是高亮真的在 payload 的释义里');
  });
}
