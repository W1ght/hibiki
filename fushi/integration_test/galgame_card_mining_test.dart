...
void main() {
  ...
  const MethodChannel voiceChannel = MethodChannel(
    'app.fushi.reader/voice_hook',
  );

  testWidgets(
    '逐个 galgame 抓真实音频并经 AnkiConnect 推带音频卡',
    (WidgetTester tester) async {
      ...
      for (int idx = 0; idx < games.length; idx++) {
        ...
        if (!File(exe).existsSync()) {
          print(
            'GALCARD RESULT game=$gameName source=skip-missing-exe '
        ...
        if (!File(injector).existsSync()) {
          print(
            'GALCARD RESULT game=$gameName source=skip-missing-injector '
            'engineHook=false pcmBytes=0 note=null fmt=na',
          );
          continue;
        }
        ...
        try {
          ...
          injectorProc = await Process.start(injector, <String>[
            '--launch',
            exe,
            '--hold',
          ]);
                ...
                onDone: () {
                  if (!pidCompleter.isCompleted) {
                    pidCompleter.complete(
                      parseInjectorHookedPid(stdoutBuf.toString()),
                    );
                  }
                },
                onError: (Object _) {
          ...
          childPid = await pidCompleter.future.timeout(
          ...
          if (childPid != null) {
            try {
              await voiceChannel.invokeMethod<void>('open', <String, Object?>{
                'pid': childPid,
              });
          ...
          final GalAudioSlice? engineSlice = await _grabEngineHook(
            voiceChannel,
            4000,
          );
          ...
          if (chosen != null) {
            ...
            created++;
            print(
              'GALCARD FILE $mediaBase wav=$wavPath '
        ...
        // l. 逐游戏结果行（无论成败）。
        print(
          'GALCARD RESULT game=$gameName source=$source '
  ...
  try {
    ...
    final WindowCaptureResult res =
        await WindowCaptureChannel.captureWindow(target.hwnd).timeout(
          ...
          onTimeout: () => const WindowCaptureResult(error: 'capture timeout'),
        );
