  ...
  testWidgets('Netflix 在内置网页播放器里起播、抓到字幕轨、bridge seek 生效',
    ...
    try {
      ...
      if (card.evaluate().isNotEmpty) {
        expect(await driver.focusWidget(card), isTrue,
            reason: '视频卡应可被焦点到达');
        await driver.activate();
      } else {
        debugPrint('[web-video-itest] card not onstage; opening via '
      ...
      final ObserveShot afterOpen =
          await captureFlutterFrame(tester, 'observe-web-video-after-open');
      expect(pageUp, isTrue,
      ...
      final ObserveShot playingShot =
          await captureFlutterFrame(tester, 'observe-web-video-playing');
      ...
      if (png != null) {
        ...
        debugPrint('[web-video-itest] webview screenshot ${f.path} '
            '(${png.length}B)');
      }
      expect(playing, isNotNull,
          reason: 'Netflix 应在 90s 内起播（未登录 / 网络不通会停在这里；'
              '截图 ${playingShot.path}）');
      ...
      final ObserveShot listShot =
          await captureFlutterFrame(tester, 'observe-web-video-subtitle-list');
      expect(listShot.nonBlank, isTrue,
          reason: '字幕列表外壳不应白屏（${listShot.path}）');
