  ...
  testWidgets(
    ...
    (WidgetTester tester) async {
      ...
      try {
        ...
        await tester.pump(const Duration(milliseconds: 500));
        expect(
          ReaderFushiSource.instance.readerViewMode,
          'vn',
        ...
        expect(
          ...
          isTrue,
          reason:
              'VN shell 必须真的装上了（找不到 .fushi-vn-stage 说明装的不是 '
        ...
        final double innerHeight =
            (geo['innerHeight'] as num?)?.toDouble() ?? -1;
        ...
        expect(
          pageWidth,
          greaterThan(0),
          reason: 'BUG-1688：VN 必须写 --page-width',
        );
        expect(
          (innerWidth - pageWidth).abs(),
          lessThanOrEqualTo(1.0),
          reason:
              'BUG-1688：WebView 视口宽 ($innerWidth CSS px) 必须与 Dart 下发的 '
        ...
        expect(
          chromeTopInset,
          greaterThanOrEqualTo(kExpectedReservePx - 1.0),
          reason:
              'BUG-1688：VN 首载稳定后 --chrome-top-inset 必须 >= 顶部进度条'
        ...
        expect(
          screenTop,
          greaterThanOrEqualTo(chromeTopInset - 1.0),
          reason:
              'BUG-1688：VN 屏顶 ($screenTop) 必须 >= --chrome-top-inset '
        ...
        expect(
          ...
          reason:
              'BUG-1688：VN 屏底 ($screenBottom) 必须 <= 视口高 ($innerHeight) '
