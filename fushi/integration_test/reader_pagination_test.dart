...
void main() {
  ...
  testWidgets(
    'M1: Reader pagination invariants hold across page turns',
    ...
    (WidgetTester tester) async {
      ...
      try {
        ...
        expect(
          ...
          findsOneWidget,
          reason:
              'ReaderFushiPage must mount after openMedia — if this '
        ...
        expect(
          ...
          isNotNull,
          reason:
              'Reader debug JS hook must be set when WebView is created. '
        ...
        final scanRaw = await eval(
          'window.fushiTestHarness.fullChapterScan();',
        );
        ...
        final RenderedSettings rendered = RenderedSettings.fromJson(
          _decode(settingsRaw),
        );
        expect(
          rendered.writingMode,
          'vertical-rl',
        ...
        expect(
          (pitch - pitch.roundToDouble()).abs(),
          greaterThan(0.01),
          reason:
              'M1 must retain the browser fractional pitch instead of '
              'testing only an integer grid',
        );
        for (int i = 0; i < pages.length && i < 4; i++) {
          final p = pages[i];
          debugPrint(
            '[M1] page ${p.pageNumber} '
        ...
        final int domMarkerCount = num.parse(
          (await eval(
            "document.querySelectorAll('[id^=m]').length",
          )).toString(),
        ).toInt();
        debugPrint(
          '[M1] Distinct markers seen: ${allMarkers.length} '
        ...
        final violations = validateChapterScan(
          pages,
          expectedMarkerCount: markerCount,
        );
        ...
        debugPrint(
          '[M1] Scroll before=${beforeState.scroll} '
        ...
        if (beforeMarkers.isNotEmpty) {
          ...
          expect(
            drift,
            lessThanOrEqualTo(beforeState.columnPitch + 1),
            reason:
                'I9: scroll drifted $drift px across chrome toggle '
        ...
        debugPrint(
          '[M1] I10 mid scroll=${midState.scroll} '
        ...
        debugPrint(
          '[M1] I10 after rapid toggle scroll=${afterRapid.scroll} '
        ...
        expect(
          ...
          greaterThan(0),
          reason:
              'I10: rapid chrome toggle reset the reader to the chapter '
...
Future<void> _toggleReaderChrome(WidgetTester tester, Finder webView) async {
